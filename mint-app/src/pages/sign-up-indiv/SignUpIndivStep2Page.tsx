...
const SignUpIndivStep2Page = (): JSX.Element => {
  ...
  return (
    <SignUpStep2
      ...
      onBack={() => navigate(ROUTES.SIGN_UP.INDIVIDUAL.STEP_1)}
      onNext={(token: string) =>
        navigate(
          `${NAVIGATION_ROUTES.SIGN_UP_STEP_3}?${PARAM_ACCESS_TOKEN}=${encodeURIComponent(token)}&${PARAM_USER_ROLE}=${encodeURIComponent(USER_ROLE.MEMBER)}`,
        )
      }
    />
  );
...
export default SignUpIndivStep2Page;
