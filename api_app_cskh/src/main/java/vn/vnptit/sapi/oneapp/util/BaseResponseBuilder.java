...
public class BaseResponseBuilder {
    ...
    public static <T> BaseResponse<T> of(T data, String request_id, String message, String status) {
        ...
        {
            obj.setMessageDetail(message);
            if(message.contains("ORA-")){
                try {
                    String ora = message.split("\n")[0];
                    String msg = ora.split(": ", 2)[1];
                    message = msg;
    ...
    public static <T, P> BasePageResponse<T, P> of(T data, P pageInfo, String request_id, String message, String status) {
        BasePageResponse<T, P> obj = new BasePageResponse<T, P>();
        obj.setErrorCode(status);
        obj.setError(status);
        obj.setRequestId(request_id);
        if (message == null) {
        ...
        obj.setData(data);
        obj.setPageInfo(pageInfo);
        return obj;
    }

    @Deprecated
    public static <T, P> BasePageResponse<T, P> of(T data, P pageInfo, String request_id, String message, HttpStatus status) {
        if (status == HttpStatus.OK) {
            return of(data, pageInfo, request_id, message, CODE_OK);
        } else {
            return of(data, pageInfo, request_id, message, "BSS-00000" + status.value());
        }
    }

    public static <T, P> BasePageResponse<T, P> of(T data, P pageInfo, String request_id, String message) {
        if (data instanceof Iterable<?>) {
            if (!((Iterable) data).iterator().hasNext())
                return of(data, pageInfo, request_id, message, CODE_NO_CONTENT);
            else
                return of(data, pageInfo, request_id, message, CODE_OK);
        } else
            return of(data, pageInfo, request_id, message, CODE_OK);
    ...
    public static <T, P> BasePageResponse<T, P> of(T data, P pageInfo, String request_id) {
        return of(data, pageInfo, request_id, null);
    }

    public static <T, P> BasePageResponse<T, P> of(T data, P pageInfo) {
        return of(data, pageInfo, null, null);
    }
}
