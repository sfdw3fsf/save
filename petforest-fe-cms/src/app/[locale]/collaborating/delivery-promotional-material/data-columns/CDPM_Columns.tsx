...
type CPH_ColumnsProps = {
  onUpdateHospitalDelivery: (id: number, values: HospitalDeliveryUpdateREQ) => void;
  managerOptions: FilterOptions;
  hospitalOptions: FilterOptions;
};
export const CPHD_Columns = ({
  onUpdateHospitalDelivery,
  managerOptions,
  hospitalOptions,
}: CPH_ColumnsProps): TableColumnsType<HospitalDeliveryDTO> => {
  ...
  return [
    {
      title: <EditableHeader title={tDeliveryPromotionalMaterial('table.hospital')} />,
      dataIndex: 'hospitalName',
      width: 150,
      sorter: true,
      shouldCellUpdate: () => true,
      // render: (value: string, record: HospitalDeliveryDTO) => <HospitalName name={value} id={record.hospitalId} />,
      render: (value: string, record) =>
        value ? (
          <div className='flex justify-between items-center'>
            <HospitalName name={value} id={record.hospitalId} />
            <CustomIconButton
              className='x-button opacity-0 hover:opacity-100'
              icon={<Image src={ICONS.CLOSE} alt='Remove' width={20} height={20} />}
              onClick={() => {
                onUpdateHospitalDelivery(record.id, { hospitalId: null });
              }}
            />
          </div>
        ) : (
          <CustomSelect
            ...
            defaultValue={value}
            onChange={handleChangeUpdate<HospitalDeliveryUpdateREQ>(record.id, 'hospitalId', value, onUpdateHospitalDelivery)}
            showSearch
