...
import { HospitalSaleListFilter } from '@/store/collaborating-hospital/request/collaborating-hospital.request';
import { getTodayRange } from '@/utils/helpers/date-time.helper';
import { orUndefined } from '@/utils/parser/io.parser';
import { DebouncedFunc } from 'lodash';
import { useSearchParams } from 'next/navigation';

type CS_TableFilterProps = {
  onFilterChange: DebouncedFunc<(filter: HospitalSaleListFilter) => void>;
  onDownloadExcel: () => void;
};

const CS_TableFilter = ({ onFilterChange, onDownloadExcel }: CS_TableFilterProps) => {
    ...
    <div className='flex flex-1 w-full flex-wrap gap-3 min-h-[38px] lg:flex-nowrap'>
      <CustomRangePicker
        ...
        onChange={(values) => {
          ...
          onFilterChange({ filter: JSON.stringify({ ['visitedAt.gte']: newValues[0], ['visitedAt.lte']: newValues[1] }) });
        }}
