...
import { getMaterialInventoryHistoryListApi, materialOtherInventoryHistoryKeys } from '@/store/material/material.api';
...
import MOFS_HistoryColumns from '../data-columns/MOFS_HistoryColumns';

const MOFS_History = () => {
  const searchParams = useSearchParams();
  const { mutateDownloadExcel } = useDownloadExcel<MaterialListREQ>({
    fileName: EXCEL_FILE_NAME.MATERIAL_OTHER_INVENTORY_DETAILS,
  ...
  const { data: materialInventoryHistoryListData } = useQuery({
    queryKey: materialOtherInventoryHistoryKeys.list(filter),
    queryFn: () =>
