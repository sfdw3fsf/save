...
import MOFS_DetailModal from '../components/MOFS_DetailModal';
import MOFS_Columns from '../data-columns/MOFS_Columns';
...
const MOFS_List = () => {
  ...
  const { arrangeOrdinal } = useArrangeOrdinal({
    url: `materials`,
    queryKey: materialOtherKeys.lists(),
    type: { materialType: MATERIAL_TYPE.OTHER },
  });
  const { mutateDownloadExcel } = useDownloadExcel<MaterialListREQ>({
    fileName: EXCEL_FILE_NAME.MATERIAL_OTHER_FUNERAL,
  ...
  const { data: materialOtherListData } = useQuery({
    queryKey: materialOtherKeys.list(filter),
    queryFn: () => getMaterialListApi({ ...filter, materialType: MATERIAL_TYPE.OTHER }),
    select: (response) => {
  ...
  const { mutate: mutateUpdateMaterial } = useMutation({
    ...
    onSuccess: () => {
      ...
      materialId.current && queryClient.invalidateQueries({ queryKey: materialOtherKeys.detail(materialId.current) });
    },
    onError: handleError,
  });
  ...
  const materialName = useMemo(() => {
    ...
    return materialOtherListData?.data.find((item) => item.id === materialId.current)?.name || '';
  }, [materialId.current, materialOtherListData?.data]);
  ...
  return (
    <>
      <div className='mt-6'>
        <TableFilter total={materialOtherListData?.total}>
      ...
      <div className='mt-4 ' id='fixedTable'>
        <CustomTable<MaterialDTO>
          columns={MOFS_Columns({ onDetail, onUpdateMaterial, onUpdateMaterialStatus })}
          data={materialOtherListData?.data || []}
          showPaging={false}
          showSelect
      ...
      <FormProvider {...methods}>
        <CustomModal
          ...
          title={materialName}>
          <MOFS_DetailModal id={materialId.current} setIsHiddingFooter={setIsHiddingFooter} />
        </CustomModal>
      </FormProvider>
