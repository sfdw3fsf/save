...
import MFS_DetailModal from '../components/MFS_DetailModal';
import { MFS_Columns } from '../data-columns/MFS_Columns';

const MFS_List = () => {
  const searchParams = useSearchParams();
  const queryClient = useQueryClient();
  const { mutateDownloadExcel } = useDownloadExcel<MaterialListREQ>({
    fileName: EXCEL_FILE_NAME.MATERIAL_FUNERAL_SUPPLIES,
  ...
  const { arrangeOrdinal } = useArrangeOrdinal({
    ...
    type: { materialType: MATERIAL_TYPE.FUNERAL },
  });
  ...
  const { data: materialFuneralListData } = useQuery({
    queryKey: materialFuneralKeys.list(filter),
    queryFn: () => getMaterialListApi({ ...filter, materialType: MATERIAL_TYPE.FUNERAL }),
    select: (response) => {
  ...
  const { mutate: mutateUpdateMaterial } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: materialFuneralKeys.list(filter) });
      materialId.current && queryClient.invalidateQueries({ queryKey: materialFuneralKeys.detail(materialId.current) });
    },
    onError: handleError,
  });
  ...
  const materialName = useMemo(() => {
    ...
    return materialFuneralListData?.data.find((item) => item.id === materialId.current)?.name || '';
  }, [materialId.current, materialFuneralListData?.data]);
  ...
  return (
    <>
      <div className='mt-6'>
        <TableFilter total={materialFuneralListData?.total}>
      ...
      <div className='mt-4 ' id='fixedTable'>
        <CustomTable<MaterialDTO>
          columns={MFS_Columns({ onDetail, onUpdateMaterial, onUpdateMaterialStatus })}
          data={materialFuneralListData?.data || []}
      ...
      <FormProvider {...methods}>
        <CustomModal
          ...
          title={materialName}>
          <MFS_DetailModal id={materialId.current} setIsHiddingFooter={setIsHiddingFooter} />
        </CustomModal>
      </FormProvider>
