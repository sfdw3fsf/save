...
import {
  ...
  materialFuneralInventoryByMonthKeys,
  materialFuneralInventoryKeys,
  materialFuneralKeys,
...
import { MFS_DetailColumns } from '../data-columns/MFS_DetailColumns';

type MFS_DetailInventoryProps = {
  id: number;
};

const MFS_DetailInventory = ({ id }: MFS_DetailInventoryProps) => {
  ...
  const { mutateDownloadExcel } = useDownloadExcel<MaterialInventoryListREQ>({
    fileName: EXCEL_FILE_NAME.MATERIAL_INVENTORY_DETAILS_SPECIFIC,
  ...
  const { mutate: mutateCreateMaterialInventory } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: materialFuneralInventoryKeys.lists() });
  ...
  const { data: materialInventoryListData } = useQuery({
    queryKey: materialFuneralInventoryKeys.list({ filter, id }),
    queryFn: () => getMaterialInventoryListApi(id, { ...filter, materialType: MATERIAL_TYPE.FUNERAL }),
    select: (response) => {
  ...
  const { mutate: mutateUpdateMaterialInventory } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: materialFuneralInventoryKeys.lists() });
      queryClient.invalidateQueries({ queryKey: materialFuneralInventoryByMonthKeys.detail(id) });
  ...
  const { data: materialInventoryByMonthData } = useQuery({
    queryKey: materialFuneralInventoryByMonthKeys.detail(id),
    ...
    <div>
      ...
      <div className='mb-6 mt-4'>
        <CustomTable<MaterialInventoryDTO>
          columns={MFS_DetailColumns({ onUpdateMaterialInventory })}
...
export default MFS_DetailInventory;
