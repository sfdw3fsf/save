...
@Service
public class GiaiPhapBackupService {
    private final DbContext dbContext;

    public GiaiPhapBackupService(DbContext dbContext) {
    ...
    public ArrayList<Map<String, Object>> getData() throws AppSqlException {
        ...
        return dbContext.executeSpWithCursorToListMap("ecms.pkg_hatang_danhmuc.sp_giaiphap_backup_danhsach", param);
    }

    public ResponseEntity<ApiResult>  capNhatGiaiPhapBackup(DanhMucDtoIn input) throws AppSqlException {
        ...
        try {
            ...
            params.add(result);
            dbContext.setConnection(ConnectionManager.Connections.CSS);
            dbContext.executeStoredProcedure("ecms.pkg_hatang_danhmuc.sp_giaiphap_backup_capnhat", params);
    ...
    public ResponseEntity<ApiResult>  xoaGiaiPhapBackup(DanhMucDtoIn input) throws AppSqlException {
        ApiResult rs = new ApiResult();
        ...
        try {
            ...
            params.add(result);
            dbContext.setConnection(ConnectionManager.Connections.CSS);
            dbContext.executeStoredProcedure("ecms.pkg_hatang_danhmuc.sp_giaiphap_backup_delete", params);
            int kq=Integer.parseInt(result.getOutValue().toString());
            if (kq==1) {
