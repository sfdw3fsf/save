...
@RestController
@RequestMapping("hatang/thietbi-khohatang-idg")
public class ThietBiHaTangIDGController {
    private final KhoHaTangIDGService service;

    public ThietBiHaTangIDGController(KhoHaTangIDGService service) {
        this.service = service;
    }

    @Operation(summary = "Thêm thiết bị hạ tầng IDG")
    @PostMapping(value = "/add-thiet-bi")
    public ResponseEntity<ApiResult> addThietBi(@RequestBody KhoHaTangIDGThietBiDTO input) {
        ApiResult rs = new ApiResult();
        try {
            rs.setData(new ResultOutput(this.service.aoHoaIDGThietBiThem(input)));
    ...
    public ResponseEntity<ApiResult> getThietBi(@RequestBody Map<String, String> input) {
            ApiResult rs = new ApiResult();
            try {
                rs.setData(this.service.aoHoaIDGThietBi(input.get("p_hatang_id")));
            } catch (AppSqlException e) {
                rs.setException(e);
            }
            return rs.getResponseEntity();
    ...
    public ResponseEntity<ApiResult> deleteThietBi(@RequestBody Map<String, String> input) {
        ApiResult rs = new ApiResult();
        try {
            rs.setData(new ResultOutput(this.service.deleteThietBiIDG(input.get("p_id"))));
        } catch (AppSqlException e) {
            rs.setException(e);
        }
        return rs.getResponseEntity(); 
