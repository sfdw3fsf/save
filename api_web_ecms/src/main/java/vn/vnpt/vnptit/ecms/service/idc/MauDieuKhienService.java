...
import vn.vnpt.vnptit.ecms.dto.mau.MauDieuKhienDto;

@Service
public class MauDieuKhienService {

    @Autowired
    private DbContext dbContext;

    /**
     * Tìm kiếm hoặc lấy danh sách thiết bị điều khiển mẫu
     */
    public List<Map<String, Object>> search(JsonNode searchJsonData) throws AppSqlException {
        ...
        return dbContext.executeSpIdxWithCursorToListMap(
                "ECMS.PKG_IDC_THIETBI_MAU.SP_SEARCH_MAU_DKH", params);
    }

    /**
     * Lấy chi tiết thiết bị điều khiển mẫu theo ID
     */
    public Map<String, Object> getById(Long id) throws AppSqlException {
        ArrayList<SqlParameter> params = new ArrayList<>();
        params.add(new SqlParameter("p_maudkh_id", id, ParameterDirection.INPUT, Types.NUMERIC));
        params.add(new SqlParameter("p_rs", null, ParameterDirection.OUTPUT, Types.REF_CURSOR));

        ArrayList<Map<String, Object>> response = dbContext.executeSpWithCursorToListMap(
                "ECMS.PKG_IDC_THIETBI_MAU.SP_GET_MAU_DKH_BY_ID", params);
    ...
    /**
     * Lấy thiết bị điều khiển mẫu theo mẫu thiết bị ID
     */
    public Map<String, Object> getByMauThietBiId(Long mautbiId) throws AppSqlException {
        ...
        ArrayList<Map<String, Object>> response = dbContext.executeSpWithCursorToListMap(
                "ECMS.PKG_IDC_THIETBI_MAU.SP_GET_MAU_DKH_BY_MAUTBI_ID", params);
        // normalize keys to lowercase
        Map<String, Object> result = response != null && !response.isEmpty() ? response.get(0) : null;
        return result != null ? normalizeKeys(result) : null;
    }
    ...
    /**
     * Tạo mới hoặc cập nhật thiết bị điều khiển mẫu
     */
    public Object save(MauDieuKhienDto dto) throws AppSqlException {
        ArrayList<SqlParameter> params = new ArrayList<>();
        SqlParameter p_id = new SqlParameter("p_maudkh_id", dto.getMaudkhId(), Types.NUMERIC);
        params.add(p_id);
        ...
        params.add(new SqlParameter("p_dl_cache", dto.getDlCache(), ParameterDirection.INPUT, Types.NUMERIC));
        ...
        Map<String, Object> result = dbContext.executeSpWithCursorToMap("ECMS.PKG_IDC_THIETBI_MAU.SP_SAVE_MAU_DKH",
                params);
        return result.get("item");
    }

    /**
     * Xóa thiết bị điều khiển mẫu
     */
    public Boolean delete(Long id) throws AppSqlException {
        ArrayList<SqlParameter> params = new ArrayList<>();
        params.add(new SqlParameter("p_maudkh_id", id, Types.NUMERIC));
        SqlParameter result = new SqlParameter("p_result", null, SqlParameter.ParameterDirection.OUTPUT, Types.NUMERIC);
        params.add(result);

        dbContext.executeStoredProcedure("ECMS.PKG_IDC_THIETBI_MAU.SP_DELETE_MAU_DKH", params);
