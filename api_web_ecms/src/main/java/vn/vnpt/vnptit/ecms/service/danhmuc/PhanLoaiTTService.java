...
@Service
public class PhanLoaiTTService {
    private final DbContext dbContext;
    public PhanLoaiTTService(DbContext dbContext) {
        this.dbContext = dbContext;
    }

    public ArrayList<Map<String, Object>> getPhanLoaiTT() throws AppSqlException {
        ...
        return dbContext.executeSpWithCursorToListMap("ecms.PKG_HATANG_DANHMUC.SP_PHANLOAI_TT_DANHSACH", param);
    }

    public ResponseEntity<ApiResult> capNhatPhanLoaiTT(DanhMucDtoIn input) throws AppSqlException {
        ...
        try {
            ...
            if (input.getTen().length() != input.getTen().trim().length() ||
                input.getGhiChu().length() != input.getGhiChu().trim().length()) {
                rs.setErrorCode("BSS-00009490");
                rs.setMessage(String.format("Nội dung nhập cần kiểm tra lại !", input.getTen()));
                return rs.getResponseEntity();
            }
            ...
            params.add(result);
            dbContext.setConnection(ConnectionManager.Connections.CSS);
            dbContext.executeStoredProcedure("ECMS.PKG_HATANG_DANHMUC.SP_PHANLOAI_TT_CAPNHAT", params);
    ...
    public ResponseEntity<ApiResult>  xoaPhanLoaiTT(DanhMucDtoIn input) throws AppSqlException {
        ApiResult rs = new ApiResult();
        ...
        try {
            ...
            params.add(result);
            dbContext.setConnection(ConnectionManager.Connections.CSS);
            dbContext.executeStoredProcedure("ecms.PKG_HATANG_DANHMUC.SP_PHANLOAI_TT_DELETE", params);
            int kq=Integer.parseInt(result.getOutValue().toString());
            if (kq==1) {
