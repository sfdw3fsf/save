...
@Service
public class QuyHoachSubnetService {
    private final DbContext dbContext;

    public QuyHoachSubnetService(DbContext dbContext) {
    ...
    public ArrayList<Map<String, Object>> getData() throws AppSqlException {
        ...
        return dbContext.executeSpWithCursorToListMap("ecms.pkg_hatang_danhmuc.sp_subnet_quyhoach_danhsach", param);
    }

    public ResponseEntity<ApiResult>  capNhatQuyHoachSubnet(DanhMucDtoIn input) throws AppSqlException {
        ...
        try {
            ...
            params.add(result);
            dbContext.setConnection(ConnectionManager.Connections.CSS);
            dbContext.executeStoredProcedure("ecms.pkg_hatang_danhmuc.sp_subnet_quyhoach_capnhat", params);
    ...
    public ResponseEntity<ApiResult>  xoaQuyHoachSubnet(DanhMucDtoIn input) throws AppSqlException {
        ApiResult rs = new ApiResult();
        ...
        try {
            ...
            params.add(result);
            dbContext.setConnection(ConnectionManager.Connections.CSS);
            dbContext.executeStoredProcedure("ecms.pkg_hatang_danhmuc.sp_subnet_quyhoach_delete", params);
            int kq=Integer.parseInt(result.getOutValue().toString());
            if (kq==1) {
