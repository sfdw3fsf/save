...
@Component
public class SmartCloudExternalClient implements ExternalApiClient {
    private final MiddlewarePropeties config;
    private final RestTemplate restTemplate;
    private final RestTokenManager restTokenManager;

    public SmartCloudExternalClient(MiddlewarePropeties config,
    ...
    public MiddlewarePropeties.SystemType getType() {
        return MiddlewarePropeties.SystemType.SMARTCLOUD_EXTERNAL;
    }
    ...
    public <T> T call(String path, HttpMethod method, Object body, Class<T> responseType) {
        T result;
        if (path.startsWith("/")) {
            path = this.config.getSmartCloudExternal().getBaseUrl() + path;
        } else {
            path = this.config.getSmartCloudExternal().getBaseUrl()+"/"+path;
        }
        HttpHeaders headers = new HttpHeaders();
        headers.set("Authorization", restTokenManager.getToken(this.getType()));
