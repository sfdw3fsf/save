...
@Component
public class SmartCloudInternalClient implements ExternalApiClient {
    private final MiddlewarePropeties config;
    private final RestTemplate restTemplate;
    private final ObjectMapper objectMapper;

    public SmartCloudInternalClient(MiddlewarePropeties config,
    ...
    public MiddlewarePropeties.SystemType getType() {
        return MiddlewarePropeties.SystemType.SMARTCLOUD_INTERNAL;
    }

    @Override
    public <T> T call(String path, HttpMethod method, Object body, Class<T> responseType) {
        if (path.startsWith("/")) {
            path = this.config.getIdg().getBaseUrl() + path;
        } else {
            path = this.config.getIdg().getBaseUrl()+"/"+path;
        }
        HttpHeaders headers = new HttpHeaders();
        headers.set("X-API-KEY", this.config.getIdg().getxApiKey());
        headers.set("X-User-Email", this.config.getIdg().getxUserEmail());
    ...
    public <T> List<T> callList(String path, HttpMethod method, Object body, Class<T> responseType) {
        ...
        ResponseEntity<RestData> response = this.restTemplate.exchange(path, method, entity, RestData.class);
        if (response.getBody()==null||String.valueOf(response.getBody().getData()).isEmpty()) return null;
