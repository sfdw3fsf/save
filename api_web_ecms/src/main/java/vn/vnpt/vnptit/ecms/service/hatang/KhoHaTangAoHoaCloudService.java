...
@Service
public class KhoHaTangAoHoaCloudService {
    @Autowired
    private DbContext dbContext;


    public Object createOrUpdate(KhoHaTangAoHoaCloudDTO dto) throws AppSqlException {
        ...
        dbContext.executeStoredProcedure("ECMS.PKG_HATANG_TAINGUYEN_QUANLY.SP_KHO_HATANG_AOHOA_INSERT_UPDATE", params);

        long resultValue = Long.parseLong(result.getOutValue().toString());
        if (resultValue > 0) {
            List<SqlParameter> deleteBackupThietBiParams = new ArrayList<>();
            deleteBackupThietBiParams.add(new SqlParameter("p_hatang", "AOHOA_THIETBI", Types.NVARCHAR));
            ...
            for (KhoHaTangThietBiHinhThanhDTO tb : dto.getThietBiDauNoiList()) {
                ...
                dbContext.executeStoredProcedure("ECMS.PKG_HATANG_TAINGUYEN_QUANLY.SP_THIETBI_HINHTHANH_HATANG_AOHOA_THIETBI_INSERT", insertSmartThietBiParams);
            }
        }
    ...
    public Object getAll() throws AppSqlException {
        ...
        ArrayList<Map<String, Object>> dbResult = dbContext.executeSpWithCursorToListMap("ECMS.PKG_HATANG_TAINGUYEN_QUANLY.SP_KHO_HATANG_AOHOA_GETALL", params);
    ...
    public Object getItem(Number id) throws AppSqlException {
        ...
        params.add(new SqlParameter("p_result", Types.REF_CURSOR));
        ArrayList<Map<String, Object>> dbResult = dbContext.executeSpWithCursorToListMap("ECMS.PKG_HATANG_TAINGUYEN_QUANLY.SP_KHO_HATANG_AOHOA_GETITEM", params);
        List<Map<String, Object>> result = dbResult.stream()
                .map(map -> map.entrySet()
                        .stream()
                        .filter(entry -> entry.getValue() != null)
    ...
    public Object deleteById(Number id) {
        try {
            List<SqlParameter> params = new ArrayList<>();
            params.add(new SqlParameter("p_id", id, Types.NUMERIC));
            params.add(new SqlParameter("p_table_name", "HATANGAOHOA", Types.NVARCHAR));
    ...
    public Object aoHoaCloudRelationship(Number id, String tabName) throws AppSqlException {
        List<SqlParameter> params = new ArrayList<>();
        params.add(new SqlParameter("p_object_id", id, Types.NUMERIC));
        params.add(new SqlParameter("p_table_name", tabName, Types.NVARCHAR));
        params.add(new SqlParameter("p_result", Types.REF_CURSOR));
        ArrayList<Map<String, Object>> dbResult = dbContext.executeSpWithCursorToListMap("ECMS.PKG_HATANG_TAINGUYEN_QUANLY.SP_KHO_HATANG_AOHOA_RELATIONSHIP", params);
        List<Map<String, Object>> result = dbResult.stream()
                .map(map -> map.entrySet()
                        .stream()
                        .filter(entry -> entry.getValue() != null)
