package vn.vnptit.sapi.oneapp.invoker.gnms;
...
import vn.vnptit.sapi.oneapp.invoker.JavaTimeFormatter;
import vn.vnptit.sapi.oneapp.invoker.RFC3339DateFormat;
import vn.vnptit.sapi.oneapp.invoker.auth.ApiKeyAuth;
import vn.vnptit.sapi.oneapp.invoker.auth.Authentication;
import vn.vnptit.sapi.oneapp.invoker.auth.HttpBasicAuth;
import vn.vnptit.sapi.oneapp.invoker.auth.HttpBearerAuth;
...
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2021-05-19T16:41:06.244+07:00[Asia/Bangkok]")
@Component
...
//    private String basePath = "https://10.157.5.2:97/nms/rest";
    private String basePath = "https://gnms-api.vnpt-technology.vn:97/nms/rest";
    ...
    protected void init() {
        ...
        apiKey.setApiKeyPrefix("Key");
        apiKey.setApiKey("7c346306450740d2882cce00530da1aa");
        authentications.put("ApiKeyAuth", apiKey);
    ...
    */
    public static WebClient buildWebClient(ObjectMapper mapper) {
        ...
        try {
            ...
            HttpClient httpClient = HttpClient.create().secure(t -> t.sslContext(sc));

            WebClient.Builder webClient = WebClient.builder().clientConnector(new ReactorClientHttpConnector(httpClient))
...
//        HttpClient httpClient = HttpClient.create().secure(t -> t.sslContext(sslContext));
    ...
    */
    ...
    public <T> Mono<T> invokeAPI(String path, HttpMethod method, Map<String, Object> pathParams, MultiValueMap<String, String> queryParams, Object body, HttpHeaders headerParams, MultiValueMap<String, String> cookieParams, MultiValueMap<String, Object> formParams, List<MediaType> accept, MediaType contentType, String[] authNames, ParameterizedTypeReference<T> returnType) throws RestClientException {
        ...
        return requestBuilder.retrieve().onStatus(
                (HttpStatus::isError), // or the code that you want
                (it -> handleError(it)) //handling error request
        )
...
//                .onStatus(httpStatus -> httpStatus.value()==501,
//                clientResponse -> Mono.error(new GnmsException(501,"Device not found")))
//                .onStatus(httpStatus -> httpStatus.value()==502,
//                        clientResponse -> Mono.error(new GnmsException(502,"Device not connected")))
//                .onStatus(httpStatus -> httpStatus.value()==503,
//                        clientResponse -> Mono.error(new GnmsException(503,"Invalid parameter")))
//                .onStatus(httpStatus -> httpStatus.value()==541,
//                        clientResponse -> Mono.error(new GnmsException(541,"Unknown error from gmns server")))
                .bodyToFlux(returnType);
    }

    private Mono<? extends Throwable> handleError(ClientResponse response) {
        int statusCode = response.statusCode().value();
        String msg="Unknown error calling gmns server";
        switch (statusCode){
            case 501:
            ...
            case 503:
                ...
                break;
            case 541:
                msg="Unknown error from gmns server";
                break;
//            default:
//                msg="Unknown error calling gmns server";

        }
//        if(response.statusCode().value()==501 ||response.statusCode().value()==502||response.statusCode().value()==503||response.statusCode().value()==541){
//
//            return response.bodyToMono(BaseResponse.class).flatMap(body -> Mono.error(new GnmsException(response.statusCode().value(),body.getMessage())));
//        }else{
//            return response.bodyToMono(BaseResponse.class).flatMap(body -> Mono.error(new GnmsException(response.statusCode().value(),body.getMessage())));
//        }
        return Mono.error(new GnmsException(statusCode,msg));
    }
