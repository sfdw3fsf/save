...
class Vnpt34InvoiceClient {
    ...
    void init(String maTinh) {
        ...
        // TODO test
        maTinh = "all"
        url = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".url34")
        username = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".username")
        password = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".password")
        thongTu = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".keyitem")
        account = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".account")
        acpass = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".acpass")
    }
    ...
    Map<String, Object> importAndPublishInv(String maTinh, InvItem inv, String pattern, String serial, String convert) {
        ...
        // TODO test
        maTinh = "all"
        String accountX = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".account")
        String acPassX = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".acpass")
        String urlX = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".url34")
        String usernameX = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".username")
        String passwordX = env.getProperty("spring.httpservices.vnptinvoice." + maTinh + ".password")
        ...
        def err_messages = [
                ...
                "51": "Chứng thư số bị thu hồi.",
                "52": "ERR:52 Sản lượng hoá đơn còn lại ({SoLuongConLai}) không đủ để tiếp tục thực hiện phát hành hóa đơn",
                "54": "Lỗi: không phát hành được với ngày hóa đơn lớn hơn ngày hiện tại",
                "58": "Lỗi: không phát hành được với ngày dịch vụ năm {item.ArisingDate.Year} cho dải số năm {serialYear}",
                "2901": "ERR:2901 Không tìm thấy keystore comId",
                "2902": "certificate null",
                "1501": "Ngày lập hóa đơn không được lớn hơn ngày hiện tại",
                "1502": "TCHDon = 5 bắt buộc nhập SBKe và NBKe",
                "1503": "Tỷ giá bắt buộc phải là VND (CurrencyUnit: {CurrencyUnit})",
                "1504": "MDVQHNSach {mDVQHNSach} không đúng quy định",
                "1505": "MDVQHNSach có dữ liệu: bắt buộc nhập Ten và DChi",
                "1506": "MST có dữ liệu: bắt buộc nhập Ten và DChi",
                "1507": "Email {EmailDeliver} không đúng định dạng",
                "1508": "HDCTTChinh = 1 chỉ cho phép cùng 1 loại TSuat = -5 hoặc CTTC",
                "1509": "vatrate == -99 vatrate không đúng định dạng.",
                "1510": "ERR:1510 LHHDTrung khong duoc de trong",
                "1511": "SKhung khong duoc de trong khi LHHDTrung = 1",
                "1512": "SMay khong duoc de trong khi LHHDTrung = 1",
                "1513": "BKSPTVChuyen khong duoc de trong khi LHHDTrung = 2",
                "1514": "DCNGHang khong duoc de trong khi LHHDTrung = 3",
                "1515": "TNGHang khong duoc de trong khi LHHDTrung = 3",
                "1516": "MSTNGHang khong duoc de trong khi LHHDTrung = 3",
                "1517": "MDDNGHang khong duoc de trong khi LHHDTrung = 3"
        ]
    ...
    Map<String, Object> hddt_confirmPaymentFkey_VTT(String provinceCode, String fKey, String pattern, String serial) {
        // TODO test
        provinceCode = "all"
        String urlVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".url34")
        String usernameVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".username")
        String passwordVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".password")
    ...
    Map<String, Object> importAndPublishInv_VTT(String provinceCode, String account, String acPass, String xmlInvData, String pattern, String serial, int convert) {
        ...
        provinceCode = "all"
        String urlVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".url34")
        String accountVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".account")
        String acPassVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".acpass")
        String usernameVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".username")
        String passwordVTT = env.getProperty("spring.httpservices.vnptinvoice." + provinceCode.toLowerCase() + ".password")
