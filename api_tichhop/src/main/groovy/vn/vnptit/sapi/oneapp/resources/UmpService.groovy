...
import vn.vnptit.sapi.oneapp.models.ump.UMP_REQ
...
class UmpService {
    ...
    @Value("\${spring.httpservices.ump.url}")
    String url

    @PostMapping("/RegisterUMP")
    @Consumes([MediaType.APPLICATION_JSON])
    @Produces([MediaType.APPLICATION_JSON])
    @LoggingInOut
    CommonResponse RegisterUMP(@Valid @RequestBody UMP_REQ body) {
        try {
            def res = post('RegisterUMP', body)
            String faultString = ""
            int errorCode = 0
            if (res.Message.status != 'success') {
                errorCode = 1//Integer.parseInt(res.resultid)
    ...
    @PostMapping("/UpdateUMP")
    @Consumes([MediaType.APPLICATION_JSON])
    @Produces([MediaType.APPLICATION_JSON])
    @LoggingInOut
    CommonResponse UpdateUMP(@Valid @RequestBody UMP_REQ body) {
        try {
            def res = post('UpdateUMP', body)
            String faultString = ""
            int errorCode = 0
            if (res.Message.status != 'success') {
                errorCode = 1//Integer.parseInt(res.resultid)
    ...
    @PostMapping("/DeleteUMP")
    @Consumes([MediaType.APPLICATION_JSON])
    @Produces([MediaType.APPLICATION_JSON])
    @LoggingInOut
    CommonResponse DeleteUMP(@Valid @RequestBody UMP_REQ body) {
        try {
            def res = post('DeleteUMP', body)
            String faultString = ""
            int errorCode = 0
            if (res.Message.status != 'success') {
                errorCode = 1//Integer.parseInt(res.resultid)
