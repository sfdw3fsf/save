...
class HKDService {
    ...
    ResponseEntity<BaseResponse<?>> executorGet(HttpServletRequest request, @RequestParam Map <String, String> requestParams) {
        try {
            String[] arrServletPath = request.getServletPath().split("hkd/", 2)
    ...
    String get(String path, Map<String, String> param) {
        ...
        String url = url_gw + "api/HKD/" + path + "?" + ls
    ...
    @GetMapping("/LayDanhSachQuan")
    @Consumes([MediaType.APPLICATION_JSON])
    @Produces([MediaType.APPLICATION_JSON])
    ResponseEntity<BaseResponse<?>> LayDanhSachQuan(@RequestParam Long tinh_id, @RequestParam Long loaitb_id) {
        try {
            String url = "https://hkd.vnpt.vn/apitichhop/api/Public/LietKeQuanHuyenTheoTinhThanh?TinhThanhPhoId=" + tinh_id
            if (loaitb_id == 303) {
                url = "https://iqms.vnpt.vn/kong/common/administrativeUnit/getDistrictByProvince/" + tinh_id
            }
            Map<String, String> header = [:]
            String resp = httpManaged.get(url, header)
    ...
    @GetMapping("/LayDanhSachPhuong")
    @Consumes([MediaType.APPLICATION_JSON])
    @Produces([MediaType.APPLICATION_JSON])
    ResponseEntity<BaseResponse<?>> LayDanhSachPhuong(@RequestParam Long tinh_id, @RequestParam Long quan_id, @RequestParam Long loaitb_id) {
        try {
            String url = "https://hkd.vnpt.vn/apitichhop/api/Public/LietKePhuongXaTheoQuanHuyen?TinhThanhPhoId=" + tinh_id + "&QuanHuyenId=" + quan_id
            if (loaitb_id == 303) {
                url = "https://iqms.vnpt.vn/kong/common/administrativeUnit/getWardByDistrict/" + quan_id
            }
            Map<String, String> header = [:]
            String resp = httpManaged.get(url, header)
            def result = new JsonSlurper().parseText(resp)
