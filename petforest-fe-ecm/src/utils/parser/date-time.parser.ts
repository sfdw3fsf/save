...
import { DATE_FORMAT } from '../constant/date.constant';
...
export const parseDayValue = (date: DayValue | null | undefined) => {
  if (!date) {
    return '';
  }
...
export const parseTimestampValue = (timestamp: number | null | undefined) => {
  if (!timestamp) {
    return '';
  }
...
export const parseDayValueToTimestamp = (day: DayValue): number | null => {
  if (!day) {
    return null;
  }
...
export const parseTimestampToDayValue = (timestamp: number | string | null | undefined): DayValue | null => {
