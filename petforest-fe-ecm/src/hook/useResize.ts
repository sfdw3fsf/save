...
const useResize = () => {
  ...
  const isTablet = useMemo(() => size.width > 0 && 500 < size.width && size.width <= 768, [size.width]);
  const isResponsive = useMemo(() => size.width > 0 && size.width && size.width < 1024, [size.width]);
