...
//    }
	...
	protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers,
		...
		if (ex instanceof MethodArgumentTypeMismatchException) {
			...
			msg = "Tham số " + e.getName() + " (" + e.getValue() + ") không đúng định dạng (" + e.getRequiredType() + ").";
		};
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		if (mostSpecificCause instanceof InvalidFormatException)
		{
			...
			for (Reference path : paths) {
				if(path.getFieldName() != null)
					msg += "Tham số " + path.getFieldName() + ": " + ModelValidation.INVALID + ".";
			}
			apiResult.setMessageDetail(msg);
		}
		else if(mostSpecificCause instanceof InputCoercionException)
		{
			...
			try {
				apiResult.setMessageDetail("Tham số " + inputCoercionException.getProcessor().getCurrentName() + " (" + inputCoercionException.getProcessor().getText() + ") không đúng định dạng (" + inputCoercionException.getTargetType().toString()
						+ ").");
			} catch (IOException e) {
