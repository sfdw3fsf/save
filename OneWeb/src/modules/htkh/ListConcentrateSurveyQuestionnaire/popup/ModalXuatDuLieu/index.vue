...
export default {
    ...
    methods: {
        onModalShown() {
            ...
            this.dsThuocTinh = dsThuocTinh.map(item => {return {NAME: item}})
        ...
        getColumnTemplateXoa(parent) {
            return function() {
                return {
                    template: {
                        template: `
                            <button class="btn btn-sm btn-danger lh14 pad3" @click="parent.onClickButtonXoaThuocTinh(data.NAME)" title="Xóa">
        ...
        onClickButtonXemDuLieu() {
            this.dsThuocTinhHeader = this.dsThuocTinhHeader.filter(item => {
                return this.dsThuocTinh.map(thuocTinh => thuocTinh.NAME).indexOf(item.name) >= 0
            })
        },
