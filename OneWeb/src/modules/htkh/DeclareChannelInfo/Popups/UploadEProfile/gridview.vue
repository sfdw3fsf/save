<template>
<div>
<div class="table-content marb0" style="overflow-x: hidden;height: 400px;"  v-if="datalist && datalist.length>0">
  <table class="table-result nowrap table-gachle" :id="id">
      <thead class="center">
          <tr>
              ...
              <th v-for="(h,hdx) in headerList" :key="h.name" :class="{'w30':hdx==headerList.length-1,'stickylastcol':(hdx==headerList.length-1) && flcol}">
                {{ h.text }}
              </th>
          </tr>
          <tr>
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td :class="{ 'highlight1' : idx==(currentrecord%recordperpage),'nowrap w30 center':hdx==headerList.length-1,'stickylastcol':(hdx==headerList.length-1) && flcol }" v-for="(h,hdx) in headerList" :key="h.name">
...
export default {
  name: 'gridview',
  props: [ 'id','datalist','headers','flcol','filegroup','lastcoltemplate' ],
  watch: {
