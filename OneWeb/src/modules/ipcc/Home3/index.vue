...
<script>
  ...
  import ThongTinChiTietOB from './components/ThongTinChiTietOB.vue'
  import TraCuuKhachHang from "@/modules/ipcc/ReceivingCallsSwitchboard/components/TraCuuDataMig.vue"
  import DanhSachThueBaoKhachHang from "@/modules/ipcc/ReceivingCallsSwitchboard/components/DanhSachThueBaoKhachHang.vue"
    ...
    methods: {
      ...
      gview_DanhSach_FocusedRowChanged: function (dr) {
        ...
        if (dr && dr.length > 0) {
          ...
          this.makh = dr[0]['ma_kh'] ? dr[0]['ma_kh'] : this.makh
          this.tenkh = dr[0]['ten_tb'] ? dr[0]['ten_tb'] : this.tenkh
        }
      },
      // Change current tab
      tab_Detail_SelectedPageChanged: async function (currentTab) {
        try {
          ...
          if (currentTab == 2) {
            ...
            await this.$refs.refThongTinChiTietOBlComponent.getListResultOB(this.tinh_id, this.khachhang_id)
          }
      ...
      async changeAgentState(status, reason_code) {
        ...
        try {
          if (this.node.show && (
            this.node.value == 0 || this.node.value == null || this.node.value == '')) {
          ...
          if (this.node.show) {
            let rs = await API.changeAgentStateV2(this.axios, params)
          } else {
