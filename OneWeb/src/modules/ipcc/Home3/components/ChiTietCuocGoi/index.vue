    ...
    watch: {
      isCalling: async function(newVal, oldVal) { // watch it
        ...
        if (newVal == 1) {
          ...
          this.maynhan = await this.$root.token.getMachine()
        } else if (newVal == 3 || newVal == 4 || newVal == 5) {
          // TH khi nhấn nút load call trên giao diện
          this.giobatdau = this.thongtincuocgoi.TimeStamp
          this.gioketthuc = this.thongtincuocgoi_end.TimeStamp
          this.maynhan = await this.$root.token.getMachine()
        } else {
          this.giobatdau = null
          this.gioketthuc = null
