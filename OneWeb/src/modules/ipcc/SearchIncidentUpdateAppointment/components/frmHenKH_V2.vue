    ...
    <ejs-dialog :enableResize="true" :allowDragging="true" :visible="false" :animationSettings="animationSettings" ref="dialogObjHenKH" :position="position" :close="closeDialog" :header="'Cập nhật thông tin hẹn'" showCloseIcon="true" width="80%" target="#app .main-wrapper">
      <div class="popup-box">
        <div class="list-actions-top">
          <ul class="list">
            <li>
              <a href="javascript:void(0);" @click.prevent="tsbtnCapNhat_Click" rel="frmHenKH_V2"> <span class="icon one-reload1"></span>Cập nhật </a>
            </li>
            <li>
...
export default {
  ...
  created() {
    this.dtpGioHen_Tu = moment().format(this.dateFormat)
    this.dtpGioHen_Den = moment().format(this.dateFormat)
  },
  methods: {
    openDialog() {
      try {
    ...
    async map_ttbh_id(baohong_id) {
      ...
      let data = this.GetData(await api.fn_tt_baohong(this.axios, { param: baohong_id, type: 4 }))
      return data
    },
    async map_ngay_bh(baohong_id) {
      ...
      // let data = this.GetData(await api.map_id(this.axios, input))
      let data = this.GetData(await api.fn_tt_baohong(this.axios, { param: baohong_id, type: 3 }))
      return data
    },
    async tsbtnCapNhat_Click() {
      try {
        ...
        if (this.Quyen_Duoc_hen == 2) {
          if ((await this.capnhat_giohen_v2()) == false) return
        }
        if (this.Quyen_Duoc_hen == 1) {
          if ((await this.capnhat_giohen_ttkd()) == false) return
        }
    ...
    async capnhat_giohen_v2() {
      ...
      let response = await frmApi.capnhat_giohen_v2(this.axios, input)
    ...
    async capnhat_giohen_ttkd() {
      let input = { vbaohong_id: this.baohong_id, vgiohen_den: this.dtpGioHen_Den, vgiohen_tu: this.dtpGioHen_Tu, vlydohen_id: this.cboLyDoHen, vnoidung: this.txtGhiChu, vphieu_id: this.phieu_id }
      let response = await frmApi.capnhat_giohen_ttkd(this.axios, input)
      if ((response.data.error === 200 || response.data.error === '200') && response.data.error_code == 'BSS-00000000') {
        return true
      } else {
        this.ShowError(`${response.data.error_code}-${response.data.message}`)
