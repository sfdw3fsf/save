...
import KetquaCSKH_OB from "./popups/KetquaCSKH_OB/KetquaCSKH_OB.vue";
...
export default {
  ...
  async created() {
    ...
    EventBus.$on('emitTimKiemLichSuBaoHong', this.emitTimKiemLichSuBaoHong)
  ...
  methods: {
    ...
    tsbtnTimKiem_Click_V2() {
      try {
        if (this.txtMaTB != '' && this.tmp_tinh_id != '') {
          this.Loading = true

          EventBus.$emit('emitChuyenTinh', {id: String(this.tmp_tinh_id), reload : false, recall : 'emitTimKiemLichSuBaoHong'});
        }
      } catch (err) {
        this.ShowError(`${err.message}`)
      } finally {
    ...
    async gridviewDanhSach_RowClicked(i, item) {
      ...
      //this.$refs.ketquaCSKH_OBModal.showModal();
    },
  ...
  destroyed() {
    ...
    EventBus.$off('emitTimKiemLichSuBaoHong', this.emitTimKiemLichSuBaoHong)
  }
}
</script>
