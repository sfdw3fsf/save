<template>
  <div>
    ...
    <b-modal title="Tra Cứu Danh Bạ" id="popupSearchAccount" size="xl"
             hide-footer
             hide-header
             hide-header-close
             body-class="modal-body p-0">
      <SearchAccount :mathuebao='mathuebao' :isPopup="true" @form-close="formClose"></SearchAccount>
    </b-modal>

    <b-modal title="Tra Cứu Hóa Đơn Điện Tử" id="popupSearchEBillByCustomer" size="xl"
             hide-footer
             body-class="modal-body p-0">
      <div class="popup-body">
        <SearchEBillByCustomer :ma_gd='ma_gd' :mathuebao='mathuebao' :isPopup="true"  @form-close="formClose"></SearchEBillByCustomer>
      </div>
    </b-modal>
  </div>
</template>
...
import SearchEBillByCustomer from '@/modules/search/subscriber/SearchEBillByCustomer';

export default {
  name: "DanhSachThueBaoKhachHang",
  components: { KDataGrid, SearchAccount, SearchEBillByCustomer },
  props: ['dulieu'],
  data: () => ({
    mathuebao: null,
    ma_gd: null,
        ...
        },
        {
          ...
          items: [
            ...
            },
            {
              id: 'frmTraCuuDanhBa',
              tag: '',
              text: 'Tra cứu danh bạ',
              path: 'https://onebss.vnpt.vn/#/search/SearchAccount?ma_tb=input_matb',
            },
            {
              id: 'frmTraCuuHoaDonDienTu',
              tag: '',
              text: 'Tra cứu hóa đơn điện tử',
              path: 'https://onebss.vnpt.vn/#/search/SearchEBillByCustomer?ma_tb=input_matb',
            },
          ]
        },
        {
          id: 'frmHoTroKhachHang',
          tag: '',
          text: 'Hỗ trợ khách hàng',
          path: '/#/cskh/TiepNhanHoTroKhachHang?thueBaoId=input_matb',
        },
        {
          id: 'frmBaoHong',
          tag: '',
          ...
          items: [
            {
              ...
              path: 'https://onebss.vnpt.vn/#/incident/ReceiveIncident?ma_tb=input_matb&taikhoan=input_matb&sdt=input_sdt&tinh=input_tinh&matinh=input_matinh&lan_bh=input_lan_bh&dichvuvt_id=dichvuvtid&loaitb_id=loaitbid',
            },
            {
              id: 'frmTraCuuPhieuBH',
              tag: '',
  ...
  methods: {
    ...
    getMaGD: async function (input) {
      let data = {}
      try {
        const response = await api.lay_tt_lapmoi_theo_thuebao(this.axios, input);
    ...
    async contextMenu_Click(args) {
      ...
      if (selected && selected.length > 0) {
        ...
        sdt = this.so_tb ? this.so_tb : (selected[0]['so_dt'] ? selected[0]['so_dt'] : selected[0]['ma_tb'])
		    //sdt = selected[0]['so_dt'] ? selected[0]['so_dt'] : selected[0]['ma_tb']
        ...
        lan_bh = selected[0]['lan_bh']
        if (lan_bh) path = path.replaceAll('input_lan_bh', lan_bh)
        else path = path.replaceAll('input_lan_bh','')
        ...
        if (path.includes('SearchAccount')) {
          this.$bvModal.show('popupSearchAccount');
          return;
        }

        if (path.includes('SearchEBillByCustomer')) {
          let data = await this.getMaGD({ "p_phanvung_id": tinh_id, "p_thuebao_id": thuebao_id });
          if (data) this.ma_gd =  data.ma_gd

          this.$bvModal.show('popupSearchEBillByCustomer');
          return;
        }
