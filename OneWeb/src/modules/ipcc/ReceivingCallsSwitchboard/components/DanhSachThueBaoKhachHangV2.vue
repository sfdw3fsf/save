<template>
  <div>
    ...
    <ModalChonDiaChi modalId="popupDiaChi_KH"  @xacnhan="getDiaChi" />
  </div>
</template>
<script>
...
import ModalChonDiaChi from '../popup/popupChonDiaChi'

export default {
  name: "DanhSachThueBaoKhachHang",
  components: { KDataGrid , ModalChonDiaChi},
  props: ['dulieu'],
  data: () => ({
    tmp_path: null,
    cuocgoi_id: 0,
    ...
    grid_DanhSach: {
      contextMenuItems: [
        ...
        },
        {
          ...
          path: 'https://onebss.vnpt.vn/#/cskh/TiepNhanHoTroKhachHang?thueBaoId=input_matb',
        },
        {
          id: 'frmBaoHong',
          tag: '',
  ...
  methods: {
    ...
    async contextMenu_Click(args) {
      ...
      if (selected && selected.length > 0) {
        ...
        if (args.item.id != 'frmTiepNhanBH' && selected[0]['loaitb_id'] != 271) { //Không phải MYOTT
      ...
      if (args.item.id == 'frmTiepNhanBH' && selected[0]['loaitb_id'] == 271) { // MYOTT
        // Show popup chọn tỉnh
        this.tmp_path = path;
        return this.$bvModal.show('popupDiaChi_KH');
      }

      window.open(path, '_blank')
    },
    replaceQueryParam(url, paramName, newValue) {
    ...
    getDiaChi(data) {
      ...
      if (data.tmp_tinh != '') {
        ...
        const myArray = data.tmp_tinh.split("-");
        console.log(myArray)

        EventBus.$emit('emitChuyenTinh', {id: String(myArray[0]), reload : false});

        let updatedUrl = this.replaceQueryParam(this.tmp_path, 'tinh', myArray[0]);
        updatedUrl = this.replaceQueryParam(updatedUrl, 'matinh', myArray[1]);
