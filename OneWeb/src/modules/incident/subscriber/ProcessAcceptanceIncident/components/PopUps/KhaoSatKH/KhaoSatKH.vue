...
import LichSu_BaoHong from '../../../../HandleAcceptanceIncidentReports/Popups/LichSu_BaoHong/LichSu_BaoHong.vue'
import TraCuuTBKS from '../../../../HandleAcceptanceIncidentReports/Popups/TraCuuTBKS/TraCuuTBKS.vue'
    ...
    methods: {
        ...
        async MAP_ID(id_neo, table, dk) {
            ...
            try {
                ...
                let response = await api.ur_41003_5_007_1_map_id(this.axios, params);
                if (response && response.data && response.data.error_code === 'BSS-00000000' && response.data.data) {
                    result = response.data.data
                }   
            } catch (e) {
        ...
        async tsbtnGhiLai_Click() {
            ...
            try {
                ...
                if ((this.isNew && !this.vdacapnhat) || this.vmophieu == 1) {
                    let isContinue = true
                    try {
                        let strKT = await this.MAP_ID("trangthai", "fims.tb_traloi", "where baohong_id = " + this.baohong_id);
                        if (strKT == "1") {
