...
import FilterTemplate from '@/components/Controls/DataGrid/FilterTemplate'
import DetailTemplate from '@/modules/incident/subscriber/SearchIncidentUpdateAppointment/DetailTemplate.vue'
import { Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate, ContextMenu, LazyLoadGroup, DetailRow } from '@syncfusion/ej2-vue-grids'
...
export default {
  ...
  provide: {
    grid: [Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate, ContextMenu, LazyLoadGroup, DetailRow]
  },
  props: {
    columns: {
      type: Array,
        ...
        if (!(this.filterConditions == null || this.filterConditions.length == 0)) {
          this.filterConditions.forEach((filterCondition) => {
            result = result.filter((x) =>
              x[filterCondition.fieldName] == null || x[filterCondition.fieldName] == undefined ? false : filterCondition.value == null || filterCondition.value == undefined ? true : x[filterCondition.fieldName].toString().toLowerCase().includes(filterCondition.value.toString().toLowerCase())
            )
          })
        }
      }
  ...
  methods: {
    ...
    getGroupedCaptionTemplate: function () {
      ...
      if (cs != null) {
        for (let i = 0; i < cs.length; i++) {
          ...
          if (this.checkIsNull(c.captionTemplate)) {
            rs = '${key} - ${count} Bản ghi'
            break
          } else {
    ...
    grid_detailDataBound: function (args) {
      console.log('grid_detailDataBound',args)
