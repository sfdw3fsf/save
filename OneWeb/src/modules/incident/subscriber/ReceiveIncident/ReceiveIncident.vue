...
import frmTruyenThongSuCo from './popups/frmTruyenThongSuCo/frmTruyenThongSuCo.vue'
...
import API from './API'
import { DichVuVienThong, DS_KHONG_XULY, KENHTN_BAOHONG, LOAI_DV, LoaiHinhTB, LOAIHONG, NHOM_TT, NHOMTON, TRANGTHAI_BH, TrangThaiTB } from '../../../../const/enums'
import moment from 'moment'

import popupChonDiaChi from '@/modules/contract/setup/InstallNewSubs/Popups/popupChonDiaChi'

import LogBNM from '@/modules/EXTS/QUANTRI/ThayDoiThongSoInternet/LogBNM'
import ChiTietKetNoiVFI from '@/modules/EXTS/QUANTRI/ThayDoiThongSoInternet/ChiTietKetNoiVFI'
...
import ModalKhoaPhieu from './popups/KhoaPhieu'
...
export default {
  ...
  watch: {
    ...
    async 'tttn.dichvuvt_id'(val) {
      await this.loadCbLoaiHinhTB(val)
      await this.loadCbNhomNN(val)
    }
  },
  methods: {
    async frmLoad() {
    ...
    async xuLyTag() {
      ...
      this.kt_bh_mytv_ott = false
      let { dichvuvt_id, loaitb_id } = this.$route.query
      ...
      if (this.$route.query.tinh != null && this.$route.query.tinh !== '' && this.$route.query.tinh.length > 0) {
        this.i_tinh = this.$route.query.tinh
      }

      if (this.$route.query.matinh != null && this.$route.query.matinh !== '' && this.$route.query.matinh.length > 0) {
        this.i_matinh = this.$route.query.matinh
      }
      ...
      console.log('tagFromRoute')
      console.log(tagFromRoute)
      // try {
      // if (tagFromRoute !== '') {
      let words = tagFromRoute.toString().split('+')
      if (words.length >= 1) this.tag = parseInt(words[0].toString())
      if (words.length >= 2) this.tag_BH_AO = parseInt(words[1].toString())
      if (words.length >= 3) this.tag_TuDong_LayTT = parseInt(words[2].toString())
      ...
      } else if (this.dichvuvt_id == 4 && this.loaitb_id == 271) {
        this.tttn.dichvuvt_id = 4
        this.tttn.loaihinh_id = 271
        this.kt_bh_mytv_ott = true
        this.tttn.ma_tb = this.i_maTB
      } else {
        console.log('here 574')
        if (this.i_maTB) {
          this.tttn.ma_tb = this.i_maTB
          await this.onEnterMaTB()
        }
    ...
    async initParams() {
      ...
      if (this.kt_bh_mytv_ott) {
        this.kt_bh_mytv_ott = true
        this.tttn.dichvuvt_id = DichVuVienThong.ADSL
        this.tttn.loaihinh_id = LoaiHinhTB.INTERNET_MYTV_OTT
      }
    ...
    async load() {
      ...
      try {
        ...
        let vuser_alt = await this.newMapData('/web-tratruoc/thongtin_tratruoc_dn/fn_tt_tinh', {
        ...
        let vpass_alt = await this.newMapData('/web-tratruoc/thongtin_tratruoc_dn/fn_tt_tinh', {
          param: this.$root.token.getPhanVungID(),
          type: 3
        })
        ...
        this.tttn.ngay_hentl_tu = this.tttn.ngay_hentl_den = this.tttn.ngay_bh = this.kqxl.ngayXN = this.ttHong.ngayTH = ngay_ht
        this.kqxl.cboKQXL = 0

        await this.loadCbDichVu()

        console.log(this.tttn.dichvuvt_id)
        ...
        if (this.KHOAPHIEU_BH_HTKH === 1) {
          //Nếu quyền khóa phiếu báo hỏng chưa nghiệm thu
          this.btnVisible = {
            ...this.btnVisible,
            tsbtnNhapMoi: false,
            tsbtnTiepNhan: false,
    ...
    async HienThi_Danhsach_Tiepnhan() {
      console.log('HienThi_Danhsach_Tiepnhan')
      try {
        let rs
        if (this.tag == 9) rs = await API.lay_ds_tb_tiepnhan_bh_tsl(this.axios)
        else {
          this.loading(true)
          rs = await API.lay_ds_tb_tiepnhan_bh(this.axios, {
            vphanvung_id: this.kt_bh_mytv_ott ? this.tinhld_id_ott : null
          })
          this.loading(false)
        }
          ...
          if (dtBH != null) {
            try {
              ...
              for (let i = 0; i < dtBH.length; i++) {
                if (dtBH[i]['dichvuvt_id'] == '9' || dtBH[i]['dichvuvt_id'] == '25') {
                  let bh_id = parseInt(dtBH[i]['baohong_id'])
                  let bhbc = await this.sp_ds_baohong_bc(bh_id)
                  if (bhbc != null) {
                    if (dtBH[i]['kieudv_id'] != bhbc[0]['daucuoi_id']) continue
                  }
                }
            ...
            // this.$refs.dsBaoHong.dsIdChon = this.dsBaoHong.length > 0 ? [this.dsBaoHong[0].baohong_id] : []
    ...
    clear() {
      ...
      this.tinhld_id_ott = 0
      this.quanld_id_ott = 0
      this.phuongld_id_ott = 0
      this.phold_id_ott = 0
      this.dacdiemld_id_ott = 0
      this.apld_id_ott = 0
      this.khuld_id_ott = 0
      this.sonhald_ott = ''
    },
        ...
        if (ma_tb != '') {
          if (this.KHOAPHIEU_BH_HTKH == 0) {
            ...
            if (data != null) {
              ...
              if (this.$route.query.lan_bh != null && this.$route.query.lan_bh !== '' && this.$route.query.lan_bh.length > 0) {
                this.ttHong.txtGhiChu = this.$route.query.lan_bh
              }
              ...
              if (this.tttn.dien_thoai == '') this.tttn.dien_thoai = data[0]['so_dt']
      ...
      try {
        if (thuebao != null) {
          ...
          if (this.KHONGCHAN_NHANHONG == 1) {
            //Nếu kcos tham số này thì cảnh báo nhận hỏng nếu có nhưng vẫn cho nhận
          ...
          if (trangthaitb_id == TrangThaiTB.DOISO) {
            if (this.NHANHONG_TB_KHOAMAY) {
              let confirm = await this.$confirm(`Thuê bao ${ma_tb} đổi số ngày ${ngay_cat}. Bạn có đồng ý tiếp tục?`, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
          ...
          if (trangthaitb_id == TrangThaiTB.THANHLY || trangthaitb_id == TrangThaiTB.THANHLY_NO) {
            if (this.NHANHONG_TB_KHOAMAY) {
              let confirm = await this.$confirm(`Thuê bao ${ma_tb} thanh lý ngày ${ngay_cat}. Bạn có đồng ý tiếp tục?`, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
                .catch(() => false)
              if (!confirm) return
              else this.nhanhong_khoamay = true
            } else {
          ...
          if (trangthaitb_id == TrangThaiTB.TAMDUNG || trangthaitb_id == TrangThaiTB.KHOA_HAICHIEU_YEUCAU) {
            if (this.NHANHONG_TB_KHOAMAY) {
              let confirm = await this.$confirm(`Thuê bao ${ma_tb} tạm dừng ngày ${ngay_td}. Bạn có đồng ý tiếp tục?`, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
          ...
          if (trangthaitb_id == TrangThaiTB.KHOA_HAICHIEU_CUONGBUC) {
            if (this.NHANHONG_TB_KHOAMAY) {
              let confirm = await this.$confirm(`Thuê bao ${ma_tb} đang bị khóa 2 chiều ngày ${ngay_td}. Bạn có đồng ý tiếp tục?`, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
          ...
          if (this.NHANHONG_TB_KHOAMAY && (trangthaitb_id == TrangThaiTB.KHOA_MOTCHIEU_YEUCAU || trangthaitb_id == TrangThaiTB.KHOA_MOTCHIEU_CUONGBUC) && dichvuvt_id != DichVuVienThong.CO_DINH && dichvuvt_id != DichVuVienThong.IMS) {
            this.nhanhong_khoamay = true
          }
          ...
          if (dtck != null) {
            if (dtck[0].chan_baohong == '1') {
              ...
              let confirm = await this.$confirm(`Bạn có muốn lưu thông tin báo hỏng không?`, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
              ...
              this.$refs.frmCatChuyenBH.initData(dtck[0]['dkcc_id'], this.tttn.thuebao_id, thuebao.ma_tb, this.tttn.dichvuvt_id, thuebao.loaitb_id)
              this.$refs.frmCatChuyenBH.showModal()

              return
            } else {
              if (dtck[0].ngaykt_dk == '') {
                let confirm = await this.$confirm('Thuê bao đang cắt chuyển cáp! \n' + 'Ngày yêu cầu cắt chuyển: ' + dtck[0].ngay_yc + '.\n' + 'Người cập nhật: ' + dtck[0].nguoi_cn + '.\n' + 'Bạn muốn tiếp tục nhận hỏng ? ', 'Thông báo', {
                  confirmButtonText: 'Có',
                  cancelButtonText: 'Không'
                })
                  .then(() => true)
    ...
    HienThiChiTietBH(thuebao) {
      ...
      if (thuebao != null) {
        let { ma_tb, thuebao_id, ma_lt, so_dt, dichvuvt_id, loaitb_id, ten_tb, diachi_ld, diachi_tb, trangthai_tb, dactrung, khachhang_id } = thuebao
        ...
        if (this.KHOAPHIEU_BH_HTKH == 0) this.HT_THONGSO_KYTHUAT(thuebao_id, dichvuvt_id)
    ...
    async popuptientrinh(thuebao_id, kt, vdichvuvt) {
      this.loading(false)
      let done = false
      await this.$confirm(`${kt}, Bạn muốn chuyển đến tiến trình báo hỏng không?`, 'Thông báo', {
        confirmButtonText: 'Có',
        cancelButtonText: 'Không'
      })
        .then(async () => {
    ...
    acceptSearchAccount(item) {
      ...
      this.tmp_thuebao_id = item.thuebao.thuebao_id
      this.onEnterMaTB()
    },
      ...
      if (this.kt_kenhtn_baohong) {
        ...
        if (item.nguoi_cn != null) {
          ...
          if (nguoi_cn == 'baohong_website') {
            ...
            this.taomoi_mabh = false
          } else this.wsappmtv = KENHTN_BAOHONG.KENHTONGDAI
        } else {
          this.wsappmtv = KENHTN_BAOHONG.KENHTONGDAI
        }
        ...
        if (item.nguoi_cn == 'app_myvnpt') this.wsappmtv = KENHTN_BAOHONG.KENH_APPMYTV
        else this.wsappmtv = KENHTN_BAOHONG.KENHTONGDAI
      }
    ...
    async btnGhiLai_Click() {
      if (this.isLoading) {
        this.$toast.info('Hệ thống đang trong quá trình xử lý, vui lòng chờ')
        return
      }
      ...
      this.loading(true)
      try {
        //#region Khối dành riêng cho Mytv OTT
        if (this.kt_bh_mytv_ott && this.tttn.loaihinh_id == LoaiHinhTB.INTERNET_MYTV_OTT) {
          console.log('btnGhiLai_Click this.kt_bh_mytv_ott')
          await this.BaoHong_MyTV_OTT()
          return
        }
        //#endregion
        ...
        if (this.ttHong.txtGhiChu == '' && this.noidung_sc != '') this.ttHong.txtGhiChu = this.noidung_sc.replaceAll('Bạn có muốn tiếp tục nhận hỏng không?', '')
        ...
        if (cboNhomTT != NHOM_TT.HOTRO_KT) {
          ...
          if (dtkt != null) {
            if (this.NHANHONG_TB_CHUAHT_HOPDONG == 1) {
              let confirm = await this.$confirm(`Thuê bao đang có hợp đồng ${dtkt[0].ten_loaihd} chưa hoàn thiện! Bạn có muốn tiếp tục nhận hỏng?`, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
          ...
          if (dtSuCo != null) {
            ...
            if (hongsuco_file) {
              let confirm = await this.$confirm(kiemtra_chan, 'Thông báo', {
                confirmButtonText: 'Có',
                cancelButtonText: 'Không'
              })
                .then(() => true)
        ...
        if (dtck != null && cboNhomTT != NHOM_TT.HOTRO_KT) {
          if (dtck[0].chan_baohong == '1') {
            ...
            this.$refs.frmCatChuyenBH.initData(dtck[0]['dkcc_id'], this.tttn.thuebao_id, this.tttn.ma_tb, this.tttn.dichvuvt_id, this.tttn.loaihinh_id)
            this.$refs.frmCatChuyenBH.showModal()
        ...
        if (this.PHAILAM('DOKIEM_TN') && this.tttn.dichvuvt_id == DichVuVienThong.ADSL) {
          ...
          let s = this.s
          if (s === 'DOWN' || s === 'KEM' || s.toUpperCase() === 'THUÊ BAO CHƯA ĐƯỢC CẤP PORT!') {
            //down
        ...
        if (this.PHAILAM('TUDONG_GIAOPHIEU_HC') && (this.giaophieu_bdg || this.giaophieu_tudong) && !this.nhanhong_khoamay) {
          //Tự động giao phiếu
    ...
    async DoKiem() {
      try {
        ...
        if (this.userInfo.att_adsl <= 0 && this.userInfo.att_gpon <= 0 && this.userInfo.snr_adsl <= 0) return 'OK'
        if (this.vuser_alt == '' || this.vpass_alt == '') return 'OK'
        //Đo kiểm trước khi nghiệm thu
        if (this.tttn.dichvuvt_id != DichVuVienThong.ADSL) return 'OK'
    ...
    async getPortStatus(vInput, vdichvuvt_id) {
      try {
        ...
        //Đo kiểm trước khi nghiệm thu
        if (vdichvuvt_id != DichVuVienThong.ADSL) return ''
    ...
    TaoDuLieu_nguyennhan(themmoi) {
      let ds_nn = []
      if (this.kqxl.cboKQXL == 0 && this.kqxl.cboNguyenNhan) {
        ...
        for (const nnId of this.kqxl.cboNguyenNhan) {
          ...
          row.CTHONG_ID = nnId
          if (this.kqxl.cboNhomNN == LOAIHONG.HONG_THIETBI_DAUCUOI && this.tttn.dichvuvt_id == DichVuVienThong.ADSL) row.HANGSX_ID = this.kqxl.cboHangSX
          ds_nn.push(row)
        }
      }
    ...
    TaoDuLieu_baoton(themmoi) {
      let ds_ton = []
      if (this.kqxl.cboKQXL == 1 && this.kqxl.cboLyDoTon) {
        // Nếu không xử lý được thì phải chọn lý do tồn
    ...
    async LayDS_SDT_DonViNhan(vdonvi_nhan_id, _vthuebao_id, _matb, _vnoidung, _vphieuid, _vhuonggiao_id) {
      ...
      if (_StringsoDT == '-1' || _StringsoDT == '') {
        if (_StringsoDT_nv != '') _StringsoDT = _StringsoDT_nv
      } else _StringsoDT = _StringsoDT + ',' + _StringsoDT_nv

      //Gán số điện thoại vào datable
      if (_StringsoDT != null && _StringsoDT != '' && _StringsoDT != '-1') {
        if (_StringsoDT.endsWith(',')) _StringsoDT = _StringsoDT.slice(0, -1)
    ...
    async POST_GET_KEYS(keyname) {
      ...
      let vphanvung_id = null
      if (this.kt_bh_mytv_ott) {
        vphanvung_id = this.tinhld_id_ott
      }
      await API.POST_GET_KEYS(this.axios, {
        phanvung_id: vphanvung_id,
        keyname: keyname
      })
        .then((response) => {
    ...
    KiemTra_BH_Ao() {
      ...
      if (this.kt_bh_mytv_ott && this.tttn.loaihinh_id == LoaiHinhTB.INTERNET_MYTV_OTT) {
        if (this.phuongld_id_ott == 0 || this.tttn.diachi_ld.trim() == '') {
          this.$root.$toast.error('Bạn hãy nhập địa chỉ lắp đặt thuê bao!')
          return false
        }
        if (this.dsBHDV_Mytv_OTT.length == 0) {
          this.$root.$toast.error('Chưa có thông tin về đơn vị thi công. Hãy chọn lại địa chỉ lắp đặt')
          return false
        }
      }
    ...
    async TaoDuLieu_BaoHong_Ao(themmoi) {
      console.log('TaoDuLieu_BaoHong_Ao')
      let dsBH_DATA = []
      ...
      row.MA_TB = this.tttn.ma_tb
      if (this.tttn.loaihinh_id == LoaiHinhTB.INTERNET_MYTV_OTT) {
        row.LOAITB_ID = LoaiHinhTB.INTERNET_MYTV_OTT
      } else {
    ...
    TaoDuLieu_BaoHong_Ao_DV(themmoi) {
      ...
      if (this.kt_bh_mytv_ott && this.tttn.loaihinh_id == LoaiHinhTB.INTERNET_MYTV_OTT) {
        row.BAOHONG_ID = this.baohong_id
        row.DONVI_ID = this.dsBHDV_Mytv_OTT[0].donvi_id
        row.LOAIDV_ID = 5
        row.KIEUDV_ID = 2
      } else {
        row.BAOHONG_ID = this.baohong_id
    ...
    async tsbtnCapNhatDoThu_Click() {
      try {
        ...
        if (this.tttn.dichvuvt_id == DichVuVienThong.ADSL && this.kqxl.cboNhomNN == LOAIHONG.HONG_THIETBI_DAUCUOI && this.tttn.loaihinh_id == LoaiHinhTB.INTERNET_FTTH) {
    ...
    async tsbtnXuatPhieu_Click() {
      try {
        ...
        if (NHANHONG_TB_KHOAMAY && trangthaitb_id == TrangThaiTB.KHOA_MOTCHIEU_YEUCAU && dichvuvt_id != DichVuVienThong.CO_DINH && dichvuvt_id != DichVuVienThong.IMS) {
          this.$root.$toast.error('Bạn đang nhận hỏng thuê bao khóa 1 chiều theo yêu cầu. Không thể giao phiếu!')
          return
        }
        if (NHANHONG_TB_KHOAMAY && trangthaitb_id == TrangThaiTB.KHOA_MOTCHIEU_CUONGBUC && dichvuvt_id != DichVuVienThong.CO_DINH && dichvuvt_id != DichVuVienThong.IMS) {
        ...
        // await f1.onSeach();
        if (this.HandoverIncidentCom) await this.onGiaoPhieuMounted()
        else {
    ...
    async onGiaoPhieuMounted() {
      ...
      //Check Giữ phiếu
      if (this.CHOPHEP_GIUPHIEU == 1) {
        if (!(await this.KIEMTRA_PHIEU('-1', 0, 3))) return
      }
    ...
    async tsbtnCapNhatPhieuTam_Click() {
      try {
        if (this.dsBaoHong.length <= 0 || this.itemDanhSachFocused == null) {
          this.$root.$toast.error('Không có thuê bao báo hỏng để giao phiếu!')
          return
        }

        if (this.tmp_thuebao_id == 0) {
          this.$root.$toast.error('Bạn chưa nhập Số máy/Acc !')
          return
        }

        //kiểm tra thiet bi tren su co cho HCM
        let rs = await API.sp_capnhat_thuebao_baohong_ao_ipcc(this.axios, {
          p_baohong_id: this.itemDanhSachFocused.baohong_id,
          p_thuebao_id: this.tmp_thuebao_id,
          p_parars: ''
        })

        if (rs.data.error_code && rs.data.error_code === 'BSS-00000000') {
          return this.$toast.success('Cập nhật phiếu tạm thành công!')
        }
      } catch (e) {
        return this.$toast.error('Lỗi cập nhật : ' + e.message)
      }
    },
    // tsbtnKhoaPhieuTam_Click
    ...
    async gridviewDanhSach_FocusedRowChanged(item) {
      ...
      try {
        ...
        if (item.baohong_tam == 1) {
          this.btnVisible.tsbtnKhoaPhieu = false
          this.btnVisible.tsbtnKhoaPhieuTam = true
          this.btnVisible.tsbtnCapNhatDoThu = false
          this.btnVisible.tsbtnCapNhatPhieuTam = true
        } else {
          this.btnVisible.tsbtnKhoaPhieu = true
          this.btnVisible.tsbtnKhoaPhieuTam = false
          this.btnVisible.tsbtnCapNhatDoThu = true
          this.btnVisible.tsbtnCapNhatPhieuTam = false
        }
        ...
        if (!kiemtra || this.KHOAPHIEU_BH_HTKH == 1) this.kqxl.cboKQXL = 0
        ...
        if (this.$root.token.getMaTinh() == 'CSS_KGG') {
          ...
          if (dichvuvt_id == '8' || dichvuvt_id == '9' || dichvuvt_id == '26' || dichvuvt_id == '25' || loaitb_id == '14' || loaitb_id == 15 || loaitb_id == 16 || loaitb_id == 17) {
            this.btnVisible = {
        ...
        if (this.PHAILAM('CHUYEN_NET_TIEPNHAN_BH')) this.btnVisible.tsbtnChuyenTTDH = true
        else this.btnVisible.tsbtnChuyenTTDH = false

        if (loaitb_id == LoaiHinhTB.INTERNET_MYTV || loaitb_id == LoaiHinhTB.Voice_Brandname) this.btnVisible.tsbtnChuyenMedia = true
        else this.btnVisible.tsbtnChuyenMedia = false

        if (this.PHAILAM('CHUYEN_IT360_TIEPNHAN_BH')) this.btnVisible.tsbtnChuyenIT360 = true
        else this.btnVisible.tsbtnChuyenIT360 = false

        if (this.PHAILAM('CHUYEN_VIP_TIEPNHAN_BH')) this.btnVisible.tsbtnChuyenVip = true
        else this.btnVisible.tsbtnChuyenVip = false
        ...
        if (this.kt_dambao_ckcl_kh) {
          ...
          } else this.ttHong.chkCamKet = false
        }
      } catch (e) {
        console.log(e)
      }
    ...
    async fn_capnhat_tiepnhanbh_v0() {
      console.log('fn_capnhat_tiepnhanbh_v0')
      try {
        ...
        if (this.KHONGGIAO_HTKT_BAOHONG == -1 || this.ttHong.cboNhomTT != NHOM_TT.HOTRO_KT) {
          // this.PHAILAM('DIEUKIEN_GIAOPHIEU_BP_TIEPTHEO') &&
          if (!this.nhanhong_khoamay) {
            console.log('vao day')
    ...
    async fn_capnhat_tiepnhanbh_v1() {
      console.log('fn_capnhat_tiepnhanbh_v1')
      try {
    ...
    tsbtnBoQua_Click() {
      ...
      try {
        ...
        this.$confirm('Bạn chắc chắn muốn bỏ qua phiếu cần xử lý không?', 'Thông báo', {
          confirmButtonText: 'Có',
          cancelButtonText: 'Không'
        }).then(() => {
          this.loading(true)
          this.sp_capnhat_trangthai_chiaphieu_vsecond(this.itemDanhSachFocused.tinh_id, this.baohong_id, this.$root.token.getUserName())
          this.HienThi_DanhSach_Tb_BaoHong_Nhanhkhac(0, 0)
          this.loading(false)
        })
      } catch (ex) {
    ...
    sp_capnhat_trangthai_chiaphieu_vsecond(phanvung_id, p_baohong_id, p_ma_nd) {
    ...
    async tsbtnXoaPhieu_Click() {
      try {
        ...
        let confirm = await this.$confirm(`Bạn thật sự muốn xoá ${lst_chon.length} phiếu báo hỏng này không ??`, 'Thông báo', {
          confirmButtonText: 'Có',
          cancelButtonText: 'Không'
        })
          .then(() => true)
    ...
    async tsbtnKhoaPhieu_Click(result) {
      ...
      try {
        if (this.frmTuVanMyTVCom && this.$refs.dialogfrmTuVanMyTV) this.$refs.dialogfrmTuVanMyTV.hideDialog() // this.$refs.dialogfrmTuVanMyTV dang bi undefined
      } catch (ex) {}
      try {
    ...
    async KiemTra_KhoaPhieu() {
      ...
      if (this.CHOPHEP_GIUPHIEU == 1) {
        if (!(await this.KIEMTRA_PHIEU('-1', 0, 3))) return false
      }
        ...
        let vds = [
          {
            ...
            SO_DT: _row.SO_DT,
            TEN_DAY_DU: this.dtTTCap.lenght > 0 ? this.dtTTCap[0].ten_day_du : "",
    ...
    async tsbtnDoKiem_Click() {
      ...
      this.checkPortDialog.input.vtinh = this.i_tinh ? this.i_tinh : this.tttn.tinh_id
      this.checkPortDialog.input.vmatinh = this.i_matinh ? this.i_matinh : this.tttn.matinh
    ...
    tsmXoaPhieu_Click(itemTiepNhan) {
      ...
      this.$confirm(`Bạn thật sự muốn xoá tiếp nhận thuê bao này không?`, 'Thông báo', {
        confirmButtonText: 'Có',
        cancelButtonText: 'Không'
      }).then(() => {
        this.loading(true)
    ...
    async tsbtnChuyenTTDH_Click() {
      ...
      try {
        ...
        let confirm = await this.$confirm('Bạn chắc chắn muốn chuyển phiếu?', 'Thông báo', {
          confirmButtonText: 'Có',
          cancelButtonText: 'Không'
        })
          .then(() => true)
        ...
        if (this.kqxl.cboKQXL == 1 && this.kqxl.cboNhomTon == NHOMTON.MANG_VT_LOP_TREN) {
          let kt = await this.Giaophieu_donvi_tieptheo_v2(this.baohong_id, this.PHIEU_ID, this.huonggiao_id)
          if (kt != '1') {
            this.$toast.error(kt)
            return
          }
    ...
    async tsbtnChuyenMedia_Click() {
      try {
        ...
        if (this.kqxl.cboKQXL == 1 && this.kqxl.cboNhomTon == NHOMTON.MANG_VT_LOP_TREN) {
          ...
          //#region Cập nhật trạng thái chia phiếu với trường hợp xử lý phiếu tồn báo hỏng nhánh khác
          if (this.tag == 3) {
            await this.sp_capnhat_trangthai_chiaphieu_vsecond(this.itemDanhSachFocused.tinh_id, this.baohong_id, this.$root.token.getUserName())
          }
    ...
    async tsbtnChuyenIT360_Click() {
      try {
        ...
        let confirm = await this.$confirm('Bạn chắc chắn muốn chuyển phiếu IT360?', 'Thông báo', {
          confirmButtonText: 'Có',
          cancelButtonText: 'Không'
        })
          .then(() => true)
        ...
        if (!(await this.CapNhat(true))) return
    ...
    async tsbtnChuyenVip_Click() {
      try {
        ...
        let confirm = await this.$confirm('Bạn chắc chắn muốn chuyển phiếu VIP?', 'Thông báo', {
          confirmButtonText: 'Có',
          cancelButtonText: 'Không'
        })
          .then(() => true)
    ...
    async tsbtnChuyenDHIT_Click() {
      if (this.tttn.dichvuvt_id == DichVuVienThong.DICHVU_CNTT) {
        ...
        if (msgFMs != 'NOK') this.$toast.success('Đã chuyển phiếu báo hỏng sang DHIT hỗ trợ')
        else this.$toast.error('Xảy ra lỗi khi chuyển phiếu báo hỏng sang DHIT')
      } else {
    ...
    async tsbtnTBCungDoiCap_Click() {
      if (this.tttn.thuebao_id === 0 || this.tttn.dichvuvt_id === 0) return
    ...
    tsbtnKhoaPhieuTam_Click() {
      this.$refs.modalKhoaphieutam.txtGhiChuKhoaPhieu = ''

      this.$bvModal.show('modal-khoa-phieu')
    },
    ...
    async dsbhChoXuLy(item) {
      try {
        ...
        let confirm = await this.$confirm(`Bạn thật sự muốn cập nhật trạng thái ${lst_chon.length} phiếu báo hỏng này không?`, 'Thông báo', {
          confirmButtonText: 'Có',
          cancelButtonText: 'Không'
        })
          .then(() => true)
    ...
    async HUY_GIUPHIEU(kieu) {
      try {
        ...
        if (!(await this.KIEMTRA_PHIEU(json_list, 0, kieu))) return
    ...
    async dsbhTTKenhTrang() {
      ...
      if (this.itemDanhSachFocused.nguoi_cn.toLowerCase() === 'webservice_hni' || _o == '0') {
        if (this.CHOPHEP_GIUPHIEU == 1) {
          if (!(await this.KIEMTRA_PHIEU('0', this.itemDanhSachFocused.phieu_id, 3))) return
        }
    ...
    async frmKenhTrang_ok(_frmkenhtrang) {
      ...
      let confirm = await this.$confirm(`Bạn có chắc chắn chọn thuê bao này để thực hiện cập nhật không`, 'Thông báo', {
        confirmButtonText: 'Có',
        cancelButtonText: 'Không'
      })
        .then(() => true)
    ...
    async lblSuCoTTVT_Click() {
      try {
        let data = {
          phanloai: this.suco.phanloai,
          db_id: this.suco.rds_id,
          item: this.suco
        }
    ...
    async ThongTinTiepNhan_tsbtnDiaChiLD_Clicked() {
      console.log('ThongTinTiepNhan_tsbtnDiaChiLD_Clicked')
      // console.log(this.$refs.popupChonDiaChi.show())
      this.$bvModal.show('popupChonDiaChi')
    },
    async popupChonDiaChi_xacnhan(obj) {
      console.log('popupChonDiaChi_xacnhan')
      console.log(obj)
      if ((obj.pho_id != 0 && obj.ap_id != 0) || (obj.pho_id != 0 && obj.khu_id != 0) || (obj.ap_id != 0 && obj.khu_id != 0)) {
        this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, obj.pho_id, obj.ap_id, obj.khu_id, obj.dacdiem_id)
        if (this.dsBHDV_Mytv_OTT.length <= 0) {
          if (obj.pho_id != 0) this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, obj.pho_id, 0, 0, 0)
          else if (obj.ap_id != 0) this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, 0, obj.ap_id, 0, 0)
          else if (obj.khu_id != 0) this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, 0, 0, obj.khu_id, 0)
        }
      } else {
        if (obj.pho_id != 0) this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, obj.pho_id, 0, 0, 0)
        if (obj.ap_id != 0) this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, 0, obj.ap_id, 0, 0)
        if (obj.khu_id != 0) this.dsBHDV_Mytv_OTT = await this.lay_donvi_theo_loaidv_diachi_ld_v2(obj.tinh_id, LOAI_DV.TRAM_VT, obj.phuong_id, 0, 0, obj.khu_id, 0)
      }
      if (this.dsBHDV_Mytv_OTT.length <= 0) {
        this.$toast.warning('Địa danh này chưa được gán Loại đơn vị Trạm thi công. Hãy liên hệ với Admin để xử lý!')
        return
      }
      console.log('this.dsBHDV_Mytv_OTT')
      console.log(this.dsBHDV_Mytv_OTT)
      this.tinhld_id_ott = obj.tinh_id
      this.quanld_id_ott = obj.quan_id
      this.phuongld_id_ott = obj.phuong_id
      this.phold_id_ott = obj.pho_id
      this.dacdiemld_id_ott = obj.dacdiem_id
      this.apld_id_ott = obj.ap_id
      this.khuld_id_ott = obj.khu_id
      this.sonhald_ott = obj.sonha
      this.sonhald_ott = obj.sonha
      this.tttn.diachi_ld = obj.diachimoi
    },
    async lay_donvi_theo_loaidv_diachi_ld_v2(phanvung_id, loaidv_id, phuong_id, pho_id, ap_id, khu_id, dacdiem_id) {
      let kq = await API.lay_donvi_theo_loaidv_diachi_ld_v2(this.axios, { phanvung_id, loaidv_id, phuong_id, pho_id, ap_id, khu_id, dacdiem_id })
      return kq.data.data
    },
    async BaoHong_MyTV_OTT() {
      // try {
      if (!this.KiemTra_BH_Ao()) return
      await this.TaoDuLieu_BaoHong_Ao(true)
      this.TaoDuLieu_BaoHong_Ao_DV(true)
      let kq = await API.fn_capnhat_tiepnhanbh_mytv_ott(this.axios, {
        vphanvung_id: null,
        vds: JSON.stringify([
          {
            HUONGGIAOTN_ID: this.huonggiao_id,
    ...
    tsbtnLogBNM_Clicked() {
      this.$bvModal.show('frmLogBNM')
    },
    tsbtnThongTinVRF_Clicked() {
      this.$bvModal.show('frmChiTietKetNoiVFI')
    }
  }
}
</script>
