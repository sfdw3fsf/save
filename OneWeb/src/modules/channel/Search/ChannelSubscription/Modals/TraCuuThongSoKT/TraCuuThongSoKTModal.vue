    ...
    methods:{
        onActionClick(action){
            if(action.id=='vattu'){
              if(this.loai==0){
                ...
                this.$refs.formVattu.phanvung_id = this.phanvung_id
        ...
        async initDuLieu(){
            ...
            if(this.dichvu_selected==DichVuVienThong.CO_DINH){
              ...
              this.ds_loaithietbi=await this.sp_lay_ds_loai_tbi(this.dichvu_selected,this.phanvung_id)
              ...
              this.ds_tocdokenh=await this.lay_ds_tocdo_kenh()
              this.ds_loaithietbi=await this.sp_lay_ds_loai_tbi(9,this.phanvung_id)
        ...
        async sp_lay_ds_loai_tbi(dichvuvt_id,phanvung_id){
            try{
                this.loading(true)
                let response = await API.sp_lay_ds_loai_tbi(this.axios, dichvuvt_id, phanvung_id)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
        ...
        async lay_ds_db_con_theo_tbid(thuebao_id, dichvuvt_id, phanvung_id){
            try{
                this.loading(true)
                let response = await API.lay_ds_db_con_theo_tbid(this.axios, thuebao_id, dichvuvt_id, phanvung_id)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
          ...
          if(this.loai==0){
            ...
            if(result.length>0){
              ...
              if(dichvuvt_id==DichVuVienThong.CO_DINH){
                ...
                await this.$refs.coDinh.hienthi_dulieu_hdtb(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.ADSL){
                this.input_so_ao=item.ma_lt!=null?item.ma_lt.toString():''
                await this.$refs.bangRongCoDinh.hienthi_dulieu_hdtb(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.MEGAWAN||dichvuvt_id==DichVuVienThong.METRONET){
                this.input_so_ao=item.ma_lt!=null?item.ma_lt.toString():''
                await this.$refs.megawanMetronet.hienthi_dulieu_hdtb(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.TSL){
                await this.$refs.kenhThueRieng.hienthi_dulieu_hdtb(result, id, this.phanvung_id)

              }else if(dichvuvt_id==DichVuVienThong.GPHONE){
                await this.$refs.gphone.hienthi_dulieu_hdtb(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.IMS){
                await this.$refs.ims.hienthi_dulieu_hdtb(item, id, this.phanvung_id)
              }
            }else{
            ...
            let result = await this.lay_ds_db_con_theo_tbid(id, dichvuvt_id, this.phanvung_id)
            if(result.length>0){
              let item=result[0]
              if(dichvuvt_id==DichVuVienThong.CO_DINH){
                await this.$refs.coDinh.hienthi_dulieu_thuebao(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.ADSL){
                this.input_so_ao=item.ma_lt!=null?item.ma_lt.toString():''
                await this.$refs.bangRongCoDinh.hienthi_dulieu_thuebao(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.MEGAWAN||dichvuvt_id==DichVuVienThong.METRONET){
                this.input_so_ao=item.ma_lt!=null?item.ma_lt.toString():''
                await this.$refs.megawanMetronet.hienthi_dulieu_thuebao(item, id,this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.TSL){
                await this.$refs.kenhThueRieng.hienthi_dulieu_thuebao(result, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.GPHONE){
                await this.$refs.gphone.hienthi_dulieu_thuebao(item, id, this.phanvung_id)
              }else if(dichvuvt_id==DichVuVienThong.IMS){
                await this.$refs.ims.hienthi_dulieu_thuebao(item, id, this.phanvung_id)
              }
