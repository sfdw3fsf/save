...
export default {
    ...
    methods:{
        ...
        async hienthi_dulieu_hdtb(data, hdtb_id, phanvung_id){
        ...
        async hienthi_dulieu_thuebao(data, thuebao_id,phanvung_id){
            for(let i=0;i<data.length;i++){
                let item=data[i]
                if(item.daucuoi_id==1){
                    //Điểm đầu
                    ...
                    let lienhe_cuoi=await this.sp_lay_thongtin_lh_db_tsl(thuebao_id, 1,phanvung_id)
                    ...
                    let lienhe_cuoi=await this.sp_lay_thongtin_lh_db_tsl(thuebao_id, 2,phanvung_id)
                    if(lienhe_cuoi.length>0){
        ...
        async sp_lay_thongtin_lh_db_tsl(thuebao_id, daucuoi_id,phanvung_id){
            try{
                this.loading(true)
                let response = await API.sp_lay_thongtin_lh_db_tsl(this.axios, thuebao_id, daucuoi_id,phanvung_id)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
