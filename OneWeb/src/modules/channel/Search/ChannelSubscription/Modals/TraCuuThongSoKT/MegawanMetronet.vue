...
export default {
    ...
    methods:{
        ...
        async hienthi_dulieu_hdtb(item, hdtb_id,phanvung_id){
            ...
            if(item.port_id!=null&&item.port_id.toString()!=''){
                let vl=await this.lay_tt_vatly_theo_port_logic(item.port_id,phanvung_id)
                if(vl.length>0){
        ...
        async hienthi_dulieu_thuebao(item, thuebao_id, phanvung_id){
            let result=await this.sp_ds_loai_tbi_theo_thuebao_id(thuebao_id, phanvung_id)
            if(result.length>0){
                this.input_thietbi_dc=result[0].loai_tbi
            }else{
                this.input_thietbi_dc=''
            ...
            if(item.huong_kn!=null){
                this.input_huongketnoi=await this.fn_lay_text_huongketnoi_v2(item.huong_kn, phanvung_id)
            }else{
                this.input_huongketnoi=''
            }
        ...
        async lay_tt_vatly_theo_port_logic(port_id,phanvung_id){
            try{
                this.loading(true)
                let response = await API.lay_tt_vatly_theo_port_logic(this.axios, port_id,phanvung_id)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
        ...
        async sp_ds_loai_tbi_theo_thuebao_id(thuebao_id, phanvung_id){
            try{
                this.loading(true)
                let response = await API.sp_ds_loai_tbi_theo_thuebao_id(this.axios, thuebao_id, phanvung_id)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
        ...
        async fn_lay_text_huongketnoi_v2(huong_kn, phanvung_id){
            try{
                this.loading(true)
                let response = await API.fn_lay_text_huongketnoi_v2(this.axios, huong_kn, phanvung_id)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
