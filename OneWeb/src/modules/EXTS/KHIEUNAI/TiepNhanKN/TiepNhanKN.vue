...
export default {
  ...
  methods: {
    ...
    async KiemTraDuLieu () {
      ...
      if(!(this.KHIEUNAI.EMAIL == null || this.KHIEUNAI.EMAIL == undefined))
      {
        let mailformat = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/
        if (!this.KHIEUNAI.EMAIL.match(mailformat) && this.KHIEUNAI.EMAIL.length > 0) {
    ...
    gridDS_TB_KHIEUNAI_onQueryCellInfo(args) {
      ...
      if (field == "MA_KN") {
        if (dataField.HETHAN_KN == 1) {
          console.log(dataField.HETHAN_KN)
