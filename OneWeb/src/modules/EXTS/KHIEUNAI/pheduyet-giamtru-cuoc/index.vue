...
export default {
  ...
  computed: {
    ...
    isDongYGiaoPhieu() {
      return this.currentItem.PHEDUYET_GTC.HTTL_ID == 1
    }      
  },
  ...
  methods: {
    ...
    async apiKhoaPhieuDeXuatGTC(params) {
      this.loading(true)
      let result = true
      try {
        const response = await APICommon.khoaPhieuDeXuatGiamTruCuoc(this.axios, params)
        if (response.data.error_code === 'BSS-00000000') {
          this.$root.toastSuccess('Phê duyệt giảm trừ cước thành công!')
        }
      } catch (error) {
        result = false
        console.log('error', error);
    ...
    async onSave() {
      let valid = true
      if (!this.currentItem.PHEDUYET_GTC.GHI_CHU || this.currentItem.PHEDUYET_GTC.GHI_CHU.trim() == '') {
        this.$root.toastError('Ghi chú ĐX GTC là bắt buộc!')
        valid = false
      }
      if (this.currentItem.PHEDUYET_GTC.HTTL_ID == null) {
      ...
      const flag = await this.$confirm('Bạn có muốn lưu Ý kiến ĐX GTC?', 'Xác nhận thực hiện', {
      ...
      const params = {
        ...
        pYKien: this.currentItem.PHEDUYET_GTC.HTTL_ID,
        pNoiDung: this.currentItem.PHEDUYET_GTC.GHI_CHU,        
      }
      const response = await this.apiCreateUpdatePhieuDeXuatGTC(params);
      if (response) await this.onLayDanhSach()
    },
    async onPheDuyetPhieu() {
      if (!this.currentItem.PHIEUKN_ID || !this.currentItem.KHIEUNAI_ID) {
        return this.$root.toastError('Bạn cần chọn phiếu để phê duyệt!')
      }
      if (this.currentItem.PHEDUYET_GTC.HTTL_ID != 1) {
        return this.$root.toastError('Trạng thái phiếu không thể phê duyệt!')
      }
      if (!this.currentItem.PHEDUYET_GTC.GHI_CHU || this.currentItem.PHEDUYET_GTC.GHI_CHU.trim() == '') {
        return this.$root.toastError('Ý kiến phê duyệt giảm trừ cước là bắt buộc!')
      }
      const flag = await this.$confirm('Bạn có muốn phế duyệt GTC?', 'Xác nhận thực hiện', {
        confirmButtonText: 'Có',
        cancelButtonText: 'Không',
        type: 'info'
      })
      ...
      const params = {
        pKhieuNaiId: this.currentItem.KHIEUNAI_ID,
        pPhieuKNId: this.currentItem.PHIEUKN_ID,
        pNoiDung: this.currentItem.PHEDUYET_GTC.YKIEN,
      }
      const res = await this.apiKhoaPhieuDeXuatGTC(params)
