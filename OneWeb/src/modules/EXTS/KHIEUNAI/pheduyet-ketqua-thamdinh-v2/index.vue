...
export default {
  ...
  computed: {
    ...
    isDongYGiaoPhieu() {
      return this.currentItem.PHEDUYET_KQTD.HTTL_ID == 1
    }    
  },
  ...
  methods: {
    ...
    async onSave() {
      let valid = true
      if (!this.currentItem.PHEDUYET_KQTD.GHI_CHU || this.currentItem.PHEDUYET_KQTD.GHI_CHU.trim() == '') {
        this.$root.toastError('Ghi chú ĐX GTC là bắt buộc!')
        valid = false
      }
      if (this.currentItem.PHEDUYET_KQTD.HTTL_ID == null) {
      ...
      const params = {
        ...
        pYKien: this.currentItem.PHEDUYET_KQTD.HTTL_ID,
        pNoiDung: this.currentItem.PHEDUYET_KQTD.GHI_CHU,        
      }
    ...
    async onGiaoPPhieu() {
      try {
        ...
        this.$refs.frmGiaoPhieuGTC.pTTKNId = TRANGTHAI_DX.PD_KQTD
