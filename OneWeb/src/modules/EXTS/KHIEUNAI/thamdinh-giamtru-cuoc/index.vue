...
export default {
  ...
  computed: {
    ...
    isDongYGiaoPhieu() {
      return this.currentItem.THAMDINH_DX_GTC.HTTL_ID == 1
    }
  },
  ...
  methods: {
    ...
    async onSave() {
      let valid = true
      if (!this.currentItem.THAMDINH_DX_GTC.GHI_CHU || this.currentItem.THAMDINH_DX_GTC.GHI_CHU.trim() == '') {
        this.$root.toastError('Ghi chú ĐX GTC là bắt buộc!')
        valid = false
      }
      if (this.currentItem.THAMDINH_DX_GTC.HTTL_ID == null) {
      ...
      const params = {
        ...
        pYKien: this.currentItem.THAMDINH_DX_GTC.HTTL_ID,
        pNoiDung: this.currentItem.THAMDINH_DX_GTC.GHI_CHU,        
      }
    ...
    async onGiaoPhieu() {
      try {
        ...
        this.$refs.frmGiaoPhieuGTC.pTTKNId = TRANGTHAI_DX.TDDX_GTC
        this.$refs.frmGiaoPhieuGTC.show()
      } catch (e) {
        console.log('onGiaoPhieu error', e);
      } finally {
        this.loading(false)
      }
    },
