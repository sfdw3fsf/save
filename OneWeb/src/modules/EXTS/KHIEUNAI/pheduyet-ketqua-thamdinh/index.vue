...
<!-- <script>
import XuLyKhieuNai from "../xu-ly-khieu-nai";
export default {
  components: {
    XuLyKhieuNai,
  },
...
export default {
  ...
  data: function () {
    return {
      ...
      dsLoaiHinhTB: [],
      dsNhanVienThucHien: [],
      dsPhieuDeXuatGiamTruCuocChuaPheDuyet: [],
      dsPhieuDeXuatGiamTruCuocDaPheDuyet: [],
      dsPhuongAnXuLy: [],
      dsGiamCuoc : [
  {
    ID: 1,
  ...
  },
  {
    ...
    VAT_PT: 10,
      ...
      //   DaGiao: false
      // }
      tabDsPhieuDeXuatGiamTruCuocSelected: {
        ChuaPheDuyet: true,
        DaPheDuyet: false
      }
    };
      ...
      if (this.isToTruong) {
        if (
          ...
          if (this.dsThaoTacControl.length > 0) {
            ...
            if (result) result = !this.isSelectedTabDaPheDuyet();
          }
        }
      }
      return result;
  ...
  methods: {
    ...
    loadDsKhieuNai: async function () {
      ...
      this.dsPhieuDeXuatGiamTruCuocChuaPheDuyet = [];
      this.dsPhieuDeXuatGiamTruCuocDaPheDuyet = [];
      this.enabledSelectFirstRowDaGiao = this.isSelectedTabDaPheDuyet();
      if (this.dsTTKNID == 3) {
        //PB xac minh
        this.dsPhieuDeXuatGiamTruCuocChuaPheDuyet = await this.getDsKhieuNaiTheoTrangThai(0);
        this.dsPhieuDeXuatGiamTruCuocDaPheDuyet = await this.getDsKhieuNaiTheoTrangThai(1);
      }
      else if (this.dsTTKNID == 5) {
        //Xu ly xong
        this.dsPhieuDeXuatGiamTruCuocDaPheDuyet = await this.getDsKhieuNaiKhac();
        ...
        if(this.tabDsKhieuNaiSelected.ChuaGiao)
          this.dsPhieuDeXuatGiamTruCuocChuaPheDuyet = await this.getDsTBKhieuNai(0, 0);
        else
          this.dsPhieuDeXuatGiamTruCuocDaPheDuyet = await this.getDsTBKhieuNai(0, 1);
      }
    },
    gridDsPhieuDeXuatGiamTruCuocChuaPheDuyet_rowSelected: async function (args) {
      this.$root.showLoading(true);
      try {
    ...
    gridDsPhieuDeXuatGiamTruCuocDaPheDuyet_rowSelected: async function (args) {
      //if (this.isSelectedTabDaPheDuyet()) {
      this.$root.showLoading(true);
      try {
        let tuyenThu = await this.getTuyenThu(args.data.THANHTOAN_ID);
        if (tuyenThu != null) {
    ...
    //   }
    // },
    tabDsPhieuDeXuatGiamTruCuoc_Click: async function (args) {
      this.tabDsPhieuDeXuatGiamTruCuocSelected.DaPheDuyet = (args.target.hash == "#tabDaPheDuyet");
      this.tabDsPhieuDeXuatGiamTruCuocSelected.ChuaPheDuyet = !this.tabDsPhieuDeXuatGiamTruCuocSelected.DaPheDuyet;
      //await this.showTab(tabDsPhieuDeXuatGiamTruCuocSelected.DaGiao);
      this.$root.showLoading(true);
      try {
        this.dsThaoTac = [];
        this.dsThaoTacControl = [];
    ...
    //   return rs;
    // },
    isSelectedTabDaPheDuyet: function () {
      return this.tabDsPhieuDeXuatGiamTruCuocSelected.DaPheDuyet;
    },
  },
};
</script>
