...
export default {
  ...
  mounted: async function () {
    ...
    try {
      ...
      this.dsXacMinhCuoc=await this.getDsXacMinhCuoc();
      this.dsKetQua=await this.getDsKetQua();
  ...
  computed: {
    IS_XACMINH_KNDN: function(){
      return this.dsTTKNID == "3" && this.dsHuongGiaoID == "35221";
    },
    dsTTKNID: function () {
      // let v = "1";
  ...
  methods: {
    ...
    gridDsThueBaoKhieuNaiDaGiao_rowDataBound(args) {
      if (args.data.HETHAN_KN === 1){
        args.row.cells[1].className = args.row.cells[1].className + " bg-danger";
      }
    },
    ...
    getDsXacMinhCuoc: async function () {
      let rs = await this.$root.context.get(
        "/web-cskh/api/danh-muc/ho-tro?loaiDanhMuc=DM_XMCUOC"
      );
      let result = rs.data;
      if (result == null || result == undefined) result = [];
      return result;
    },
    getDsKetQua: async function () {
      let rs = await this.$root.context.get(
        "/web-cskh/api/danh-muc/ho-tro?loaiDanhMuc=DM_KETQUA"
      );
      let result = rs.data;
      if (result == null || result == undefined) result = [];
      return result;
