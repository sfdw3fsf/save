  ...
  >
    <div class="modal-content popup-box">
      <div class="list-actions-top">
        <ul class="list">
          <li>
            <a href="javascript: void(0)" @click="doSearch"> <span class="icon one-search"></span> Lấy DS </a>
          </li>
          <li>
            <a href="javascript: void(0)" @click="doChenNo"> <span class="icon one-excel"></span> Chèn nợ </a>
          </li>
          <li>
      ...
      <div class="popup-body">
        <div class="box-form">
          <div class="legend-title mart0">
            ...
            <div class="pull-right">
              <span class="note text-main fw6  block">
                <span class="-ap icon-warning3 text-warning f20 mar10"></span> Tìm tháng nợ của các thuê bao trong các
                kỳ cước trước
              </span>
            </div>
          ...
          <div class="row">
            <div class="col-sm-6 col-12">
              <div class="info-row">
                ...
                <div class="value">
                  <input readonly type="text" class="form-control highlight" v-model="params.p_ma_tt" />
                </div>
              </div>
            </div>
            <div class="col-sm-6 col-12">
          ...
          <div class="info-row">
            ...
            <div class="value">
              <input readonly type="text" class="form-control highlight" v-model="params.p_ten_tt" />
            </div>
          </div>
        </div>
        <div class="box-form">
...
import DeXuatGiamTruCuocAPI from '../api/DeXuatGiamTruCuocAPI'
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      p_default_kycuoc: moment(new Date())
        .subtract(2, 'months')
        .startOf('month')
        .format('YYYYMMDD'),
      results: [],
  ...
  methods: {
    async getDSChenNo() {
      ...
      await DeXuatGiamTruCuocAPI.getDSChenNo(this.axios, postData)
        .then((res) => {
          // onsole.log(res)
    ...
    async getTongNoTheoKyHoaDon() {
      try {
        ...
        const results = await DeXuatGiamTruCuocAPI.getTongNoTheoKyHoaDon(this.axios, postData)
        return results
      } catch (err) {
        console.log(err)
      }
    ...
    async getPayment_Info_ForPay() {
      try {
        const results = await DeXuatGiamTruCuocAPI.getPayment_Info_ForPay(this.axios, {
          pKyCuoc: this.params.p_kycuoc_dc,
          pPaymentCode: this.params.p_ma_tt
        })
    ...
    onDialogOpen() {
      ...
      this.params.p_default_kycuoc = moment(this.kyhoadon, 'YYYYMMDD')
        .subtract(1, 'months')
        .startOf('month')
        .format('YYYYMMDD')
    ...
    clearForm() {
      this.params.p_kycuoc_dc = moment(new Date())
        .subtract(2, 'months')
        .startOf('month')
        .format('YYYYMMDD')
    ...
    doChenNo: function () {
      ...
      if (this.$refs.dsThueBaoChenNo.getSelectedRecords().length <= 0) {
        this.$toast.error('Bạn chưa chọn thuê bao trên lưới để chèn nợ GTC!')
        return false
      }
      this.currentRow = this.$refs.dsThueBaoChenNo.getSelectedRecords();
      this.$emit('chenNoSuccess', this.currentRow);
      this.closeDialog();
      // let arr = []

      // this.$refs.dsThueBaoChenNo.getSelectedRecords().forEach((item, index) => {
      //   arr.push({
      //     LOAITIEN_ID: item.LOAITIEN_ID ? item.LOAITIEN_ID : 1,
      //     CHUKYNO: item.CHUKYNO,
      //     KY_GC: this.params.p_kycuoc_dc
      //   })
      // })
      // let postData = {
      //   pKhieuNaiId: this.khieunai_id,
      //   pMaTT: this.params.p_ma_tt,
      //   pTaoMoi: 0, // 1: tạo mới, 0: chỉnh sửa
      ...
      // DeXuatGiamTruCuocAPI.doChenNo(this.axios, postData)
      //   .then((res) => {
    ...
    async getThongTinTTTheoDBHienTai() {
      try {
        const results = await DeXuatGiamTruCuocAPI.getTTThanhhToanTheoDBHienTai(this.axios, this.params.p_ma_tt)
        const data = await results.data
  ...
  watch: {
    kyhoadon: {
      handler: function(newValue, oldValue) {
        this.p_default_kycuoc = moment(newValue, 'YYYYMMDD')
          .subtract(1, 'months')
          .startOf('month')
          .format('YYYYMMDD')
      }
