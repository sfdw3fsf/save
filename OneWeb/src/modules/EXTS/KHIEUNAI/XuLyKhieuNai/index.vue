...
export default {
  ...
  watch: {
    ...
    'Selected.PAXL_ID'(val) {
      this.Button.enableDeXuat = val == '1' ? true : false
    },
  ...
  methods: {
    ...
    layKetQuaXacMinhGiamCuoc: async function(phieukn_id) {
      let rs = await this.$root.context.get(
        '/web-gqkn/xu-ly-khieu-nai/lay-danhmuc-xacminh-theo-khieunai-id/' + phieukn_id
      )
      return rs.data
    },
    ...
    async getTB_KHIEUNAI(tab) {
      try {
        ...
        if (this.activetab == 1)
          if (this.DS_TB_KHIEUNAI.length > 0) {
            rs = { data: this.DS_TB_KHIEUNAI[0] }
            await this.gridDS_TB_rowSelected(rs)
          }

        if (this.activetab == 2)
          if (this.DS_TB_KHIEUNAI_DAGIAO.length > 0) {
            rs = { data: this.DS_TB_KHIEUNAI_DAGIAO[0] }
            await this.gridDS_TB_rowSelected(rs)
          }
      } catch (error) {
      ...
      try {
        if (this.DS_TB_KHIEUNAI.length > 0) {
          ...
          if (!this.isEmptyObj(this.KHIEUNAI)) {
            ...
            data = {
              KHIEUNAI_ID: khieunai_id,
              KETQUAKN_ID: this.Selected.NGUYENNHAN_ID,
              NOIDUNG_GQ: this.KETQUA_KHIEUNAI.ND_TRALOI
            }
            ...
            await this.getTB_KHIEUNAI(this.activetab)
          }
        } else {
          this.loading(false)
    ...
    async tsbtnGhiLai_Click() {
      ...
      try {
        this.loading(true)
        let ketqua = await this.capNhatKetQuaXuLy()
        if (ketqua == 'ok') this.$toast.success('Cập nhật kết quả xử lý thành công')
        else this.$toast.error(ketqua)
        this.loading(false)
      } catch (error) {
        this.loading(false)
      }
    ...
    async capNhatKetQuaXuLy() {
      try {
        ...
        if (rs.error_code == 'BSS-00000000') {
          ...
          return this.extractOracleMessage(rs.message_detail)
        }
      } catch (error) {
        return this.extractOracleMessage(error.response.data.message_detail)
      }
    },
    ...
    async grid_SelectedRowChanged(row) {
      ...
      if (row) {
        try {
          ...
          this.DS_NGUYENNHAN_LOI = await this.getDanhSachNguyenNhanLoi(
            this.KHIEUNAI.CHUDEKN_ID,
            this.KHIEUNAI.LOAITB_ID
          )

          const danhSachXacMinh = await this.layKetQuaXacMinhGiamCuoc(this.KHIEUNAI.KHIEUNAI_ID)
          this.DS_XACMINH_GIAMCUOC = danhSachXacMinh.length > 0 ? JSON.parse(danhSachXacMinh[0].NOIDUNG_XACMINH) : []
          this.XACMINH_KHIEUNAI.ND_XACMINH = danhSachXacMinh.length > 0 ? danhSachXacMinh[0].XACMINH_KHAC : null;

          this.KETQUA_KHIEUNAI.DOITUONG_LOI = this.DS_NGUYENNHAN_LOI.filter(
            (item) => item.NGUYENNHAN_ID == this.Selected.NGUYENNHAN_ID
          )[0]?.DOITUONG_LOI_ID
          ...
          console.log('grid_SelectedRowChanged ~ error', error)
        } finally {
          this.loading(false)
        }
      } else {
    ...
    huyNhanVienTH: async function(khieunai_id) {
      ...
      try {
        const response = await this.$root.context.post('/web-gqkn/xu-ly-khieu-nai/huyNhanVienTH', {
          id: khieunai_id
        })
        if (response.error_code === 'BSS-00000000') {
          resultDetail = response.data
        } else {
            ...
            if (this.checkEmptyString(this.KHIEUNAI.ND_KHIEUNAI)) {
              ...
              if (this.checkPhaiLam('XACMINH') && this.checkEmptyString(this.XACMINH_KHIEUNAI.ND_XACMINH)) {
                ...
                if (t11 != 'ok') {
                  ...
                  if (t1.RS > 0) {
                    this.$root.toastError('Phiếu không thể hoàn thành do vẫn còn phiếu xác minh chưa xử lý!')
                  } else {
                    let huongGiaoID = 0
        ...
        if (this.checkEmptyNumber(this.Selected.NHANVIEN_ID)) {
          ...
          if (ck) {
            if (this.checkPhaiLam('XACMINH') && this.checkEmptyString(this.XACMINH_KHIEUNAI.ND_XACMINH)) {
              ...
              await this.btnLayTT_Click(null)
            }
          }
        }
      } finally {
    ...
    khoaPhieu: async function(args) {
      ...
      try {
        let param = {
          ...
          pNoiDung: this.NGHIEMTHU.NOIDUNG_NT
        }
        this.loading(true)
        var rs = await this.$root.context.post('/web-gqkn/xu-ly-khieu-nai/khoaphieu-nghiemthu-khieunai', param)
        if (rs.error_code == 'BSS-00000000') {
    ...
    btnHoSo_Click: function(args) {
      if (!this.checkEmptyNumber(this.KHIEUNAI.KHIEUNAI_ID)) {
        this.$refs.frmHoSoKhieuNai.khieunai_id = this.KHIEUNAI.KHIEUNAI_ID
        this.$refs.frmHoSoKhieuNai.show()
      }
    ...
    async btnGiamCuoc_Click() {
      let ketqua = await this.capNhatKetQuaXuLy()
      if (ketqua == 'ok') {
        let Link = this.$router.resolve({
          path: 'de-xuat-giam-tru-cuoc-kn',
    ...
    async hoanThanhTraPhieu() {
      this.$refs.modalTraPhieuKhieuNai.hide()
      await this.btnLayTT_Click(null)
    },
    ...
    modalTraPhieuKhieuNai_Shown: async function (args) {
     this.KHIEUNAI.NGAY_TN = moment(this.KHIEUNAI.NGAY_TN).format('DD/MM/YYYY'),
    ...
    async getDanhSachNguyenNhanLoi(chudeknid, loaitbid) {
      try {
        ...
        var rs = await this.$root.context.post('/web-gqkn/khieunai/lay-danhsach-nguyennhan-loi', param)
        if (rs.data.length > 0) {
          return rs.data
        } else {
          return null
