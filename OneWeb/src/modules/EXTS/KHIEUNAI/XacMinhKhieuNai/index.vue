...
//import ModalDocFile from '../TiepNhanKhieuNai/modal/DocFileV2'
//import ModalNhanVienLoi from '../TiepNhanKhieuNai/modal/KetQuaLoi/NhanVienLoi'
//import ModalDonViLoi from '../TiepNhanKhieuNai/modal/KetQuaLoi/DonViLoi'
//import ModalGiaoPhieuKN from '../TiepNhanKhieuNai/modal/GiaoPhieu/index.vue'
//import GiaoPhieuPhoiHop from '../GiaoPhoiHop'
  ...
  data() {
    return {
      ...
      Button: {
        ...
        enableHuyGiao: true,
        enableTraPhieu: true,
        enableGiaoPhieu: true,
        enableHoanThanh: true,
        enableHoanThanh_GQKN: true,
        enableKhoaPhieu: true,
        enableGiaoPhoiHop: true,
        enableGhiLai: true,
        enableDeXuat: true,
      },
      ...
      dsBollean: [
        { KETQUA_ID: 0, KETQUA: 'Đúng' },
        { KETQUA_ID: 1, KETQUA: 'Sai' }
      ],
      lyDoParams() {
        let lydoElem; // khai báo biến local
        let lydoObj;  // khai báo biến local
        return {
          create: () => {
          ...
          write: () => {
            lydoObj = new DropDownList({
              dataSource: this.dsBollean,
              fields: { value: 'KETQUA_ID', text: 'KETQUA' },
              enabled: true,
  ...
  methods: {
    ...
    async getTB_KHIEUNAI(tab) {
      try {
        ...
        if (tab == null || tab == 1)
          this.DS_TB_KHIEUNAI = await this.getDanhSachKhieuNai(0);

        if (tab == null || tab == 2)
          this.DS_TB_KHIEUNAI_DAGIAO = await this.getDanhSachKhieuNai(1);

        if (this.activetab == 1)
          if (this.DS_TB_KHIEUNAI.length > 0) {
            let bien = { data: this.DS_TB_KHIEUNAI[0] }
            await this.gridDS_TB_rowSelected(bien)
          }
        if (this.activetab == 2)
    ...
    async apiGhiLai() {
      try {
        let param = {
          ...
          pNoiDung: this.XACMINH_KHIEUNAI.ND_XACMINH,
          dsXacMinhCuoc: JSON.stringify(this.DS_XACMINH_GIAMCUOC),
          phanvung_id:this.KHIEUNAI.PHANVUNG_ID
        }
        let rs = await this.$root.context.post('/web-gqkn/khieunai/capnhat-danhsach-xacminh-giamcuoc', param)
        this.$toast.success('Cập nhật thành công');
      } catch (error) { this.$toast.error(error.response.data.message_detail) }
    },
    async tsbtnGhiLai_Click() {
      //if (!this.Button.tsbtnGhiLai) return false
      if (this.XACMINH_KHIEUNAI.ND_XACMINH == null || this.XACMINH_KHIEUNAI.ND_XACMINH == undefined) {
        this.$toast.error('Bạn chưa nhập nội dung xác minh')
        return;
      }
      try {
        this.loading(true)
        await this.apiGhiLai();
        this.loading(false)
      } catch (error) {
        this.Button.tsbtnGhiLai = true
        this.loading(false)
    ...
    getDanhSachGiamCuoc: async function (khieuNaiId) {
      let rs = await this.$root.context.get(
        "/web-gqkn/xu-ly-khieu-nai/lay-danhsach-khieunai-giamcuoc/"+khieuNaiId);
      return rs.data;
    },
    ...
    // ) {
    //   let rs = await this.$root.context.post(
    //     "/web-gqkn/khieunai/capnhat-danhsach-xacminh-giamcuoc",
    //     {
    //       pKhieuNaiId: khieunai_id,
    //       pPhieuKNId: phieukn_id,
    //       pNoiDung: noi_dung,
    //       dsXacMinhCuoc:ds_xacminh_cuoc,
    ...
    async grid_SelectedRowChanged(row) {
      ...
      if (row) {
        try {
          ...
          await this.enableButtons(this.activetab);
          ...
          this.Selected.KETQUAKN_ID = parseInt(this.KHIEUNAI.KETQUAKN_ID)
          ...
          let danhSachXacMinh=await this.layKetQuaXacMinhGiamCuoc(this.KHIEUNAI.KHIEUNAI_ID)
          this.DS_XACMINH_GIAMCUOC=JSON.parse(danhSachXacMinh[0].NOIDUNG_XACMINH)
          this.XACMINH_KHIEUNAI.ND_XACMINH=danhSachXacMinh[0].XACMINH_KHAC
          
          this.DsGC=await this.getDanhSachGiamCuoc(this.KHIEUNAI.KHIEUNAI_ID)
          ...
          //   }
          // } 
          this.SetButton(this.currentActivetab)
        } catch (error) {
    ...
    //           let row = this.$refs.gridDS_TB_KHIEUNAI.grid.getRowInfo(
    //             this.$refs.gridDS_TB_KHIEUNAI.grid.getRowByIndex(rowCell.rowIndex)
    //           )
    //           if (!this.dsIdChon.includes(row.rowData.KHIEUNAI_ID)) {
    //             this.dsIdChon.push(row.rowData.KHIEUNAI_ID)
    //           }
    //         }
    //       }
    //     }
    ...
    // },
    // gridDS_TB_KHIEUNAI_DAGIAOOnCellSelectedChange: async function(args) {
    ...
    //       if (rowCell.cellIndexes.length == 1) {
    //         if (rowCell.cellIndexes.includes(1)) {
    //           let row = this.$refs.gridDS_TB_KHIEUNAI_DAGIAO.grid.getRowInfo(
    //             this.$refs.gridDS_TB_KHIEUNAI_DAGIAO.grid.getRowByIndex(rowCell.rowIndex)
    //           )
    //           if (!this.dsIdChon.includes(row.rowData.KHIEUNAI_ID)) {
    //             this.dsIdChon.push(row.rowData.KHIEUNAI_ID)
    //           }
    ...
    //   }
    //   await this.gridDS_TB_KHIEUNAI_SelectedRowChanged(args.data)
    //   this.$refs.gridDS_TB_KHIEUNAI_DAGIAO.grid.clearSelection()
    // },
    ...
    onclickTab: function (currentActivetab) {
      try {
        ...
        if (currentActivetab == 1) {
          ...
          this.Button.enableHuyGiao = false
          this.Button.enableGiaoPhoiHop = false
          this.Button.enableTraPhieu = true
        } else {
          this.Button.enableGiaoViec = false
          this.Button.enableHuyGiao = true
          this.Button.enableGiaoPhoiHop = true
          this.Button.enableTraPhieu = false
        }

        this.activetab = currentActivetab
        this.columnFields = GRID_COLUMS[currentActivetab]
        this.getTB_KHIEUNAI(currentActivetab)        
        this.setButton(currentActivetab)      
    ...
    enableButtons: async function (tab) {

      if (this.isToTruong) {
        if (!(this.dsThaoTacControl == null || this.dsThaoTacControl == undefined)) {
          if (this.dsThaoTacControl.length > 0) {
            this.enableBtnGiaoViec = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbrnGiaoViec') > -1
            this.enableBtnHuyGiao = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnHuyGiao') > -1
            this.enableBtnTraPhieu = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnTraPhieu') > -1
            this.enableBtnGiaoPhieu = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnGiaoPhieu') > -1
            this.enableBtnHoanThanh = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnHoanThanh') > -1
            this.enableBtnHoanThanh_GQKN = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnHoanThanh_GQKN') > -1
            this.enableBtnKhoaPhieu = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnKhoaPhieu') > -1
            this.enableBtnGiaoPhoiHop = this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnGiaoPhoiHop') > -1
            this.enableBtnGhiLai =this.dsThaoTacControl.findIndex((x) => x.ENABLE == 1 && x.CONTROL_NAME == 'tsbtnGhiLai') > -1
          }
        }
      }
    ...
    enableBtnKetQuaKNMore: function () {
      ...
      if (this.currentItem.selectedKhieuNai != null) {
        result =
          this.currentItem.selectedKhieuNai.KETQUAKN_ID == 7 || this.currentItem.selectedKhieuNai.KETQUAKN_ID == 9
      }
      return result
    },
    ...
    giaoPhieuNhanVien: async function (
      ...
      let rs = await this.$root.context.post(
        "/web-gqkn/khieunai/giao-nhanvien-xacminh",
        {
          phieuKNId: phieukn_id,
          nhanVienTHId: nhanvien_th_id,          
          ghiChu: ghichu,
          phanvung_id:phanvung_id 
        }
      );
    ...
    xuLyThongBaoNVTH: async function (phieukn_id) {
      let rs = await this.$root.context.post(
        "/web-gqkn/xu-ly-khieu-nai/xuLyThongBaoNVTH",
        {
          id: phieukn_id
        }
      );
    ...
    xuLyThongBaoHuyGiao: async function (phieukn_id) {
      let rs = await this.$root.context.post(
        "/web-gqkn/xu-ly-khieu-nai/xuLyThongBaoHuyGiao",
        {
          id: phieukn_id
        }
      );
    ...
    btnHuyGiaoNV_Click: async function (args) {
      this.$root.showLoading(true)
      try {
          let rs=await this.huyNhanVienTH(this.KHIEUNAI.PHIEUKN_ID,this.KHIEUNAI.PHANVUNG_ID)
          if (rs[0].KETQUA=='ok') {
            this.$root.toastSuccess('Hủy giao phiếu nhân viên thành công')
          } else this.$root.toastError('Hủy giao phiếu nhân viên thất bại')
        if (this.nhanTinGiaoViec) {
    ...
    huyNhanVienTH: async function (phieuknId,phanvung_id) {
      let rs = await this.$root.context.post(
        "/web-gqkn/khieunai/huy-giao-nhanvien-xacminh",
        {
          id: phieuknId,
          phanvung_id:  phanvung_id
        }
    ...
    btnHoanThanh_Click_1: async function (args) {
      ...
      try {
        ...
        if (this.checkPhaiLam('XACMINH') && this.checkEmptyString(this.XACMINH_KHIEUNAI.ND_XACMINH)) {
          ...
          let rs=await this.capNhatTTPhieuXacMinh(this.KHIEUNAI.PHIEUKN_ID,this.KHIEUNAI.PHANVUNG_ID)
          //if (rs.Result)   
          // this.$root.toastSuccess('Hoàn thành công việc thành công !')
          await this.getDanhSachKhieuNai(this.activetab);
        }
      }
      finally {
        this.$root.showLoading(false)
    ...
    capNhatTTPhieuXacMinh: async function (
      phieukn_id,phanvung_id 
    ) {
      let rs = await this.$root.context.post(
    ...
    getDanhSachKhieuNai: async function (dagiao) {
      try {        
        let rs = await this.$root.context.post(
          "/web-gqkn/khieunai/lay-danhsach-khieunai-xacminh",
          {
            pDaGiao: dagiao
          }
        );
    ...
    btnGiamCuoc_Click() {
      let Link = this.$router.resolve({
        ...
        query: { MaKN: this.currentItem.selectedKhieuNai.MA_KN }
      })
    ...
    modalTraPhieuKhieuNai_Shown: async function (args) {
      await this.$refs.formTraPhieuKhieuNai.bindData(
        this.KHIEUNAI
      );
    },
    ...
    getDsNhanVienThucHien: async function () {
      let rs = await this.$root.context.post(
        "/web-gqkn/xu-ly-khieu-nai/getDsNhanVienXuLyKNV2"
      );
      return rs.data;
    },
    ...
    actionComplete(args) {
      if (args.action === 'edit' && args.requestType === 'save') {
        // clone mảng gốc
        let data = [...this.DS_XACMINH_GIAMCUOC];

        for (let i = 0; i < data.length; i++) {
          if (data[i].ID === args.data.ID) {
            data[i].KETQUA_ID = args.data.KETQUA_ID;
            data[i].GHI_CHU = args.data.GHI_CHU;  // nhớ cập nhật luôn GHI_CHU
            break;
          }
        }

        // gán lại để Vue reactive cập nhật grid
        this.DS_XACMINH_GIAMCUOC = [...data];
      }
    },
    setButton(kieu){
      if(kieu==1)
