...
const DSThueBaoDDV = () => import('./components/ThongTinThueBao/Popups/ThueBaoGoiDDV.vue')
...
export default {
  ...
  methods: {
    ...
    frmChuyenDich_Load: async function () {
      try {
        ...
        if (this.thamso_md !== null) {
          // console.log('get_thamso_md', this.thamso_md)
    ...
    txtMaTB_KeyPress: async function (args) {
      ...
      try {
        ...
        if (args.data !== null && args.data.length > 0) {
          let ds_tb = await this.dsThuebaoGoiDDV(args.data[0].THUEBAO_ID)
          if (ds_tb.length > 0) {
            this.dsTBGoiDDV = [...ds_tb]
            this.$refs.ref_thuebao_ddv.openDialog()
          }
        }
    ...
    HienThiTT_DanhBa_KH_NhanCQ: async function (ds) {
      try {
        ...
        let ds_tb = await this.dsThuebaoGoiDDV(ds[0].THUEBAO_ID)
        if (ds_tb.length > 0) {
          this.dsTBGoiDDV = [...ds_tb]
          this.$refs.ref_thuebao_ddv.openDialog()
        }
    ...
    async LAY_DS_THAMSO_MD (vkieu) {
      try {
        ...
        return rs.data
        // let data = rs.data.map(x => ({ TEN_TS: x.ten_ts, MA_TS: x.ma_ts}))
        // return data
      } catch (error) {
        return []
      }
    ...
    dsThuebaoGoiDDV: async function (thuebao_id) {
      try {
        let rs = await this.$root.context.get(
          '/web-hopdong/cap-nhat-thue-bao-dat-coc/ds_thuebao_goiddv/' + thuebao_id
        )
        if (rs.data.length > 0) {
          return this.ThongTinThueBao.UpperCasePropertyList(rs.data)
        }
        return []
      } catch (error) {
        console.log(error)
  ...
  mounted: async function () {
    try {
      ...
      let r = this.thamso_md.find(x => x.ma_ts === 'NVKD_BATBUOC_NHAP_LOAINHA')
      // console.log(r)
      this.f_nhap_loainha = r === undefined || r.ten_ts === 0 ? false : true

      console.log('this.f_nhap_loainha', this.f_nhap_loainha)
    } catch (error) {
