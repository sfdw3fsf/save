...
export default {
  ...
  data: function () {
    return {
      fieldsTraCuu: { text: 'kieu_tk', value: 'kieutk_id' },
      loaitb_id: null,
      resolvePromise: null,
      ma_tn: null,
      ma_doicap: null,
  ...
  methods: {
    ...
    // },
    show() {
      this.$bvModal.show('frmDSTBCungDoiCap_kh_khac')
    ...
    hide(data) {
      ...
      this.Close()
      this.$bvModal.hide('frmDSTBCungDoiCap_kh_khac')
    },
    ...
    async tsbtnChapNhan_Click() {
      ...
      if (vdvvt_id == Enum.DichVuVienThong.ANTOAN_BAOMAT_TT || vdvvt_id == Enum.DichVuVienThong.DICHVU_CNTT ||
        ...
        // if (await this.KIEMTRA_MATB_TRUYNHAP(this.ma_doicap, this.loaitb_id, Enum.KieuLapDat.LAP_LINE_SAN) == 0 ||
        if (await this.KIEMTRA_MATB_TRUYNHAP(this.ma_doicap, this.loaitb_id, 2) == 0 ||
    ...
    async HienThi_DS_Khachhang() {
      try {
        ...
        let value0 = null, value1 = null, value2 = null, value3 = null
        this.txTextBoxTC = this.txTextBoxTC ? this.txTextBoxTC.trim() : ""

        if (this.txTextBoxTC === null || this.txTextBoxTC === '') {
          dieukien1 = 0
        } else {
          dieukien1 = this.cboDKTraCuu.Value
          value1 = this.txTextBoxTC.trim()
        }
        if (this.chkDK2) {
          this.txtTraCuu2 = this.txtTraCuu2 ? this.txtTraCuu2.trim() : ""
          if (this.txtTraCuu2 === null || this.txtTraCuu2 === '')
            dieukien2 = 0
          else {
            dieukien2 = this.cboDK2.Value
            value2 = this.txtTraCuu2.trim()
          }
        }
        if (this.chkDK3) {
          ...
          if (this.txtDK3 == "")
            ...
            dieukien3 = this.cboDK3.Value
            value3 = this.txtDK3.trim()
          }
        }
        /*
        console.log('this.cboDKTraCuu.Value', this.cboDKTraCuu.Value)
        if (this.txTextBoxTC == "")
          dieukien1 = 0
        else dieukien1 = this.cboDKTraCuu.Value // this.cboDKTraCuu.Value.replace(":values", this.txTextBoxTC)
        if (this.chkDK2) {
          this.txtTraCuu2 = this.txtTraCuu2 ? this.txtTraCuu2.trim() : ""
          if (this.txtTraCuu2 == "")
            dieukien2 = 0
          else dieukien2 = this.cboDK2.Value.replace(":values", this.txtTraCuu2)
        }
        if (this.chkDK3) {
          this.txtDK3 = this.txtDK3 ? this.txtDK3.trim() : ""
          if (this.txtDK3 == "")
            dieukien3 = 0
          else dieukien3 = this.cboDK3.Value.replace(":values", this.txtDK3)
        }
        if (dieukien1 == 0)
          if (dieukien2 == 0)
            if (dieukien3 == 0)
        ...
        */
        ...
        this.vcvlan = ""
        this.vsite_id = ""
        let tracuu = this.cboDKTraCuu.DataSource.find(x => x.dieukien == this.cboDKTraCuu.Value)
        if (tracuu && tracuu.kieu_tk == "Svlan/Cvlan") {
          ...
          if (tracuu && tracuu.kieu_tk == "Số ảo")
            ...
            // this.dtHDKH = await this.TRA_CUU_DANH_BA(dieukien, dichvuvt_id, donvi_id)
            this.dtHDKH = await this.TRA_CUU_DANH_BA_2(dieukien1, dieukien2, dieukien3, value1, value2, value3, dichvuvt_id, donvi_id)
          }
        }
    ...
    async HT_Kieu_TK_Cbo_Dev(cbo, nhom_id) {
      ...
      cbo.DataSource = rs.data
      if (cbo.DataSource.length > 0) cbo.Value = rs.data[0].kieutk_id // rs.data[0].dieukien
    },
    ...
    async TRA_CUU_DANH_BA(dieukien, dichvuvt_id, donvi_id) {
      let rs = await this.$root.context.get("web-hopdong/chuyendich-bancheo/dstb-cungdoicap-kh-khac/tra_cuu_danh_ba", {
        dieukien: dieukien,
        donvi_id: donvi_id,
        dichvuvt_id: dichvuvt_id
      })
    ...
    async TRA_CUU_DANH_BA_2(dieukien1, dieukien2, dieukien3, value1, value2, value3, dichvuvt_id, donvi_id) {
      let rs = await this.$root.context.post("web-hopdong/chuyendich-bancheo/dstb-cungdoicap-kh-khac/tra_cuu_danh_ba", {
        kieutk_id1: dieukien1,
        kieutk_id2: dieukien2,
        kieutk_id3: dieukien3,
        tracuu1: value1,
        tracuu2: value2,
        tracuu3: value3,
        donvi_id: donvi_id,
