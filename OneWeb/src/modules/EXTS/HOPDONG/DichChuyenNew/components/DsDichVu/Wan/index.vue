...
export default {
  ...
  methods: {
    ...
    async HT_COMBO_DONVI () {
      let dsLoaiDonVi = await this.lay_ds_loai_donvi_v2({
        loaihinh_tb: this.idLoaihinhTb,
        kieu_ld: this.idKieuLd,
        // tocdo_id: this.cboTocDoKenhTSL.Value,
        dichvu_cd_id: this.parent.inputValue.dichvuvt_id,
        loaikenh_id: this.parent.inputValue.vloaikenh_id,
      })
    ...
    async TaoDuLieu_HDTB_MGWAN (themmoi) {
      ...
      Object.keys(firstRow).forEach(key => rowHDTB_MGWAN[key.toUpperCase()] !== undefined ? rowHDTB_MGWAN[key.toUpperCase()] = firstRow[key] : null)
      // console.log('rowHDTB_MGWAN dòng 438', rowHDTB_MGWAN)
    ...
    async lay_ds_loai_donvi_v2 (data) {
      try {
        let response = await this.$root.context.post(`/web-hopdong/chuyendich-v2/lay_ds_loai_donvi_v2`, data)
        return response.data
      } catch (err) {
        return []
      }
