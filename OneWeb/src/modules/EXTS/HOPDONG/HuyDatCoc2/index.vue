...
import Vue from "vue";
import { Group, Page, Sort, Filter, Aggregate } from "@syncfusion/ej2-vue-grids";
...
export default {
    ...
    methods: {
        ...
        async txtMaGD_KeyPress() {
            ...
            try {
                ...
                let ds = await this.LAY_DS_THUEBAO_DATCOC_THEOGD(this.txtMaGD.Text)
                if (ds.length <= 0) {
                    this.$toast.error('Không tìm thấy hợp đồng huỷ có mã giao dịch ' + this.txtMaGD.Text)
                    this.Clear()
                    return
                }
                
                this.dgvDatCoc.DataSource = ds.map(x => ({ ...x, _CHECK: true, _KEY:  this.makeKey()}))
                this.ds_datcoc_bk = [...this.dgvDatCoc.DataSource]
                ...
                if (this.tthd_id === 1) {
                    ...
                    this.tsbtnCapNhatDB.Enabled = false
                } else if (this.tthd_id === 2) {
                    this.tsbtnThanhToan.Enabled = false  
                    this.tsbtnCapNhatDB.Enabled = true
                }
        ...
        async HienThiTTHopDongKH() {
            ...
            let thongTinHopDong = this.THUEBAO
            console.log('thongTinHopDong', thongTinHopDong)
            ...
            this.tthd_id = thongTinHopDong.TTHD_ID
            ...
            this.txtTenKH.Text = thongTinHopDong["TEN_TB"]
            this.txtDiaChiKH.Text = thongTinHopDong["DIACHI_LD"]
            this.ten_kh = thongTinHopDong["TEN_TB"]
            this.diachi_kh = thongTinHopDong["DIACHI_LD"]
            this.txtGhiChu.Text = !thongTinHopDong["GHICHU"] ? '' : thongTinHopDong["GHICHU"]
            this.ten_goi_ddv = thongTinHopDong.TEN_GOI_DDV
            this.ten_tocdo = thongTinHopDong.TEN_TOCDO
            this.txtMaTB.Text = thongTinHopDong.MA_TB
            this.cboDichVuVT.Value = thongTinHopDong.DICHVUVT_ID
            ...
            if (!thongTinHopDong["HDKH_ID"]) this.SetButton(1)
            else this.SetButton(3)
        ...
        async HienThiTT_Datcoc() {
            ...
            let postData = {
                p_data: JSON.stringify({"thuebao_id" : this.thuebao_id, "loai_hd" : 32})
            }
            let ds_datcoc = await this.LAY_DS_DATCOC_THEO_MATB(postData)

            // console.log(ds_datcoc)
            this.dgvDatCoc.DataSource = [...ds_datcoc.map(x => ({ ...x, _CHECK: false, _KEY:  this.makeKey()}))]
        ...
        async txtMaTB_KeyPress() {
            ...
            try {
                ...
                let ds = await this.LAY_DS_THUEBAO_DATCOC_THEOTB(this.txtMaTB.Text)

                if (!ds || ds.length <= 0) {
                    this.$alert('Không tìm thấy thông tin về mã thuê bao ' + this.txtMaTB.Text, 'Thông báo', {
                    confirmButtonText: 'Đóng',
                    callback: action => {
                    }
                    })
                }
                const dsUnique = ds.filter(x => x.MA_TB === this.txtMaTB.Text)
                ...
                this.dgvDatCoc.DataSource = ds.map(x => ({ ...x, _CHECK: true, _KEY:  this.makeKey()}))
                this.ds_datcoc_bk = [...this.dgvDatCoc.DataSource]
                this.HienThiTTHopDongKH(this.dgvDatCoc.DataSource[0])
        ...
        async TaoDuLieu_HDTB_v2(themmoi) {
            ...
            for (let i = 0; i < this.dgvDatCoc.DataSource.length; i++) {
                ...
                node.setAttribute("ten_tb", r["TEN_TB"])
                node.setAttribute("diachi_ld", r["DIACHI_LD"])
                node.setAttribute("diachi_tb", r["DIACHI_LD"])
                node.setAttribute("kieuld_id", kieuld_id)
                node.setAttribute("doituong_id", parseInt(r["DOITUONG_ID"]))
                node.setAttribute("tthd_id", this.TrangThaiHD.MOI)
                node.setAttribute("loaitb_id", parseInt(r["LOAITB_ID"]))
                node.setAttribute("dichvuvt_id", r["DICHVUVT_ID"])
                node.setAttribute("donvi_id", parseInt(r["DONVIQL_ID"]))
                node.setAttribute("nguoi_cn", this.$root.token.getUserName())
                node.setAttribute("may_cn", this.$root.token.getUserName())
                node.setAttribute("ghichu", this.txtGhiChu.Text)
                node.setAttribute("quytrinh_id", quytrinh_id)
        ...
        async TaoDuLieu_HDTB_DATCOC_v2(themmoi) {
            ...
            for (let i = 0; i < this.dgvDatCoc.DataSource.length; i++) {
                ...
                node.setAttribute("ghichu", this.txtGhiChu.Text)
                node.setAttribute("thang_thoaitra", this.yyyymm(this.dtpThangThoaiTra.Value))
                node.setAttribute("nhomtb_id", r["NHOMTB_ID"])
                if (this.TINHCUOC_NGAY == 1)
                    node.setAttribute("ngay_thoai", this.ddmmyyyy(this.dtpNgayThoai.Value));
        ...
        async cboDichVuVT_SelectedIndexChanged() {
            ...
            this.cboLoaiHinhTB.Value = this.THUEBAO.LOAITB_ID
        ...
        async dgvDatCoc_SelectionChanged(r) {
            ...
            this.thuebao_thoaitra_id = !r["THUEBAO_THOAITRA_ID"] ? r['THUEBAO_DC_ID'] : r["THUEBAO_THOAITRA_ID"]
            this._thoaitra = parseInt(r["TIEN_THOAI"]);
        ...
        async CapNhatV2() {
            if (this.tsbtnNhapMoi.Enabled == false) {
                // let Kiemtra_Dulieu = await this.Kiemtra_Dulieu_V2(true)
            ...
            let result = await this.ThemMoi_HopDong_V2()
        },
        async ThemMoi_HopDong_V2 () {
            try {
                ...
                for( let i = 0; i < selected.length; i++) {
                    ...
                    //   continue
                    // }
                    let tien = (Math.round(parseFloat(item.TIEN_THOAI) / 1.1))
                    let vat = (parseFloat(item.TIEN_THOAI) - tien)
                    ...
                    let info = {
                        ...
                        "thuebao_dc_id": !item.HDTB_ID ? null : item.THUEBAO_DC_ID,
                        "thuebao_thoaitra_id": !!item.THUEBAO_THOAITRA_ID ? item.THUEBAO_THOAITRA_ID : item.THUEBAO_DC_ID,
                        "rkm_id": item.RKM_ID,
                        "hdtb_id": !!item.HDTB_ID ? item.HDTB_ID : null,
                        "hdkh_id": !!item.HDKH_ID ? item.HDKH_ID : null,
                        "ma_gd": !!item.MA_GD ? item.MA_GD : null,
                        "khachhang_id": !!item.KHACHHANG_ID ? item.KHACHHANG_ID : null,
                        "nhom_datcoc_id": item.NHOM_DATCOC_ID,
                        "thang_bd": item.THANG_BD,
                        "thang_kt": item.THANG_KT,
                        "thang_bd_mg": 0,
                        "thang_kt_mg": 0,
                        "ngay_dk": this.ddmmyyyy(this.dtpNgayYeuCau.Value),
                        "ghichu": item.GHICHU,
                        "thang_thoaitra": item.THANG_THOAI,
                        "ngay_thoai": item.NGAY_THOAI,
                        "nhomtb_id": item.NHOMTB_ID,
                ...
                let rs = await this.$root.context.post('/web-hopdong/ThayDoiDatCocHuy/themmoi_hopdong_v2', { dsData : JSON.stringify(postData)})
                if (rs.data === null) {
                    this.$root.$toast.error(rs.message)
                    return false
                }
                if (rs.data.length > 0) {
                    this.txtMaGD.Text = rs.data[0].ma_gd.split(',')[0]
        ...
        async Kiemtra_Dulieu_V2 (themmoi){
            let selected = this.dgvDatCoc.DataSource.filter(x => x._CHECK === true)

            for( let i = 0; i < selected.length; i++) {
            }
        },
        async KiemTra_DuLieu_TB (themmoi, thuebao) {
            let thuebao_bk = this.ds_datcoc_bk.find(x => x._KEY === thuebao._KEY)
            let errorMessage = []
            let ngayCn = await this.lay_ngay_cn()
            let thanght = parseInt(this.yyyymm(DateTimeLib.toDateObjectWithFormat(ngayCn, "YYYY-MM-DD HH:mm:ss")))
            let phanVungId = this.$root.token.getPhanVungID()

            if (!(thuebao.nhom_datcoc_id === 2 && phanVungId === 26/*HPG*/)) {
                if (thuebao.thang_kt < thanght && thuebao.thang_bd > 0) {
                    errorMessage.push('Đặt cọc đã hết hạn không thay đổi được !')
                }
            }
            ...
            if (!thuebao.TIEN_THOAI && parseFloat(thuebao.TIEN_THOAI) !== 0) {
                errorMessage.push("Tiền thoái trả không được để trống !")
            }

            if (!!thuebao.TIEN_THOAI && parseInt(thuebao.TIEN_THOAI) > thuebao_bk.TIEN_THOAI && this._KHONG_VUOTQUA_TIENTHOAI > 0) {
                errorMessage.push("Không được nhập lớn hơn số tiền thoái. Hãy nhập lại.!");
                this.txtTienThoaiTra.hasError = true
            }
        ...
        async dsTBDatCocRecordClick (row) {
            // if (row.target.classList.contains('e-check')) { // click check/uncheck
            // }
            console.log('🚀 ~ file: index.vue:4135 ~ dsTBDatCocRecordClick ~ row:', row)
            this.$root.showLoading(true)
            try {
                if (this.dgvDatCoc.DataSource.length === 0) return false
                if (row === undefined) return false
                if (row) {
                    this.THUEBAO = row.rowData
                    await this.HienThiTTHopDongKH()
                    await this.dgvDatCoc_SelectionChanged(row.rowData)                
                }
            } catch (error) {
        ...
        dsTBDatCocRowDeselected (args) {
            ...
            if (!args.isHeaderCheckboxClicked && args.isInteracted) { // không phải call by click checkbox header & paging
                let f = this.dgvDatCoc.DataSource.find(x => x._KEY === args.data._KEY)
                console.log('dsTBDatCocRowDeselected', f)
        ...
        async btnXacNhanTTDatCoc () {
            ...
            let ngayThoai = moment(this.dtpNgayThoai.Value).format('DD/MM/YYYY')
            let ds = []
            for (const element of this.dgvDatCoc.DataSource) {
                let item = {...element}
                if (element.GOI_DDV_ID === this.datCocDuocChon.GOI_DDV_ID && element.NGAY_THOAI !== ngayThoai && this.TINHCUOC_NGAY == 1) {
                    let ret = await this.Lay_tienthoai_datcoc_v4({
                        rkm_id: element.RKM_ID,
                        // thuebao_dc_id: !element.HDKH_ID  ? element.THUEBAO_DC_ID : element.THUEBAO_THOAITRA_ID,
                        thuebao_dc_id: !element["THUEBAO_THOAITRA_ID"] ? element['THUEBAO_DC_ID'] : element["THUEBAO_THOAITRA_ID"],
                        thang_thoai: this.yyyymm(this.dtpThangThoaiTra.Value),
                        ngay_thoai: this.yyyymmdd(this.dtpNgayThoai.Value)
                    })
                    // item.THUEBAO_THOAITRA_ID = element.THUEBAO_THOAITRA_ID
                    item.NGAY_THOAI = ngayThoai
                    item.THANG_THOAI = this.yyyymm(this.dtpThangThoaiTra.Value)
                    item.TIEN_THOAI = ret
                    item.GHICHU = this.txtGhiChu.Text !== null ? this.txtGhiChu.Text.trim() : null
                    item._CHECK = true
                }
                ds.push(item)
            }
    ...
    provide: {
        grid: [Group, Page, Sort, Filter, Aggregate]
    }
}
</script>
<style>
