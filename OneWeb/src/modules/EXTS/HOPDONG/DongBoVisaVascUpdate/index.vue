...
export default {
  ...
  methods: {
    ...
    setLoginLimit: async function (account, phien) {
      return await vdc.setLoginLimit(account, phien)
    },
    ...
    hoangcong_hopdong_bancheo: async function (in_hdtb_id) {
      ...
      if (ds[0].hdtb_ban > 0) {
        ...
        await this.hoancong_hopdong_bancheo_v3(ds[0].tinhban, ds[0].hdtb_ban, ds[0].khachhang_tc_id,
    ...
    hoancong_hopdong_bancheo_v3: async function (tinh_ban, hdtb_ban, khachhang_tc_id,
    ...
    async thucHienDongBo () {
      ...
      let fTamDung = !!this.quytrinh_id && this.dsQuyTrinh.find(x => x.quytrinh_id.toString() === this.quytrinh_id.toString()).quytrinh.toLowerCase().includes('tạm dừng internet')

      if (this.loaihd_id == Enum.LoaiHopDong.CHAMDUT_SD || fTamDung) {
        console.log('this.currentItem', this.currentItem)
        if (this.loaihd_id == Enum.LoaiHopDong.THAY_DOI_DV && this.currentItem.ngay_bd_td !== null) {
          let f = moment(this.ngay_th).isAfter(moment(this.currentItem.ngay_bd_td, 'DD/MM/YYYY'))
          if (!f) {
            msg = 'Chưa đến ngày khách hàng yêu cầu tạm dừng. Bạn chắc chắn muốn tạm dừng dịch vụ của khách hàng ?'
          }
        } else if (this.loaihd_id == Enum.LoaiHopDong.CHAMDUT_SD) {
          let f = moment(this.ngay_th).isAfter(moment(this.currentItem.ngay_bd_td, 'DD/MM/YYYY'))
          if (!f) {
            msg = 'Chưa đến ngày khách hàng yêu cầu thanh lý. Bạn chắc chắn muốn thanh lý dịch vụ của khách hàng ?'
          }
        }
                    ...
                    if (kieu_mega == "1") {
                      ...
                      // #region Thay đổi tốc độ
                        ...
                        if (tocdo_id != tocdo_cu_id) {
                          ...
                          let f_loginLimitNew = 1
                          let f_loginLimitOld = 1
                          let phien_toida_new = 1
                          let phien_toida_old = 1

                          // set phiên
                          try {
                            f_loginLimitNew = this.dsTocDoADSL.find(x => x.tocdo_id === this.tocdo_adsl_id)
                            f_loginLimitOld = this.dsTocDoADSL.find(x => x.tocdo_id === this.currentItem.tdcu_id)
                            // console.log('set Phiên dòng 3842: ', this.tocdo_adsl_id, f_loginLimit)
                            // console.log('set Phiên dòng 3843: ', tocdo_cu_id, f_loginLimitOld)

                            if (f_loginLimitNew !== undefined && f_loginLimitOld !== undefined) {
                              if (f_loginLimitNew.phien_toida > -1 || f_loginLimitOld.phien_toida > -1) {
                                phien_toida_new = f_loginLimitNew.phien_toida > -1 ? f_loginLimitNew.phien_toida : 1
                                let f = await this.setLoginLimit(this.account, phien_toida_new)
                                if (!f) {
                                  this.$root.toastError(
                                    "Có lỗi trong quá trình set phiên !"
                                  )
                                  return 
                                }
                              }
                          ...
                          if (doitocdoadsl) {                            
                            ...
                            // set lại phiên cũ nếu lỗi
                            try {
                              if (f_loginLimitOld !== undefined) {
                                if (f_loginLimitOld.phien_toida > -1) {
                                  phien_toida_old = f_loginLimitOld.phien_toida
                                }
                              }
                              let f = await this.setLoginLimit(this.account, phien_toida_old)
                            } catch (error) {
                              console.log('Lỗi set lại phiên cũ: ', error)
                            }
                            return;
                          }
                        } else {
    ...
    Lay_ds_hdtb_gd_kichhoat: async function () {
      try {
        ...
        let data = {
          loai_hd_id: Number(this.loaihd_id),
          dichvu_vt_id: this.dichvuvt_id,
    ...
    async getSysDate () {
      try {
        ...
        return moment(rps.data[0].ngay_sys, 'DD/MM/YYYY HH:mm').toDate()
      } catch (error) {
        console.log(error)
        return null
      }
    ...
    async getQuyenTamNgung (nhanvien_id) {
      try {
        let res = await this.$root.context.get('/web-hopdong/dongbo-visa-vasc/get_quyen_tamngung_hd/' + nhanvien_id)
        return res.data        
      } catch (err) {
        console.log(err)
        return 0
    ...
    async kiemtraNgayTDTL (hdtb_id) {
      try {
        let res = await this.$root.context.get('/web-thicong/capnhatthongtin/fn_kiemtra_ngay_tdtl', input)
        if (res.data.length > 0) {
          id = res.data[0].id
        }
        return id
  ...
  created: async function () {
    try {
      ...
      let r = await this.getQuyenTamNgung(this.$auth.getNhanVienID())
      console.log('getQuyenTamNgung', r)
    } catch (e) {
      console.log(e);
    } finally {
      this.$root.showLoading(false);
