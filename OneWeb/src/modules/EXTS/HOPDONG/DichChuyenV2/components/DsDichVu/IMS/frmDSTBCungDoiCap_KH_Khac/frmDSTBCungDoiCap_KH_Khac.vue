  ...
  methods: {
    ...
    async HienThi_DS_Khachhang() {
      try {
        ...
        if (this.txTextBoxTC == "")
          dieukien1 = 0
        else dieukien1 = this.cboDKTraCuu.Value.replace(":values", this.txTextBoxTC)
        if (this.chkDK2) {
        ...
        if (tracuu && tracuu.kieu_tk == "Svlan/Cvlan") {
          ...
          else this.dtHDKH = await this.TRA_CUU_DANH_BA(dieukien, dichvuvt_id, donvi_id)
        }
    ...
    async HT_Kieu_TK_Cbo_Dev(cbo, nhom_id) {
      ...
      cbo.DataSource = rs.data
      if (cbo.DataSource.length > 0) cbo.Value = rs.data[0].dieukien
    },
