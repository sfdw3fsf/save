...
// import DSTBCungDoiCapBCModal from '@/modules/contract/setup/ChangeSubsTypeCrossSale/popups/DSTBCungDoiCapBCModal.vue'
// import DSTBLapKemBCModal from '@/modules/contract/setup/ChangeSubsTypeCrossSale/popups/DSTBLapKemBCModal.vue'
export default {
  name: 'frmThongSoWan',
  components: {
    api,
  ...
  methods: {
    async frmThongSoWan_Load () {
      try {
        if (this.loaitb == LoaiHinhTB.MGW_INTERNET_TRUCTIEP) {
          this.label38 = 'Tốc độ tổng'
          this.label21 = 'Tốc độ tổng'
          console.log('🚀 ~ file: frmThongSoWan.vue:212 ~ frmThongSoWan_Load ~ this.ThongsoBan:', this.ThongsoBan)
          console.log('🚀 ~ file: frmThongSoWan.vue:213 ~ frmThongSoWan_Load ~ this.ThongsoBan.phanloai_id:', this.ThongsoBan.phanloai_id)
          console.log('🚀 ~ file: frmThongSoWan.vue:215 ~ frmThongSoWan_Load ~ this.ThongsoBan.phantach_id:', this.ThongsoBan.phantach_id)
          console.log('🚀 ~ file: frmThongSoWan.vue:219 ~ frmThongSoWan_Load ~ this.cboPhanTachTDList:', this.cboPhanTachTDList)
          ...
          console.log('🚀 ~ file: frmThongSoWan.vue:217 ~ frmThongSoWan_Load ~ this.cboPhanTachTD:', this.cboPhanTachTD)
          console.log('🚀 ~ file: frmThongSoWan.vue:217 ~ frmThongSoWan_Load ~ this.cboPhanLoaiMGW:', this.cboPhanLoaiMGW)
    ...
    SetValues () {
      ...
      try {
        console.log('🚀 ~ file: frmThongSoWan.vue:323 ~ SetValues ~ this.ThongsoBan:', this.ThongsoBan)
        if (this.ThongsoBan == null) {
          console.log('Không có thông số bán để gán')
          return
        } else {
          if (this.ThongsoBan.loaikenh_id != null && this.ThongsoBan.loaikenh_id > 0) this.cboLoaiKenhBan = this.ThongsoBan.loaikenh_id
          ...
          if (this.ThongsoBan.tocdo_nix != null && this.ThongsoBan.tocdo_nix > 0) {
            this.chkTocDo_NIX = true
            console.log('🚀 ~ file: frmThongSoWan.vue:339 ~ SetValues ~ this.cboTocDo_NIXBanList:', this.cboTocDo_NIXBanList)
            this.cboTocDo_NIXBan = this.ThongsoBan.tocdo_nix
            this.changeTocDoNixBan()
          }
      ...
      try {
        console.log('🚀 ~ file: frmThongSoWan.vue:366 ~ SetValues ~ this.ThongsoTc:', this.ThongsoTc)
    ...
    async openDialog1 (p_tinh, ploaitb_ld, p_loaihinhtb, vloaihd, vdichvu_vt, tsBan, tsTc) {
      ...
      try {
        ...
        if (tsBan != null) this.ThongsoBan = tsBan
        else this.ThongsoBan = {}
        if (tsTc != null) this.ThongsoTc = tsTc
        else this.ThongsoTc = {}
        await this.loadData()
    ...
    async tsbtnCapNhat_Click () {
      try {
        ...
        if (this.loaitb == LoaiHinhTB.MGW_INTERNET_TRUCTIEP) {
          ...
          else if (parseInt(this.cboPhanTachTD) == 0) {
        ...
        if (parseInt(this.cboLoaiKenhBan) == 9) {
          ...
          try {
            ...
            if (dt_kieu_nt_tc.length == 0 || dt_kieu_nt_ban.length == 0) {
    ...
    async bancheo_kiemtra_thongsokt () {
      ...
      let obj = {
        ...
        mtu_size: this.txtMTU_Size_BAN,
        auto_nego: '',
        sncp: '',
        cap_chinh: '',
        cap_duphong: '',
        diem_pop: this.txtDiemPOP_BAN,
        wan_ip: this.txtIP_WAN_BAN,
        lan_ip: this.txtIP_LAN_BAN,
        ce_ip: this.txtIP_CE_BAN,
        tt_ip: this.txtThongTinIP_BAN,
        sl_ip: this.txtSL_IP_BAN,
        ten_trunk_nni: this.txtTenTrunkNNI_BAN,
        so_as: this.txtSo_AS_BAN,
        hqos: this.txtHqos_BAN,
        cos: this.txtCos_BAN,
        svlan: this.txtSVLAN_BAN,
        cvlan: this.txtCLANID_BAN,
        vlan_transparent: this.txtVLAN_Trans_BAN,
        tt_port: this.txtPort_POP_BAN
      }
...
*/
    ...
    async sp_lay_kieu_nghiemthu () {
      ...
      if (data != null && data.length > 0) {
        ...
        this.cboNT_DV_BANList = options[0].id
    ...
    async getcboLoaiKenhBanList () {
      ...
      console.log('🚀 ~ file: frmThongSoWan.vue:1754 ~ getcboLoaiKenhBanList ~ this.cboLoaiKenhBanList:', this.cboLoaiKenhBanList)
    ...
    async cboLoaiKenhBan_SelectedValueChanged (val) {
      try {
        ...
        else this.panelControl1_Visible = false
    ...
    changeTocDoNixBan (val) {
      // console.log('🚀 ~ file: frmThongSoWan.vue:2177 ~ changeTocDoNixBan ~ val:', this.cboTocDo_NIXBan)
      // console.log('🚀 ~ file: frmThongSoWan.vue:2181 ~ changeTocDoNixBan ~ this.cboTocDo_NIXBanList:', this.cboTocDo_NIXBanList)
      // let listNixBan = this.cboTocDo_NIXBanList.filter((x) => x.id === this.cboTocDo_NIXBan)
      // console.log('🚀 ~ file: frmThongSoWan.vue:2181 ~ changeTocDoNixBan ~ listNixBan:', listNixBan)
      // if (listNixBan.length > 0) {
      //   let listNixTC = this.cboTocDo_NIXTCList.filter((x) => x.text === listNixBan[0].text)
      //   console.log('🚀 ~ file: frmThongSoWan.vue:2185 ~ changeTocDoNixBan ~ listNixTC:', listNixTC)
      //   if (listNixTC.length > 0) {
      //     this.cboTocDo_NIXTc = listNixTC[0].id
      //     console.log('🚀 ~ file: frmThongSoWan.vue:2188 ~ changeTocDoNixBan ~ this.cboTocDo_NIXTc:', this.cboTocDo_NIXTc)
      //   }
      // }
      this.cboTocDo_NIXTc = this.cboTocDo_NIXBan
      console.log('🚀 ~ file: frmThongSoWan.vue:2192 ~ changeTocDoNixBan ~ this.cboTocDo_NIXTc:', this.cboTocDo_NIXTc)
    },
    changeTocDoISPBan (val) {
      console.log('🚀 ~ file: frmThongSoWan.vue:2181 ~ changeTocDoISPBan ~ val:', val)
