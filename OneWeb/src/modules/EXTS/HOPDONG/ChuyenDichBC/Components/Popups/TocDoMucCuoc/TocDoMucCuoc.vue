  ...
  <div class="popup-body">
    ...
    <ejs-dialog :enableResize='true' :visible='false' ref='DSTBCungDoiCapBCDialog' :position='position'
      ...
      <DSTBCungDoiCapBC ref='DSTBCungDoiCapBCPopup' :khachhang_id="khachhang_id"
                        :tinhtc="tinhtc" :kieuld_id="Number(cboKieuLDTC.SelectedValue)" @accept="acceptDSTBCungDoiCapBC" />
    ...
    <ejs-dialog :enableResize='true' :visible='false' ref='DSTBLapKemBCDialog' :position='position'
      ...
      <DSTBLapKemBC ref='DSTBLapKemBCPopup' :hdkh_id="hdkh_id"
                    :vkieuld_id="Number(thongso_tc.kieuld_id)" @accept="acceptDSTBLapKemBC" />
...
import MucCuocThueBao from '../MucCuocThueBao/MucCuocThueBao'
...
export default {
  ...
  watch: {
    cboTocDoBan: {
      handler: async function () {
        ...
        if (!this.kt_load) { await this.Lay_DS_MucCuoc_TB_Ban(this.pLoaihinhTB, this.cboDoiTuongBan.SelectedValue, moment().format('DD/MM/YYYY'), 3) }
        if (this.chkTocDoTC.Checked == true) {
          ...
          try {
            if (this._vloaihd == LoaiHopDong.CHUYEN_QUYEN || this._vloaihd == LoaiHopDong.DI_CHUYEN || this._vloaihd == LoaiHopDong.KHOIPHUC_SD) {
              ...
              console.log('🚀 ~ file: TocDoMucCuoc.vue:669 ~ response1:', response1)
            ...
            this.cboTocDoTC.SelectedValue = 0
          }
          try {
    ...
    cboTocDoTC: {
      handler: async function () {
        if (this.cboTocDoTC.list.length > 0) {
          ...
          if (!this.kt_load) {
            console.log('🚀 ~ file: TocDoMucCuoc.vue:743 ~ this.kt_load:', this.kt_load)
    ...
    cboTrangBiBan: {
      handler: async function () {
        ...
        if (!this.kt_load) { await this.Lay_DS_MucCuoc_TB_Ban(this.pLoaihinhTB, this.cboDoiTuongBan.SelectedValue, moment().format('DD/MM/YYYY'), this.cboTrangBiBan.SelectedValue) }
      },
      deep: true
    ...
    cboDoiTuongTC: {
      handler: async function () {
        if (!this.kt_load) {
          console.log('🚀 ~ file: TocDoMucCuoc.vue:771 ~ this.kt_load:', this.kt_load)
          await this.Lay_DS_MucCuoc_TB_TC(this.pLoaihinhTB, this.cboDoiTuongTC.SelectedValue, moment().format('DD/MM/YYYY'), this.cboTrangBiTC.SelectedValue)
        }
      },
      deep: true
    ...
    cboTrangBiTC: {
      handler: async function () {
        if (!this.kt_load) {
          console.log('🚀 ~ file: TocDoMucCuoc.vue:781 ~ this.kt_load:', this.kt_load)
          await this.Lay_DS_MucCuoc_TB_TC(this.pLoaihinhTB, this.cboDoiTuongTC.SelectedValue, moment().format('DD/MM/YYYY'), this.cboTrangBiTC.SelectedValue)
        }
      },
      deep: true
  ...
  methods: {
    async frmTocDoMucCuoc_Load () {
      try {
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1008 ~ frmTocDoMucCuoc_Load ~ this.thongso_ban:', this.thongso_ban)
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1008 ~ frmTocDoMucCuoc_Load ~ this.thongso_tc:', this.thongso_tc)
        let dsLoaiHinhLocTocDo = ',11,'
        if (dsLoaiHinhLocTocDo.indexOf(',' + this._ploaitb_td + ',') > 1) { this.chkTocDoTC.Visible = true }
        ...
        if (this._vloaihd == LoaiHopDong.DAT_MOI) { await this.load_dm_tocdo_adsl() } else { await this.load_dm_tocdo_adsl_dc() }
        ...
        if (this.thongso_ban.congnghe_id > 0) { this.cboCongNgheBan.SelectedValue = this.thongso_ban.congnghe_id }

        if (this.thongso_tc.congnghe_id > 0) { this.cboCongNgheTC.SelectedValue = this.thongso_tc.congnghe_id }

        let diachiban = this.Diachi_null()
        this.diachi_ban = {...diachiban}
        if (this.thuebao_id_tc != 0) { await this.lay_diachi_dbtb_bancheo() } else {
        ...
        if (this.thongso_tc.kieuld_id == this.KieuLapDat.THAYDOI_KIEUTRA_MYTV) {
          ...
          if (this.thongso_ban.kieutra_id == this.KieuTra.TRASAU) { this.cboKieuTraBan.SelectedValue = this.KieuTra.TRATRUOC } else { this.cboKieuTraBan.SelectedValue = this.KieuTra.TRASAU }
        }
        if (this.thongso_ban.kieutra_id == 0) { this.cboKieuTraBan.SelectedValue = 2 } else { this.cboKieuTraBan.SelectedValue = this.thongso_ban.kieutra_id }
        ...
        if (this.thongso_ban.kieuld_id == this.KieuLapDat.THAYDOI_MUCCUOC_ADSL || this.thongso_ban.kieuld_id == this.KieuLapDat.THAYDOI_TOCDO_MUCCUOC_ADSL) {
          if (this.thongso_ban.muccuoc_id == this.MUCCUOC.LUU_LUONG) { this.cboMucCuocBan.SelectedValue = this.MUCCUOC.TRON_GOI }
          this.cboMucCuocTC.SelectedValue = this.MUCCUOC.TRON_GOI
          if (this.thongso_ban.muccuoc_id == this.MUCCUOC.TRON_GOI) { this.cboMucCuocBan.SelectedValue = this.MUCCUOC.LUU_LUONG }
        ...
        if (this.thongso_ban.tocdo_id != 0) {
          if (this._ploaitb_td == LoaiHinhTB.INTERNET_MYTV && this.thongso_ban.kieuld_id != this.KieuLapDat.THAYDOI_KIEUTRA_MYTV) {
            ...
            console.log('🚀 ~ file: TocDoMucCuoc.vue:1132 ~ frmTocDoMucCuoc_Load ~ this.cboTocDoTC.SelectedValue:', this.cboTocDoTC.SelectedValue)
        ...
        if (this.thongso_ban.muccuoctb_id > 0) { this.cboMucCuocTBBan.SelectedValue = this.thongso_ban.muccuoctb_id }
        if (this.thongso_tc.muccuoctb_id > 0) {
          console.log('🚀 ~ file: TocDoMucCuoc.vue:1135 ~ frmTocDoMucCuoc_Load ~ this.thongso_tc.muccuoctb_id :', this.thongso_tc.muccuoctb_id)
          this.cboMucCuocTBTC.SelectedValue = this.thongso_tc.muccuoctb_id
        }

        this.kt_load = false
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1148 ~ frmTocDoMucCuoc_Load ~ this.kt_load:', this.kt_load)

        if (this._vloaihd == LoaiHopDong.KHOIPHUC_SD) { this.thongso_khoiphuc(this.dichvuvt_id) }
        ...
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1180 ~ frmTocDoMucCuoc_Load ~ error:', error)
      }
    },
    ...
    // Sự kiện button
    tsbtnCapNhat_Click () {
      console.log(this.cboMucCuocTBBan.list)
    ...
    async chkTocDoTC_CheckedChanged () {
      console.log('🚀 ~ file: TocDoMucCuoc.vue:1365 ~ chkTocDoTC_CheckedChanged ~ this.chkTocDoTC.Checked:', this.chkTocDoTC.Checked)
      if (this.chkTocDoTC.Checked === true) {
        let maTocDoTinhBan = ''
        if (this.cboTocDoBan.list.filter(x => x.TOCDO_ID == this.cboTocDoBan.SelectedValue).length > 0) {
          maTocDoTinhBan = this.cboTocDoBan.list.filter(x => x.TOCDO_ID == this.cboTocDoBan.SelectedValue)[0].MATOCDO
        ...
        if (this.thongso_tc.tocdo_id == 0) { this.cboTocDoTC.SelectedValue = this.cboTocDoTC.list[0].tocdo_id } else { this.cboTocDoTC.SelectedValue = this.thongso_tc.tocdo_id }
      }
    },
    // API
    async Lay_DS_MucCuoc_TB_Ban (vloaitb_id, vdoituong_id, ngay, vtrangbi_id) {
      try {
        ...
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1417 ~ Lay_DS_MucCuoc_TB_Ban ~ this._vloaihd:', this._vloaihd)
        if (this._vloaihd == LoaiHopDong.DAT_MOI || this._vloaihd == LoaiHopDong.THAY_DOI_TOCDO_ADSL || this._vloaihd == LoaiHopDong.CHUYENDOI_LH) {
          ...
          if (response1.data.error_code === 'BSS-00000000' && response1.data.data) {
            ...
            this.cboMucCuocBan.SelectedValue = this.cboMucCuocTBBan.list[0].muccuoctb_id
          } else {
            this.cboMucCuocTBBan.list = []
          }
        } else {
          let response = await api.lay_ds_muccuoc_tb_ban_frmtocdomuccuoc(this.axios, {
            'ds_para': JSON.stringify({'LOAIHD_ID': this._vloaihd,
              'LOAITB_ID': vloaitb_id,
              'DOITUONG_ID': vdoituong_id,
              'TOCDO_ID': vtocdo_id,
              'MUCCCUOC_ID': vmuccuoc_id,
              'NGAY': ngay,
              'TRANGBI_ID': vtrangbi_id,
              'TOCDO_BAN_ID': this.cboTocDoBan.SelectedValue})
          })
          console.log('🚀 ~ file: TocDoMucCuoc.vue:1458 ~ Lay_DS_MucCuoc_TB_Ban ~ response:', response)
          if (response.data.error_code === 'BSS-00000000' && response.data.data) {
    ...
    async Lay_DS_MucCuoc_TB_TC (vloaitb_id, vdoituong_id, ngay, vtrangbi_id) {
      console.log('🚀 ~ file: TocDoMucCuoc.vue:1487 ~ Lay_DS_MucCuoc_TB_TC ~ vloaitb_id, vdoituong_id, ngay, vtrangbi_id:', vloaitb_id, vdoituong_id, ngay, vtrangbi_id)
      try {
        let vtocdo_id = 0
        let vmuccuoc_id = 0 // 2 Lưu lượng
        ...
        if (this._vloaihd == LoaiHopDong.DAT_MOI || this._vloaihd == LoaiHopDong.THAY_DOI_TOCDO_ADSL || this._vloaihd == LoaiHopDong.CHUYENDOI_LH) {
          ...
          if (response1.data.error_code === 'BSS-00000000' && response1.data.data) {
            ...
            console.log('🚀 ~ file: TocDoMucCuoc.vue:1522 ~ Lay_DS_MucCuoc_TB_TC ~ this.cboMucCuocTBTC.SelectedValue:', this.cboMucCuocTBTC.SelectedValue)
          } else {
            this.cboMucCuocTBTC.list = []
          }
        } else {
          ...
          console.log('🚀 ~ file: TocDoMucCuoc.vue:1533 ~ Lay_DS_MucCuoc_TB_TC ~ response:', response)
          if (response.data.error_code === 'BSS-00000000' && response.data.data) {
            this.cboMucCuocTBTC.list = response.data.data
            console.log('🚀 ~ file: TocDoMucCuoc.vue:1536 ~ Lay_DS_MucCuoc_TB_TC ~ this.cboMucCuocTBTC.list:', this.cboMucCuocTBTC.list)
            if (this.cboMucCuocTBTC.list.length > 0) {
              this.cboMucCuocTBTC.SelectedValue = this.cboMucCuocTBTC.list[0].mucuoctb_id
              console.log('🚀 ~ file: TocDoMucCuoc.vue:1547 ~ Lay_DS_MucCuoc_TB_TC ~ this.cboMucCuocTBTC.list[0].muccuoctb_id:', this.cboMucCuocTBTC.list[0].mucuoctb_id)
            }
            console.log('🚀 ~ file: TocDoMucCuoc.vue:1534 ~ Lay_DS_MucCuoc_TB_TC ~ this.cboMucCuocTBTC.SelectedValue:', this.cboMucCuocTBTC.SelectedValue)
          } else {
            this.cboMucCuocTBTC.list = []
          }
        }
    ...
    async ht_kieu_ld_combobox () {
      ...
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
        ...
        if (this.thongso_tc.kieuld_id == 0) { this.cboKieuLDTC.SelectedValue = this.cboKieuLDTC.list[0].kieuld_id } else { this.cboKieuLDTC.SelectedValue = this.thongso_tc.kieuld_id }
      }
    },
    ...
    async load_dm_tocdo_adsl () {
      ...
      console.log('🚀 ~ file: TocDoMucCuoc.vue:1588 ~ load_dm_tocdo_adsl ~ response:', response)
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
    ...
    async load_dm_tocdo_adsl_dc () {
      ...
      console.log('🚀 ~ file: TocDoMucCuoc.vue:1602 ~ load_dm_tocdo_adsl_dc ~ response:', response)
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
        this.cboTocDoTC.list = response.data.data
        // this.cboTocDoTC.SelectedValue = this.cboTocDoTC.list[0].tocdo_id
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1608 ~ load_dm_tocdo_adsl_dc ~ this.thongso_tc.tocdo_id:', this.thongso_tc.tocdo_id)
        if (this.thongso_tc.tocdo_id === 0) { this.cboTocDoTC.SelectedValue = this.cboTocDoTC.list[0].tocdo_id } else { this.cboTocDoTC.SelectedValue = this.thongso_tc.tocdo_id }
      }
    },
    async formload_frmtocdomuccuoc () {
      console.log('🚀 ~ file: TocDoMucCuoc.vue:1614 ~ formload_frmtocdomuccuoc ~ this.thongso_ban:', this.thongso_ban)
      ...
      if (response.data.error_code === 'BSS-00000000' && response.data.data && response.data.data.ERROR_CODE == 1) {
        ...
        console.log('🚀 ~ file: TocDoMucCuoc.vue:1623 ~ formload_frmtocdomuccuoc ~ result:', result)
        ...
        if (this.cboTocDoBan.list.length > 0) {
          ...
          if (this.thongso_ban.tocdo_id == 0) { this.cboTocDoBan.SelectedValue = this.cboTocDoBan.list[0].TOCDO_ID } else { this.cboTocDoBan.SelectedValue = this.thongso_ban.tocdo_id }
        }
        this.cboKieuTraBan.list = result.DS_KIEUTRA
        this.cboKieuTraBan.SelectedValue = this.cboKieuTraBan.list[0].KIEUTRA_ID
        if (this.thongso_ban.kieutra_id == 0) { this.cboKieuTraBan.SelectedValue = this.cboKieuTraBan.list[0].KIEUTRA_ID } else { this.cboKieuTraBan.SelectedValue = this.thongso_ban.kieutra_id }
        this.cboMucCuocBan.list = result.CBO_MUCCUOC_BAN
        this.cboMucCuocBan.SelectedValue = this.cboMucCuocBan.list[0].MUCCUOC_ID
        if (this.thongso_ban.muccuoc_id == 0) { this.cboMucCuocBan.SelectedValue = this.cboMucCuocBan.list[0].MUCCUOC_ID } else { this.cboMucCuocBan.SelectedValue = this.thongso_ban.muccuoc_id }
        ...
        if (this.thongso_tc.muccuoc_id === 0) {
          this.cboMucCuocTC.SelectedValue = this.cboMucCuocTC.list[0].MUCCUOC_ID
          console.log('🚀 ~ file: TocDoMucCuoc.vue:1622 A~ formload_frmtocdomuccuoc ~ this.thongso_tc.muccuoc_id :', this.thongso_tc.muccuoc_id)
        } else {
          this.cboMucCuocTC.SelectedValue = this.thongso_tc.muccuoc_id
          console.log('🚀 ~ file: TocDoMucCuoc.vue:1622 B~ formload_frmtocdomuccuoc ~ this.thongso_tc.muccuoc_id :', this.thongso_tc.muccuoc_id)
        }
        ...
        if (result.DS_CQ_TC.filter(x => x.TINH_ID == this.$root.token.getPhanVungID()).length > 0) { this.cboChuQuan_TC.SelectedValue = result.DS_CQ_TC.filter(x => x.TINH_ID == this.$root.token.getPhanVungID())[0].CHUQUAN_ID }
        this.cboChuQuanLD_TC.list = result.DS_CQ_TC
        this.cboChuQuanLD_TC.SelectedValue = this.cboChuQuanLD_TC.list[0].CHUQUAN_ID
        if (result.DS_CQ_TC.filter(x => x.TINH_ID == this.tinhtc).length > 0) { this.cboChuQuanLD_TC.SelectedValue = result.DS_CQ_TC.filter(x => x.TINH_ID == this.tinhtc)[0].CHUQUAN_ID }

        this.cboDoiTuongBan.list = result.CBO_DOITUONG_BAN
        this.cboDoiTuongBan.SelectedValue = this.cboDoiTuongBan.list[0].DOITUONG_ID
        if (this.thongso_ban.doituong_id == 0) { this.cboDoiTuongBan.SelectedValue = this.cboDoiTuongBan.list[0].DOITUONG_ID } else { this.cboDoiTuongBan.SelectedValue = this.thongso_ban.doituong_id }

        this.cboTrangBiBan.list = result.CBO_TRANGBI_BAN
        this.cboTrangBiBan.SelectedValue = this.cboTrangBiBan.list[0].TRANGBI_ID
        if (this.thongso_ban.trangbi_id > 0) { this.cboTrangBiBan.SelectedValue = this.thongso_ban.trangbi_id }
      }
    },
    async ht_loai_tb_lhtb_combobox (kieucb) {
      ...
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
        if (kieucb == 1) {
          ...
          if (this.thongso_ban.thietbidc_id == 0) { this.cboThietBiDCBan.SelectedValue = this.cboThietBiDCBan.list[0].thietbidc_id } else { this.cboThietBiDCBan.SelectedValue = this.thongso_ban.thietbidc_id }
        } else if (kieucb == 2) {
          this.cboThietBiDCTC.list = response.data.data
          this.cboThietBiDCTC.SelectedValue = this.cboThietBiDCTC.list[0].thietbidc_id
          if (this.thongso_tc.thietbidc_id > 0) { this.cboThietBiDCTC.SelectedValue = this.thongso_tc.thietbidc_id }
    ...
    async load_dm_doi_tuong () {
      ...
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
        ...
        if (this.thongso_tc.doituong_id == 0) { this.cboDoiTuongTC.SelectedValue = this.cboDoiTuongTC.list[0].doituong_id } else { this.cboDoiTuongTC.SelectedValue = this.thongso_tc.doituong_id }
      }
    },
    async load_dm_trang_bi () {
      ...
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
        ...
        if (this.thongso_tc.trangbi_id > 0) { this.cboTrangBiTC.SelectedValue = this.thongso_tc.trangbi_id }
      }
    },
    ...
    async layMaTN_CD () {
      ...
      console.log('🚀 ~ file: TocDoMucCuoc.vue:1708 ~ layMaTN_CD ~ response:', response)
      if (response.data.error_code === 'BSS-00000000' && response.data.data) {
      ...
      if (kieu == 2) // Lắp mới trên đường có sẵn
      {
        if (this.khachhang_id > 0) {
          // await this.$refs.DSTBCungDoiCapBCDialog.$children[0].loadData()
