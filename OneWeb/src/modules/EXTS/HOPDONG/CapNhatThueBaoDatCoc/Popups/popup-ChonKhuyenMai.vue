    ...
    <div class="popup-body">
        ...
        <div class="info-row">
          <div class="key w140">
            <div class="check-action">
              <input type="checkbox" class="check" v-model="filterbox.congvan.isenabled" @change="HienThiKhuyenMai" />
              <span class="name">Số công văn</span>
            </div>
          </div>
          <div class="value" :class="{disabled:!filterbox.congvan.isenabled}">
            <div class="select-custom">
              <select2 :disabled="!filterbox.congvan.isenabled" v-model="filterbox.congvan.value" class="select-custom" :options="filterbox.congvan.list.map(ls=>({id:ls.congvan_id,text:ls.ten_cv}))" @change="HienThiKhuyenMai"></select2>
            </div>
          </div>
        </div>
        <div class="row">
      ...
      <div class="box-form">
        ...
        <div class="table-content">
            <DataGrid class="table-result table-gachle table-filter"
  ...
  methods: {
    ...
    async HienThiKhuyenMai () {
      try {
        ...
        this.filterbox.khuyenmai.list = rs.data === null ? [] : rs.data
