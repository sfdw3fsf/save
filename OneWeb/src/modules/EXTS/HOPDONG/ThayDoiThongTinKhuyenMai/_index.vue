  ...
  created() {
    this.$watch(
      ...
      (newValue, oldValue) => {
        ...
        if (this.ItemList) {
          // console.log('this.$refs["SO_THANG"]', this.$refs["SO_THANG"])
          if (this.$refs["SO_THANG"] !== undefined && this.$refs["SO_THANG"] !== null && this.$refs["SO_THANG"].length > 0) {
  ...
  methods: {
    ...
    hienThiThongTinHopDongTB: async function (items) {
      try {
        ...
        if (items.length > 0) {
          let data = items[0]
          console.log('hienThiThongTinHopDongTB', data)
          this.cbo_chuquan_id.Value = data.chuquan_id;
    ...
    async TaoDuLieu_HDTB_TSL(themmoi) {
      ...
      try {
        ...
        if (dt.data === null && dt.data.length <= 0) return
        for (let item of dt.data) {
          ...
          // for(const key in Object.keys(rowHDTB_TSL)) {
          //   let k = key.toUpperCase()
          //   row[k] = rowHDTB_TSL[key]
          // }
          console.log(row)
