...
<script>
import ModalThuHoiIP from './ModalThuHoiIP/index.vue'
import ModalThuHoiDaiIP from './ModalThuHoiDaiIP/index.vue'
import ModalThuHoiThietBiVaPort from './ModalThuHoiThietBiVaPort/index.vue'
...
export default {
  ...
  computed: {
    ...
    shouldHideButtons() {
      ...
      if (this.$root.token.getNhanVienID() != this.thongTinCongViec.NHANVIEN_TH_ID 
        && this.thongTinCongViec.TRANGTHAI_ID_PHIEU_YC != 21) {
        return true;
      }
    ...
    // computed property để kiểm tra tất cả IP đã được thu hồi chưa
    isAllIPThuHoi() {
      // Nếu không có data thì return false
      if (!this.childGridDaiIP.dataSource || this.childGridDaiIP.dataSource.length === 0) {
        return false;
      }
      // Kiểm tra xem tất cả các IP có đều ở trạng thái "đã thu hồi" không
      return this.childGridDaiIP.dataSource.every(item => {
        const trangThai = item.TRANGTHAICAPPHAT;
        return trangThai && (
          trangThai.toLowerCase().includes('đã thu hồi')
        );
      });
    },
    ...
    isHoanThanhDisabled() {
      // Nếu không có dải IP nào thì disable
      if (this.isDataGridEmpty) {
        return true;
      }
      // Nếu có dải IP nhưng chưa có IP con nào thì disable
      if (this.isDataChildGridEmpty) {
        return true;
      }
      // Chỉ enable khi tất cả IP đã được thu hồi
      return !this.isAllIPThuHoi;
    },
    isEditMode() {
  ...
  methods: {
    ...
    async loadThongTinCongViec() {
      try {
        ...
        if (response.data.length > 0) {
          ...
          this.isThuHoiDaiIP = response.data[0].LOAI_CV.toLowerCase().includes('dải ip');
        } else {
    ...
    openModalThuHoi() {
      ...
      if (this.isThuHoiDaiIP) {
        this.$refs.ModalThuHoiDaiIP.openDialog();
      } else {
        this.$refs.ModalThuHoiIP.openDialog();
      }
    },
      ...
      return function () {
        ...
        if (self.shouldHideButtons) {
          ...
          return {
              template: {
                  template: `<div class="action-buttons gap-1">
                    <button class="btn btn-sm btn-outline-success" v-if="canThuHoi()" title="Thu hồi thiết bị" @click="onCapPhatThietBi()">
                      <i class="fa fa-cogs"></i> Thu hồi
                    </button>
                  </div>`,
                  methods: {
                    canThuHoi() {
                      const rowData = this.dataItem || this.rowData || this.data || this;
                      const thietBiID = rowData.THIETBI_ID ?? null;
                      const congID = rowData.CONG_ID ?? null;
                      return thietBiID && congID && rowData.IS_THUHOI == 0;
                    },
                    onCapPhatThietBi: function () {
                      ...
                      self.openModalThuHoiThietBiVaPort();
                    }
                  }
              },
          };
    ...
    openModalThuHoiThietBiVaPort() {
      ...
      this.$refs.ModalThuHoiThietBiVaPort.openDialog({
