<template>
  <b-modal id="drawAislePolygonModal" ref="drawAislePolygonModal" size="xl" hide-footer @hidden="onModalHidden">
    <template #modal-header>
      <h5 class="modal-title">
        <i :class="isEditingExisting ? 'fa fa-edit' : 'fa fa-draw-polygon'"></i>
        {{ isEditingExisting ? 'Sửa hành lang: ' : 'Vẽ hành lang: ' }}{{ aisleData ? aisleData.name : '' }}
      </h5>
    ...
    <div v-if="aisleData">
      <div v-if="!editMode">
        ...
        <div class="text-info small">
          <span
            >Loại: <span :style="{ color: aisleData.color }">{{ getAisleTypeLabel(aisleData.type) }}</span></span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    show (aisleData, existingPolygon = null) {
    ...
    hide () {
      this.$refs.drawAislePolygonModal.hide()
    },

    getAisleTypeLabel (type) {
      const typeMap = {
        hot: '🔥 Hành lang nóng',
        cold: '❄️ Hành lang lạnh',
        main: '🚪 Hành lang chính',
        intermediate: '🌡️ Hành lang trung gian',
        emergency: '🚶 Hành lang khẩn cấp'
      }
    ...
    drawCanvas () {
      ...
      if (this.polygonPoints.length > 0) {
        ...
        if (this.polygonPoints.length >= 3) {
          ...
          ctx.fillStyle = (this.aisleData.color || '#2196F3') + '20'
          ctx.fill()
        }

        ctx.strokeStyle = this.aisleData.color || '#2196F3'
        ctx.lineWidth = 2
        ctx.stroke()
        ...
        this.polygonPoints.forEach((point, index) => {
          ...
          if (isSelected) {
            ...
            ctx.fillStyle = this.aisleData.color || '#2196F3'
          }
    ...
    onKeyDown (event) {
      if (!this.$refs.drawAislePolygonModal || !this.$refs.drawAislePolygonModal.$data.isVisible) {
        return
      }
