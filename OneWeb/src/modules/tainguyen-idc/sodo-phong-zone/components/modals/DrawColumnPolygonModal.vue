<template>
  <b-modal id="drawColumnPolygonModal" ref="drawColumnPolygonModal" size="xl" hide-footer @hidden="onModalHidden">
    <template #modal-header>
      <h5 class="modal-title">
        <i :class="isEditingExisting ? 'fa fa-edit' : 'fa fa-draw-polygon'"></i>
        {{ isEditingExisting ? 'Sửa cột: ' : 'Vẽ cột: ' }}{{ columnData ? columnData.name : '' }}
      </h5>
    ...
    <div v-if="columnData">
      <div v-if="!editMode">
        ...
        <div class="text-info small">
          <span>Loại: <span :style="{ color: columnData.color }">{{ columnData.name }}</span></span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    show (columnData, existingPolygon = null) {
      ...
      this.$refs.drawColumnPolygonModal.show()
    ...
    hide () {
      this.$refs.drawColumnPolygonModal.hide()
    },
    ...
    drawCanvas () {
      ...
      if (this.polygonPoints.length > 0) {
        ...
        if (this.polygonPoints.length >= 3) {
          ...
          ctx.fillStyle = (this.columnData.color || '#9C27B0') + '20'
          ctx.fill()
        }

        ctx.strokeStyle = this.columnData.color || '#9C27B0'
        ctx.lineWidth = 2
        ctx.stroke()
        ...
        this.polygonPoints.forEach((point, index) => {
          ...
          if (isSelected) {
            ...
            ctx.fillStyle = this.columnData.color || '#9C27B0'
          }
    ...
    drawGrid (ctx) {
      ...
      // ✅ Luôn vẽ grid toàn bộ phòng (như drawCage)
    ...
    onKeyDown (event) {
      if (!this.$refs.drawColumnPolygonModal || !this.$refs.drawColumnPolygonModal.$data.isVisible) {
        return
      }
