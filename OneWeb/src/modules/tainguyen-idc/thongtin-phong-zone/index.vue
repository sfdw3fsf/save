...
import PopupPhongZoneDetail from './components/PopupPhoneZoneDetail/PopupPhongZoneDetail.vue'
import api from './api'
...
export default {
  ...
  data() {
    return {
      ...
      datePickerHasInteraction: false
    }
  },
  computed: {
    grdItems: function () {
    ...
    thuocTinhColumns() {
      return [
        ...
        { fieldName: 'GIATRI', headerText: 'Giá trị', allowFiltering: true, allowSorting: true },
        {
          fieldName: 'ACTIONS',
  ...
  created: async function () {
    try {
      ...
      const idxMatSan = this.importConfig.validationRules.findIndex(
        (r) => r.field === 'matsanId' && r.validator === null
      )
      if (idxMatSan !== -1) {
  ...
  watch: {
    ...
    'form.matsanId'(val) {
      this.validateField('matsanId', val)
    },
    'form.ten'(val) {
  ...
  methods: {
    ...
    openEditPopup(itemId) {
      ...
      this.$nextTick(() => {
        this.$refs.popupPhongZone.open(itemId)
      })
    },
    ...
    validateField(field, value) {
      ...
      const fieldValidationMap = {
        matsanId: { control: 'selectMatSan', type: 'select' },
    ...
    processPhongZoneExcelData(dataRows) {
      ...
      return nonEmptyRows.map((row) => {
        return {
          matsanId: row[0] ? row[0].toString().trim() : '',
    ...
    formatPhongZoneCellValue(value, fieldName) {
      ...
      // Format cho trường mặt sàn (matsanId)
      if (fieldName === 'matsanId' && this.matSanSelectionList && this.matSanSelectionList.length > 0) {
    ...
    processImportData: async function (importData) {
      try {
        ...
        const validItems = (importData || []).map((item) => {
          return {
            id: null,
            matsanId: this.convertMatSanNameToId(item.matsanId),
            ten: item.ten?.trim() || '',
    ...
    async handleCheckData(checkDataInfo) {
      try {
        const payload = (checkDataInfo?.data || []).map((row) => {
          return {
            id: '',
            matsanId: this.convertMatSanNameToId(row.matsanId),
            ten: row.ten || '',
        ...
        if (flag) {
          try {
            ...
            if (!this.isFromModal) {
              ...
              const currentItem = dsPhongZone.find((pz) => pz.ID == this.roomId)
              if (currentItem) {
                this.setCurrentItem(currentItem)
                this.dsPhongZone = [currentItem]
              }
    ...
    setCurrentItem: async function (item) {
      ...
      this.form.matsanId = item.ID_MATSAN
      ...
      this.form.slRackThietKe = item.SL_RACK_THIETKE || null
      this.form.slRackDaLapDat = item.SL_RACK_DALAP || null
      this.form.slRackKhaDung = item.SL_RACK_THIETKE - item.SL_RACK_DALAP
    ...
    onDatePickerChange(args) {
      this.datePickerHasInteraction = true
    },
    validateDate(value) {
      // If user has interacted with datepicker and value is null, it means invalid input
      if ((value === null || value === undefined) && this.datePickerHasInteraction && this.buttons.btnSave.enabled) {
