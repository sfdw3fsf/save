  ...
  methods: {
    async loadLoaiCongViec() {
      ...
      if (response && response.data && response.data.length > 0) {
        this.searchConditions.tenCv = response.data.find(item => item.LOAICV_ID == 31)?.TEN || '';
      }
    },
    async loadDanhMuc() {
      try {
    ...
    dsLapDatRack() {
      this.$router.push({ path: `/tainguyen-idc/danhsach-congviec?loaicv=31` })
    },

    viewDetails(data) {
      this.$router.push({
        path: '/tainguyen-idc/lapdat-rack-form',
            ...
            computed: {
              isEditable() {
                ...
                const editable =
                  ...
                  console.log('isEditable', currentNhanVienId, this.data.NHANVIEN_TH_ID);
                return editable;
              }
            },
            methods: {
