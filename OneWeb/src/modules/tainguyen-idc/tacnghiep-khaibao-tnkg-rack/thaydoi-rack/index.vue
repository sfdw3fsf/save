...
import ThayDoiRackModal from './modals/thaydoi-rack-modal.vue';
import ThiCongThayDoiRack from './modals/thicong-thaydoi-rack.vue';
  ...
  methods: {
    ...
    loadAllocatedRacks: async function () {
      try {
        ...
        console.error('Error loading allocated racks:', error);
        this.dataGrid2Items = [];
      }
    },
    ...
    onComplete: async function () {
      ...
      if (flag) {
        try {
          let rs = false;
          let response = null;
            response = await this.$root.context.post(`/web-ecms/thicong-rack/hoanthanh-thaydoi-rack-idc/${this.phieutc}`);
          rs = response.data.data;
          if (response || rs) {
            this.$toast.success('Hoàn thành phiếu thành công');
            this.$router.push({
              path: '/tainguyen-idc/ketqua-thaydoi-rack'
            })
          } else {
    ...
    thiCongThayDoiRack(rackId) {
      if (this.$refs.popupThiCongThayDoiRack) {
        this.$refs.popupThiCongThayDoiRack.dialogOpen(rackId);
      }
    },
    ...
    xoaThayDoiRack: async function (data) {
      if (!this.phieutc) {
      ...
      let flag = await this.$confirm(
        `Bạn có chắc chắn muốn gỡ thay đổi rack "${data.TEN_RACK}" không?`,
        "Xác nhận gỡ",
        {
          confirmButtonText: "Xác nhận",
          cancelButtonText: "Hủy",
      ...
      if (flag) {
        try {
          ...
          if (response && response.error && response.error == "200") {
            this.$toast.success('Gỡ thay đổi Rack thành công');
            await this.loadAllocatedRacks();
          } else {
            this.$toast.error('Gỡ thay đổi Rack thất bại');
          }
        } catch (error) {
          console.error('Error deleting rack allocation:', error);
          this.$toast.error('Có lỗi xảy ra khi gỡ thay đổi Rack');
        } finally {
          this.$root.showLoading(false);
        }
      }
        ...
        return {
          template: {
            template: `<div class="action-buttons">
                <button v-if="onRemoveRack" class="btn btn-sm btn-outline-danger ml-1" title="Gỡ thay đổi" @click="deleteItem()">
                ...
                <button v-if="onProcessRack && !data.isProcessed" class="btn btn-sm btn-outline-success" title="Xử lý" @click="xuLyItem()">
                  <i class="fa fa-cogs"></i> Thay đổi
                </button>
                <button v-if="data.isProcessed && onProcessRack" class="btn btn-sm btn-success" title="Đã thay đổi" disabled>
                  <i class="fa fa-check"></i> Đã thay đổi
                </button>
            ...
            methods: {
              deleteItem() {
                parent.xoaThayDoiRack(this.data)
              },
              xuLyItem() {
                parent.rackId = this.data.RACK_ID;
                if (parent.rackId != null) {
                  parent.thiCongThayDoiRack(parent.rackId);
                }
              }
            }
          }
