  ...
  methods: {
    async loadLoaiCongViec() {
      ...
      if (response && response.data && response.data.length > 0) {
        this.searchConditions.tenCv = response.data.find(item => item.LOAICV_ID == 32)?.TEN || '';
      }
    },
    async loadDanhMuc() {
      try {
    ...
    dsThaoDoRack() {
      this.$router.push({ path: `/tainguyen-idc/danhsach-congviec?loaicv=32` })
    },

    viewDetails(data) {
      this.$router.push({
        path: '/tainguyen-idc/thaodo-rack-form',
