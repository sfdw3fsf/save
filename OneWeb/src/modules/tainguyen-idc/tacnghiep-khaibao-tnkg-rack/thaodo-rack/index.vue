...
import ThaoDoRackModal from './modals/thaodo-rack-modal.vue';
import ThiCongThaoDoRack from './modals/thicong-thaodo-rack-modal.vue';
...
export default {
  ...
  methods: {
    ...
    onComplete: async function () {
      ...
      if (flag) {
        try {
          let response = null;
          let rs = false;
          response = await this.$root.context.post(`/web-ecms/thicong-rack/hoanthanh-thaodo-rack-idc/${this.phieutc}`);
    ...
    thaoDoRack() {
      if (this.$refs.popupThaoDoRack) {
        this.$refs.popupThaoDoRack.dialogOpen();
      }
    },
    thiCongThaoDoRack(rackId) {
      if (this.$refs.popupThiCongThaoDoRack) {
        this.$refs.popupThiCongThaoDoRack.dialogOpen(rackId);
      }
    },
    ...
    xoaThaoDoRack: async function (data) {
      if (!this.phieutc) {
      ...
      let flag = await this.$confirm(
        `Bạn có chắc chắn muốn gỡ tháo dỡ rack "${data.TEN_RACK}" không?`,
        "Xác nhận gỡ",
        {
          confirmButtonText: "Xác nhận",
          cancelButtonText: "Hủy",
      ...
      if (flag) {
        try {
          ...
          if (response && response.error && response.error == "200") {
            this.$toast.success('Gỡ tháo dỡ Rack thành công');
            await this.loadAllocatedRacks();
          } else {
            this.$toast.error('Gỡ tháo dỡ Rack thất bại');
          }
        } catch (error) {
          console.error('Error deleting rack allocation:', error);
          this.$toast.error('Có lỗi xảy ra khi gỡ tháo dỡ Rack');
        } finally {
          this.$root.showLoading(false);
        }
      }
        ...
        return {
          template: {
            template: `<div class="action-buttons">
                <button v-if="onRemoveRack" class="btn btn-sm btn-outline-danger ml-1" title="Gỡ thu hồi" @click="deleteItem()">
                ...
                <button v-if="data.isProcessed && onProcessRack" class="btn btn-sm btn-success" title="Đã thay đổi" disabled>
                  <i class="fa fa-check"></i> Đã tháo dỡ
                </button>
            ...
            methods: {
              deleteItem() {
                parent.xoaThaoDoRack(this.data)
              },
              xuLyItem() {
                parent.rackId = this.data.RACK_ID;
                if (parent.rackId != null) {
                  parent.thiCongThaoDoRack(parent.rackId);
                }
              }
            }
          }
