...
export default {
  ...
  created: async function () {
    try {
      ...
      this.nhanVienTen = this.$root.context.user.getProperty("ten_nv");
  ...
  methods: {
    ...
    getDsBangthongCapPhat: async function () {
      try {
        const rs = await this.$root.context.get(`web-ecms/ha-tang/thongtin-u/thicong-bangthong/dsKetqua?loaiKq=CP`)
        return rs.data
      } catch (error) {
    ...
    loadDonVi: async function() {
      ...
      try {
        const rs = await this.$root.context.post(`/web-ecms/idc/danh-muc/common`, { loaiDanhMuc: 'IDC-DS-DONVI' })
        return rs.data
      } catch (error) {
        this.$toast.error(error.response?.data?.message_detail || 'Không lấy được thông tin')
        return []
    ...
    resetSearch() {
      this.searchConditions = {
        ...
        ngayKetthuc: null,
        nhanvienThId: null,
        tenCv: 'Cấp phát băng thông mạng',
        trangthaiId: null,
        tinhId: null,
        loaiThiCongId: null,
        maTb: null,
