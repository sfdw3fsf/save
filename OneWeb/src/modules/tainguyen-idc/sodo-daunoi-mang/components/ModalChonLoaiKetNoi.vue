                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select v-model="formData.loaiKetNoi" class="form-control">
                                ...
                                <option value="DUONG_QUANG">Đường quang</option>
                                <option value="DUONG_DONG">Đường đồng</option>
                                <option value="DUONG_DU_PHONG">Đường kết nối dự phòng</option>
...
import { DIAGRAM_CONFIG } from '../diagram-config.js'
    ...
    methods: {
        async loadPortNguon() {
            ...
            try {
                ...
                // deviceId trong addInfo là số thuần (không có prefix 'device_')
                const deviceId = String(this.sourceNode.addInfo.deviceId)
                const response = await this.$root.context.post(
                    '/web-ecms/tainguyen-idc/sodo-daunoi-mang/ds-port-thietbi-mang-chua-ket-noi',
                    {
                        thietBiId: deviceId
                    }
                )
        ...
        async loadPortDich() {
            ...
            try {
                ...
                // deviceId trong addInfo là số thuần (không có prefix 'device_')
                const deviceId = String(this.targetNode.addInfo.deviceId)
                const response = await this.$root.context.post(
                    '/web-ecms/tainguyen-idc/sodo-daunoi-mang/ds-port-thietbi-mang-chua-ket-noi',
                    {
                        thietBiId: deviceId
        ...
        async handleOk(bvModalEvt) {
            ...
            try {
                ...
                // Gọi API tạo kết nối
                // deviceId trong addInfo là số thuần (không có prefix 'device_')
                const sourceDeviceId = String(this.sourceNode.addInfo.deviceId)
                const targetDeviceId = String(this.targetNode.addInfo.deviceId)
                
                const response = await this.$root.context.post(
                    '/web-ecms/tainguyen-idc/sodo-daunoi-mang/ket-noi-thiet-bi-mang',
                    {
                        ...
                        loaiKetNoiId: loaiKetNoiMap[this.formData.loaiKetNoi] || 1,
                ...
                if (response.error_code === 'BSS-00000000') {
                    ...
                    // selectedType = loaiKetNoi từ formData
                    this.$emit('create-connection', {
                        sourceNode: this.sourceNode,
                        targetNode: this.targetNode,
                        selectedType: this.formData.loaiKetNoi,
                        connectionData: { 
                            ...this.formData,
        ...
        getDeviceTypeName(type) {
            ...
            // Normalize type (có thể có hoặc không có dấu gạch dưới)
            const normalizedType = type.toUpperCase().replace(/\s+/g, '_')
            
            // Kiểm tra trong DIAGRAM_CONFIG trước
            const configNames = DIAGRAM_CONFIG.DEVICE.TYPE_NAMES
            if (configNames[normalizedType]) {
                return configNames[normalizedType]
            }
            
            // Fallback: thử các biến thể khác
            const fallbackMap = {
                ...
                'PATCH_PANEL': 'Patch Pannel',
                'PATCHPANEL': 'Patch Pannel'
            }
            
            return fallbackMap[normalizedType] || configNames[normalizedType] || type
        }
    }
}
</script>
