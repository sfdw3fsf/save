  ...
  methods: {
    async loadLoaiCongViec() {
      ...
      if (response && response.data && response.data.length > 0) {
        this.searchConditions.tenCv = response.data.find(item => item.LOAICV_ID == 42)?.TEN || '';
      }
    },
    async loadDanhMuc() {
      try {
    ...
    async onSearch() {
      try {
        ...
        searchParams.loaiCongViec = 'THU_HOI_U'
        ...
        console.error('Lỗi khi tìm kiếm kết quả thu hồi U:', error)
    ...
    createThuHoiU() {
      this.$router.push({ path: `/tainguyen-idc/danhsach-congviec?loaicv=42` })
    },

    viewDetails(data) {
      this.$router.push({ 
        path: '/tainguyen-idc/thuhoi-u-form', 
