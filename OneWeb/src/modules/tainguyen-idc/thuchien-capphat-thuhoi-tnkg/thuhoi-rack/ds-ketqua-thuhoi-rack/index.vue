  ...
  methods: {
    async loadLoaiCongViec() {
      ...
      if (response && response.data && response.data.length > 0) {
        this.searchConditions.tenCv = response.data.find(item => item.LOAICV_ID == 62)?.TEN || '';
      }
    },
    async loadDanhMuc() {
      try {
    ...
    async onSearch() {
      try {
        ...
        searchParams.loaiCongViec = 'THU_HOI_RACK'
        const data = searchParams;

        const response = await this.$root.context.post('/web-ecms/idc/thi-cong/thu-hoi-rack/ds-ketqua-thuhoi-rack', {
          loaiCvId: 62,
        ...
        console.error('Lỗi khi tìm kiếm kết quả thu hồi Rack:', error)
    ...
    createThuHoiRack() {
      this.$router.push({ path: `/tainguyen-idc/danhsach-congviec?loaicv=62` })
    },

    viewDetails(data) {
      this.$router.push({
        path: '/tainguyen-idc/thuhoi-rack-form',
