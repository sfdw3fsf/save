  ...
  methods: {
    async loadLoaiCongViec() {
      ...
      if (response && response.data && response.data.length > 0) {
        this.searchConditions.tenCv = response.data.find(item => item.LOAICV_ID == 36)?.TEN || '';
      }
    },
    ...
    async onSearch() {
      try {
        ...
        searchParams.loaiCongViec = 'CAP_PHAT_U'

        const data = searchParams;
        
        const response = await this.$root.context.post(`/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/ds-ketqua-u`,
          {
            loaiCvId: 36,
            ...
            tenU: data.tenU ? data.tenU.trim() : null,
          }
        )
        this.danhSachKetQua = response.data || []
      } catch (error) {
        console.error('Lỗi khi tìm kiếm kết quả cấp phát U:', error)
    ...
    createCapPhatU() {
      this.$router.push({ path: `/tainguyen-idc/danhsach-congviec?loaicv=36` })
    },

    viewDetails(data) {
      this.$router.push({ 
        path: '/tainguyen-idc/capphat-u-form', 
