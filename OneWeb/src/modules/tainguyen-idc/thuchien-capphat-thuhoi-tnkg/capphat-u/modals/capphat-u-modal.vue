<template src="./capphat-u-modal.html">
</template>
    ...
    methods: {
        async dialogOpen() {
            try {
                ...
                if (this.$refs.dialogCapPhatU) {
                    this.$refs.dialogCapPhatU.show();
                } else {
        ...
        closeDialog() {
            ...
            this.dataGrid1Items = [];
            this.$refs.dialogCapPhatU.hide();
        },
        loadRackOptions: async function () {
            try {
                let response = await this.$root.context.post(`/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/ds-rack-sansang-capphat-u/${this.phieuThiCongId}`);
                // let response = await this.$http.post('http://localhost:10990/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/ds-rack-sansang-capphat-u');
        ...
        loadAvailableUs: async function (rackId) {
            ...
            try {
                let response = await this.$root.context.post(`/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/ds-u-sansang-capphat?idPhieuThiCong=${this.phieuThiCongId}&rackId=${rackId}`);
                if (response && response.data) {
        ...
        async onCapPhatU() {
            if (!this.selectedUs || this.selectedUs.length === 0) {
                this.$toast.warning('Vui lòng chọn ít nhất một U để cấp phát');
                return;
            }
            ...
            const message = this.selectedUs.length === 1 
                ? "Bạn có muốn cấp phát U này không ?"
                : `Bạn có muốn cấp phát ${this.selectedUs.length} U đã chọn không ?`;
            ...
            if (flag) {
                try {
                    ...
                    let response = await this.$root.context.post('/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/capphat-u', requestBody);
                    
                    if (response) {
                        const successMessage = this.selectedUs.length === 1 
                            ? "Cấp phát U thành công"
                            : `Cấp phát ${this.selectedUs.length} U thành công`;
                        ...
                        if (response && response.message_detail) {
                            ...
                            this.$root.toastError("Cấp phát U thất bại");
                        }
                    }
                } catch (err) {
