...
import CapPhatUModal from './modals/capphat-u-modal.vue';
...
export default {
  ...
  methods: {
    ...
    loadAllocatedUs: async function () {
      try {
        let response = await this.$root.context.post(`/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/ds-u-trong-phieutc-capphat/${this.phieutc}`);
        if (response && response.data) {
    ...
    onComplete: async function () {
      if (!this.phieutc) {
        this.$toast.warning('Không tìm thấy phiếu thi công');
        return;
      }
      ...
      if (flag) {
        try {
          let response = await this.$root.context.post(`/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/hoan-thanh-capphat/${this.phieutc}`);
          
          if (response && response.error && response.error == "200") {
            this.$toast.success('Hoàn thành phiếu thành công');
            this.$router.push({ 
              path: '/tainguyen-idc/ketqua-capphat-u'
            })
          } else {
    ...
    capPhatU() {
      if (this.$refs.capPhatUModal) {
        this.$refs.capPhatUModal.dialogOpen();
      }
    },
    onUAllocated(uData) {
      this.loadRackData();
    ...
    xoaCapPhatU: async function (data) {
      if (!this.phieutc) {
      ...
      let flag = await this.$confirm(
        `Bạn có chắc chắn muốn xóa cấp phát U "${data.TEN_U}" không?`,
        "Xác nhận xóa cấp phát U",
        {
          confirmButtonText: "Xác nhận",
          cancelButtonText: "Huỷ",
          type: "warning",
      ...
      if (flag) {
        try {
          ...
          let response = await this.$root.context.post('/web-ecms/idc/thi-cong/cap-phat-thu-hoi-u/xoa-capphat-u', requestBody);
          
          if (response && response.error && response.error == "200") {
            this.$toast.success('Xóa cấp phát U thành công');
            await this.loadRackData();
            await this.loadAllocatedUs();
            this.selectedItems = [];
          } else {
            this.$toast.error('Xóa cấp phát U thất bại');
          }
        } catch (error) {
          console.error('Error deleting U allocation:', error);
          this.$toast.error('Có lỗi xảy ra khi xóa cấp phát U');
        } finally {
          this.$root.showLoading(false);
        }
      }
    },
    xoaCapPhatUNhieuItem: async function () {
      if (!this.phieutc) {
        this.$toast.warning('Không có thông tin phiếu thi công');
        return;
      }
      ...
      let flag = await this.$confirm(
        `Bạn có chắc chắn muốn xóa cấp phát ${this.selectedItems.length} U đã chọn không?<br/><strong>Danh sách:</strong> ${selectedNames}`,
        "Xác nhận xóa cấp phát U",
        {
          confirmButtonText: "Xác nhận",
          cancelButtonText: "Huỷ",
        ...
        return {
          template: {
            template: `<div class="action-buttons">
                ...
                <button class="btn btn-sm btn-outline-danger ml-1" title="Xóa cấp phát U" @click="deleteItem()" :disabled="isDeleteDisabled">
