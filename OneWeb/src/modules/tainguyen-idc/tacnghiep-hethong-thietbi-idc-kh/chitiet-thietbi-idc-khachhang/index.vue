...
export default {
  ...
  methods: {
    ...
    validateForm: function() {
      ...
      // 10. Trạng thái Tài sản (bắt buộc)
    ...
    onChangePhongZone: async function() {
      try {
        if (this.currentItem.phongZoneId && this.currentItem.lapDatTaiIdcId) {
          let rs = await api.getDanhMucRackByPhongZone(this.axios, this.currentItem.phongZoneId, this.currentItem.hdtbId ? this.phieutc : null)
          this.rackList = rs || []
        } else {
          this.rackList = []
        }
    ...
    onChangeRack: async function() {
      this.uTrenList = []
      this.uDuoiList = []
      this.currentItem.uTrenId = null
      this.currentItem.uDuoiId = null
