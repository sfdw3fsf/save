<template src="./ChuyenDoiCongSuatDien.html"></template>
...
import ChuyenDoiDienPopup from '../components/ChuyenDoiDienPopup.vue'
...
export default {
  components: {
    ...
    popupChuyenDoiDien: ChuyenDoiDienPopup,
  ...
  methods: {
    ...
    async loadData(phieutc_id) {
      try {
        ...
        if (resCv.data.idLoaiCv != 81) {
          this.$toast.error('Công việc không phải công việc chuyển đổi công suất điện')
          return this.$router.push('/tainguyen-idc/danhsach-congviec?loaicv=81')
        }
        ...
        const resKetQua = await api.getKetQuaChuyenDoiByPhieu(this.$root.context, phieutc_id)
        this.dsKetQuaThietBiChuyenDoi = (resKetQua.data || []).map((item) => ({
          ...item,
    ...
    async handleChuyenXuLy(result) {
      ...
      try {
        ...
        this.$router.push('/tainguyen-idc/ketqua-congsuatdien?loaicv=81')
      } catch (e) {
    ...
    async handleChuyenDoiDien(result) {
      try {
        const payload = {
          phieuTcId: this.congviec.phieuTcId,
          thietbiId: result.thietbiId,
          thanhnguonId: result.idThanhNguon,
          ocamMoiId: result.ocamMoiId,
          ocamCuId: result.ocamCuId,
          noiDung: result.noiDung,
          trangthaiId: this.congviec.idTrangThaiThucHien
        }
        await api.chuyenDoiCongSuatDien(this.$root.context, payload)
        this.$toast.success('Chuyển đổi công suất điện thành công')
        ...
        if (e.response && e.response.data) {
          this.$toast.error('Lỗi chuyển đổi công suất điện: ' + e.response.data.message)
        } else {
          this.$toast.error('Lỗi chuyển đổi công suất điện: ' + e.message)
        }
      }
    },
    onChuyenDoiDien() {
      this.$refs.dlgChuyenDoiDien.openDialog()
    },
    ...
    openChuyenDoiDienPopup() {
      this.$refs.dlgChuyenDoiDien.phieuTcId = this.congviec.phieuTcId
      this.$refs.dlgChuyenDoiDien.selectedThietBi = null
      this.$nextTick(() => {
        this.$refs.dlgChuyenDoiDien.openDialog()
      })
    },
    ...
    async tsbtnGhiLai_Click() {
      return this.$router.push('/tainguyen-idc/ketqua-congsuatdien?loaicv=81')
    },
    ...
    async tsbtnHoanThanh_Click() {
      if (this.dsKetQuaThietBiChuyenDoi < 1) {
        this.$toast.error('Vui lòng chuyển đổi điện cho ít nhất một thiết bị trước khi hoàn thành công việc')
        return
      }
    ...
    actionColumn(parent) {
      ...
      return function() {
        return {
          ...
          <div class="action-buttons gap-1">
                        <button 
                          ...
                          title="Chuyển đổi điện cho thiết bị" 
                          @click="onCapPhat()">
                          <i class="fa fa-plug"></i> Chuyển đổi
                        </button>
            <button 
        ...
        `,
            methods: {
              async onCapPhat() {
                self.$refs.dlgChuyenDoiDien.selectedThietBi = this.data
                self.$refs.dlgChuyenDoiDien.phieuTcId = null
                self.$nextTick(() => {
                  self.$refs.dlgChuyenDoiDien.openDialog()
                })
              },
              async onDelete() {
                const payload = {
                ...
                try {
                  await self.$confirm('Bạn có chắc muốn hoàn tác thi công này không?', 'Xác nhận hoàn tác', {
                    confirmButtonText: 'Có',
                    cancelButtonText: 'Không',
                    type: 'warning'
                  })
                  await api.deleteKetQuaCongSuatDien(self.$root.context, payload)
                  self.$toast.success('Gỡ thiết bị thi công thành công')
                  await self.loadData(self.congviec.phieuTcId)
                } catch (e) {
                  if (e.response?.data?.message) {
                    self.$toast.error('Lỗi gỡ thiết bị thi công: ' + e.response.data.message)
                  } else {
                    self.$toast.error('Lỗi gỡ thiết bị thi công: ' + e.message)
                  }
                }
              }
            }
