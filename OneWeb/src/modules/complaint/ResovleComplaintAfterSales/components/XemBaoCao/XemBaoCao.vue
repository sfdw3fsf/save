...
export default {
  ...
  methods: {
    ...
    async XemNgay () {
      try {
        this.loading(true)
        // let caulenh = await this.FN_MAP_ID("where nhom_bc_id=901 and ma_bc ='" + this.ma_bc + "'", 'caulenh', 'admin.baocao')
        let caulenh = await this.FN_TT_BAOCAO(901, this.ma_bc)
        if (caulenh != '-1') {
          this.path = caulenh
        ...
        this.popupComponent = () => import('@/modules/print/components/PreviewReport.vue')
    ...
    //   return result
    // },
    async FN_TT_BAOCAO (nhombcId, mabc) {
      let result = ''
      const req = {
        type: 1,
        param1: nhombcId,
      ...
      let data = this.GetData(await API.FN_TT_BAOCAO(this.axios, req))
      if (data) {
        result = data
      }
      return result
