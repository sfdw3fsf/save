          ...
          <tr :class="{ 'highlight4' : d.i===(currentrecord%recordperpage) }"  v-for="(d,idx) in pagedDataList" :key="idx"  @click="oneClick(d.i,true,$event)">
             <template v-if="d.type==0">
                 <td :colspan="headerList.length+2">
                     <div class="tree-arrow"></div> Ngày giao: {{d.data}}
                  </td>
             </template>
...
export default {
  name: 'treegridview',
  props: [ 'id', 'datalist', 'headers', 'flcol', 'filegroup', 'lastcoltemplate', 'showheader', 'styleObject', 'groupKey' ],
  watch: {
  ...
  computed: {
    ...
    pagedDataList () {
      let data = this.tabledata.slice(this.currentpage * this.recordperpage, (this.currentpage + 1) * this.recordperpage)
      var keys = []
      var pagerData = []
