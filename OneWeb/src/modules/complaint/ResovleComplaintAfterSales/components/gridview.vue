<template>
<div>
<div class="table-content marb0" style="max-height: 400px;"  v-if="(datalist && datalist.length>0) || showHeader">
  <table class="table-result nowrap table-gachle" :id="id">
      <thead class="center">
          <tr>
              <th class="w20"></th>
              <th v-if="showCheck" class="w20">
                  <div class="check-action">
                      <input type="checkbox" class="check" v-model="checkAll" @change="checkAllEvent($event)">
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td class="selected" v-if="showCheck">
                  <div class="check-action">
                      <input type="checkbox" class="check" v-model="d.ischecked" @change="checkedItemsEvent">
                      <span class="name"></span>
                  </div>
              </td>
...
<div class="tc-pagination" v-if="tabledata && tabledata.length>0">
 ...
 <div v-if="!datalist || datalist.length<=0">
    <slot name="nodata"> <p class="text-center">Không có bản ghi nào để hiển thị</p> </slot>
</div>
</div>
</template>
<style scoped>
...
export default {
  name: 'gridview',
  props: [ 'id', 'datalist', 'headers', 'flcol', 'filegroup', 'lastcoltemplate', 'showheader', 'styleObject', 'showCheck' ],
  watch: {
