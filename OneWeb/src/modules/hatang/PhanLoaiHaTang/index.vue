...
export default {
  ...
  methods: {
    ...
    apiListDanhSach: async function () {
      let result = [];
      try {
        let rs = await this.$root.context.get("/web-ecms/danhmuc/phan-loai-tt/danh-sach");
        result = rs.data;
      } catch(e) {
    ...
    apiSave: async function (args) {
      let result = false;
      try {
        let rs = await this.$root.context.post("/web-ecms/danhmuc/phan-loai-tt/cap-nhat", {
          id: args.id,
          ten: args.ten.trim(),
    ...
    apiDelete: async function (id) {
      let result = false;
      try {
        let rs = await this.$root.context.post("/web-ecms/danhmuc/phan-loai-tt/xoa", {
          id: id,
        });
        result = rs.error_code=='BSS-00000000';
      } catch(e) {
