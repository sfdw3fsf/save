...
export default {
  ...
  methods: {
    ...
    updateItem: async function (args) {
      let result = false;
      try {
        let rs = await this.$root.context.post("/web-ecms/loai-ip/capnhat", {
    ...
    deleteItem: async function (id) {
      let result = false;
      try {
        let rs = await this.$root.context.post("/web-ecms/loai-ip/xoa", {
          id: id,
        });
        result = rs.data.result;
      } catch(e) {
    ...
    btnGhiLai_Click: async function (args) {
      if (this.validateControls()) {
        let flag = await this.$confirm(
          "Có chắc bạn muốn lưu lại loại IP đang chọn?",
          "Xác nhận thực hiện",
          {
        ...
        if (flag) {
          try {
            ...
            if (rs) {             
              if (this.currentItem.id === 0) {
                this.$root.toastSuccess("Thêm loại IP thành công");
              }
              else{
                this.$root.toastSuccess("Cập nhật loại IP thành công");
              }
              this.dataItems = await this.getItems();
              ...
              if (this.currentItem.id === 0) {
                this.$root.toastError("Thêm loại IP thất bại");
              }
              else{
                this.$root.toastError("Cập nhật loại IP thất bại");
              }
            }
    ...
    btnXoa_Click: async function (args) {
      ...
      let flag = await this.$confirm(
        "Có chắc bạn muốn xóa loại IP đang chọn?",
        "Xác nhận thực hiện",
        {
          confirmButtonText: "Có",
          cancelButtonText: "Không",
