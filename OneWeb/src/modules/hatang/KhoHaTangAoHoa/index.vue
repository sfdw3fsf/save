...
export default {
  ...
  methods: {
    ...
    apiListItems: async function () {
      ...
      try {
        if (this.modalItemID > 0) {
          ...
          let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/get-all')
          result = rs.data
        }
    ...
    apiListHistory: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/change-history', {
          id: id
        })
        result = rs.data
      } catch (e) {
    ...
    apiListMayao: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/may-ao', { id: id })
        result = rs.data
      } catch (e) {
        this.$toast.error(e.response.data.message_detail)
      }
    ...
    apiListHaTangIdg: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/hatang-idg', {
          id: id
        })
        result = rs.data
      } catch (e) {
    ...
    apiListSanPhamDichVu: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/sanpham-dichvu', {
          id: id
        })
        result = rs.data
      } catch (e) {
    ...
    apiListTuDia: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/thietbi-tudia', {
          id: id
        })
        result = rs.data
      } catch (e) {
    ...
    apiListThietBiDauNoi: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/thiet-bi', {
          id: id
        })
        result = rs.data
      } catch (e) {
    ...
    apiListThieBiTuDiaByIDCId: async function (id) {
      let result = []
      try {
        let rs = await this.$root.context.get('/web-ecms/danhmuc/thietbi-tudia-by-idc-id', { id: id })
        result = rs.data
        result.forEach((element) => {
          element.id = element.ID
          element.label = element.TEN
    ...
    apiSave: async function (args, thietbi_list) {
      let result
      try {
        result = await this.$root.context.post('/web-ecms/aohoa-cloud/create-or-update', {
          id: Number(args.id),
          ...
          thietbi_tudia_id: args.thietbi_tudia_id.map((item) => item),
          ...
          thietBiDauNoiList: thietbi_list.map((item) => ({
            id: item.id,
    ...
    apiDelete: async function (id) {
      let result = false
      try {
        let rs = await this.$root.context.get('/web-ecms/aohoa-cloud/delete', { id: id })
    ...
    onSave: async function () {
      if (this.validateControls() && this.validateResourceFields()) {
        ...
        if (flag) {
          try {
            ...
            if (rs.data > 0) {
              ...
              this.$root.toastError('Tên hạ tầng ảo hoá/cloud đã tồn tại.')
            } else if (rs.data == -2) {
              this.$root.toastError('Mã hạ tầng ảo hoá/cloud đã tồn tại.')
            } else if (rs.data == -3) {
              this.$root.toastError('Tên và Mã hạ tầng ảo hoá/cloud đã tồn tại.')
            } else {
    ...
    themThietBi: async function () {
      ...
      if (values.length > 0) {
        this.current_item.nangluc_luutru = this.select_nangluc_luutru_tudia.filter((obj) => values.includes(obj.ID))

        this.current_item.nangluc_luutru.forEach((obj) => {
        ...
        this.$root.toastError('Danh sách năng lực tủ đĩa không được để trống.')
      }
      return
    },
    openThietBiModal() {
      if (this.current_item.idc_id) {
    ...
    grid_onCommandClicked_thietbi(name, dataItem) {
      ...
      if (name == 'XOA') {
        this.thietbi_daunoi_items = this.thietbi_daunoi_items.filter((item) => item.thietbi_id != dataItem.thietbi_id)
      }
    }
  }
}
