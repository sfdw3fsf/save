...
export default {
  ...
  methods: {
    ...
    async getDsYeuCauThuHoi() {
      ...
      if (!this.spdvId) {
        this.$root.toastError("Vui lòng chọn Sản phẩm dịch vụ!");
        return result;
      }
      try {
        const rs = await this.$root.context.post('/web-ecms/toiuu-tainguyen/lay-ds-yeucau-thuhoi', {
          p_thang: this.thangTimKiem.value,
          p_spdv_id: this.spdvId ?? 1,
        });
        result = rs.data;
      } catch (e) {
    ...
    xuatExcel: async function () {
      try {
        if (this.dataSource == []) {
          ...
          XLSX.utils.book_append_sheet(workbook, worksheet, "Danh sách yêu cầu thu hồi")
          XLSX.writeFile(workbook, "Yêu cầu thu hồi.xlsx")
        }
      } catch (ex) {
    ...
    async importExcel () {
      ...
      try {
        ...
        formData.append('file', this.file); // Thêm tệp Excel vào FormData
        formData.append('monthYear', this.thangTimKiem.value); 

        // Gửi yêu cầu POST với FormData
        let rs = await this.$root.context.post('/web-ecms/toiuu-tainguyen/upload', formData, {
          headers: {
        ...
        if (this.dataSource.length > 0) {
          this.$root.toastSuccess(`Đã import thành công ` + this.dataSource.length + ` dòng dữ liệu`);
        } else {
          this.$root.toastSuccess(`Đã có lỗi xảy ra trong quá trình import dữ liệu hoặc file rỗng`);
        }
    ...
    apiDownFile: async function() {
      let result = {};
      try {
        let rs = await this.$root.context.post("/web-ecms/storage/getPresignedUrl", { fileSource: 'HATANGCNTT/IT/insert_hieunang_thang_template.xlsx' });
        result = rs.data;
        // Tạo một thẻ <a> ẩn để tải file
        const link = document.createElement('a');
        link.href = result.result;
        link.download = ''; // Để trống thì trình duyệt sẽ tự lấy tên file từ URL
        document.body.appendChild(link);
        link.click();

        // Xoá thẻ <a> sau khi tải
