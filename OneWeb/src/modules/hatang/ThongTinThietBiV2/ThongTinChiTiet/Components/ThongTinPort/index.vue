...
<script>
import PortData from './components/PortData/PortData.vue'
import CreatePortType from './components/CreatePortType/index.vue'
...
export default {
  ...
  computed: {
    currentTab() {
      const portTypeMapping = {
        [PORT_TYPE.INTERFACE.NAME]: 1,
        [PORT_TYPE.POWER.NAME]: 2,
        [PORT_TYPE.CONSOLE.NAME]: 3
      }
      return portTypeMapping[this.cardModuleInfo.portType]
    },
    currentItem() {
      if (this.isInModal) return this.$parent.$parent.currentItem
      return this.deviceData || this.$parent?.currentItem || {}
    },
    disabledLineNumber() {
      return this.cardModuleInfo.isCardModuleMode ? true : !this.controls.lineNumber.enabled
    },
    isLineCard() {
      return !this.cardModuleInfo.isCardModuleMode || this.cardModuleInfo.portType === PORT_TYPE.INTERFACE.NAME
    },
    isPowerCard() {
      return !this.cardModuleInfo.isCardModuleMode || this.cardModuleInfo.portType === PORT_TYPE.POWER.NAME
    },
    isConsoleCard() {
      return !this.cardModuleInfo.isCardModuleMode || this.cardModuleInfo.portType === PORT_TYPE.CONSOLE.NAME
    },
    lineNumber: {
      get() {
      ...
      set(newValue) {
        ...
        if (propName && this.currentItem) {
          this.currentItem[propName] = numValue
          if (this.setCurrentItemValByProp) {
  ...
  methods: {
    ...
    openModal() {
      console.log("Todo: implement modal all ports");
    },
    getTotalPort(tab) {
      const portList = {
    ...
    setTotalPort(tab, value) {
      ...
      if (portMapping[tab] && this.currentItem) {
        //this.$set() được dùng để đảm bảo Vue nhận diện thay đổi khi cập nhật currentItem
        this.$set(this.currentItem, portMapping[tab], value)
      }
    },
    // Method để lấy dữ liệu port types cho parent component
    getPortTypeData() {
      return {
        portTypeInterface: this.portTypeInterfaceList,
        portTypePower: this.portTypePowerList,
        portTypeConsole: this.portTypeConsoleList
      }
    },
  }
}
