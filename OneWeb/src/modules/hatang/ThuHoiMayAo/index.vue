...
export default {
  ...
  methods: {
    apiListItems: async function () {
      let result = [];
      try {
        let rs = await this.$root.context.get("/web-ecms/hatang/thu-hoi-mayao/get-all");
        result = rs.data;
      } catch(e) {
    ...
    apiListYeuCauThuHoi: async function(id) {
      let result = [];
      try {
        let rs = await this.$root.context.get("/web-ecms/hatang/thu-hoi-mayao/danhsach-yeucau/"+id);
        result = rs.data;
      } catch(e) {
        this.$toast.error(e.response.data.message_detail)
      }
    ...
    apiHoanThanhThuHoiMayAo: async function(id, ykien) {
      let result = [];
      try {
        let rs = await this.$root.context.get("/web-ecms/hatang/thu-hoi-mayao/hoanthanh-thuhoi-mayao"
        , {id: id, ykien: ykien == null ? '' : ykien});
        result = rs.data;
      } catch(e) {
        this.$toast.error(e.response.data.message_detail)
      }
    ...
    apiYeuCauThuHoiIP: async function(id) {
      let result = [];
      try {
        let rs = await this.$root.context.get("/web-ecms/hatang/thu-hoi-mayao/yeucau-thuhoi-ip", {id: id});
        result = rs.data;
      } catch(e) {
        this.$toast.error(e.response.data.message_detail)
      }
    ...
    setCurrentItem: async function (item) {
      ...
      this.current_item.hoanthanh_mayao_enable = item.hoanthanh_mayao_enable
    ...
    grdItems_RowSelected: async function (args) {
      try {
        this.$root.showLoading(true);      
        this.buttons.btnComplete.enabled = (args.data.hoanthanh_mayao_enable == 1);
    ...
    grdItems_RowDeselected: async function () {
      ...
      if (!(items == null || items.length == 0)) {
        try {
          this.$root.showLoading(true);
          this.buttons.btnComplete.enabled = (items[0].hoanthanh_mayao_enable == 1);
          this.buttons.btnRequestIPRecall.enabled = (items[0].yeucau_ip_enable == 1);
    ...
    onComplete: async function (args) {
      if (this.current_item.ykien_thuchien_thuhoi == null || this.current_item.ykien_thuchien_thuhoi.trim() == '') {
          this.$root.toastError("Ý kiến thực hiện thu hồi là bắt buộc!")
          return;
      }
      ...
      if (flag) {
        try {
            this.$root.showLoading(true);
            let rs = await this.apiHoanThanhThuHoiMayAo(this.current_item.id, this.current_item.ykien_thuchien_thuhoi);
            if (rs > 0) {
              this.$root.toastSuccess('Hoàn thành thu hồi máy ảo thành công');
              this.thuhoi_mayao_items = await this.apiListItems();
            } else {
    ...
    onRequestIPRecall: async function (args) {
      let flag = await this.$confirm("Bạn có muốn yêu cầu thu hồi IP này không?","Xác nhận",
        {
          confirmButtonText: "Có",
          cancelButtonText: "Không",
          type: "warning",
      ...
      if (flag) {
        try {
            this.$root.showLoading(true);
            let rs = await this.apiYeuCauThuHoiIP(this.current_item.id);
            if (rs > 0) {
              this.thuhoi_mayao_items = await this.apiListItems();
              this.$root.toastSuccess('Yêu cầu thu hồi IP thành công!')
            } else {
              this.$root.toastError('Chức năng lỗi.');
            }
          } finally {
