...
import { getLsThayDoi, getLsCongSuatThucTe } from './api';
...
export default {
        ...
        return {
            ...
            lsCongSuatThucTeGrid: {
                columns: [
                    ...
                    {
                        fieldName: "GIATRI_CU",
                        headerText: "Giá trị đo cũ",
                        textAlign: "Center",
                        width: "220"
                    },
                    {
                        fieldName: "GIATRI_MOI",
                        headerText: "Giá trị đo mới",
                        textAlign: "Center",
                        width: "320"
                    },
                    {
    ...
    methods: {
        loadInitialData: async function () {
            ...
            result = await getLsCongSuatThucTe(this.axios, this.currentItem.id);
            if (result.error) {
                this.$root.toastError(result.error + " Vui lòng tải lại trang!");
                return;
            } else {
                this.lsCongSuatThucTeGrid.dataSource = result.data.map((item, index) => {
                    return {
                        STT: index + 1,
                        ...item
                    }
