...
import { getDeviceUrls, getDeviceTypeIdByListUrl, DEVICE_TYPE_TO_MANG_THIET_BI } from './const'

export default {
  name: 'TimKiemThietBiIDC',
  components: {
    breadcrumb,
    KDataGrid
  },
  ...
  computed: {
    deviceTypeId() {
      // Lấy deviceTypeId từ URL hiện tại
      const currentPath = this.$route.path;
      console.log('Current Path:', currentPath, getDeviceTypeIdByListUrl(currentPath));
      return getDeviceTypeIdByListUrl(currentPath)
    },
    deviceUrls() {
  ...
  methods: {
    ...
    async loadLoaiThietBi() {
      try{
        ...
        this.loaiThietBiList = await api.getDanhMucLoaiThietBi(this.axios, mangThietBiId)
      }catch{
        console.error('Error loading loaiThietBi:', error)
    ...
    onAddNew() {
      ...
      this.$router.push(`${this.deviceUrls.detailUrl}?action=add`)
    },
    onView(row) {
      if (!this.deviceUrls || !this.deviceUrls.detailUrl) {
      ...
      this.$router.push(`${this.deviceUrls.detailUrl}?id=${row.ID}`)
    },
    onEdit(row) {
       if (!this.deviceUrls || !this.deviceUrls.detailUrl) {
        this.$root.toastWarning('Không tìm thấy loại thiết bị')
      ...
      this.$router.push(`${this.deviceUrls.detailUrl}?id=${row.ID}&mode=edit`)
    },
    ...
    // Get danh sách thiết bị với param theo loại thiết bị ID
        async loadDevicesByTypeId(deviceTypeId) {
          try {
            this.showGrid = false;
            this.searchForm.loaiThietBiId = deviceTypeId;
            await this.onSearch()
          } catch (error) {
