...
export default {
  ...
  upsertThietBiIDC: async (axios, data) => {
    try {
      ...
      const url = isUpdate
        ? 'web-ecms/idc/hatang-cntt/thiet-bi/sua'
        : 'web-ecms/idc/hatang-cntt/thiet-bi/them-moi'
      
      const rs = await axios.post(url, payload)
      return rs && rs.data ? rs.data.data[0].ID : null
    } catch (error) {
      handleApiError(error)
    }
  },
  ...
  getChiTietThietBiIDC: async (axios, id) => {
    try {
      const rs = await axios.post('web-ecms/idc/hatang-cntt/thiet-bi/chi-tiet-thiet-bi', { id })
  ...
  getDanhMucLoaiThietBi: async (axios, mangThietBiId = null) => {
    if (mangThietBiId && mangThietBiId !== 0 && mangThietBiId !== '0' && mangThietBiId !== '') {
  ...
  // Lấy danh mục loại ổ cắm
  getDanhMucLoaiOCam: async (axios) => {
    return await getDanhMucCommon(axios, '/loai-o-cam')
  },

  // Lấy danh mục thiết bị kết nối
  getDanhMucThietBiKetNoi: async (axios) => {
    return await getDanhMucCommon(axios, '/thiet-bi-ket-noi')
  },
  ...
  getDSDonViQuanLy: async (axios) => {
    //return await getDanhMucCommon(axios, 'IDC-DONVI-QLY', 1)
    try {
      const rs = await axios.get('/web-ecms/idc/hatang-cntt/danhmuc/idc-common', { params: { loaiDanhMuc: 'IDC-DONVI-QLY',thamSo1:1 } })
      return rs && rs.data ? rs.data.data : []
    } catch (error) {
      handleApiError(error)
      return []
  ...
  // Lấy DS phòng/tổ quản lý
  getDSToQuanLy: async (axios, donviql_id) => {
    //return await getDanhMucCommon(axios, 'IDC-TO-QLY', params)
    try {
      const rs = await axios.get('/web-ecms/idc/hatang-cntt/danhmuc/idc-common', { params: { loaiDanhMuc: 'IDC-TO-QLY',thamSo1:donviql_id } })
      return rs && rs.data ? rs.data.data : []
    } catch (error) {
      handleApiError(error)
      return []
  ...
  // Lấy DS nhân viên quản lý
  getDSNhanVienQuanLy: async (axios, toql_id) => {
    //return await getDanhMucCommon(axios, 'IDC-NHANVIEN-QLY', params)
    try {
      const rs = await axios.get('/web-ecms/idc/hatang-cntt/danhmuc/idc-common', { params: { loaiDanhMuc: 'IDC-NHANVIEN-QLY',thamSo1:toql_id } })
      return rs && rs.data ? rs.data.data : []
    } catch (error) {
      handleApiError(error)
      return []
...
const extractFirstDataEntry = (response) => {
