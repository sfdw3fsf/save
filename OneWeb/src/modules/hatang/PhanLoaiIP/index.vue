...
export default {
  ...
  methods: {
    ...
    updateItem: async function (args) {
      let result = false;
      try {
        let rs = await this.$root.context.post("/web-ecms/phanloai-ip/capnhat", {
    ...
    deleteItem: async function (id) {
      let result = false;
      try {
        let rs = await this.$root.context.post("/web-ecms/phanloai-ip/xoa", {
          id: id,
        });
        result = rs.data.result;
      } catch(e) {
    ...
    validateControls: function () {
      ...
      if(this.currentItem.ten.length >= 100){
        msg = " Tên không được vượt quá 100 ký tự.";
      }
    ...
    btnGhiLai_Click: async function (args) {
      if (this.validateControls()) {
        let flag = await this.$confirm(
          "Có chắc bạn muốn lưu lại phân loại IP đang chọn?",
          "Xác nhận thực hiện",
          {
        ...
        if (flag) {
          try {
            ...
            if (rs) {
              if (this.currentItem.id === 0) {
                this.$root.toastSuccess("Thêm phân loại IP thành công");
              }
              else{
                this.$root.toastSuccess("Cập nhật phân loại IP thành công");
              }
              this.dataItems = await this.getItems();
            }
            else{
              if (this.currentItem.id === 0) {
                this.$root.toastError("Thêm phân loại IP thất bại");
              }
              else{
                this.$root.toastError("Cập nhật phân loại IP thất bại");
              }
            }
    ...
    btnXoa_Click: async function (args) {
      ...
      let flag = await this.$confirm(
        "Có chắc bạn muốn xóa phân loại IP đang chọn?",
        "Xác nhận thực hiện",
        {
          confirmButtonText: "Có",
          cancelButtonText: "Không",
