...
export default {
  ...
  watch: {
    ...
    "currentItem.ten": function (value) {
      this.controls.txtTen.invalid = value == null || value.trim() == "";
    },
    "currentItem.bangThong": function (value) {
      this.controls.bangThong.invalid = value == null || value < 0 || value >= 1000;
    },
  },
  ...
  methods: {
    ...
    apiListDanhSach: async function () {
      let result = [];
      try {       
        const rs = await this.$root.context.post(`/web-ecms/danhmuc/thong-tin-module/danhsach`);      
        result = rs.data;
      } catch(e) {
    ...
    apiSave: async function (args) {
      let result = false;  
      try {
        let rs = await this.$root.context.post("/web-ecms/danhmuc/thong-tin-module/capnhat", {
          id: args.id,
          ten: args.ten.trim(),
          bangThong: parseFloat(args.bangThong),
          ghiChu: args.ghiChu == null ? "" : args.ghiChu.trim(),
          hieuLuc: args.hieuLuc,
          kieu: args.id == null ? 1 : 2 // 1 thêm mới, 2 sửa
        });
        result = {
          success: rs.error_code=='BSS-00000000',
          kieu: args.id == null ? 1 : 2, // 1 thêm mới, 2 sửa
        };
      } catch(e) {
    ...
    enableControls: function (args) {
      ...
      this.controls.bangThong.enabled = args;
    ...
    validateControls: function () {
      ...
      if(this.controls.bangThong.invalid){
        msg ="Vui lòng nhập băng thông nguyên dương và nhỏ hơn 1000 GB";        
      }   
    ...
    onDelete: async function (args) {
      ...
      if (flag) {
        try {
          // const issetActiveItem = items.some(item => item.HIEU_LUC === 1);// Có đối tượng hiệu lực
          // if( issetActiveItem) {
          //   this.$root.toastError("Hiện đối tượng đang thao tác đã có dữ liệu khác tham chiếu sử dụng.");
          //   return;
          // }
          this.$root.showLoading(true);
          let  isSuccess=false;
