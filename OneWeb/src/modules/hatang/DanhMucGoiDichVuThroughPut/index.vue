...
<script>
export default {
  name: 'DanhMucGoiDichVuThroughPut',
  data() {
    return {
      currentIdUpdate: 0,
      dsSelected: [],
  ...
  methods: {
    getDanhSach: async function() {
      try {
        const rs = await this.$root.context.post(`/web-ecms/throughput/get-all-throughput`)
        if (rs.data) this.dsThroughPut = rs.data.data
        this.dsThroughPut.forEach((item) => {
          item.HIEU_LUC_TEXT = item.HIEU_LUC == 1 ? 'Hiệu lực' : 'Không hiệu lực'
        })

        setTimeout(() => {
          if (this.currentIdUpdate > 0) {
            const pageSize = this.$refs.dsPhanLoaiHaTangGrid.pageSize
            this.$refs.dsPhanLoaiHaTangGrid.selectRow(this.dsThroughPut.findIndex(x => x.ID == this.currentIdUpdate) % pageSize)
    ...
    checkForm: function() {
      ...
      if (this.form.giatri == null) {
        this.$root.toastError('Không được để trống giá trị')
        kq = false
      }
      if (this.form.giatri < 0 || this.form.giatri > 99999) {
        this.$root.toastError('Giá trị phải lớn hơn 0 và nhỏ hơn 99999')
        kq = false
      }
      return kq
    },
    updateThroughPut: function() {
      if (this.$refs.dsPhanLoaiHaTangGrid) {
        // Vô hiệu hóa tất cả checkbox
        const checkboxes = this.$refs.dsPhanLoaiHaTangGrid.$el.querySelectorAll('input[type="checkbox"]');
        checkboxes.forEach(checkbox => {
          checkbox.disabled = true;
        });

        // Vô hiệu hóa tất cả sự kiện click trên các dòng
        const rows = this.$refs.dsPhanLoaiHaTangGrid.$el.querySelectorAll('tr');
        rows.forEach(row => {
      ...
      // Chọn dòng hiện tại
      this.$refs.dsPhanLoaiHaTangGrid.selectRow(
        this.dsThroughPut.findIndex(x => x.ID == this.form.id) % pageSize
      )
    ...
    createThroughPut: async function() {
      if (this.checkForm() && this.form.id == 0) {
        try {
          ...
          const rs = await this.$root.context.post(`/web-ecms/throughput/insert-throughput`, this.form)
          if (rs.error == '200' && rs.error_code == 'BSS-00000000') {
            this.$toast.success('Thêm mới thành công')
          }
          await this.getDanhSach()
          this.canceleThroughPut()
        } catch (error) {
          this.$toast.error(error.response?.data?.message_detail || error.message_detail || 'Có lỗi xảy ra')
        }
        ...
        try {
          ...
          this.form.ten = this.form.ten?.trim()
          this.form.ghichu = this.form.ghichu?.trim()
          const rs = await this.$root.context.post(`/web-ecms/throughput/update-throughput`, this.form)
          if (rs.error == '200' && rs.error_code == 'BSS-00000000') {
    ...
    deleteDanhSach: async function() {
      try {
        if (this.dsSelected.length == 0) {
          this.$toast.error('Vui lòng chọn gói dịch vụ ThroughPut để xóa')
          return
        }
        if (this.buttons.btnXoa.enabled) {
          ...
          for (let item of this.dsSelected) {
            const rs = await this.$root.context.post(`/web-ecms/throughput/delete-throughput/`, { id: item })
    ...
    canceleThroughPut: async function () {
      if (this.$refs.dsPhanLoaiHaTangGrid) {
    ...
    onSelectRowChanged(selected) {
      ...
      this.form.giatri = selected.data.GIATRI ?? selected.GIATRI
