  ...
  <table class="table-result nowrap table-gachle" :id="id">
      <thead class="center">
          <tr>
              ...
              <th v-if="orderColumn" class="w20">STT</th>
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td  v-if="orderColumn">{{ d['stt'] }}</td>
              <td :class="{ 'highlight1' : idx==(currentrecord%recordperpage),'nowrap w30 center':hdx==headerList.length-1,'stickylastcol':(hdx==headerList.length-1) && flcol }" v-for="(h,hdx) in headerList" :key="hdx">
                <template v-if="((hdx<headerList.length-1) || (!lastcoltemplate) || (lastcoltemplate=='0') )&& (h.html==='' || h.html===null || h.html==undefined)">
                  <span>{{ d[h.name] }} </span>
                </template>
                <template v-if="!(h.html==='')">
                    <span v-if="d[h.name]" v-html="h.html">{{h.html}} </span>
                    <span v-if="!d[h.name]" v-html="h.html1">{{h.html1}}  </span>
                </template>
                ...
                <template v-if="(hdx==headerList.length-1) && lastcoltemplate && (lastcoltemplate=='3')">
                  <a href="#" class="act f20 text-main nc-icon-glyph ui-1_circle-add" @click.prevent="addItemEvent(d)" ></a>
                </template>
              </td>
          </tr>
      </tbody>
...
export default {
  name: 'gridview',
  props: [ 'id', 'datalist', 'headers', 'flcol', 'filegroup', 'lastcoltemplate', 'checkheader', 'orderColumn', 'updateui' ],
  watch: {
  ...
  methods: {
    ...
    addItemEvent: function (d) {
      console.log('addItemEvent')
      console.log(d)
      this.$emit('addItemEvent', d)
    }
