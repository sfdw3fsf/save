<template>
  <div style="height: 600px">
    ...
    />
    <div
      id="mapVNPTXetDuyetPhanTuMangHoDanThuThap"
      class="mapVNPTXetDuyetPhanTuMangHoDanThuThap"
    ></div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getMarkersVNPT () {
      if(this.markers == undefined || this.markers == null){
        let defaultConfig = this.$root.context.user.getMapConfigDefault();
        let e = {
          lngLat: {
            lng: defaultConfig.longtitus,
            lat: defaultConfig.latitus
          }
        }
        this.dblclickMap(e);
      }
