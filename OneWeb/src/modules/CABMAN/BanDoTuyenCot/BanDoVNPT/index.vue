...
<script>
import VNPTMap from '../components/VNPTMap.vue';
...
export default {
  ...
  watch: {
    getgian_do_cot: function (val) {
      ...
      map.setLocationEx(this.nodes[0].getPosition());
    }
  },
  methods: {
    updateSizeMapBanDo: function () {
      this.$refs.map.updateSizeMap();
    },
    ...
    async xoaCap(id) {
      ...
      if (vdoituongid[2] === '0') {
        this.$confirm(`Bạn có muốn xóa cáp đang chọn không ?`, 'Giản đồ mạng cáp', {
          ...
          .then(() => {
            this.loading(true);
            this.api_set_xoa_cap({ cap_id: vdoituongid[1], xoa_batbuoc: 1 })
              .then((rep) => {
    ...
    async xoaKetcuoi(id) {
      ...
      this.$confirm(`Bạn có muốn xóa kết cuối đang chọn không ?`, 'Giản đồ mạng cáp', {
        ...
        .then(() => {
          this.loading(true);
          this.api_set_xoa_ketcuoi({ ketcuoi_id: vdoituongid[1], xoa_batbuoc: 1 })
            .then((rep) => {
              // this.$refs.map.reload()
              // Call Reload  gian do
              this.loading(false);
