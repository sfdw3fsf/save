...
export default {
  components: {XemToaDo, BssRequiredMarker, BanDoVNPT},
  name: "QuanLyDoiTuong",
  props: {
  ...
  methods: {
    ...
    async onShownModalDT() {
      ...
      this.kinhDo = this.getPosition.lng
      this.viDo = this.getPosition.lat
      if (this.objUpdate) {
    ...
    modalToaDo_Show: async function () {
      await this.$refs.banDoVNPT.bindData(
