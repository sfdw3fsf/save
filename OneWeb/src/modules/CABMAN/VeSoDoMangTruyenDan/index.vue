...
export default {
  ...
  , capnhatDoiTuong: function (item) {
     ...
     window.dsDoiTuongAfterChange = this.dsDoiTuongAfterChange
      ...
      this.objUpdate.TEN_LOAI_DT = this.tenTuyenSelected
      this.objUpdate.TEN_TTD = this.tenLoaiDoiTuongSelected
      ...
      let idNguon = this.dsLienKet.filter(x => x.LIENKET_ID === parseInt(item.id))[0].DOITUONG_N_ID
      let idDich =this.dsLienKet.filter(x => x.LIENKET_ID === parseInt(item.id))[0].DOITUONG_D_ID
      let loailk_id =this.dsLienKet.filter(x => x.LIENKET_ID === parseInt(item.id))[0].LOAILK_ID
        ...
        await this.$root.context.post(
          ...
          if (dsLienKet.length > 0) {
            dsLienKet.forEach(itemLK => {
              ...
              if (thongtinNodeN){
                let nodeN = this.taoNode("DT_" + thongtinNodeN.DOITUONG_ID + "", thongtinNodeN.TEN_HT, "SPLITTER")
                // console.log(nodeN)
                let thongtinNodeD = dsDoiTuong.filter(item => item.DOITUONG_ID === itemLK.DOITUONG_D_ID)[0]
                if (thongtinNodeD){
                  let nodeD = this.taoNode("DT_" + thongtinNodeD.DOITUONG_ID + "", thongtinNodeD.TEN_HT, "SPLITTER")
                  //"1-2-3-1"  số 2 chọn kiểu default
                  let connector = this.taoKetNoi("1-1-3-1", itemLK.LIENKET_ID + "", nodeN, nodeD, itemLK.LOAILK_ID, itemLK.TEN_HT + "", itemLK.CONG_DICH + "")
