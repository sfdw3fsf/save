...
export default {
  ...
  computed: {
      ...
      if (this.columns != null) {
        for (let i = 0; i < this.columns.length; i++) {
          let column = {
            ...
            editTemplate: this.columns[i].editTemplate
          }
      ...
      if (!(this.commandColumn == undefined || this.commandColumn == null)) {
        if (this.commandColumn.show == true) {
          ...
          if (cmds.length > 0) {
            ...
            if (this.commandColumn.firstColumn == true) { allColumns.splice(0, 0, column) } else allColumns.push(column)
          }
        }
      }
      return allColumns
    ...
    hasData: function () {
      if (this.filteredRecords == -1) { return !(this.dataItems == null || this.dataItems.length == 0) } else {
  ...
  methods: {
    ...
    getColumnCustomAttributes: function (column) {
      ...
      if (!(css == null || css == undefined || css == '')) {
        ...
        ) { customAttributes.class = ' ' + css } else customAttributes.class = css
      }
    ...
    getSelectedValues: function (selectedRecords) {
      ...
      if (selectedRecords != null) {
        for (let i = 0; i < selectedRecords.length; i++) {
          ...
          if (this.dataKeyField.trim() != '') { result.push(item[this.dataKeyField.trim()]) } else result.push(item)
        }
      }
      return result
    },
    ...
    grid_ActionComplete: function (args) {
      if (args.action == 'filter' || args.action == 'clearFilter') {
        ...
        if (args.action == 'filter') { this.filteredRecords = this.grid.getFilteredRecords().length } else this.filteredRecords = -1
      } else {
    ...
    getModuleName: function () {
      if (!(this.$route.matched == null || this.$route.matched.length == 0)) { return this.$route.matched[0].name } else return 'ecms'
    },
