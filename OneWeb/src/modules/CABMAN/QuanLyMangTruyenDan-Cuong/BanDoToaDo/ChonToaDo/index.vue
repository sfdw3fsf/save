...
export default {
  ...
  methods: {
    ...
    getThongTinBanDo: async function () {
      try {
        ...
        if (rs.data != null && rs.data.length > 0) {
          ...
          this.searchAddressInput = parseFloat(this.thongtinbando.C_LATITUS)+','+parseFloat(this.thongtinbando.C_LONGTITUS)
    ...
    dblclickMap (event) {
      ...
      this.searchAddressInput = this.positionKetCuoi.lat +"," +this.positionKetCuoi.lng
    }
  },
  watch: {
    position (val) {
