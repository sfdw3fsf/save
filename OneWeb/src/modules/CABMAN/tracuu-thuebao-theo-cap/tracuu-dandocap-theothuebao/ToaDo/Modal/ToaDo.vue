<template src="./ToaDo.html">
  
</template>
<script>
import './../../../../../../utils/gmaps'
// import GMap from '../../Gmap/GMap.vue'
import FormToaDo from '../../Gmap/FormToaDo.vue'
import GraphNode from '../../structures/GraphNode'
export default {
  name: 'ToaDo',
  props: ['modalId', 'data'],
  created: async function () { },
  components: { FormToaDo },
  // components: { GMap },
  computed: {},
  watch: {
    nodes: async function (val) {
      let map = this.$refs.formToaDo
      map.clear()
  ...
  methods: {
    focusMyElement: async function () {
      ...
      if (this.data == null) {
        ...
        if (this.data.toado == '0,0' || this.data.toado == 'null,null' || this.data.toado == ',') this.toado = macdinh
        else {
    ...
    xacnhan: async function () {
      ...
      try {
        ...
        this.toado = this.$refs.formToaDo.positionDefault.lat + ',' + this.$refs.formToaDo.positionDefault.lng;
        this.data.toado = this.toado
    ...
    checkToaDo: function () {
      if (this.query == '') return false
        ...
        if (isNaN(parseFloat(this.query.split(',')[0])) || isNaN(parseFloat(this.query.split(',')[1]))) return false
      }
      return true
    }
  },
