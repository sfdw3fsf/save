import api from "../../GiamSatMangNgoaiVi/api";
...
export const actions = {
  ...
  async api_get_gian_do_cap({ commit }, data) {
    try {
      const response = await apiManager.api_get_gian_do_cap(data.CAP_ID);
      response.data.forEach(item => {
        item.kyHieuPhanBiet = data.kyHieuPhanBiet;
        item.thuTu = data.thuTu
      });
      commit("set_gian_do_cap", response.data);
      commit("set_node_focus", data.kyHieuPhanBiet);
      return response;
    } catch (error) {
      if (error.response && error .response.status === 400) {
        throw new Error(error.response.data.message_detail);
  ...
  async api_get_gian_do_cong({ commit }, data) {
    try {
      console.log(data)
      const response = await apiManager.api_get_gian_do_tuyen_cong(data.TUYENCB_ID);
      response.data.dsBeCap.forEach(item => {
        item.kyHieuPhanBiet = data.kyHieuPhanBiet;
        item.thuTu = data.thuTu
      });
      response.data.dsDoanCong.forEach(item => {
        item.kyHieuPhanBiet = data.kyHieuPhanBiet;
        item.thuTu = data.thuTu
      });
      commit("set_gian_do_cong", response.data);
      commit("set_node_focus", response.data.dsBeCap[0].BECAP_ID + '_' + data.kyHieuPhanBiet);
      return response;
    } catch (error) {
      if (error.response && error.response.status === 400) {
  ...
  async api_get_gian_do_cot({ commit }, data) {
    try {
      const response = await apiManager.api_get_gian_do_tuyen_cot(data.TUYENCOT_ID);
      response.data.dscot.forEach(item => {
        if(item.COTSAU_ID != null){
          item.COTSAU_ID = item.COTSAU_ID.toString() + '_' + data.kyHieuPhanBiet;
        }
        item.COT_ID = item.COT_ID.toString() + '_' + data.kyHieuPhanBiet;
        item.kyHieuPhanBiet = data.kyHieuPhanBiet;
        item.thuTu = data.thuTu
      });
      commit("set_gian_do_cot", response.data);
      commit("set_node_focus",  data.kyHieuPhanBiet);
      return response;
    } catch (error) {
      if (error.response && error.response.status === 400) {
  ...
  async deleteCapGoc({ commit }, data) {
    commit("set_ky_hieu_phan_biet_delete", data);
  },
};
