...
import { ComplexHierarchicalTree, ConnectorConstraints, NodeConstraints, SelectorConstraints, DiagramPlugin, Diagram, DiagramTools, LineDistribution, SnapConstraints, LineRouting, DiagramConstraints, PrintAndExport, DiagramContextMenu, ConnectionPointOrigin } from '@syncfusion/ej2-vue-diagrams'
...
export default Vue.extend({
  components: { QuanLyCapReNhanh, DaunoiThietBi, ModalQLCapExt, ModalGianDoTreeList, ModalQLBe },
  data: function () {
    return {
      dataSend: {},
      ds_dv: [],
      ...
      contextMenuOpen: function (args) {
        if (this.selectedItems.nodes.length > 0) {
          if (this.selectedItems.nodes[0].id.startsWith('TRAM_')) {
            ...
            if (this.selectedItems.nodes[0].id.startsWith('COT_')) {
              for (let index = 0; index < this.contextMenuSettings.items.length; index++) {
                const element = this.contextMenuSettings.items[index]
                if (element.group && element.group !== 'COT') {
                  args.hiddenItems.push(element.id)
                }
              }
              return
            }
            if (this.selectedItems.nodes[0].id.startsWith('BC_')) {
              for (let index = 0; index < this.contextMenuSettings.items.length; index++) {
                const element = this.contextMenuSettings.items[index]
                if (element.group && element.group !== 'CONG_BE') {
                  args.hiddenItems.push(element.id)
                }
              }
              return
          ...
          if (this.selectedItems.connectors[0].id.startsWith('DC_')) {
            for (let index = 0; index < this.contextMenuSettings.items.length; index++) {
              const element = this.contextMenuSettings.items[index]
              if (element.group && element.group !== 'CAP_CONG_BE') {
                args.hiddenItems.push(element.id)
              }
            }
            return
          }
          if (this.selectedItems.connectors[0].id.startsWith('CAP_')) {
            for (let index = 0; index < this.contextMenuSettings.items.length; index++) {
              const element = this.contextMenuSettings.items[index]
              if (element.group && element.group !== 'CAP_GOC') {
                args.hiddenItems.push(element.id)
              }
            }
            return
          }
          if (this.selectedItems.connectors[0].id.startsWith('LINK_COT_')) {
            for (let index = 0; index < this.contextMenuSettings.items.length; index++) {
              const element = this.contextMenuSettings.items[index]
              if (element.group && element.group !== 'CAP_COT') {
                args.hiddenItems.push(element.id)
              }
            }
            return
      ...
      contextMenuSettings: {
        ...
        items: [
          ...
          {
            id: 'MENU_KET_CUOI_ITEM_SEPARATOR_1',
            separator: true,
            group: 'KETCUOI'
          },
          {
          ...
          {
            id: 'MENU_KET_CUOI_ITEM_SEPARATOR_2',
            separator: true,
            group: 'KETCUOI'
          },
          {
          ...
          {
            id: 'MENU_COT_ITEM_SEPARATOR_1',
            separator: true,
            group: 'COT'
          },
          {
            text: 'Thuộc tính',
            id: 'MENU_ITEM_THUOC_TINH_COT',
            target: '.e-diagramcontent',
            iconCss: 'icon text-main f20 -ap icon-info',
            group: 'COT'
          },
          ...
          {
            text: 'Thuộc tính',
            id: 'MENU_ITEM_THUOC_TINH_CONG_BE',
            target: '.e-diagramcontent',
            iconCss: 'icon text-main f20 -ap icon-info',
            group: 'CONG_BE'
          },
          ...
          {
            id: 'MENU_CAP_CONG_BE_ITEM_SEPARATOR_1',
            separator: true,
            group: 'CAP_CONG_BE'
          },
          {
  ...
  methods: {
    ...
    focusByNodeID(nodeID) {
      setTimeout(() => {
        let nodeFocus = diagramInstance.nodes.find((node) => node.id == String(nodeID))
        if (nodeFocus == undefined) {
          nodeFocus = diagramInstance.nodes.filter((node) => node.id.includes(nodeID))[0]
        }
        if (nodeFocus) {
          diagramInstance.bringToCenter({
            x: nodeFocus.offsetX,
            y: nodeFocus.offsetY,
            width: nodeFocus.width,
            height: nodeFocus.height
          })
        }
      }, 1000)
    },
        ...
        try {
          if (obj.objData) {
            if (obj.objData.LOAIDT === 'KETCUOI' || obj.objData.LOAIDT === 'THUEBAO') {
              ...
              let kcNodeActive = diagramInstance.nodes.filter((node) => node.id.includes(objNodeId))[0]
              if (kcNodeActive) {
                let BringToCenter = {
            ...
            console.log(obj, 'hieu check')
            console.log(diagramInstance.connectors, 'hieu check')
            if (obj.dsDANDO_VEGOC && obj.dsDANDO_VEGOC.length > 0) {
              obj.dsDANDO_VEGOC.forEach((element) => {
                ...
                let capEdgeActive = diagramInstance.connectors.filter((con) => con.id.includes(objEdgeId))[0]
                if (capEdgeActive) {
                  let idxColor = this.idxColor
                  let color = ['blue ', '#FF5733']
                  capEdgeActive.style.strokeColor = color[idxColor]
    ...
    contextMenuClick: function (args) {
      ...
      if (args.element.id === 'MENU_ITEM_THUOC_TINH_DC') {
        if (diagramInstance.selectedItems.connectors.length > 0) {
          let id = diagramInstance.selectedItems.connectors[0].id.split('_')[1]
          this.$emit('thuocTinhCapCongBe', id)
        }
        return
      }
      ...
      if (args.element.id === 'MENU_ITEM_THUOC_TINH_CONG_BE') {
        if (diagramInstance.selectedItems.nodes.length > 0) {
          let id = diagramInstance.selectedItems.nodes[0].dataBe.BECAP_ID.split('_')[0]
      ...
      if (args.element.id === 'MENU_ITEM_THUOC_TINH_COT') {
        if (diagramInstance.selectedItems.nodes.length > 0) {
          let id = diagramInstance.selectedItems.nodes[0].id.split('_')[1]
          let obj = {
            cotId: id
          }
