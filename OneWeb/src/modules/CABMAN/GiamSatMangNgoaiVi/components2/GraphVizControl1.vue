<template>
  <div>
    ...
    <div id="content1" style="display: none; padding: 10px">
...
export default {
  ...
  methods: {
    ...
    save () {
      console.log('🚀 ~ file: GraphVizControl1.vue:551 ~ save ~ document.getElementById', document.getElementById(`${this.graphID}`))
    ...
    async menuEdge_Clicked (event) {
      ...
      if (event.option.slug === 'edge_xoa_ket_noi') {
        if (!this.CHECK_PERMIT_AND_MESSAGE()) return
        if (this.selectedComponents === null) return
        let ds_dn = [...new Set(this.selectedComponents.nodes())]
        for (let i = 0; i < ds_dn.length; i++) {
          let vdaunoi_id = ds_dn[i].id
          if (vdaunoi_id.startsWith('dn')) {
            let ketqua = await this.XOA_DAU_NOI(vdaunoi_id.substring(3).split('_'))
            if (ketqua) {
              this.deleteSelectedComponents_v2(ds_dn[i])
            }
            ...
            console.log('🚀 ~ file: GraphVizControl.vue ~ line 197 ~ menuEdge_Clicked ~ dnInfo', dnInfo)
            vcap_id = dnInfo[0]
            vmidspan_id = dnInfo[1]
            soi = dnInfo[2]
            if (!this.CHECK_PERMIT_AND_MESSAGE()) return
            let ketqua = await this.DAUNOI_CAP_V2(
              vcap_id,
              vmidspan_id,
              soi,
              vitri_n,
            ...
            console.log('🚀 ~ file: GraphVizControl.vue ~ line 207 ~ menuEdge_Clicked ~ ketqua', ketqua)
            if (ketqua) {
              this.deleteSelectedComponents_v2(ds_dn[i])
            }
          }
    ...
    deleteSelectedComponents_v2 (component) {
      console.log('🚀 ~ file: GraphVizControl1.vue:1498 ~ deleteSelectedComponents_v2 ~ component:', component)
      this.selectedComponents.style('display', 'none')
      // let self = this
      // let componentName = component.class
      // console.log('🚀 ~ file: GraphVizControl1.vue:1502 ~ deleteSelectedComponents_v2 ~ componentName:', componentName)
      // var componentName = component.select('title').text()
      // if (component.attr('class') === 'node') {
      //   self.dotGraph.deleteNode(componentName)
      // } else {
    ...
    async loadData () {
      try {
        this.setState({busy: true})
        // this.setDataGraphviz('digraph G {}')
        this.startNode = null
        this.endNode = null
        this.selectedComponents = d3_selectAll(null)
        ...
        let rs = await this.$root.context.get(
          '/web-cabman/ban-do-mang-cap/lay-gd-ketcuoi-con-v4' + '?ketcuoi_id=' + ketcuoi_id
        )
        let ds = rs.data
        ...
        this.KETCUOI = ds.dsKC
        this.VITRI = ds.dsVT
        if (ds.dsKC.length > 0) {
          if (ds.dsKC[0].LOAIKC_ID === 5) {
            this.btnThemKC_Caption = 'Thêm khay ODF'
          }
        }
        var firstInputPair = ''
        var firstOutputPair = ''
        ...
        sw =
          sw +
          ` graph [id=\"gd\" pad=1 splines=curve rankdir=\"LR\" clusterrank=local fontname=\"Courier New\" fontsize=14 bold=true margin=5 nodesep=0.05];\n`
        sw =
          sw +
          ` node [width=0.3 height=0.3 fontname=\"Courier New\" fontsize=14 shape=circle style=filled fillcolor=white color=black margin=0 fixedsize=shape];\n`

        // Cáp vào(s)
        sw = sw + ` subgraph cluster_capvaos {\n`
        sw =
          sw +
          ` graph [label=<<b>Cáp vào</b>> style=dashed bgcolor=white margin=20 fontsize=18 pencolor=\"Gray40\"];\n`

        ds.dsCAPVAO.forEach((cap) => {
          var node_tmps = []
          var tmp = ds.dsDN_CAP_VAO.filter(function (x) {
            return x.CAP_ID === cap.CAP_ID
          })
          tmp.forEach((soivao) => {
            var midspan = soivao.MIDSPAN === null ? ' ' : soivao.MIDSPAN
            var node_tmp = this.Format(
              'capvao{0}_{1}_node_tmp1|capvao{0}_{1}_node_tmp2|{2}',
              soivao.CAP_ID,
              soivao.MIDSPAN_ID,
              midspan
            )
            if (!node_tmps.includes(node_tmp)) {
              node_tmps.push(node_tmp)
            }
          })
          ...
          sw =
            sw +
            this.Format(
              '      graph [id="capvao_{0}" label=<<b>{1}</b>> style=rounded bgcolor=lightgrey fontsize=14];\n',
              cap.CAP_ID,
              cap.KYHIEU.replaceAll('>', '-')
            )
          // Vẽ đoạn cáp ở đầu
          sw =
            sw +
            `      node [style=filled label=\"\" width=0.2 height=0.2 fillcolor=black];`
          node_tmps.forEach((node_tmp) => {
            sw = sw + this.Format('      {0} [style=invis];', node_tmp.split('|')[0])
            sw =
              sw +
              this.Format(
                '      {0}->{1} [penwidth=7 dir=none label="{2}"]',
                node_tmp.split('|')[0],
                node_tmp.split('|')[1],
                node_tmp.split('|')[2]
              )
          ...
          tmp.forEach((soivao) => {
            ...
            var nodeName = this.Format(
              'soivao{0}_{1}_{2}',
              soivao['CAP_ID'],
              soivao['MIDSPAN_ID'],
              soivao['SOI']
            )
            var nodeID = this.Format(
              'sv_{0}_{1}_{2}',
              soivao['CAP_ID'],
              soivao['MIDSPAN_ID'],
              soivao['SOI']
        ...
        sw =
          sw +
          `  graph [label=<<b>Kết cuối</b>> style=dashed bgcolor=white margin=20 fontsize=18 pencolor=\"Gray40\"];\n`
        if (ds.dsKC.length > 0) {
          ...
          for (let index = 0; index < ds.dsKC.length; index++) {
            ...
            // Thông về cáp gốc
            let tmpVITRI = ds.dsVT.filter((e) => {
              return (
                e.KETCUOI_ID === kc.KETCUOI_ID &&
                e.VITRI === 1 &&
                e.MAT === 1 &&
                e.IS_ONLINE !== 0
              )
            ...
            if (isSplitter && isOnline) {
              ...
              let portOLTInfo = await this.$root.context.post(
                '/web-cabman/ban-do-mang-cap/sp_lay_port_olt_theocap',
                data
              )
            ...
            sw =
              sw +
              this.Format(
                '      graph [id="kc_{0}" label=<<b>{1}</b>> style=boxed bgcolor="' +
                  clusterBgColor +
                  '" fontsize=14];\n',
                kc['KETCUOI_ID'],
                kc['KYHIEU']
            ...
            var tmpdsVT = ds.dsVT.filter((e) => {
              return e.KETCUOI_ID === kc.KETCUOI_ID
            })
            for (let index = 0; index < tmpdsVT.length; index++) {
              ...
              sw =
                sw +
                this.Format(
                  '      "p{0}_{1}_{2}" [label="{1}" {3} id="vt_{0}_{1}_{2}"];\n',
                  vt['KETCUOI_ID'],
                  vt['VITRI'],
                  vt['MAT'],
                  nodeStyle
            ...
            for (let m = 1; m <= 2; m++) {
              ...
              var q = ds.dsVT
                .filter((e) => {
                  return e.KETCUOI_ID === kc.KETCUOI_ID && e.MAT === m
                })
                .sort(this.compareNumbers2)
              q.forEach((vt) => {
        ...
        sw =
          sw +
          `  graph [label=<<b>Cáp ra</b>> style=dashed bgcolor=white margin=20 fontsize=18 pencolor=\"Gray40\"];\n`
        for (let index = 0; index < ds.dsCAPRA.length; index++) {
          const cap = ds.dsCAPRA[index]
          var node_tmps = []
          var tmp = ds.dsDN_CAP_RA.filter(function (x) {
            return x.CAP_ID === cap.CAP_ID
          })
          tmp.forEach((soivao) => {
            var midspan = soivao.MIDSPAN === null ? ' ' : soivao.MIDSPAN
          ...
          sw =
            sw +
            `      graph [id=\"capra_${cap.CAP_ID}\" label=<<b>${cap.KYHIEU}</b>> style=rounded bgcolor=lightgrey fontsize=14];\n`
          ...
          tmp.forEach((soira) => {
            ...
            sw =
              sw +
              `      ${nodeName} [label=\"${soira.SOI}\" fillcolor=white id=\"${nodeID}\"];\n`
            svNodes.push(nodeName)
          })
          ...
          node_tmps.forEach((node_tmp) => {
            ...
            sw =
              ...
              `  ${node_tmp.split('|')[1]}->${
                node_tmp.split('|')[0]
              } [penwidth=7 dir=none label=\"${node_tmp.split('|')[2]}\"]`
            sw = sw + `  node [style=filled margin=0 width=0.3 height=0.3];`
          })
        ...
        sw = sw + `  }\n`
        if (firstInputPair !== '') {
          sw = sw + `  ${firstInputPair}->${firstJumper} [style=invis];\n`
        }

        if (firstOutputPair !== '') {
          sw = sw + `  ${lastJumper}->${firstOutputPair} [style=invis];\n`
        }
        // Đấu nối kết cuối
        var tmpdsDN = ds.dsDN.filter((e) => {
          return e.LOAITBI_N_ID === 25 && e.LOAITBI_D_ID === 25
        })
        tmpdsDN.forEach((dn) => {
          ...
          var t =
            (f1 ? ' style="dashed"' : '') +
            (disableConstraint ? ' constraint=false' : '')
          sw =
            sw +
            this.Format(
              '    "p{0}_{1}_{2}" -> "p{3}_{4}_{5}" [id="dn_{0}_{1}_{2}_{3}_{4}_{5}"{6} color=DarkBlue]\n',
              dn['THIETBI_N_ID'],
              dn['VITRI_N'],
              dn['MAT_N'],
              dn['THIETBI_D_ID'],
        ...
        for (let index = 0; index < ds.dsDN_CAP_VAO.length; index++) {
          ...
          if (dn.THIETBI_D_ID === null || dn.VITRI_D === null) {
            continue
          }
          ...
          sw =
            sw +
            this.Format(
              '    soivao{0}_{1}_{2} -> "p{3}_{4}_{5}" [id="dv_{0}_{1}_{2}_{3}_{4}_{5}"{6} dir=none];\n',
              dn['CAP_ID'],
              dn['MIDSPAN_ID'],
              dn['SOI'],
              dn['THIETBI_D_ID'],
        ...
        for (let index = 0; index < ds.dsDN_CAP_RA.length; index++) {
          ...
          if (dn.THIETBI_N_ID === null || dn.VITRI_N === null) {
            continue
          }
          let t = chkToiUuGianDo ? ' ' : ' constraint=false'
          sw =
            sw +
            this.Format(
              '    "p{3}_{4}_{5}" -> soira{0}_{1}_{2} [id="dr_{0}_{1}_{2}_{3}_{4}_{5}"{6} dir=none];\n',
              dn['CAP_ID'],
              dn['MIDSPAN_ID'],
              dn['SOI'],
              dn['THIETBI_N_ID'],
        ...
        ds.dsCAPVAO.forEach((capvao) => {
          ds.dsCAPRA.forEach((capra) => {
            if (capvao.CAP_ID === capra.CAP_ID) {
              let isVong =
                ds.dsDN_CAP_VAO.filter((e) => {
                  return (
                    e.CAP_ID === capvao.CAP_ID && e.MIDSPAN !== null && e.MIDSPAN !== 0
                  )
                }).length > 0
              if (isVong) {
                isVong =
                  ds.dsDN_CAP_RA.filter((e) => {
                    return (
                      e.CAP_ID === capvao.CAP_ID && e.MIDSPAN !== null && e.MIDSPAN !== 0
                    )
                  }).length > 0
              ...
              if (isVong) {
                sw =
                  sw +
                  `  capra${capvao.CAP_ID}_0_node_tmp1->capvao${capvao.CAP_ID}_0_node_tmp1 [constraint=fals penwidth=7 style=dashed];\n`
              }
            }
          })
        })
    ...
    renderGraph (dataGraphviz) {
      console.log('🚀 ~ file: GraphVizControl1.vue:2018 ~ renderGraph ~ dataGraphviz:', dataGraphviz)
      let fit = this.fit
    ...
    handleRightClickEdge (d, i) {
      ...
      if (event.which === 3) {
        ...
        // console.log('🚀 ~ file: GraphVizControl1.vue:2192 ~ handleRightClickEdge ~ extendSelection:', extendSelection)
    ...
    handleClickEdge (d, i) {
      ...
      let extendSelection = event.ctrlKey || event.shiftKey
      console.log('🚀 ~ file: GraphVizControl1.vue:2218 ~ handleClickEdge ~ extendSelection:', extendSelection)
    ...
    selectComponents (components, extendSelection = false) {
      if (extendSelection) {
        let _node = components.nodes()
        if (this.selectedComponents.nodes().length === 0) {
          this.selectedComponents = d3_selectAll(
            _node
          )
        } else {
          if ((this.selectedComponents.nodes()).includes(_node) === false) {
            this.selectedComponents = d3_selectAll(
              this.selectedComponents.nodes().concat(_node)
            )
          }
        }
        console.log('🚀 ~ file: GraphVizControl1.vue:2983 ~ selectComponents ~ this.selectedComponents:', this.selectedComponents.nodes())
      } else {
        this.unSelectComponents()
        this.selectedComponents = components
        console.log('🚀 ~ file: GraphVizControl1.vue:2984 ~ selectComponents ~ this.selectedComponents:', this.selectedComponents.nodes())
      }
