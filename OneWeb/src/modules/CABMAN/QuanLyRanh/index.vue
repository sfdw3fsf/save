...
export default {
  ...
  methods: {
    ...
    napLai() {
      //Load lại danh sách ranh và bản đồ
      EventBus.$emit('LOAD_RANHTRAM', 'LOAD_RANHTRAM')
    },
    LuuRanhTram() {
      ...
      reader.onload = async (e) => {
        ...
        if (sGeoJSON.type === 'FeatureCollection' && sGeoJSON.features.length > 0) {
          ...
          try {
            ...
            if(rs.error == '200'){
              this.$root.$toast.success('Import file KML thành công!')
            } else{
              this.$root.$toast.error('Import file KML thất bại!')
            }
    ...
    //xuất excel và tải file excel về máy
    async exportExcel() {
      try {
        this.$root.showLoading(true)
        await this.api_export_excel()
        this.$root.showLoading(false)
      } catch (error) {
        console.log("🚀 ~ file: index.vue:107 ~ reader.onload= ~ error:", error)
        this.$root.showLoading(false)
