...
export default class DotGraph {
    ...
    insertEdge(startNodeName, endNodeName, attributes) {
        ...
        console.log(
            '🚀 ~ file: dot.js ~ line 49 ~ DotGraph ~ insertEdge ~ newEdgeString',
            newEdgeString
        );
    ...
    parseChildren(children, parent) {
        children.forEach((child, i) => {
            if (child.type === 'node_stmt') {
                ...
                const attributes = child.attr_list.reduce(function(
                    attrs,
                    attr,
                    i
                ) {
                ...
                {});
                Object.assign(
                    this.nodes[child.node_id.id].attributes,
                    attributes
                );
                ...
                if (parent.type === 'edge_stmt') {
                    if (i > 0) {
                        ...
                        const nodeNames = nodeIds.map(
                            (nodeId) =>
                                nodeId.id +
                                (nodeId.port ? ':' + nodeId.port.id : '')
                        );
                        const edgeId =
                            nodeNames[0] + this.edgeop + nodeNames[1];
        ...
        children.forEach((child, i) => {
            ...
            if (child.type === 'node_stmt') {
                ...
                if (typeof child.eq === 'object') {
                    ...
                    this.str +=
                        quoteIdIfNecessary(child.id) +
                        '=' +
                        quoteIdIfNecessary(child.eq);
                }
            } else if (child.type === 'edge_stmt') {
    ...
    deleteComponentInStatementList(statementList, type, id, edgeRHSId, erase) {
        statementList.forEach((statement, i) => {
            ...
            if (statement.type === 'attr_stmt') {
                ...
                const eraseNode =
                    type === 'node' && statement.node_id.id === id;
                ...
                if (erasedLastEdgeStatement) {
                    ...
                    if (statement.attr_list.length > 0) {
                        if (
                            !statementSeparators.includes(
                                this.dotSrc[this.index - 1]
                            ) &&
                            !statementSeparators.includes(
                                this.dotSrc[this.index]
                            )
                        ) {
                            this.insert(' ');
                        }
    ...
    insert(string) {
        this.dotSrc =
            this.dotSrc.slice(0, this.index) +
            string +
            this.dotSrc.slice(this.index);
