<template>
  <div class="map-canvas-box">
    ...
    <ModalThemMoiTuyenDuong modalID="ModalThemMoiTuyenDuong" ref="ModalThemMoiTuyenDuong"></ModalThemMoiTuyenDuong>
  </div>
</template>
...
import ModalThemMoiTuyenDuong from '../modal/ThemMoiTuyenDuong'
import GraphNode from '../structures/GraphNode'
const TOKEN = 'VNPT-f76bf568-b093-45cf-9761-f6339d3e7d9a'
export default {
  components: { 'vue-simple-context-menu': VueSimpleContextMenu, MeasureTool, html2canvas, DaunoiThietBi, ModalQLCapExt, ModalThemMoiTuyenDuong },
  name: 'BanDoVNPT',
  props: {},
  async created() {
  ...
  data() {
    return {
      ...
      optionsNodeMenu: [
        ...
        {
          name: '<span class="icon text-main f20 one-taocap"></span> Tạo cáp tới kết cuối cùng tuyến',
          slug: 'tao_cap_toi_ketcuoi_cungtuyen'
        },
        {
      ...
      optionsMapMenu: [
        ...
        {
          name: '<span class="icon text-main f20 nc-icon-glyph location_crosshair"></span> Tạo kết cuối',
          slug: 'tao_ket_cuoi_vnptmap'
        },
        {
          name: '<span class="icon text-main f20 nc-icon-glyph location_crosshair"></span> Tạo tên đường',
          slug: 'tao_ten_duong_vnptmap'
        }
      ],
      optionsPolyline: [
        {
  ...
  watch: {
    ...
    ds_ketcuoi_cnc: function (val) {
      let cnc = this.pointLayers.filter((element) => {
        return element.cnc
      })
      cnc.forEach((element) => {
        element.remove()
      ...
      val.forEach((element) => {
        ...
        node_d.id = 'KETCUOI_' + element.KETCUOI_ID;
        node_d.position = { lat: element.KINHDO, lng: element.VIDO };
        node_d.setIcon(element.ICON_D);
        node_d.setPosition(element.TOADO_D);
        node_d.setSymbol(element.KYHIEU_D);
        const point = [node_d.position.lng, node_d.position.lat];
        const obj = {
          iconUrl: node_d.icon.url,
          id: node_d.id,
          iconWidth: node_d.icon.size.width,
          iconHeight: node_d.icon.size.height,
          isDraggable: false,
          className: node_d.id + 'maps'
        }
        var pointLayer = VnptMap.initPoint(point, obj)
        pointLayer.symbol = node_d.symbol
        pointLayer.setLabel(node_d.symbol)
        pointLayer.setLabelVisible(true)
        pointLayer.id = node_d.id
        pointLayer.id_polyline = node_d.id_polyline
        pointLayer.modified = node_d.modified
        pointLayer.modifiedDate = node_d.modifiedDate
        pointLayer.draggable = false
        pointLayer.isDiemuon = node_d.isDiemuon
        pointLayer.visible = node_d.visible && node_d.position.lat * node_d.position.lng > 0
        pointLayer.setVisible(pointLayer.visible)
        pointLayer.position = node_d.position
        pointLayer.icon = node_d.icon
        pointLayer.tooltipText = node_d.tooltipText
        pointLayer.cnc = true
        pointLayer.on(this.mapVNPT, 'contextmenu', (e) => {
          this.point.x = e.pointEvent.x
          this.point.y = e.pointEvent.y
          this.rightclickNode(e, pointLayer.id)
        })
  ...
  async mounted() {
    ...
    vm.mapVNPT.on('draw.create', (e) => {
      this.$refs.ModalThemMoiTuyenDuong.show(e)
    })
  },
  methods: {
      ...
      try {
        if (obj.objData) {
          ...
          if (obj.objData.LOAIDT === 'CAP') {
            ...
            if (activePolyline) {
              let colorCab = this.cabColor(obj.objData.KYHIEU)
              clearInterval(this.stylePolylineInterval)
              this.isBlueColor = true
              this.stylePolylineInterval = setInterval(function () {
                if (this.isBlueColor) {
                  activePolyline.setStyle({ color: colorCab })
                } else {
    ...
    cabColor(kyHieu) {
      for (const element of this.gian_do_cap) {
        if (element.KYHIEU === kyHieu) {
          switch (element.TTDA_ID) {
            case 1:
              return 'blue'
            case 2:
              return '#FFFF00'
        ...
        case 'xoa_diem_uon':
          ...
          if (objIndex > -1) {
            ...
            if (idx2 > -1) {
              ...
              this.polylineLayers[idx].modifiedDate = new Date();
    ...
    luuToaDoBanDo() {
      ...
      for (let index = 0; index < this.pointLayers.length; index++) {
        ...
        if (node.modified && !node.id.startsWith('DIEMUON_')) {
          ...
          let tg = node.modifiedDate
          nodes.push({ loai: arr[0], id: arr[1], lat: lat, lng: lng, modifiedDate: tg })
        }
      }
      var polylines = []
      ...
      if (clickedItem) {
        if (this.vaction === 'TaoCapKetNoi') {
          if (this.vnguon_id === null || this.vnguon_id.length === 0) {
            ...
            let vnguon_ids = clickedItem.split('_')
            this.addHighlightNguon(args)
            this.setState({ key: 'vnguon_id', value: vnguon_ids })
    ...
    leftclickMap(event) {
      ...
      if (clickedItem) {
        this.vdoituong_id = clickedItem.split('_')
        this.setState({ key: 'vdich_id', value: clickedItem.split('_') })
        console.log(this.vaction)
        if (this.vaction !== 'None') {
          if (this.vaction === 'TaoCap') {
            console.log('🚀 ~ file: GianDo.vue ~ line 747 ~ onClick ~ this.vnguon_id', this.vnguon_id)
            // if (this.vnguon_id === null || this.vnguon_id.length === 0) {
            ...
            //   return
            // }
            if (this.vnguon_id[0] !== this.vdich_id[0] || this.vnguon_id[1] !== this.vdich_id[1]) {
              if (this.vdich_id[0] == 'CAP') {
                this.toast && this.toast.close()
                this.setState({ key: 'vaction', value: 'None' })
                this.setState({ key: 'vnguon_id', value: null })
                this.setState({ key: 'vdich_id', value: null })
            ...
            if (this.polylineLayers[index].source === ketcuoi_id || this.polylineLayers[index].dest === ketcuoi_id) {
              for (let index2 = 0; index2 < this.polylineLayers[index].path.length; index2++) {
                if (this.polylineLayers[index].path[index2].id === ketcuoi_id) {
                  ...
                  this.polylineLayers[index].modifiedDate = new Date()
    ...
    menuMap_Clicked(event) {
      switch (event.option.slug) {
        ...
        case 'tao_ten_duong_vnptmap':
          ...
          // this.$refs.ModalThemMoiTuyenDuong.show(this.locationrightclickMap)
          break
      }
    },
    menuPolyline_Clicked(event) {
      switch (event.option.slug) {
        ...
        case 'sua_toado_cap':
          ...
          event.item.modifiedDate = new Date()
            ...
            if (evt.paths.length > 0) {
              ...
              for (let i = 0; i < ddata.decodedData.length; ++i) {
                ...
                pointLayer.onDrag('drag', (e) => {
                  ...
                  this.pointDiemUonLayers[objIndex].modifiedDate = new Date()
                    ...
                    if (nodeid === polyline.dest || nodeid === polyline.source || nodeid.startsWith('DIEMUON_')) {
                      ...
                      for (let j = 0; j < path.length; j++) {
                        if (path[j].id === nodeid) {
                          ...
                          if(polyline.modified)
                            polyline.modifiedDate = new Date();
                          else
                          polyline.modifiedDate = null;
                          break
                        }
                      }
                      polyline.path = path
    ...
    setNodes(nodes) {
      ...
      for (let index = 0; index < _nodes.length; index++) {
        ...
        pointLayer.modifiedDate = node.modifiedDate
        pointLayer.draggable = false
        ...
        pointLayer.onDrag('drag', (e) => {
          ...
          this.pointLayers[objIndex].modifiedDate = new Date()
          ...
          if (nodeid === polyline.dest || nodeid === polyline.source || nodeid.startsWith('DIEMUON_')) {
            ...
            for (let j = 0; j < path.length; j++) {
              if (path[j].id === nodeid) {
                ...
                path[j].lng = lng
                polyline.modified = !!nodeid.startsWith('DIEMUON_')
                polyline.modifiedDate = polyline.modified ? new Date() : null;
                break
              }
            }
            polyline.path = path
    ...
    reAllPolyline() {
      for (let i = 0; i < this.polylineLayers.length - 1; i++) {
        ...
        polylineLayer.modifiedDate = polyline.modifiedDate
        ...
        polylineLayer.on('mouseout', (e) => {
          me.popupInstance && me.popupInstance.remove()
          polylineLayer.setStyle({ color: polyline.color })
        })
