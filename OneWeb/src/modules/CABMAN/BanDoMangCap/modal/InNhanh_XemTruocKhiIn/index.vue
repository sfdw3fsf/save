...
export default {
  ...
  mounted() {
    setTimeout(function() {
      $('.diagram_hiddenUserHandleTemplate').remove()
      //  $('#diagram_gridline_svg').remove();
    }, 1500)
  },
  methods: {
    onDragStart(event) {
      // console.log('🚀 ~ file: index.vue:89 ~ onDragStart ~ event:', event)
    ...
    getHtmlDiagram() {
      ...
      html.insertAdjacentHTML('beforeend', "<div id='insert-element'></div>")
      document.getElementById('insert-element').insertAdjacentHTML('beforeend', document.getElementById('diagramGianDo_diagramLayer_div').innerHTML)
      document.getElementById('insert-element').insertAdjacentHTML('beforeend', document.getElementById('diagramGianDo_htmlLayer').innerHTML)
      ...
      if (parseInt(this.scaleDiagram.toString(), 10) === 1) {
        ...
        let diagram_width = $('#diagram_diagramLayer_svg').width()
        let ratio = (frame_width / diagram_width) * 100
        div.style.zoom = ratio + '%'
        this.range = ratio / 100
      } else {
        div.style.zoom = this.scaleDiagram + '%'
        this.range = parseInt(this.scaleDiagram.toString(), 10) / 100
      }
    },
    setZoom() {
      $('#insert-element').css('zoom', this.range)
    },
    onHiddenModal() {
      this.isShown = false
      this.count = 0
...
.xem-truoc-khi-in .table-result td,
.table-result th {
  padding: 5px 7px;
  border: 2px solid #212529 !important;
  height: 32px;
}
.xem-truoc-khi-in .table-result thead tr {
