...
export default {
  ...
  data() {
    var demoVue = Vue.component("demo", {
      template:
        "<div><div class='inline' style ='float:left'>{{data.NAME}}</div>" +
      ...
      async created() {
        if (this.data.STATUS != "") this.showDIv = true;
      },
    });
    return {
      key: 1,
  ...
  methods: {
    ...
    async onShownModal() {
      ...
      try {
        ...
        await this.NAP_DS_TOKT();
        //await this.NAP_DS_TRAMTB()
        if (this.allowChoose != null) this.isEnableDonVi = this.allowChoose;
      } catch (error) {
      } finally {
        this.loading(false);
      }
    ...
    getDonVi: async function (tram_tbi_id) {
      ...
      if (exch_id !== 0) {
        let rs = await this.$root.context.post(
          `/web-cabman/daunoi-thietbi/load-daunoi-tbi?donvi_id=${exch_id}`
        );
        if (rs.data.length > 0) {
          let arr = rs.data.filter(
            (x) => this.dataCbo.cboTTVT.filter((y) => y.DONVI_ID == x.TTVT_ID).length > 0
          );
    ...
    NAP_DS_TTVT: async function () {
      try {
        ...
        let rs = await this.$root.context.get(
          "/web-cabman/chon-ketcuoi/lay-ds-ttvt-theo-nv",
    ...
    NAP_DS_TOKT: async function () {
      try {
        ...
        if (this.modelCbo.mTTVT !== -1) {
          let rs = await this.$root.context.get(
            "/web-cabman/chon-ketcuoi/lay-ds-tokt-theo-nv",
            {
              ttvt_id: this.modelCbo.mTTVT,
              nhanvien_id: this.$root.token.getNhanVienID(),
            }
          );
          ...
          if (rs.data != null && rs.data.length > 0) {
            this.modelCbo.mToKT =
              this.cbodonviExt.tokt != -1 ? this.cbodonviExt.tokt : rs.data[0].DONVI_ID;
            this.cbodonviExt.tokt = -1;
    ...
    NAP_DS_TRAMTB: async function () {
      try {
        ...
        if (this.modelCbo.mToKT !== -1) {
          ...
          let rs = await this.$root.context.get(
            "/web-cabman/daunoi-thietbi/lay-ds-tram-tbi",
            { tokt_id: tokt_id }
          );
          this.dataCbo.cboTramTB = rs.data;
          if (rs.data != null && rs.data.length > 0) {
            this.modelCbo.mTramTB =
              this.cbodonviExt.tramtb != -1
                ? this.cbodonviExt.tramtb
                : rs.data[0].DONVI_ID;
            this.cbodonviExt.tramtb = -1;
    ...
    NAP_DS_LOAI_TBI: async function () {
      try {
        ...
        if (this.modelCbo.mTramTB !== -1) {
          ...
          let rs = await this.$root.context.get(
            `/web-cabman/daunoi-thietbi/lay-ds-loai-tbi?donvi_id=${donvi_id}`
          );
    ...
    addTree: function (items) {
      ...
      if (items != null) {
        for (let i = 0; i < items.length; i++) {
          ...
          if (this.tree.findIndex((x) => x.ID == node.ID) == -1) ds.push(node);
        }
    ...
    NAP_DS_THIETBI: async function () {
      try {
        ...
        await this.$root.context
          .get("/web-cabman/daunoi-thietbi/lay-ds-tbi", {
            donvi_id: tramtb_id,
    ...
    getDuLieuThietBiCon: async function (id) {
      ...
      if (node) {
        ...
        if (node.HAS_PORT === 1 || node.CO_PORT === 1) {
          ...
          await this.$root.context
            .get("/web-cabman/daunoi-thietbi/lay-ds-cong-con", {
              kieu: 1,
              loaitbi_id: loaitbi_id,
    ...
    NAP_DS_THIETBI_CON: async function (node) {
      try {
        if (node !== null) {
          const [loaitbi_id, thietbi_id] = node.ID.split("-");
          await this.$root.context
            .get("/web-cabman/daunoi-thietbi/lay-ds-tbi-con", {
              loaitbi_id: loaitbi_id,
              thietbi_id: thietbi_id,
            })
            .then((rs) => {
    ...
    NAP_DS_CONG_CON: async function (node) {
      try {
        if (node !== null) {
          ...
          if (node.HAS_PORT === "1") {
            let rs = await this.$root.context.get(
              "/web-cabman/daunoi-thietbi/lay-ds-cong-con",
    ...
    NAP_LAI_CONG: async function (p0, p1, p2) {
      try {
        ...
        let rs = await this.$root.context.post(
          "/web-cabman/daunoi-thietbi/lay-thongtin-cong1",
          data_json
        );
    ...
    btnTaoMoi: function () {
      ...
      if (currentNode.length > 0) {
        if (!(currentNode[0].IMAGE == null || currentNode[0].IMAGE == undefined)) {
          if (currentNode[0].IMAGE.split("_").length > 2) {
            kieu_dn = currentNode[0].IMAGE.split("_")[2];
          }
        }
      }
    ...
    KIEMTRA: function () {
      ...
      if (this.Visible.tlpSoi_1) {
        ...
        if (
          this.hidden.port1 &&
          (this.modelCbo.mCong === -1 || this.modelCbo.mCong == "")
        ) {
          if (!flag) {
      ...
      if (this.Visible.tlpSoi_2) {
        ...
        if (
          this.hidden.port2 &&
          (this.modelCbo.mCong2 === -1 || this.modelCbo.mCong2 == "")
        ) {
          if (!flag) {
    ...
    btnGhiLai: async function () {
      if (this.KIEMTRA()) await this.GHILAI();
    },
    GHILAI: async function () {
      try {
        ...
        if (this.checkKyHieu) {
          ...
          kyhieu = this.dataCbo.cboKyHieu.filter(
            (x) => x.CAPTD_ID == this.modelCbo.mKyHieu
          )[0].KYHIEU;
        } else {
          id_cap = -1;
        ...
        let data = {
          ...
          kh: kyhieu,
          td: this.txtTenCap,
          dd: Number(this.rdgKieuDauNoi) + 1,
        ...
        if (this.Visible.tlpSoi_1 && this.dataCbo.cboCong.length > 0) {
          p1 =
            this.modelCbo.mCong !== "" && this.modelCbo.mCong !== -1
              ? this.modelCbo.mCong.split("-")
              : "";
          if (p1.length == 3) {
            const [lt, tb, vt] = p1;
            data["lt2"] = lt;
        ...
        if (this.Visible.tlpSoi_2 && this.dataCbo.cboCong2.length > 0) {
          p2 =
            this.modelCbo.mCong2 !== "" && this.modelCbo.mCong2 !== -1
              ? this.modelCbo.mCong2.split("-")
              : "";
          if (p2.length == 3) {
            const [lt, tb, vt] = p2;
            data["lt3"] = lt;
        ...
        await this.$root.context.post(
          "/web-cabman/daunoi-thietbi/capnhat-dl-daynhay",
          data_json
        );
        if (!this.isDisableBtnNhapMoi) this.$root.toastSuccess("Tạo dây nhảy thành công");
    ...
    btnXoa: async function () {
      this.$bvModal
        .msgBoxConfirm("Bạn có thực sự muốn xóa dây nhảy đang chọn không ?", {
    ...
    XOA: async function () {
      try {
        ...
        await this.$root.context.post(
          `/web-cabman/daunoi-thietbi/xoa-cap-td?captd_id=${captd_id}`
        );
    ...
    LOAD_TBI_CONG1: function () {
      ...
      let loaitb1 = this.dataCbo.cboLoaiTbi.map((item) => {
        return { ...item };
      }); //[...ds1]
      if (loaitb1.length > 0) {
    ...
    LOAD_TBI_CONG2: function () {
      ...
      let loaitb2 = this.dataCbo.cboLoaiTbi.map((item) => {
        return { ...item };
      }); //[...ds1]
      if (loaitb2.length > 0) {
    ...
    NAP_DS_KIEU_DC: async function () {
      try {
        let rs = await this.$root.context.get(
          "/web-cabman/daunoi-thietbi/lay-ds-kieu-dc"
        );
    ...
    NAP_DS_LOAI_CAP: async function () {
      try {
        let rs = await this.$root.context.get(
          "/web-cabman/daunoi-thietbi/lay-ds-loai-cap"
        );
    ...
    NAP_DS_DAYNHAY: async function (pid) {
      try {
        ...
        let rs = await this.$root.context.get(
          `/web-cabman/daunoi-thietbi/lay-ds-daynhay-cong?loaitbi_id=${loaitbi_id}&thietbi_id=${thietbi_id}&vitri=${vitri}`
        );
        if (rs.data != null && rs.data.length > 0) {
    ...
    changeKyHieu: async function () {
      try {
        ...
        let selectedDslam = dsKyHieu.filter(
          (x) => x.CAPTD_ID == this.modelCbo.mKyHieu
        )[0];
        if (dsKyHieu.length > 0) {
          if (selectedDslam != null) {
            ...
            if (pid.length == 3) {
              ...
              let d1 = await this.$root.context.post(
                "/web-cabman/daunoi-thietbi/lay-tt-tbi-kn",
                data
              );
              ...
              if (d1.data != null && d1.data.length > 0) {
                ...
                //console.log('po', po)
                this.modelCbo.mCong = this.port1 =
                  po[0].LOAITBI_ID + "-" + po[0].THIETBI_ID + "-" + po[0].VITRI;
              } else {
              ...
              if (this.Visible.tlpSoi_2) {
                ...
                let d2 = await this.$root.context.post(
                  "/web-cabman/daunoi-thietbi/lay-tt-tbi-kn",
                  data
                );
                this.resetSoi2();
                ...
                if (d2.data != null && d2.data.length > 0) {
                  ...
                  if (this.hidden.thietbi2) {
                    this.modelCbo.mThietbi2 =
                      item2[0].LOAITBI_ID + "-" + item2[0].THIETBI_ID;
                    await this.cboThietBi_2_EditValueChanged();
                  }
                  ...
                  this.modelCbo.mCong2 = this.port2 =
                    po[0].LOAITBI_ID + "-" + po[0].THIETBI_ID + "-" + po[0].VITRI;
                } else {
        ...
        if (!this.isDisableBtnNhapMoi) this.XOA_THONGTIN_DAYNHAY();
      } catch (ex) {
        console.log(ex);
      }
    },
    ...
    NAP_DS_THUEBAO_CONG: async function (pid) {
      ...
      let rs = await this.$root.context.get(
        `/web-cabman/daunoi-thietbi/lay-ds-cong-con?kieu=2&loaitbi_id=${loaitbi_id}&thietbi_id=${thietbi_id}`
      );
      if (rs.data != null && rs.data.length > 0) {
        for (let dr in rs.data) {
      ...
      try {
        ...
        if (rs.data != null) {
          if (cbo === "thietbi1") {
            ...
            if (rs.data.length > 0) {
              if (!this.firstLoaded) this.modelCbo.mThietbi = rs.data[0].ID;
            }
          } else if (cbo === "thietbi2") {
            this.dataCbo.cboThietbi2 = rs.data;
            if (rs.data.length > 0) {
              if (!this.firstLoaded) this.modelCbo.mThietbi2 = rs.data[0].ID;
            }
          }
        }
      } catch (error) {
    ...
    getdsThietBiCon: async function (loaitbi_id, thietbi_id) {
      ...
      try {
        let rs = await this.$root.context.get(
          "/web-cabman/daunoi-thietbi/lay-ds-tbi-con",
          { loaitbi_id: loaitbi_id, thietbi_id: thietbi_id }
        );

        if (rs.data != null && rs.data.length > 0) ds = rs.data;
        return ds;
      } catch (e) {
        return ds;
      }
    ...
    fillCboFrame: async function (type, loaitbi_id, thietbi_id) {
      if (type === 1) {
        ...
        if (rs.data != null) {
          ...
          if (rs.data.length > 0) {
            if (!this.firstLoaded) {
              this.modelCbo.mFrame =
                this.modelCbo.mFrame != -1 ? this.modelCbo.mFrame : rs.data[0].ID;
            }
          }
        }
        await this.cboFrame_1_EditValueChanged();
        ...
        this.dataCbo.cboFrame2 = [];
        let rs = await this.$root.context.get(
          `/web-cabman/daunoi-thietbi/lay-ds-tbi-con?loaitbi_id=${loaitbi_id}&thietbi_id=${thietbi_id}`
        );
        if (rs.data != null) {
