  ...
  methods: {
    ...
    onClickToLeft() {
      if(this.daGanSelected.length > 0) {
        const itemTransf = this.dsMDDaGan.filter(item=> this.daGanSelected.includes(item.MUCDICH_ID))
    ...
    onClickToRight() {
     if(this.chuaGanSelected.length > 0) {
        const itemTransf = this.dsMDChuaGan.filter(item=> this.chuaGanSelected.includes(item.MUCDICH_ID))
    ...
    resetForm(form) {
      Object.keys(form).forEach(function(key){
        if(key !== 'ckbZalo' && key !='rlhId' && key != 'ckbOA') form[key] = ""
      });
      this.form.ckbZalo = false;
    ...
    onClickCapNhat() {
      ...
      let pDSDK = this.addNewClicked ? this.makeDsDkAddNew() : this.makeDsDk();
      const dsIdMucDich =[];
    ...
    makeDsDk() {
      ...
      this.dsTLHKH.map(item=> {
        let CbkDk = document.getElementById(`dk${this.form.ttlhId}_${item.MA_KH}`);
        let CbkHuy = document.getElementById(`huy${this.form.ttlhId}_${item.MA_KH}`);
      ...
      this.dsLHThanhToan.map(item=> {
        let CbkDk = document.getElementById(`dk${this.form.ttlhId}_${item.MA_TT}`);
        let CbkHuy = document.getElementById(`huy${this.form.ttlhId}_${item.MA_TT}`);
        if (CbkDk && CbkDk.checked) {
          pDSDK.push({
      ...
      this.dsLHThueBao.map(item=> {
        let CbkDk = document.getElementById(`dk${this.form.ttlhId}_${item.MA_TB}`);
        let CbkHuy = document.getElementById(`huy${this.form.ttlhId}_${item.MA_TB}`);
        if (CbkDk && CbkDk.checked) {
          pDSDK.push({
        ...
        this.dsTLHKH = rs.data.map(item=> {
          return {
            ...
            DANGKY: `<div class="check-action">
                        <input type="checkbox" id="dk${this.form.ttlhId}_${item.MA_KH}" class="check" ${item.SUDUNG == 'Dang SD' ? 'disabled' : ''}>
                        <span class="name"></span>
                    </div>`,
            HUY :  `<div class="check-action">
                        <input type="checkbox"  id="huy${this.form.ttlhId}_${item.MA_KH}" class="check" ${item.SUDUNG != 'Dang SD' ? 'disabled' : ''}>
                        <span class="name"></span>
                    </div>`,
          }
        });
      ...
      try {
        ...
        this.dsLHThanhToan = rs.data.map(item=> {
          return {
            ...item,
            DANGKY: `<div class="check-action">
                        <input type="checkbox" id="dk${this.form.ttlhId}_${item.MA_TT}" class="check" ${item.SUDUNG == 'Dang SD' ? 'disabled' : ''}>
                        <span class="name"></span>
                    </div>`,
            HUY :  `<div class="check-action">
                        <input type="checkbox"  id="huy${this.form.ttlhId}_${item.MA_TT}" class="check" ${item.SUDUNG != 'Dang SD' ? 'disabled' : ''}>
                        <span class="name"></span>
                    </div>`,
          }
        });
      ...
      try {
        ...
        this.dsLHThueBao = rs.data.map(item=> {
          return {
            ...item,
            DANGKY: `<div class="check-action">
                        <input type="checkbox" id="dk${this.form.ttlhId}_${item.MA_TB}" class="check" ${item.SUDUNG == 'Dang SD' ? 'disabled' : ''}>
                        <span class="name"></span>
                    </div>`,
            HUY :  `<div class="check-action">
                        <input type="checkbox"  id="huy${this.form.ttlhId}_${item.MA_TB}" class="check" ${item.SUDUNG != 'Dang SD' ? 'disabled' : ''}>
                        <span class="name"></span>
                    </div>`,
          }
        });
    ...
    getDiaChiLienHe(data){
      this.form.diaChi = data.diachimoi || ""
    }
  },
};
</script>
