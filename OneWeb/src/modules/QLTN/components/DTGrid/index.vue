...
export default {
  ...
  computed: {
    ...
    //=== Add for footer ===
    countFooter() {
        return this.columns.filter(i => i.footer === 'Count');
    },
    sumFooter() {
        return this.columns.filter(i => i.footer === 'Sum');
    },
    countSelectedFooter() {
      return this.columns.filter(i => i.footer === 'CountSelected');
    },      
    sumSelectedFooter() {
      return this.columns.filter(i => i.footer === 'SumSelected');
    }
  },
  methods: {
    ...
    grid_RowSelected: function (args) {
      ...
      if (this.countSelectedFooter.length > 0) {
        ...
        this.footerData.countSelected = args.rowIndexes.length + '/' + this.dataSource.length;          
      }
      if (this.sumSelectedFooter.length > 0) {
        isAggregateRefresh = true;          
        let checkedItems = this.$refs.grid.getSelectedRecords();
        this.footerData.sumSelected = [];
        this.sumSelectedFooter.forEach(item => {
          let data = this.calculateSum(checkedItems, item.fieldName) + '/' + this.calculateSum(this.dataSource, item.fieldName);           
          this.footerData.sumSelected.push({ key: item.fieldName, data: data });
          let index = this.columnAggregate.findIndex(e => e.field === item.fieldName);
          this.columnAggregate.splice(index, 1);
          this.columnAggregate.push({ field: item.fieldName, type: 'Custom', footerTemplate: this.footerCustomSelected, customAggregate: () => (data)});
        });
      }
      // Refresh footer aggregates:
    ...
    grid_RowDeselected: function (args) {
      ...
      if (this.countSelectedFooter.length > 0) {
        isAggregateRefresh = true;
        this.footerData.countSelected = this.$refs.grid.getSelectedRecords().length + '/' + this.dataSource.length;          
      }
      if (this.sumSelectedFooter.length > 0) {
        isAggregateRefresh = true;          
        let checkedItems = this.$refs.grid.getSelectedRecords();
    ...
    grid_DataBound: function (args) {
      ...
      this.countFooter.forEach(item => {
        this.columnAggregate.push({ field: item.fieldName, type: 'Count', format: 'N0', footerTemplate: this.footerCount });
      });
      this.sumFooter.forEach(item => {
        this.columnAggregate.push({ field: item.fieldName, type: 'Sum', format: 'N0', footerTemplate: this.footerSum });
      });      
      if (this.countSelectedFooter.length > 0) {
        this.footerData.countSelected = '0/' + this.dataSource.length;
        this.countSelectedFooter.forEach(item => {
          this.columnAggregate.push({ field: item.fieldName, type: 'Custom', footerTemplate: this.footerCustomSelected, customAggregate: this.countSelectedAggregateFn });
        });
      }
      if (this.sumSelectedFooter.length > 0) {
        this.footerData.sumSelected = [];
        this.sumSelectedFooter.forEach(item => {
          let data = '0/' + this.calculateSum(this.dataSource, item.fieldName);           
          this.footerData.sumSelected.push({ key: item.fieldName, data: data });
        });
        this.footerData.sumSelected.forEach(item => {
          this.columnAggregate.push({ field: item.fieldName, type: 'Custom', footerTemplate: this.footerCustomSelected, customAggregate: () => (item.data)});
        })          
      }
      this.$refs.grid.ej2Instances.setProperties({
        aggregates: [{ columns: this.columnAggregate }]
