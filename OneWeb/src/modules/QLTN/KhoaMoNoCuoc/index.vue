...
export default {
  ...
  methods: {
      ...
      if (value) {
        ...
        if (lstChosen.length > 0) {
          ...
          lstChosen.forEach((element) => {
            if (element.NGAY_BD_TD!='' & moment(element.NGAY_BD_TD,'DD/MM/YYYY').toDate()>new Date())
            {
              soPhieuYeucau=soPhieuYeucau+1;
            }
          })
          ...
          if (soPhieuYeucau>0)
          {
            let isConfirm = false
                    await this.$bvModal
                      .msgBoxConfirm('Có '+soPhieuYeucau+' phiếu chưa đến ngày khách hàng yêu cầu thanh lý/tạm dừng. \n Bạn chắc chắn muốn thanh lý/tạm dừng dịch vụ của khách hàng', {
                        title: 'Thông báo',
                        centered: true,
                        size: 'md',
                        okTitle: 'Đồng ý',
    ...
    async onClickTraPhieu() {
      ...
      //this.msgKichHoat = this.msgThongBao[50];
      const lstChosen = this.$refs.tableThueBao.getSelectedRecords() || [];
      if (lstChosen.length == 0) {
        this.$toast.error("Bạn chưa chọn thuê bao để hoàn trả ");
        return;
      }
      //this.$bvModal.show("notice-kichhoat");
      let so_loi = 0;
      lstChosen.forEach((element) => {
        //   console.log(element);          
        if (element.STATUS != 5 && element.STATUS != 7)

          this.hoantra(element);
        else so_loi = so_loi + 1;

      });
      if (so_loi == 0)
        this.$toast.success("Hoàn trả thành công!");
      else
        this.$toast.success("Hoàn trả thành công! có " + so_loi + " thuê bao không hoàn trả do đã kích hoạt");
      this.getDsThueBao();
    },
    hoantra: async function (body) {
      const bd = {
        hDTBId: body.HDTB_ID,
        phieuId: body.PHIEU_ID
      }
      try {
        const { data } = await this.axios.post(`/web-thuno/api/thu-no/xu-ly-thue-bao/tra-phieu-lai-huong-giao-truoc`, bd);
        if (data && data.error == "200") {
          //TODO
        } else {
          this.$toast.error("Hoàn trả phiếu thất bại thuê bao : " + body.MA_TB);
        }
      } catch (error) {
        this.$toast.error("Hoàn trả phiếu thất bại thuê bao : " + body.MA_TB);
      }
