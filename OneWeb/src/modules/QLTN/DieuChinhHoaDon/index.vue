      ...
      try {
        ...
        if (response.data.error_code === "BSS-00000000") {
          if (response.data.data.hasOwnProperty("Invoice")) {
            ...
            if (result.NDHDon.NMua.MST)
              this.objHoaDonDienTu.MSTKH =result.NDHDon.NMua.MST
            else            
            if (result.NDHDon.NMua.TTKhac)
              if (result.NDHDon.NMua.TTKhac.length>1)
                this.objHoaDonDienTu.MSTKH = result.NDHDon.NMua.TTKhac[1].TTin.TTruong == 'CusTaxCode' ? result.NDHDon.NMua.TTKhac[1].TTin.DLieu : "";
              else
                this.objHoaDonDienTu.MSTKH =  "";
