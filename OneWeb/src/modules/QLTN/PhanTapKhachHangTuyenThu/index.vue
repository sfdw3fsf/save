...
methods : {    
    ...
    getDSTuyenThuDonVi : async function (donviid) {
         try {
                const rs = await this.$root.context.get(
                `/web-thuno/api/thu-no/phan-tap-doanh-nghiep/ds-tuyen-thu/don-vi/${donviid}`
                );
   ...
   kiemtra: function(){
    ...
    getDSDiaChiChuaGan:async function(khuvuc_id,loainvid,nhanvienid){
        ...
        try {
                const rs = await this.$root.context.get(
                `/web-thuno/api/thu-no/phan-tap-doanh-nghiep/ds-dia-chi-tuyen-thu?loai=1&nhanVienId=${nhanvienid}&khuVucId=${khuvuc_id}&loaiNVId=${loainvid}`
                );
                this.dsChuaGan = rs.data;   
    ...
    getDSDiaChiDaGan:async function(khuvuc_id,loainvid,nhanvienid){
        ...
        try {
                const rs = await this.$root.context.get(
                `/web-thuno/api/thu-no/phan-tap-doanh-nghiep/ds-dia-chi-tuyen-thu?loai=2&nhanVienId=${nhanvienid}&khuVucId=${khuvuc_id}&loaiNVId=${loainvid}`
                );
                this.dsDaGan = rs.data;   

            } catch (error) {}    
    },
    postDSDiaChiTuyenThu:async function(nhanvienid,loainvid,quanid,phuongid,phoid,apid,khuid,dacdiemid,khuvucid,kieu){
        let param={
        ...
        try {
                ...
                `/web-thuno/api/thu-no/phan-tap-doanh-nghiep/cap-nhat-tuyen-thu-phuong-xa`,param
                );   
          ...
          if (this.nodeID===2)
            ...
            if (this.nodeID===3)
              ...
              dsSelected_canChuyen.map((val) => {
                ...
                if (idx >= 0) {
                  this.postDSDiaChiTuyenThu(this.nhanVienID,this.loaiNVID,this.dsChuaGan[idx].QUAN_ID,this.dsChuaGan[idx].PHUONG_ID,this.dsChuaGan[idx].PHO_ID,this.dsChuaGan[idx].AP_ID,this.dsChuaGan[idx].KHU_ID,this.dsChuaGan[idx].DACDIEM_ID,this.dsChuaGan[idx].KHUVUC_ID,1);
                  arr.push(this.dsChuaGan[idx]);
      ...
      if (this.nodeID===2)
      {
        ...
        if (this.nodeID===3)
              ...
              dsSelected_canChuyen.map((val) => {
                ...
                if (idx >= 0) {
                  this.postDSDiaChiTuyenThu(this.nhanVienID,this.loaiNVID,this.dsDaGan[idx].QUAN_ID,this.dsDaGan[idx].PHUONG_ID,this.dsDaGan[idx].PHO_ID,this.dsChuaGan[idx].AP_ID,this.dsDaGan[idx].KHU_ID,this.dsDaGan[idx].DACDIEM_ID,this.dsDaGan[idx].KHUVUC_ID,2);
                  arr.push(this.dsDaGan[idx]);
    ...
    selectedItemsNhanVien: async function (data) {
      this.dsNhanVienSelected = data;      
      this.nhanVienID=data[0].NHANVIEN_ID;
      await this.load_danhsach(this.nodeID,this.nhanVienID,this.loaiNVID,this.khuVucID);
   ...
   showLeave(id){
    ...
    click_leaves :async function(id,text){
       ...
       if (this.loaiNVID!=0);
       this.getDSTuyenThuDonVi(this.donViID,this.loaiNVID);
...
load_danhsach:async function(nodeID,nhanvien,loainv,khuvuc){
      if (nodeID===1)
        {
        await  this.getDSNVLKHChuaGan(nhanvien,loainv);
        await this.getDSNVLKHDaGan(nhanvien,loainv);
        }
       if (nodeID===2)
        {
        await this.getDSNVNNChuaGan(nhanvien,loainv);
        await this.getDSNVNNDaGan(nhanvien,loainv);
        } 
        if (nodeID===3)
        {
            
        await this.getDSDiaChiChuaGan(khuvuc,loainv,nhanvien);
        await this.getDSDiaChiDaGan(khuvuc,loainv,nhanvien);
        }         
        if (nodeID===4)
        {
      
        await this.getDSDTDaGan(nhanvien,loainv);
        } 
        if (nodeID===5)
        {
       
        await this.getDSDTNDaGan(nhanvien,loainv);
        } 
...
watch: {    
    ...
    "loaiNVID":function(newval,newold){
      this.getDSTuyenThuDonVi(this.donViID,newval);
    }
},
}
</script>
