  ...
  methods: {
    getSplitters: function () {
      ...
      if (this.thongTinGianDo != null) {
        for (let i = 0; i < this.thongTinGianDo.length; i++) {
          ...
          if (gianDo.group == null && (gianDo.TT_SP1 != null || gianDo.TT_SP2 != null)) {
            let ck = false;
      ...
      if (this.thongTinGianDo != null) {
        for (let i = 0; i < this.thongTinGianDo.length; i++) {
          ...
          if(gianDo.SOI == null)
            gianDo.SOI = 0;
          if (gianDo.group == null && (!(gianDo.TT_SP1 != null || gianDo.TT_SP2 != null))) {
            let ck = rs.find(x => x.CAP_ID == gianDo.CAP_ID && x.NGUON == gianDo.NGUON && x.DICH == gianDo.DICH && x.DIACHI_DICH == gianDo.DIACHI_DICH);
            if(ck != null){
              if(ck.SOI < gianDo.SOI)
                ck.SOI = ck.SOI.toString() + "," + gianDo.SOI.toString();
              else 
                ck.SOI = gianDo.SOI.toString() + "," + ck.SOI.toString();
            }
            else {
              rs.push({
                CAP_ID: gianDo.CAP_ID,
                KYHIEU_CAP: gianDo.KYHIEU_CAP,
                SODOIDAY: gianDo.SODOIDAY,
                SOI: gianDo.SOI.toString(),
                NGUON: gianDo.NGUON,
                DICH: gianDo.DICH,
                DIACHI_DICH: gianDo.DIACHI_DICH,
                CHIEUDAI: gianDo.CHIEUDAI
              });
            }
          }
        }
    ...
    getGroups: function () {
      ...
      if (this.thongTinGianDo != null) {
        for (let i = 0; i < this.thongTinGianDo.length; i++) {
          const gianDo = this.thongTinGianDo[i];
          if (gianDo.group != null) {
            if (rs.findIndex(x => x.groupName == gianDo.group) == -1) {
              rs.push(
                {
                  groupName: gianDo.group,
                  groupHTML: this.getGroupHtml(gianDo.group)
                }
              );
            }
          }
    ...
    getGroupHtml: function (value) {
      if (value != null) return "<td colspan='8'><span class='text-bold'>" + value + "</span></td>";
      return "";
    },
    getDsCapByGroup: function (value) {
      let rs = [];
      if (this.thongTinGianDo != null) {
        let ds = this.thongTinGianDo.filter(x => x.group == value);
        ds = ds.sort((a, b) => (a.SOI < b.SOI ? -1 : 1));
    ...
    getTieuDe: function (args) {
      let rs = "THÔNG TIN CỔNG/CÁP/ĐỊA BÀN";
      if(args.DICHVUVT_ID == 9)
        rs += " ĐIỂM ĐẦU";
      return rs;
    },
    getThueBaoCungCap: function (args) {
      let dsTbCungCap = "(Không có)";
