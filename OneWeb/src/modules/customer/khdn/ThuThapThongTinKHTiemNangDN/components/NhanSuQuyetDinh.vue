        ...
        return {
            ...
            grid_danhsach: {
                ...
                columns:[
                    ...
                    { fieldName: 'officerName', headerText: 'Họ tên(*)', textAlign: 'center', allowFiltering: true, width: 200, allowEditing: false , template: this.getInputTemplate(this, 'officerName', 'gridDanhSach')},
    ...
    methods: {
        ...
        handleRemoveRow(args) {
                this.$confirm(`Bạn có chắc chắn muốn xóa dòng này ?`, {
                    ...
                    this.grid_danhsach.dataSource = this.grid_danhsach.dataSource.filter((item) => item.order !== args.order).map((item, index) => {
                        item.order = index + 1;
                        return item;
                    });
                });
