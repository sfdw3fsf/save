<template>
  <div>
    ...
    <div class="list-actions-top">
      <ul class="list">
        <li>
          <a href="javascript:;" @click="GetListMessageQueue">
    ...
    <div class="page-content">      
      ...
      <div class="box-form">
        <div class="legend-title">Danh sách request từ BSS</div>
        <div class="row"> 
          <DataGrid :columns="grdBNMRequests.header"
            :dataSource="grdBNMRequests.list" :enable-paging-server="false" :allowPaging="true"
            :showFilter="true" :showColumnCheckbox="false" :enabledSelectFirstRow="true" ref="grdBNMRequests" panelDataHeight="300px"
...
export default {
  ...
  data: () => ({
    header: {
      ...
      list: [
        {
          name: 'Danh sách Yêu cầu tạo Script từ BSS',
          link: { 
            name: 'Ui.buttons',
            active: true 
          }
    ...
    dtpTuNgay: {text: 'Từ ngày', value: moment(new Date()).format('DD/MM/YYYY'), Enabled: true},
    dtpDenNgay: {text: 'Đến ngày', value: moment(new Date()).format('DD/MM/YYYY'), Enabled: true},
  ...
  methods: {
    formLoad: async function () {
      ...
      // this.cboHangSX.value = 8
      // this.cboDichVu.value = 59
      // this.cboLoaiCH.value = 1
      // this.dtpTuNgay.value = "08/12/2023"
      // this.dtpDenNgay.value = "08/12/2024"
      await this.GetListMessageQueue()
    ...
    getDSTinh: async function () {      
      try {
        ...
        this.cboTinh.Items.forEach(item => {
          if (item.code === matinh_ccos.matinh_ccos){
    ...
    chuyenDoiNgay : function(ngay) {
      var parts = ngay.split('/');
      var ngay = parts[0];
      var thang = parts[1];
      var nam = parts[2];
      var ngayMoi = ngay + '-' + thang + '-' + nam;
      return ngayMoi;
    },

    GetListMessageQueue: async function () {
     // await this.testapi()
      try {
        ...
        var input = {
          ...
          tungay :this.chuyenDoiNgay(this.dtpTuNgay.value),
          denngay: this.chuyenDoiNgay(this.dtpDenNgay.value),
        };
        let res = await api.GetListMessageQueue(this.axios, input);
        if (res.data.errorCode != 0) {
          this.$toast.error(res.data.faultString)
        }
        if (res.data.data && res.data.data.length > 0) {
          var List_BSS=res.data.data
          List_BSS.forEach(item => {
            let cauhinh = this.cboLoaiCH.Items.find(item => (item.id === this.cboLoaiCH.value ? item.config_name : ""))
            item.cauhinh =  cauhinh.config_name
            item.created_date = this.convertDate(item.created_date)
            item.trangthai = (item.status === 1) ? "OK" : "NOT-OK"
          })

          this.grdBNMRequests.list =  List_BSS
        }
      } catch (ex) {
        this.ShowError("Lỗi: " + (ex?.data?.faultString || ex));
      }
    },
    convertDate: function (isoDateString) {
    ...
    testapi : async function () {
      ...
      var input = {
        ...
        let res = await api.getDetailLogScript(this.axios, input);
        if (res.data.errorCode != 0) {
          this.$toast.error(res.data.faultString)
        }
        if (res.data.data && res.data.data.length > 0) {
    ...
    btnChitiet_click: function (item) {
      ...
      this.popupComponent = () =>
        import(
          "@/modules/admin/bnm/BNMRequestOneBss/popup/BNMRequestBssDetail.vue"
        );
