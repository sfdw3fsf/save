  ...
  <div>
    ...
    <div class="list-actions-top">
      <ul class="list">
        ...
        <li>
          <a href="javascript:;" v-if="isShowForm == true && isEdit == false" @click="btnEdit_click" >
            <span class="icon one-edit"></span> Sửa
          </a>
        </li>
        <li>
          <a href="javascript:;" v-if="isShowForm == true" @click="btnTest_click">
            <span class="icon one-file-check"></span> Sinh Script
          </a>
        </li>
        <li>
          <a href="javascript:;" v-if="isShowForm == false" @click="btnthunghiem_click">
        ...
        <div class="row" v-if="isShowForm == true">
          <div class="col-sm-4 col-12" style="vertical-align: baseline">
            <div class="box-form">
                      ...
                      <li v-for="(item, index) in gridParamBSS.list.map(key =>key.param_name )">
                        <div class="row">
                          ...
                          <div class="col-sm-6 col-12">
                            <div class="info-row">
                              <div class="key w100">
                                <input type="text" v-model="ValuesParam[item]" ></input>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <br>
              <div v-if="cboHangSX.value == 8 ? true : false">
            ...
            <div class="box-form">
              ...
              <div class="info-row">
                <div class="value" >
                  ...
                  <textarea name="scriptTemp" class="form-control" style="height: 1200px;" v-bind:value="scriptTemp"
                    @change="e => scriptTemp = e.target.value" :disabled="isEdit == false"></textarea>
                </div>                  
              </div>
            </div>          
          </div>      
...
export default {
  ...
  data: () => ({
    ...
    cboHangSX: {text: 'Hãng SX', value: '', Items: [], isLocked:false},
    cboDichVu: {text: 'Dịch vụ', value: '', Items: [],isLocked:false},
    cboLoaiCH: {text: 'Loại cấu hình', value: '', Items: [],isLocked:false},
  ...
  methods: {
    formLoad: async function () {
      ...
      if (this.dulieu) {
        ...
        this.scriptTemp = tmp
        this.cboHangSX.isLocked=true
        this.cboDichVu.isLocked=true
        this.cboLoaiCH.isLocked=true
      }
    ...
    btnEdit_click:function () {
      ...
      this.scriptTemp_goc=this.scriptTemp
    },
    ...
    btncancel_click:function (){
      this.isEdit = !this.isEdit
      this.scriptTemp = this.scriptTemp_goc
    ...
    sp_update_profile_bnm: async function(){
    let input = {
      vftth_bandwidth_profile: "",
      vftth_shaper_profile: "",
      vhangsx_id: this.cboHangSX.value.length > 0 ? Number(this.cboHangSX.value) : "",
      vims_bandwidth_profile: "",
      vims_shaper_profile: "",
      vmytv_bandwidth_profile: "",
      vmytv_shaper_profile: "",
      vpass_telnet: "",
      vphanvung_id: this.phanvung_id.length > 0 ? Number(this.phanvung_id) : "",
      vtcon_ims: "",
      vtcon_profile: "",
      vtr069_gem_profile: "",
      vtype: "R",
    ...
    var response = await api.sp_update_profile_bnm(this.axios, input)
        if (response && response.data && response.data.error_code && response.data.error_code == "BSS-00000000"
        ){
          this.profile_bnm=response.data.data[0]
          return true
        }
        else {
          this.$toast.error(" Lỗi lấy profile_bnm")
          return false
      }
  },
  },
