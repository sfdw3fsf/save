...
export default {
  ...
  watch: {
    storeInfo: async function () {   
      ...
      this.doitac_id = this.storeInfo.doitac_id
    },
    getSelectedStore: async function (newValue, oldeValue) {      
      ...
      this.doitac_id = newValue.doitac_id
  ...
  methods: {
    ...
    loadDSDoiTac: async function () {
      ...
      try {
        const resgetQLVT_DOITAC = await api.getQLVT_DOITAC(axios)
        
        if (resgetQLVT_DOITAC.data && resgetQLVT_DOITAC.data.data && resgetQLVT_DOITAC.data.data.length > 0) {
          this.dsDoitac = resgetQLVT_DOITAC.data.data
        }
      } catch (error) {
        console.log(error.toString())
        this.$toast.error(error.toString())
