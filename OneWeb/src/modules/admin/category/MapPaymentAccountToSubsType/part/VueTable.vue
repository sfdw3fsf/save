      ...
      <table class="table-result table-gachle">
        ...
        <tbody>
            <tr v-if="!options.length && !loading">
              <td class="text-center" colspan="100%">Không có bản ghi nào hiển thị</td>
            </tr>
          <template v-if="!loading">
            ...
            <tr
              ...
              :class="compare(option, choose) ? 'highlight' : ''"
            >
...
export default {
  ...
  computed: {
    ...
    current_value() {
      if (this.multiple) return this.multiple_value;
      else return this.choose;
    },
  },
  mounted() {
    if (this.multiple) this.multiple_value = this.value ? this.value : [];
    else this.chooseItem(this.value);
    ...
    checkAllItem(event) {
      if (event.target.checked === true) {
        this.options.forEach((item) => {
          ...
          if (index < 0) this.multiple_value.push(item);
        });
      } else {
        this.multiple_value = [];
      }
