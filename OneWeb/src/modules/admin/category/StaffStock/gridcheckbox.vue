  ...
  <table class="table-result nowrap table-gachle" :id="id">
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td> <span>{{ d.ten_kho }}</span></td>
              <td> <span>{{ d.ten_dv }}</span></td>
              <td class="selected">  
                 <div class="check-action">
                      <input type="checkbox" class="check" v-model="d.quyen_kg" @change="someHandler(d)">
                      <span class="name"></span>
                  </div>
              </td>
              <td class="selected">
                 <div class="check-action">
                      <input type="checkbox" class="check" v-model="d.quyen_kn" @change="someHandler(d)">
                      <span class="name"></span>
                  </div>
              </td>
...
export default {
  ...
  methods: {
    ...
    someHandler(data){
      ...
      this.list_change.push(data);
       console.log(this.list_change);
        this.$emit('prop_update',this.list_change);
    },
  }
}
</script> 