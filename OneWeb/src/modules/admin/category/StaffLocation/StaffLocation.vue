                    ...
                    <div class="box-move-select-table">
                        ...
                        <div class="box-col box-form">
                            ...
                            <div class="info-row">
                                ...
                                <div class="value">
                                    <div class="select-custom">
                                        <select @change="changeLoaiNV" v-model="loai_nv_selected" class="form-control">
                                            <option v-for="item in danhsach_loai_nv" :key="item.loainv_id" :value="item.loainv_id">{{item.ten}}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="info-row">
                                ...
                                <div class="value">
                                    <!-- <div class="select-custom">
                                        <select @change="changeDV" v-model="donvi_selected" class="form-control">
                                            <option v-for="item in danhsach_donvi" :key="item.donvi_id" :value="item.donvi_id">{{item.ten_dv}}</option>
                                        </select>
                                    </div> -->
                                    <ejs-dropdownlist 
                                        :allowFiltering="true"
                            ...
                            <div class="info-row">
                                ...
                                <div class="value">
                                    <div class="select-custom">
                                        <select v-model="nhiemvu_selected" class="form-control">
                                            <option v-for="item in danhsach_nhiemvu" :key="item.id" :value="item.id">{{item.name}}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
...
import CapNhatNhanVienPopup from './CapNhatNhanVienPopup.vue'
import DSCacKhuVucPopup from './DSCacKhuVucPopup.vue'
...
export default {
    ...
    methods:{
        ...
        async lichSuCapNhat(){
            try{
                this.loading(true)
                let response=await StaffLocationAPI.lay_ds_thongtin_log_khuvuc(this.axios, this.khuvuc_id_selected)
                this.loading(false)
        ...
        async changeDV(){
            if(this.isFirstChangeDonvi){
                this.isFirstChangeDonvi=false
                return
            }
            await this.sp_hienthi_nhanvien_theo_kv(1)
            await this.sp_hienthi_nhanvien_theo_kv(2)
        ...
        async sp_lay_ds_thamso_md_mats(){
            try{
                let response=await StaffLocationAPI.sp_lay_ds_thamso_md_mats(this.axios)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
        ...
        async sp_lay_loainv_nhomlv(){
            try{
                ...
                let response=await StaffLocationAPI.sp_lay_loainv_nhomlv(this.axios)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    //sort client
                    this.danhsach_loai_nv=response.data.data.sort((a, b)=>a.ten.localeCompare(b.ten))

                    if(this.danhsach_loai_nv.length>0){
                        //default 52: Nhân viên quản lý kinh doanh
                        const index =this.danhsach_loai_nv.findIndex(x=>x.loainv_id==52)
                        if(index>-1){
        ...
        async sp_lay_ds_loai_kv(){
            try{
                ...
                let response=await StaffLocationAPI.sp_lay_ds_loai_kv(this.axios)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
        ...
        async sp_lay_ds_donvi_theo_nhomlnv(){
            try{
                this.danhsach_donvi=[]
                let response=await StaffLocationAPI.sp_lay_ds_donvi_theo_nhomlnv(this.axios)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    this.danhsach_donvi=response.data.data.map(x=>{
                        return {
                            donvi_id:x.donvi_id,
                            ten_dv:x.donvi_id!=-1?(x.ma_dv+" - "+x.ten_dv):x.ten_dv
                        }
                    })
        ...
        async sp_lay_ds_khuvuc_theo_loaikv(data){
            try{
                ...
                let response=await StaffLocationAPI.sp_lay_ds_khuvuc_theo_loaikv(this.axios, data)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                   
                    //this.dataSourcesKhuVuc=response.data.data
                    response.data.data.forEach((item, index)=>{
                        this.dataSourcesKhuVuc.push(Object.assign(item, {
                            stt:index+1
                        }))
                    })
                    if(this.dataSourcesKhuVuc.length>0){
        ...
        async sp_hienthi_nhanvien_theo_kv(loai){
            try{
                ...
                this.loading(true)
                let response=await StaffLocationAPI.sp_hienthi_nhanvien_theo_kv(this.axios, data)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    if(loai==1){
        ...
        async sp_ghilai_gan_nv_khuvuc(data){
            try{
                this.loading(true)
                let response=await StaffLocationAPI.sp_ghilai_gan_nv_khuvuc(this.axios, data)
                this.loading(false)
