 ...
 * @author: IT3.Web2
 */
import { Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate, ContextMenu, DetailRow } from '@syncfusion/ej2-vue-grids'
...
export default {
  ...
  provide: {
    grid: [Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate, ContextMenu, DetailRow]
  },
  props: {
    childMapping: {
      type: String,
  ...
  methods: {
    ...
    grid_ActionComplete: function(args) {
      if (args.action == 'filter' || args.action == 'clearFilter') {
        ...
        if (args.action == 'filter') this.filteredRecords = this.grid.getFilteredRecords().length
    ...
    grid_headerCellInfo: function(args) {
