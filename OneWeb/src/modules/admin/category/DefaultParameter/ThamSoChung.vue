...
export default {
    ...
    methods:{
        async initDuLieu(){
            await this.lay_ds_thamso_macdinh(0)
        },
        async clearDuLieu(){
            this.dataSources=[]
        },
        ...
        layDulieuCapNhat(){
            let changes = this.$refs.gridThamChung.getBatchChanges().changedRecords
            if(changes.length>0){
        ...
        async sp_ct_tsmd_update(data){
            try{
                ...
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000') {
                    this.$toast.success('Cập nhật tham số chung thành công!')
                    await this.lay_ds_thamso_macdinh(0)
                }else {
                    if(response && response.data &&response.data.message){
                        this.$toast.error(response.data.message)
                    }else{
                        this.$toast.error('Đã xảy ra lỗi, cập nhật tham số chung không thành công!')
                    }
                }
            }catch(error){
                this.loading(false)
