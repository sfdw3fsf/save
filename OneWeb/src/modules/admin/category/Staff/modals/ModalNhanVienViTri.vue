<template>
  <ejs-dialog
    ...
    :visible="false"
    :isModal="true"
    ref="modalNhanVienViTri"
    :header='"Nhân viên - Vị trí"'
    showCloseIcon=true
    :target="'.main-wrapper'"
    width='60%'
    @open="openPopupNhanVienViTri"
    height="800">
  ...
  methods: {
    ...
    updateGanViTriNhanVien () {
      const param = {
        ...
        js_data: this.danhSachViTriDaGan.map(item => ({
          NHANVIEN_ID: this.nhanVienId,
          VITRI_ID: item.vitri_id
        })) || []
      }
      this.loading(true)
      updateGanViTriNhanVien(this.axios, param).then(() => {
