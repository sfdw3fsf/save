      ...
      <table class="table-result table-gachle">
        ...
        <tbody>
          ...
          <tr v-bind:key="(option && option[unique]) ? (option[unique] + page) : ('col4' + j + page)" v-for="(option, j) in options_perpage" :class="compare(option, choose) ? 'highlight' : ''">
            ...
            <td :class="item.align ? ('text-' + item.align) : ''" :style="item.color ? ('color: ' + item.color) : ''" @click="chooseItem(option)" v-bind:key="'col3' + i" v-for="(item, i) in config">
             <!-- {{item.format? item.format(option[item.key]): ''}} -->
                        {{ option ? 
                        (item.format ? item.format(option[item.key]): option[item.key]): ''}}
