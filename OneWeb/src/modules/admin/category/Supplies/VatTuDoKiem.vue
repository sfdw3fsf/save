...
export default {
    ...
    methods:{
        async initDuLieu(){
            ...
            this.dataSourcesChuaGan=await this.lay_ds_vattu_dokiem(0)
            this.dataSourcesGan=await this.lay_ds_vattu_dokiem(1)
        },
        async clearData(){
            this.dataSourcesGan=[]
            this.dataSourcesChuaGan=[]
        ...
        async boGan(){
            ...
            let result=await this.capnhat_vattu_dokiem_giaodien(data)
            if(result=='1'){
        ...
        async lay_ds_vattu_dokiem(kieu){
            try{
                this.loading(true)
                let response = await SuppliesAPI.lay_ds_vattu_dokiem(this.axios, kieu)
                this.loading(false)
        ...
        async capnhat_vattu_dokiem_giaodien(data){
            try{
                this.loading(true)
                let response = await SuppliesAPI.capnhat_vattu_dokiem_giaodien(this.axios, data)
                this.loading(false)
                if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data
                }else{
