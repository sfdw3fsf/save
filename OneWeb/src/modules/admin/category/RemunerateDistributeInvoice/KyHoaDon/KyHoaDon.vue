...
export default {
  ...
  computed: {
    ds_dagan: {
      get() {
        this.get_khd_dagan();
        return this.ds_khd_dagan;
      },
      set(value) {
        return (this.ds_khd_dagan = value);
      },
    },
    ds_chuagan: {
      get() {
        this.get_khd_chuagan();
        return this.ds_khd_chuagan;
      },
      set(value) {
        return (this.ds_khd_chuagan = value);
      },
    },
    cp_selected_dagan: {
      get() {
  ...
  methods: {
    get_khd_dagan: async function () {
      var input = {
        thulao_ck_id: this.thulao_ck_id,
        type: 1,
      };
      let data = this.GetData(await api.ds_kyhoadon(this.axios, input));
      this.ds_khd_dagan.list = data;
      this.$emit("assignedItemsChanged", this.getSelectedValues(data));
    },
    get_khd_chuagan: async function () {
      var input = {
        thulao_ck_id: this.thulao_ck_id,
        type: 0,
      };
      let data = this.GetData(await api.ds_kyhoadon(this.axios, input));
      this.ds_khd_chuagan.list = data;
    },
    ...
    selectRight: function () {
      this.Loading = true;
      var dagan = this.ds_khd_dagan.list;
      var chuagan = this.ds_khd_chuagan.list;
      ...
      dagan = dagan
        ...
        .sort((a, b) => parseFloat(b.id) - parseFloat(a.id));
      this.ds_khd_dagan.list = dagan;
      this.ds_khd_chuagan.list = chuagan;
    ...
    getSelectedValues: function (selectedRecords) {
      ...
      if (selectedRecords != null && selectedRecords.length > 0) {
        for (let i = 0; i < selectedRecords.length; i++) {
          const item = { CHUKYNO: selectedRecords[i].id.toString() };
          result.push(item);
        }
      }
      return result;
