...
export default {
  ...
  computed: {
    ds_dagan: {
      get() {
        this.get_khuvuc_dagan();
        return this.ds_khuvuc_dagan;
      },
      set(value) {
        return (this.ds_khuvuc_dagan = value);
      },
    },
    ds_chuagan: {
      get() {
        this.get_khuvuc_chuagan();
        return this.ds_khuvuc_chuagan;
      },
      set(value) {
        return (this.ds_khuvuc_chuagan = value);
      },
    },
    cp_selected_dagan: {
      get() {
    ...
    cp_selected_chuagan: {
      ...
      set(value) {
        value.length > 0 ? (this.btnLeft = false) : (this.btnLeft = true);
        return (this.selected_chuagan = value);
      },
    },
  },
  methods: {
    get_khuvuc_dagan: async function () {
      var input = {
        thulao_ck_id: this.thulao_ck_id,
        type: 1,
      };
      let data = this.GetData(await api.ds_khuvuc(this.axios, input));
      this.ds_khuvuc_dagan.list = data;
      this.$emit("assignedItemsChanged", this.getSelectedValues(data));
    },
    get_khuvuc_chuagan: async function () {
      var input = {
        thulao_ck_id: this.thulao_ck_id,
        type: 0,
      };
      let data = this.GetData(await api.ds_khuvuc(this.axios, input));
      this.ds_khuvuc_chuagan.list = data;
    },
    ...
    selectRight: function () {
      this.Loading = true;
      var dagan = this.ds_khuvuc_dagan.list;
      var chuagan = this.ds_khuvuc_chuagan.list;
      ...
      dagan = dagan
        ...
        .sort((a, b) => parseFloat(b.id) - parseFloat(a.id));
      this.ds_khuvuc_dagan.list = dagan;
      this.ds_khuvuc_chuagan.list = chuagan;
    ...
    getSelectedValues: function (selectedRecords) {
      ...
      if (selectedRecords != null && selectedRecords.length > 0) {
        for (let i = 0; i < selectedRecords.length; i++) {
          const item = { KHUVUC_ID: selectedRecords[i].khuvuc_id };
          result.push(item);
        }
      }
      return result;
