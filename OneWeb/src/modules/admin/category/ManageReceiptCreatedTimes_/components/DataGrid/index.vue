  ...
  */
      ...
      if (this.columns != null) {
        for (let i = 0; i < this.columns.length; i++) {
          let column = {
            ...
            columns:
              this.columns[i].columns == undefined ? null : this.columns[i].columns,
      ...
      if (!(this.commandColumn == undefined || this.commandColumn == null)) {
        if (this.commandColumn.show == true) {
          ...
          if (cmds.length > 0) {
            ...
            if (this.commandColumn.firstColumn == true) allColumns.splice(0, 0, column);
  ...
  methods: {
    getGridColumnType: function (column) {
      ...
      */
      if (
        (column.template == undefined || column.template == null) &&
    ...
    getColumnCommands: function () {
      ...
      if (!(this.commands == null || this.commands.length == 0)) {
        for (let i = 0; i < this.commands.length; i++) {
          ...
          if (
            !(this.commands[i].cssClass == undefined || this.commands[i].cssClass == "")
          )
    ...
    grid_RowSelected: function (args) {
      // this.bus.$emit("row-selected", true);
    ...
    grid_RowDeselected: function (args) {
      // this.bus.$emit("row-selected", false);
      let records = this.getSelectedRecords();
          ...
          for (let i = 0; i < rows.length; i++) {
            ...
            for (let j = 0; j < gridColumns.length; j++) {
              ...
              if (
                ...
                let cellItem = $(cell);
                if (
                  !(cellItem == undefined || cellItem == null || cellItem.length == 0)
                ) {
