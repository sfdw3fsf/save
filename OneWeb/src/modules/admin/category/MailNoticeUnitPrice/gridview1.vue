  ...
  <table class="table-result nowrap table-gachle" :id="id">
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td :class="{ 'highlight1' : idx==(currentrecord%recordperpage),'nowrap w30':hdx==headerList.length-1,'stickylastcol':(hdx==headerList.length-1) && flcol }" v-for="(h,hdx) in headerList" :key="hdx">
...
<div class="tc-pagination">
  ...
  <div class="dropdown inline">
      <div class="fw6 input-number" data-toggle="dropdown">
          <span class="text-main">{{  totalrecord>0? currentrecord +1 : currentrecord }}</span>/{{ totalrecord }}
      </div>
