  ...
  methods: {
    ...
    grid_DataBound: function (args) {
      ...
      if (!this.flagBeginFilter) {
        console.log('!this.flagBeginFilter');
        if (this.defaultColumnCheckboxChecked == true) {
          console.log('this.defaultColumnCheckboxChecked');
          this.flagSelectedRowIndexes = [];
          ...
          if(rows.length){
            for (let i = 0; i < rows.length; i++) {
              ...
              // console.log('row: ',row);
              // customize to be comfortable with require 
              if(row.rowData['trangthai'] == "1"){
                this.flagSelectedRowIndexes.push(row.rowIndex);
              }
            }
          }
          console.log('this.flagSelectedRowIndexes: ', this.flagSelectedRowIndexes);
        }
