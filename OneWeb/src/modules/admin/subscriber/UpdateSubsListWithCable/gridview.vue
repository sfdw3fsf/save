  ...
  <table class="table-result nowrap table-gachle" :id="id">
      <thead class="center">
          <tr>
              ...
              <th v-for="(h,hdx) in headerList" :key="hdx" :class="{'w30':hdx==headerList.length-1,'':(hdx==headerList.length-1) && flcol}">
                {{ h.text }}
              </th>
          </tr>
          <tr>
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td :class="{ 'highlight1' : idx==(currentrecord%recordperpage),'nowrap w30 center':hdx==headerList.length-1,
              'stickylastcol':(hdx==headerList.length-1) && flcol }" v-for="(h,hdx) in headerList" :key="hdx">
