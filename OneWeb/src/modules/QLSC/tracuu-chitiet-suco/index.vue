...
export default {
    ...
    data() {
        return {
            ...
            idSuCoDuocChon: null,
            tabDuocChon: 1,
            allowRefreshGridNhanVien: false,
            allowRefreshGridBaoTon: true,
            dsSuCo: [],
            dsTienTrinh: [],
            dsNhanVienXuLy: [],
            dsBaoTon: [],
    ...
    watch: {
        ...
        tabDuocChon(newVal) {
            if (newVal == 2 && this.allowRefreshGridBaoTon) {
                this.allowRefreshGridBaoTon = false
                setTimeout(() => {
                    this.$refs['grid-baoton'].grid.refresh()
                }, 200)
            }

            if (newVal == 1 && this.allowRefreshGridNhanVien) {
                this.allowRefreshGridNhanVien = false
                setTimeout(() => {
                    this.$refs['grid-nhanvien-xuly'].grid.refresh()
                }, 200)
            }
        }
    },
    ...
    methods: {
        onActionCompleteDsNhanVien(args) {
            if (args.requestType == "refresh" && this.tabDuocChon != 1) {
                this.allowRefreshGridNhanVien = true
            }
        },
        onActionCompleteDsBaoTon(args) {
            if (args.requestType == "refresh" && this.tabDuocChon != 2) {
                this.allowRefreshGridBaoTon = true
            }
        },
        onRowSelectedTienTrinh(data) {
        ...
        async onSelectRowSuCo(rowData) {
            ...
            this.idSuCoDuocChon = rowData.SUCO_ID
            try {
                this.loading(true)
                let loading1 = this.layDsTienTrinh(this.idSuCoDuocChon)
                let loading2 = this.layDsCongBiAnhHuong(this.idSuCoDuocChon)
                await loading1
                await loading2
            } catch (err) {

            } finally {
                if (this.dsTienTrinh.length == 0) {
                    this.loading(false)
                }
            }
        },
        ...
        onClickBtnExcelThongTinTienTrinh() {
            ...
            if ([1, 2, 3].indexOf(this.tabDuocChon) < 0) {
                return
            }
            ...
            if (this.tabDuocChon == 1) {
                ...
                fileName = 'ds_cong_anh_huong'

                this.dsCongBiAnhHuong.forEach(item => {
                    exportData.push({
                        'Thiết bị': item.TEN_TBI,
                        'Cổng': item.CONG
                    })
                })  
            }
        ...
        async onClickBtnLayThongTin() {
            ...
            let data = {
                donViId: 0,
                maSuCo: !this.maSuCo ? '' : this.maSuCo.trim(),
            ...
            try {
                ...
                if (response.error_code === 'BSS-00000000' && response.data != null) {
                    response.data.forEach(item => {
                        item.NGAY_BT = this.ddmmyyyy(item.NGAY_BT)
