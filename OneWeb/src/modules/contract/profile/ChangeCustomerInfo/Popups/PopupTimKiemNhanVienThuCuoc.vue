...
import API from '@/modules/QLTN/api/ChuyenPhieuXLNLacHuongAPI.js'//'../../api/ChuyenPhieuXLNLacHuongAPI.js'
...
export default {
    ...
    methods: {
        ...
        dialogOpen() {
            ...
            this.params.quan_Selected = this.input.quan_Id ? this.input.quan_Id : (this.input.QUAN_ID?this.input.QUAN_ID:this.params.quan_Selected);
            this.params.phuong_Checked = false;
            this.params.phuong_Selected = this.input.phuong_Id ? this.input.phuong_Id : (this.input.PHUONG_ID?this.input.PHUONG_ID:this.params.phuong_Selected);
            this.params.pho_Checked = false;
            this.params.pho_Selected = this.input.pho_Id ? this.input.pho_Id : (this.input.PHO_ID?this.input.PHO_ID:this.params.pho_Selected);
            this.params.ap_Checked = false;
            this.params.ap_Selected = this.input.ap_Id ? this.input.ap_Id : (this.input.AP_ID?this.input.AP_ID:this.params.ap_Selected);
            this.params.khu_Checked = false;
            this.params.khu_Selected = this.input.khu_Id ? this.input.khu_Id : (this.input.KHU_ID?this.input.KHU_ID:this.params.khu_Selected);
            this.params.dacDiem_Checked = false;
            this.params.dacDiem_Selected = this.input.dacDiem_Id ? this.input.dacDiem_Id : (this.input.DACDIEM_ID?this.input.DACDIEM_ID:this.params.dacDiem_Selected);
            ...
            this.params.tenNVTC = '';
            this.params.soDT = '';
            if (this.input.donViQL_Id || this.dvql) {
                let dvql = this.input.donViQL_Id?this.input.donViQL_Id:this.dvql;
                let idx = this.params.donViQL_Options.findIndex(e => e.id === Number(dvql))
                if (idx !== -1)
                    this.params.donViQL_Selected = dvql;
                else
                    this.$toast.error(`Không lấy được đơn vị quản lý: donviql_id = ${dvql}`);
            } else
