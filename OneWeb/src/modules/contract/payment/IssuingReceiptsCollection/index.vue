<template src="./IssuingReceiptsCollection.html"></template>
<style type="text/css">
 a:hover {
  cursor:pointer;
 }
...
export default {
  ...
  methods:{
    ...
    async loadForm(){
          ...
          try
            {
                if (this.ds_Gan.length > 0)
                {
                    ...
                    this.$toast.error("Bạn chưa chọn hóa đơn để phát hành");
                }
            }
            catch (ex)
            {
     ...
     async onChangeLuotTao(){
         try {
            let change = await this.lay_dsBienLai(this.luottao_id);
            } catch (err) {
            this.$toast.error("Load dữ liệu không thành công");
            }
  ...
  created(){
        ...
        // }
        this.tag = 1;
        console.log("params: " + this.$route.params.tag);
        console.log("query: " + this.$route.query.tag);
