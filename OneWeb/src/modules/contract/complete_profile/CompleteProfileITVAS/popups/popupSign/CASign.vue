  ...
  >
    ...
    <div class="" v-if='!is_loading'>
      <div class="info-row">
        ...
        <div class="value">
<!--          <ejs-combobox ref='cboChonCTSA' :popupHeight='300' @change='this.cboChonCTS_changed' :allowFiltering='true'-->
<!--                        :fields='fields_cts' v-model="selected_serial" :dataSource='this.ds_cts'></ejs-combobox>-->
          <select2 v-model="selected_serial"
                   :options="ds_cts.map(ls=>({id:ls.serialnumber,text:ls.subjectdn}))"
                   @change="cboChonCTS_changed"></select2>
        </div>
      </div>
      <div class="info-row">
    ...
    async show (dulieu_ky, kieuky, return_func, lydo) {
     // $this.loading(true)
      console.log('Test')
			let return_value2 = ""
			this.reset_form()
      this.$bvModal.show('catoken_sign')
         ...
         /* while(!this.ktraloi_form())
          ...
          if(!this.matkhau)
          {
            ...
            return;
					...
					if (kieuky == "hash") {
						event.preventDefault();
            this.loading_text = 'Đang ký số giao dịch'
            this.is_loading = true
            this.disable_button_huy = true
            this.disable_button_kyso = true
