...
export default {
  ...
  data: () => {
    return {
      ...
      requestFormInfo: {
        ...
        ngayYCTL: moment(new Date()).format('DD/MM/YYYY'),
        cbxNgayHT: true,
        cbxNgayTL: true,
        cbxNgayHTEnable: true,
        cbxNgayTLEnable: true,
        cuocNong: '0'
      },
  ...
  mounted () {
    ...
    this.$refs.dtpNgayTL.disabled = true
  },
  destroyed () {
  ...
  methods: {
    ...
    clear () {
      ...
      this.requestFormInfo.cbxNgayTL = false
    ...
    chkNgayTL_CheckedChanged() {
      this.$refs.dtpNgayTL.disabled = !this.$refs.chkNgayTL.checked
    },
    nhapMoi (e) {
      if (e) {
        e.preventDefault()
    ...
    async btnGhiLaiClick () {
      ...
      if(!this.$refs.chkNgayTL.checked){
        this.$toast.error('Bạn phải nhập ngày thanh lý')
        return
      }
      const selectedDate = moment(this.requestFormInfo.ngayYCTL, 'DD/MM/YYYY').format('YYYY-MM-DD');
      const today = moment().format('YYYY-MM-DD');
      if (selectedDate < today) {
        this.$toast.error('Ngày thanh lý không được nhỏ hơn ngày hiện tại.');
        return;
      }
      ...
      const dataset = await api.layhuonggiao_tiepnhan_v2(LOAI_HOPDONG.CHAMDUT_SD, this.loaitb_id, this.tocdo_id, muocCuocId, parseInt(this.requestFormInfo.kieuYeuCau), parseInt(this.loaikenh_id), parseInt(this.requestFormInfo.dichVu))
      let dset = dataset[0]
      console.log(99999)
      console.log(dset)
      // let input = {
    ...
    async taoDuLieu (themmoi, themTB) {
      ...
      this.dsThemTB_chamducSD = [{
        ...
        'CHKNGAYTL': this.requestFormInfo.cbxNgayTL ? 1 : 0,
        ...
        'NGAY_YCTL': this.requestFormInfo.ngayYCTL,
    ...
    async taoDuLieuHDTB (themmoi) {
      ...
      this.dsHDTB[0] = {
        ...
        LOAITB_ID: this.loaitb_id,
        NGAY_BD_TD: this.requestFormInfo.ngayYCTL,
        DICHVUVT_ID: this.dichvuvt_id,
        MA_TB: this.subscriptionFormInfo.soMayAcc,
              ...
              if (this.TRANGTHAI_DONGBO && this.requestFormInfo.dichVu == DICHVU_VIENTHONG.DI_DONG) {
                ...
                if (ds_hd && ds_hd.length > 0) {
                  ...
                  if ("MA_HD" in columnNames && "MA_TB" in columnNames) {
                    const current_somay = "84"+this.dsThueBao[i]["ma_TB"].toString()
                    const ma_hd_thanhly_neo = ds_hd.filter(r => r['MA_TB'] === current_somay && r["TRANGTHAIHD_ID"] == 2).map(r => r["MA_HD"])
                    if (ma_hd_thanhly_neo && ma_hd_thanhly_neo.length > 0) {
    ...
    async kiemTraSlFileCa(){
      if (this.loaitb_id != 116 && this.loaitb_id != 288)
        return true;
      this.loading(true)
      try{
        let params = {
        ...
        return await api.kt_soluong_file_ca_v3(params)
          .then(response => {
            this.loading(false)
    ...
    async giaoPhieu (e) {
      ...
      try {
        ...
        let kiemtra_file_hs_ca = await this.kiemTraSlFileCa()
        if(!kiemtra_file_hs_ca)
          return;
        try {
    ...
    async onMaGiaoDichKeyPress (e) {
      ...
      if (this.requestFormInfo.maGiaoDich && this.requestFormInfo.maGiaoDich.length > 0) {
        ...
        console.log('onMaGiaoDichKeyPress')
        console.log(rs)
        if(rs&& rs.length > 0){
            this.hienThiTTHopDongKH(rs);
            this.hienThiThongTinDanhBa(rs[0].ma_tb);
        }else{
          this.$toast.warning('Không tìm thấy thông tin về hợp đồng có mã giao dịch: '+this.requestFormInfo.maGiaoDich+'. Bạn hãy kiểm tra lại')
          this.loading(false)
          return;
        }
        this.loading(false)
    ...
    async hienThiTTDBMoRong () {
      ...
      switch (+this.dichvuvt_id) {
        case DICHVU_VIENTHONG.CO_DINH:
          ...
          if (rsKieuTB[0]['loaikenh_id']) {
              this.loaikenh_id = rsKieuTB[0]['loaikenh_id']
          }
          break
        case DICHVU_VIENTHONG.ADSL:
        case DICHVU_VIENTHONG.BRCD_DOITAC:
    ...
    async hienThiTTHopDongKH (data) {
      ...
      this.requestFormInfo.dichVu = data[0].dichvuvt_id
    ...
    async hienThiThongTinDanhBa (tbid) {
      ...
      if (rsDsKM && rsDsKM.length > 0) {
        let tb = 'Thuê bao ' + this.subscriptionFormInfo.soMayAcc
      ...
      this.loaikenh_id = rs[0].loaikenh_id
    ...
    async onAcceptSearchContract (e) {
      ...
      if (this.requestFormInfo.maGiaoDich && this.requestFormInfo.maGiaoDich.length > 0) {
        this.dsThueBao = []
        const rs = await api.lay_ds_hopdong_theo_ma_gd(this.requestFormInfo.maGiaoDich)
        console.log('onAcceptSearchContract')
        console.log(rs)
        this.hienThiTTHopDongKH(rs)
      }
    },
