...
export default {
  ...
  methods: {
    ...
    frmTraCuuHDTB_TTHD_Load: async function () {
      console.log("frmTraCuuHDTB_TTHD_Load")
    ...
    changeCBO: async function () {
      if (this.chkLoaiHD && this.modelCbo.vcboLoaiHD.length > 0) { await this.timkiem() }
    },
    changeCBO2: async function () {
      if (this.chkDVVT && this.modelCbo.vcboDVVT.length > 0) { await this.timkiem() }
    },
    ...
    HienThiDanhSach: async function () {
      try {
        ...
        // Hiếu - 20.10.2010: Chỉ cho hiển thị tất cả đối với loại hợp đồng chuyển quyền
        // Chỉ user nào lập hợp đồng mới được hoàn thiện hồ sơ chuyển quyền
        console.log("LAY_DS_HDTB_TTHD", Enum.LoaiHopDong.DD_HSSV)
        if (loaihd_id == Enum.LoaiHopDong.DD_HSSV) {
        ...
        if (this.dtList.length > 0) { this.isDisableBtn = false } else { this.isDisableBtn = true }
      } catch (e) {
        console.log(e)
      }
    },
