...
export default {
  ...
  methods: {
    ...
    TAO_KHTT: async function (data) {
      try {
        var kq_return = await api.TAO_KHTT(this.axios, data);
        console.log(kq_return)
        if (kq_return.data.data.data.agentInfo) {
          if (kq_return.data.data.data.agentInfo != null) {

            var input = {
              hdtb_id: this.hdtb_id,
              agent_id: kq_return.data.data.data.agentInfo.agent_id,
              adser_id: kq_return.data.data.data.agentInfo.adser_id,
              contract_id: kq_return.data.data.data.agentInfo.contract_id,
              username: kq_return.data.data.data.agentLogin.username,
              password: kq_return.data.data.data.agentLogin.password,
          ...
          this.$toast.error(kq_return.data.data.data.message);
        }
        return false;
      } catch (ex) {
        console.log(ex);
    ...
    btnNhapMoi_Click: async function () {
      try {
        ...
        if(data != null) {          
          const kq= await this.TAO_KHTT(data);  
          if(kq) this.LoadThongTinKhachHang();
        }
