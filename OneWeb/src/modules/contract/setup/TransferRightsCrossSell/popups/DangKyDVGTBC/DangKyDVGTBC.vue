<template src="./DangKyDVGTBC.html"></template>
<script>
import Vue from 'vue'
        ...
        return {
            ...
            dgvDanhSach: {
                ...
                headers: [
                    ...
                    { fieldName: 'ma_dvgt', headerText: 'Mã dịch vụ', allowFiltering: true, allowSorting: false,  autoFit: "true" },
                    { fieldName: 'ten_dvgt', headerText: 'Tên dịch vụ', allowFiltering: true, allowSorting: false, autoFit: "true" },         
                    { fieldName: 'sudung', headerText: 'Trạng thái', allowFiltering: true, allowSorting: false, autoFit: "true", visible: true},
                    { fieldName: 'dangky', headerText: '', allowFiltering: false, allowSorting: false, width: "120px", textAlign: "Center",
                        headerTemplate: () => {
                            return {
                        ...
                        template: function () {
                            return {
                                template: Vue.component('action-check', {
                                    template: `
                                        <div class="check-action">
                                            <input v-model="parent.dgvDanhSach.checked_DK" :value="data.ma_dvgt"
                    ...
                    { fieldName: 'huy', headerText: '', allowFiltering: false, allowSorting: false, visible: true, width: "90px", textAlign: "Center",
                        headerTemplate: () => {
                            return {
                                template: Vue.component("action-check-all", {
                                    template: `
                        ...
                        template: function () {
                            return {
                                template: Vue.component('action-check', {
                                    template: `
                                        <div class="check-action">
                                            <input v-model="parent.dgvDanhSach.checked_Huy" :value="data.ma_dvgt"
                                                type="checkbox" class="check" @change="onChangeProcessed(data)"
                                            />
                                            <span class="name"></span>
                                        </div>
                    ...
                    { fieldName: 'sua', headerText: '', allowFiltering: false, allowSorting: false, visible: true, width: "90px", textAlign: "Center",
                        headerTemplate: () => {
                            return {
                                template: Vue.component("action-check-all", {
                                    template: `
                        ...
                        template: function () {
                            return {
                                template: Vue.component('action-check', {
                                    template: `
                                        <div class="check-action">
                                            <input v-model="parent.dgvDanhSach.checked_Sua" :value="data.ma_dvgt"
                                                type="checkbox" class="check" @change="onChangeProcessed(data)"
                                            />
                                            <span class="name"></span>
                                        </div>
                        ...
                        template: function () {
                            return {
                                template: Vue.component('action-check', {
                                    template: `
                                        <div class="check-action">
                                            <input v-model="parent.dgvDanhSachTC.checked_DKTC" :value="data.ma_dvgt"
                                                type="checkbox" class="check" @change="onChangeProcessed(data)"
                                            />
                                            <span class="name"></span>
                                        </div>
                        ...
                        template: function () {
                            return {
                                template: Vue.component('action-check', {
                                    template: `
                                        <div class="check-action">
                                            <input v-model="parent.dgvDanhSachTC.checked_HuyTC" :value="data.ma_dvgt"
                                                type="checkbox" class="check" @change="onChangeProcessed(data)"
                                            />
                                            <span class="name"></span>
                                        </div>
                        ...
                        template: function () {
                            return {
                                template: Vue.component('action-check', {
                                    template: `
                                        <div class="check-action">
                                            <input v-model="parent.dgvDanhSachTC.checked_SuaTC" :value="data.ma_dvgt"
                                                type="checkbox" class="check" @change="onChangeProcessed(data)"
                                            />
                                            <span class="name"></span>
                                        </div>
    ...
    methods: {
        ...
        btnChapNhan_Click() {
            ...
            for (let i = 0; i < this.dgvDanhSach.data.length; i++) {
                ...
                let checkDK = this.dgvDanhSach.checked_DK.find((item) => item == this.dgvDanhSach.data[i].ma_dvgt)
                if (checkDK != undefined && checkDK != null) isDK = true
                let checkSua = this.dgvDanhSach.checked_Sua.find((item) => item == this.dgvDanhSach.data[i].ma_dvgt)
                    ...
                    this.$confirm("Bạn có nhập '" + this.dgvDanhSach.data[i].thongbao + "' cho dịch vụ '" + this.dgvDanhSach.data[i].ten_dvgt + "' hay không ?", {
                        ...
                        if (v) {
                            ...
                            */
                            ...
                            if (this.dgvDanhSach.data[i].cuoc_ld == "")
                                this.dgvDanhSach.data[i].cuoc_ld = "0";
                            if (this.dgvDanhSach.data[i].vat_ld == "")
                                this.dgvDanhSach.data[i].vat_ld = "0";
                            if (this.dgvDanhSach.data[i].cuoc_sd == "")
                                this.dgvDanhSach.data[i].cuoc_sd = "0";
                            if (this.dgvDanhSach.data[i].vat_sd == "")
                                this.dgvDanhSach.data[i].vat_sd = "0";
                            if (this.dgvDanhSach.data[i].tien == "")
                                this.dgvDanhSach.data[i].tien = "0";

                            if (isDK && this.dgvDanhSach.data[i].ma_dvgt == "PAYTV" 
                                    && this.dgvDanhSach.data[i].noidung != undefined && this.dgvDanhSach.data[i].noidung != "") {
                                try {
            ...
            this.dsDK_DVGT.dsdvgt_ban.forEach(item => {
                item.dangky = "0"
                item.huy = "0"
                item.sua = "0"
                let oFind = this.dgvDanhSach.checked_DK.find(itemF => itemF == item.ma_dvgt)
                if (oFind && oFind != null) item.dangky = "1"
                oFind = this.dgvDanhSach.checked_Huy.find(itemF => itemF == item.ma_dvgt)
                if (oFind && oFind != null) item.huy = "1"
                oFind = this.dgvDanhSach.checked_Sua.find(itemF => itemF == item.ma_dvgt)
                if (oFind && oFind != null) item.sua = "1"
            })

            this.dsDK_DVGT.dsdvgt_tc.forEach(item => {
                item.dangky = "0"
                item.huy = "0"
                item.sua = "0"
                let oFind = this.dgvDanhSachTC.checked_DKTC.find(itemF => itemF == item.ma_dvgt)
                if (oFind && oFind != null) item.dangky = "1"
                oFind = this.dgvDanhSachTC.checked_HuyTC.find(itemF => itemF == item.ma_dvgt)
                if (oFind && oFind != null) item.huy = "1"
                oFind = this.dgvDanhSachTC.checked_SuaTC.find(itemF => itemF == item.ma_dvgt)
                if (oFind && oFind != null) item.sua = "1"
            })

            console.log(this.dsDK_DVGT)
        ...
        Set_dsDK_DVGT_TC() {
            ...
            for (let i = 0; i < this.dgvDanhSachTC.data.length; i++) {
                let isDK = false, isSua = false
                let checkDK = this.dgvDanhSachTC.checked_DKTC.find((item) => item == this.dgvDanhSachTC.data[i].ma_dvgt)
                if (checkDK != undefined && checkDK != null) isDK = true
                let checkSua = this.dgvDanhSachTC.checked_SuaTC.find((item) => item == this.dgvDanhSachTC.data[i].ma_dvgt)
                if (checkSua != undefined && checkSua != null) isSua = true
                    ...
                    this.$confirm("Bạn có nhập '" + this.dgvDanhSachTC.data[i].thongbao + "' cho dịch vụ '" + this.dgvDanhSachTC.data[i].ten_dvgt + "' hay không ?", {
                        ...
                        if (v) {
                            ...
                            */
                            ...
                            if (this.dgvDanhSachTC.data[i].cuoc_ld == "")
                                this.dgvDanhSachTC.data[i].cuoc_ld = "0";
                            if (this.dgvDanhSachTC.data[i].vat_ld == "")
                                this.dgvDanhSachTC.data[i].vat_ld = "0";
                            if (this.dgvDanhSachTC.data[i].cuoc_sd == "")
                                this.dgvDanhSachTC.data[i].cuoc_sd = "0";
                            if (this.dgvDanhSachTC.data[i].vat_sd == "")
                                this.dgvDanhSachTC.data[i].vat_sd = "0";
                            if (this.dgvDanhSachTC.data[i].tien == "")
                                this.dgvDanhSachTC.data[i].tien = "0";

                            if (isDK && this.dgvDanhSachTC.data[i].ma_dvgt == "PAYTV" 
                                    && this.dgvDanhSachTC.data[i].noidung != undefined && this.dgvDanhSachTC.data[i].noidung != "") {
                                try {
        ...
        gview_DanhSach_queryCellInfo(args) {
            if (args.data != undefined) {
                let iFind = -1
                for (let i=0; i<this.dgvDanhSach.data.length; i++) {
                    if (this.dgvDanhSach.data[i].ma_dvgt == args.data.ma_dvgt) {
                        iFind = i
                        break
                    }
                }
        ...
        gview_DanhSachTC_queryCellInfo(args) {
            if (args.data != undefined) {
                let iFind = -1
                for (let i=0; i<this.dgvDanhSachTC.data.length; i++) {
                    if (this.dgvDanhSachTC.data[i].ma_dvgt == args.data.ma_dvgt) {
                        iFind = i
                        break
                    }
                }
