...
export default {
  ...
  methods: {
        ...
        for (let i in this.dtControl) {
          for (let j = 0; j < this.ds_tien_caidat.length; j++) {
            if (this.dtControl[i]["khoanmuctt_id"] == this.ds_tien_caidat[j]["khoanmuctt_id"]) {
              ...
              this.$set(this.pnlDynamicControl[i].txt, 'Text', Math.round(Number(this.ds_tien_caidat[j]["tien"])))
              this.$set(this.pnlDynamicControl[i].txtvat, 'Text', Math.round(Number(this.ds_tien_caidat[j]["vat"])))
    ...
    EnterTongTien($event, i, tyle_vat) {
      ...
      this.$set(this.pnlDynamicControl[i].txttongtien,'Text',  Math.round((Number(value))))
      console.log(value)
      this.$set(this.pnlDynamicControl[i].txtvat,'Text',  Math.round((Number(value)) * Number(tyle_vat) / (100 + tyle_vat)))
      this.$set(this.pnlDynamicControl[i].txt,'Text', Math.round((Number(value)) * (100) / (100 + tyle_vat)))
    },
    EnterTien($event,i, tyle_vat) {
      let value = $event.target.value.replaceAll(",", "")
      this.$set(this.pnlDynamicControl[i].txt,'Text',  Math.round(Number(value)))
      this.$set(this.pnlDynamicControl[i].txtvat,'Text',  Math.round((Number(value)) * Number(tyle_vat) / 100))
