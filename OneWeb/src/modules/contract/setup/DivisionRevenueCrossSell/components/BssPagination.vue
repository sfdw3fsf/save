...
export default {
    ...
    watch: {
        totalRows (newVal) {
            ...
            if(newVal!=0){
                let phanNguyen = Math.floor(newVal/ 10)
                console.log("phanNguyen", phanNguyen)
                if ( this.currentPage <= phanNguyen  &&  this.index_currentPage*10 < newVal ){
                    this.textMain = this.currentPage*10 
                }
                else {
                    this.textMain = newVal
                }
            }

            this.currentPage = newVal > 0 ? this.index_currentPage : 0
