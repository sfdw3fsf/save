...
export default {
      ...
      let temp = this.dataSourcesHen
        .map(x =>
          Object.assign(x, {
            ...
            giohen_tu:
              x.giohen_tu != null
                ? moment(x.giohen_tu, "YYYY-MM-DD HH:mm:ss").format(
                    "DD/MM/YYYY HH:mm:ss"
                  )
                : "",
            giohen_den:
              x.giohen_den != null
                ? moment(x.giohen_den, "YYYY-MM-DD HH:mm:ss").format(
                    "DD/MM/YYYY HH:mm:ss"
                  )
                : "",
            ngay_cn:
              x.ngay_cn != null
                ? moment(x.ngay_cn, "YYYY-MM-DD HH:mm:ss").format(
                    "DD/MM/YYYY HH:mm:ss"
                  )
                : ""
          })
  ...
  methods: {
    ...
    async sendSMS(item) {
      ...
      if (item.so_dt == null) {
        this.$toast.error("Không có thông tin số điện thoại để gửi tin nhắn");
        return;
      }
      //send sms kh
      let content = "test";
      let result = await this.sendSMSAPI(item.so_dt, content);
      if (result) {
    ...
    changeDenNgay(data) {
      this.den_ngay = moment(new Date(data.value)).format(
        "DD/MM/YYYY HH:mm:ss"
      );
    },
    ...
    async ghiDuLieu() {
      ...
      let result = await this.sp_ins_hdtb_hen(
        JSON.stringify(dulieu.result),
        isInsert ? 1 : 2
      );
      if (result) {
        this.$toast.success(
          isInsert ? "Tạo lịch hẹn thành công" : "Cập nhật lịch hẹn thành công"
        );
        //send sms
    ...
    async kiemtra_dulieu(isInsert) {
      ...
      if (!isInsert) {
        //Bỏ chế độ cập nhật
        this.$toast.warning(
          "Không được cập nhật thông tin hẹn. Chỉ được hẹn mới hoặc Xóa"
        );
        return false;
      } else {
        ...
        if (checkDaHoanThanh.length > 0) {
          this.$toast.warning(
            "Thuê bao " +
              checkDaHoanThanh[0].ma_tb +
              " đã hoàn thành. Không được hẹn lắp đặt với thuê bao này"
          );
          return false;
        }
        ...
        if (checkDaThiCong.length > 0) {
          this.$toast.warning(
            "Thuê bao " +
              checkDaThiCong[0].ma_tb +
              " đã thi công xong. Không được hẹn lắp đặt với thuê bao này"
          );
          return false;
        }
        ...
        if (checkDaThoaiTra.length > 0) {
          this.$toast.warning(
            "Thuê bao " +
              checkDaThoaiTra[0].ma_tb +
              " đã thoái trả. Không được hẹn lắp đặt với thuê bao này"
          );
          return false;
        }
        ...
        if ((this.hen_ld == 1 && this.gdkh) || this.check_henld) {
          ...
          if (result.length > 0) {
            ...
            if (result[0].nhanvien_id != nhanvien_id) {
              this.$toast.warning(
                "Bạn không được cập nhật ngày hẹn do user giao dịch khác lập hợp đồng !"
              );
              return false;
            }
          }
        ...
        if (
          moment(this.tu_ngay, "DD/MM/YYYY HH:mm:ss")
            .add(10, "minutes")
            .isBefore(new Date())
        ) {
          this.$toast.warning(
            "Hẹn từ ngày + 10p không được nhỏ hơn ngày hiện tại !"
          );
          return false;
        }
      }
      ...
      if (
        moment(this.den_ngay, "DD/MM/YYYY HH:mm:ss").isBefore(
          moment(this.tu_ngay, "DD/MM/YYYY HH:mm:ss")
        )
      ) {
      ...
      if (this.kt_thoigian_hen_thicong) {
        ...
        if (dtSoNgay.length > 0) {
          ...
          if (
            moment(this.den_ngay, "DD/MM/YYYY HH:mm:ss").diff(
              moment(this.tu_ngay, "DD/MM/YYYY HH:mm:ss"),
              "days"
            ) > songay
          ) {
            this.$toast.warning(
              "Đối với phiếu thi công thời gian hẹn không quá " +
                songay +
                " ngày!"
            );
            return false;
        ...
        moment(this.den_ngay, "DD/MM/YYYY HH:mm:ss").isAfter(
          moment(new Date(), "DD/MM/YYYY HH:mm:ss").add(1, "years")
        )
      ) {
        this.$toast.warning(
          "Ngày hẹn đến không được quá 1 năm so với ngày hiện tại !"
        );
        return false;
      }
      return true;
    ...
    async getDuLieu(isInsert) {
      ...
      if (isInsert) {
        ...
        result.push({
          ...
          DONVI_ID: this.$root.token.getDonViID(),
          NHANVIEN_ID: this.$root.token.getNhanVienID(),
          DAUCUOI_ID: this.selectedRowHen.diemcuoi
            ? 2
            : this.selectedRowHen.diemdau
            ? 1
            : 0,
          KIEU: 0
        });
    ...
    async onSelectedRowDsThueBao(item) {
      ...
      this.vloaihd_id = Number(
        await this.fn_map_id(
          {
            id_neo: "loaihd_id",
            table: "css.hd_khachhang",
            dk: "where hdkh_id = " + this.hdkh_id
          },
          "0"
        )
      );
    ...
    async clickXoaLichHen() {
      ...
      if (tthd_id == "6") {
        this.$toast.error(
          "Thuê bao " + matb + " đã hoàn thành. Không được xóa thông tin hẹn"
        );
        return;
      } else if (tthd_id == "5") {
        this.$toast.error(
          "Thuê bao " + matb + " đã thi công xong. Không được xóa thông tin hẹn"
        );
        return;
      } else if (tthd_id == "7") {
        this.$toast.error(
          "Thuê bao " + matb + " đã thoái trả. Không được xóa thông tin hẹn"
        );
        return;
      }
      ...
      try {
        ...
        if (
          ...
          this.$toast.error(
            "Đã có lỗi xảy ra, xoá thông tin hẹn không thành công"
          );
        }
      } catch (e) {
        this.loading(false);
    ...
    async khoitao_thamso() {
      ...
      if (dsThamSo.length > 0) {
        ...
        if (
          ...
          //quyen_duoc_hen
          let ttkd_capnhat_hen = await this.lay_ts_macdinh_dv(
            donvi_nd,
            "TTKD_CAPNHAT_HEN"
          );
          if (
            ttkd_capnhat_hen == "1" ||
            ttkd_capnhat_hen == "TTKD_CAPNHAT_HEN"
          ) {
    ...
    async hienthi_ds_hdtb() {
      try {
        this.loading(true);
        this.dataSources = [];
        let response = await API.sp_lay_ds_hdtb_theo_hdkh(
          this.axios,
          this.hdkh_id
        );
        this.loading(false);
        if (
          ...
          this.dataSources = response.data.data.map((x, index) =>
            Object.assign(x, { id: index + 1, diemdau: false, diemcuoi: false })
          );
    ...
    async hienthi_ds_hdtb_hen() {
      try {
        ...
        if (
          ...
          this.dataSourcesHen = response.data.data.map(x =>
            Object.assign(x, {
              diemdau: x.diemdau != null && x.diemdau == "1" ? true : false,
              diemcuoi: x.diemcuoi != null && x.diemcuoi == "1" ? true : false
            })
          );
          if (this.dataSourcesHen.length == 0) {
            this.setActiveActions(1);
    ...
    async sp_lay_nhanvien_theo_manv_loainv(ma_nv, loainv_id) {
      try {
        this.loading(true);
        let response = await API.sp_lay_nhanvien_theo_manv_loainv(
          this.axios,
          ma_nv,
          loainv_id
        );
    ...
    async lay_nhanvien_id_theo_hdkh_id(hdkh_id) {
      try {
        this.loading(true);
        let response = await API.lay_nhanvien_id_theo_hdkh_id(
          this.axios,
          hdkh_id
        );
        this.loading(false);
    ...
    async fn_map_id(data, defaultValue) {
      try {
        let response = await API.fn_map_id(this.axios, data);
        if (
          response &&
          response.data &&
          response.data.error_code &&
      ...
      try {
        ...
        if (
          ...
          if (response && response.data && response.data.message) {
            ...
            this.$toast.error(
              kieu == 1
                ? "Đã xảy ra lỗi, tạo lịch hẹn không thành công"
                : "Đã xảy ra lỗi, cập nhật lịch hẹn không thành công"
            );
          }
          return false;
        }
  ...
  watch: {
    statusPopupHenKH(val) {
      if (val) this.$refs["popupThongTinHenKhachHang"].show();
      else this.$refs["popupThongTinHenKhachHang"].hide();
    }
  }
};
</script>
