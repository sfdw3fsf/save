                            ...
                            <div class="value">
                                <div class="select-custom" ref="ds_tocdo_thuc">
                                    <select2 :settings="{ dropdownParent: $refs['ds_tocdo_thuc'] }" ref="cboTocDoThuc"
                                        ...
                                        :disabled="!checkTocDoThuc||disabledTocDoThucMoi">
                                    </select2>
                                </div>
                            </div>
                        </div>
...
import DSTBCungDoiCapModal from '../../ChangeSubsType/popups/DSTBCungDoiCapModal.vue'
import DSTBLapKemModal from '../../ChangeSubsType/popups/DSTBLapKemModal.vue'
import ThongTinTCModal from '../../ChangeSubsType/popups/ThongTinTCModal.vue'
export default {
    name:'BangRongCoDinh',
    props:{
      khachhang_id:{
    ...
    data(){
        return{
            ...
            enableLoaiTBi:true,
            disabledTrangBi:false,
            disabledMucCuoc:false,
            disabledTocDoThucMoi:false
    ...
    methods:{
        ...
        async HienThiTTHopDongTB_B2(item){
            ...
            this.input_khuvuc=item.TEN_KV?item.TEN_KV:''
        },
        async HienThiTTMoRong_DB(item){
            ...
            if(item.tocdothuc_id){
                this.tocdothuc_selected=item.tocdothuc_id
            }else{
                this.tocdothuc_selected=-1
            }
            this.muccuoc_selected=item.muccuoc_id?item.muccuoc_id:0
            this.loaithietbi_selected=item.thietbidc_id?item.thietbidc_id:0
            this.trangbi_selected=item.trangbi_id?item.trangbi_id:0
            ...
            this.input_matkhau_adsl_cu=item.password?item.password:''
            this.input_matkhau_adsl=this.input_matkhau_adsl_cu
            this.input_thongtin_tc_cu=item.thongtin_tc?item.thongtin_tc:''

            this.input_soao=item.ma_lt?item.ma_lt.toString():''

        },
        async HienThiTTMoRong_HD(item){
            console.log('HienThiTTMoRong_HD ADSL',item)
            this.input_email_adsl=item.email?item.email.toString():''
            this.input_matkhau_adsl=item.password?item.password.toString():''
            ...
            // }
            this.muccuoc_moi_selected=item.muccuoc_id?item.muccuoc_id:null
            this.loaithietbi_moi_selected=item.thietbidc_id!=null?item.thietbidc_id:null
            if(item.tucthi&&Number(item.tucthi)!=0){
                this.checkTucThi=true
            }else{
                this.checkTucThi=false
            }
            this.input_madoicap_adsl=item.madoicap?item.madoicap.toString():''
            this.trangbi_moi_selected=item.trangbi_id?item.trangbi_id:null
