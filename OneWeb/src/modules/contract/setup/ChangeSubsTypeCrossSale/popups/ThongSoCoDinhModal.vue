<template>
    <b-modal ref="popupThongSoCoDinh" size="lg" hide-header hide-header-close hide-footer body-class="p-0"
        ...
        <div class="modal-content popup-box">
                        ...
                        <div class="info-row">
                            ...
                            <div class="value">
                                <div class="select-custom" ref="ds_congnghe_ban">
                                    <select2 :settings="{ dropdownParent: $refs['ds_congnghe_ban'] }"
                                        ref="cboCongNgheBan" v-model="thongso_bancheo.congnghe_id" class="select2"
                            ...
                            <div class="value">
                                <div class="select-custom" ref="ds_doituong_tc">
                                    <select2 :settings="{ dropdownParent: $refs['ds_doituong_tc'] }" ref="cboDoiTuongTC"
                                        v-model="thongso_thicong.doituong_id" class="select2" @change="changeDoiTuongTC"
                            ...
                            <div class="value">
                                <div class="select-custom" ref="ds_trangbi_tc">
                                    <select2 :settings="{ dropdownParent: $refs['ds_trangbi_tc'] }" ref="cboTrangBiTC"
                                        v-model="thongso_thicong.trangbi_id" class="select2" @change="changeTrangBiTC"
                        ...
                        <div class="info-row">
                            ...
                            <div class="value">
                                <div class="select-custom" ref="ds_muccuoctb_tc">
                                    <select2 :settings="{ dropdownParent: $refs['ds_muccuoctb_tc'] }"
                                        ref="cboMucCuocTBTC" v-model="thongso_thicong.muccuoctb_id" class="select2"
            ...
            <DSTBCungDoiCapBCModal ref="dSTBCungDoiCapBCModal" :khachhang_id="Number(khachhang_id)"
                :tinh_tc="Number(tinh_tc)" :kieuld_id="Number(thongso_thicong.kieuld_id)"
                @accept="acceptDSTBCungDoiCapBC" />
            <DSTBLapKemBCModal ref="dSTBLapKemBCModal" :hdkh_id="Number(hdkh_id)" :tinh_tc="Number(tinh_tc)"
                :kieuld_id="Number(thongso_thicong.kieuld_id)" @accept="acceptDSTBLapKemBC" />
...
export default {
    ...
    methods: {
        ...
        async CapNhat() {
            ...
            console.log('emit modal', JSON.stringify({
                thongso_ban: this.thongso_bancheo,
                thongso_tc: this.thongso_thicong
            }));
            this.hideModal()
        ...
        async initDuLieu() {
            ...
            //fn_lay_dulieu_danhmuc_cd_ims
            let result = await this.fn_lay_dulieu_danhmuc_cd_ims(JSON.stringify({
                TINHTHICONG_ID: this.tinh_tc,
                KIEULD_ID_BAN: this.thongso_bancheo.kieuld_id,
                KIEULD_ID_TC: this.thongso_thicong.kieuld_id,
            }))
            if(result && result.ERROR_CODE == 1){
                ...
                this.ds_doituong_tc = result.RESULT.CBO_DOITUONG_TC
                ...
                this.ds_trangbi_tc = result.RESULT.CBO_TRANGBI_TC
                ...
                console.log('tinh_chuquan_id', tinh_chuquan_id)
    
                //Chủ quản bán, vs đơn vị tc
                // this.ds_chuquan_ban = await this.lay_ds_chuquan_ban()
                this.ds_chuquan_ban = result.RESULT.CBO_CHUQUAN
                let det = []
                if (tinh_chuquan_id == '-1') {
                    det = this.ds_chuquan_ban.filter(x => x.TINH_ID && x.TINH_ID == this.$root.token.getPhanVungID())
                } else {
                    det = this.ds_chuquan_ban.filter(x => x.TINH_ID && x.TINH_ID == tinh_chuquan_id)
                }
                if (det.length > 0) {
                ...
                this.ds_chuquan_tc = result.RESULT.CBO_CHUQUAN_TC
                let dut = []
                if (tinh_chuquan_id == '-1') {
                    dut = this.ds_chuquan_tc.filter(x => x.TINH_ID && x.TINH_ID == this.$root.token.getPhanVungID())
                } else {
                    dut = this.ds_chuquan_tc.filter(x => x.TINH_ID && x.TINH_ID == tinh_chuquan_id)
                }
                if (dut.length > 0) {
        ...
        async fn_lay_dulieu_danhmuc_cd_ims(ds_para){
            try {
                this.loading(true)
                let response = await this.axios.post('/web-bancheo/tracuu/fn_lay_dulieu_danhmuc_cd_ims', {
                    p_ds_para: ds_para
                })
                this.loading(false)
        ...
        async lay_ds_chuquan_ban() {
            try {
                ...
                this.loading(false)
                if (response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
                    return response.data.data.filter(x => x.hieuluc == 1)
                } else {
                    return []
                }
            } catch (e) {
        ...
        async LAY_CHUQUAN_MACDINH_THEO_DONVI_BANCHEO() {
            let result = ""
            try {
                let response = await this.axios.get('web-hopdong/hopdong/lay_chuquan_macdinh_theo_donvi_bancheo');
