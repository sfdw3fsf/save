  ...
  mutations: {
    addToCache(state, { key, subkey, data, expirationTime }) {
      const cacheItem = {
        ...
        expirationTime: Date.now() + (expirationTime || 5 * 60 * 1000) // Calculate the expiration time based on the current time and the specified expiration period or default to 60 minutes (60 * 60 * 1000 milliseconds)
      };
