    ...
    if (vthaotac == 2) // Điều chỉnh hóa đơn
    {
        if (vkieu == 1 || vkieu == 2 || vkieu == 3)
        {
            ...
            try
            {
                ...
                var xml = "";
                var tt78 = false;
                // let kq = await API.post_map_id(this.axios, {
                //   "in_id_neo": "ten_ts",
                        ...
                        */
                        ...
                        if (xml.indexOf('<DLHDon')>0)
                        {
                            ...
                            // {
                                ...
                                objHoaDonDienTu.Amount = getXML(xml,"DLHDon.NDHDon.TToan.TgTTTBSo");
                                objHoaDonDienTu.Amount_words = getXML(xml,"DLHDon.NDHDon.TToan.TgTTTBChu");
                                objHoaDonDienTu.ArisingDate = getXML(xml,"DLHDon.TTChung.NLap");
                                xmlValue = getXML(xml,"DLHDon.NDHDon.NMua.HVTNMHang");
                                if(xmlValue) objHoaDonDienTu.Buyer = xmlValue;
                                objHoaDonDienTu.ComAddress = getXML(xml,"DLHDon.NDHDon.NBan.DChi");
                                objHoaDonDienTu.ComName = getXML(xml,"DLHDon.NDHDon.NBan.Ten");
                                objHoaDonDienTu.ComPhone = getXML(xml,"DLHDon.NDHDon.NBan.SDThoai");
                                objHoaDonDienTu.ComTaxCode = getXML(xml,"DLHDon.NDHDon.NBan.MST");
                                objHoaDonDienTu.CusAddress = getXML(xml,"DLHDon.NDHDon.NMua.DChi");
                                xmlValue = getXML(xml,"DLHDon.NDHDon.NMua.TNHang");
                                if(xmlValue) objHoaDonDienTu.CusBankName = xmlValue;
                                objHoaDonDienTu.CusCode = getXML(xml,"DLHDon.NDHDon.NMua.MKHang");
                                objHoaDonDienTu.CusName = getXML(xml,"DLHDon.NDHDon.NMua.Ten");
                                xmlValue = getXML(xml,"DLHDon.NDHDon.NMua.SDThoai");
                                if(xmlValue) objHoaDonDienTu.CusPhone = xmlValue;
                                xmlValue = getXML(xml,"DLHDon.NDHDon.NMua.MST");
                                if(xmlValue) objHoaDonDienTu.CusTaxCode = xmlValue;
                                xmlValue = getXML(xml,"DLHDon.TTChung.TTKhac.TTruong");//DLHDon.TTChung.TTKhac[0].TTruong
                                if (xmlValue) objHoaDonDienTu.Extra = xmlValue;
                                objHoaDonDienTu.InvoiceName = getXML(xml,"DLHDon.TTChung.THDon");
                                objHoaDonDienTu.InvoiceNo = getXML(xml,"DLHDon.TTChung.SHDon");
                                objHoaDonDienTu.InvoicePattern = getXML(xml,"DLHDon.TTChung.KHMSHDon");
                                objHoaDonDienTu.Kind_of_Payment = getXML(xml,"DLHDon.TTChung.HTTToan");
                                objHoaDonDienTu.SerialNo = getXML(xml,"DLHDon.TTChung.KHHDon");
                                objHoaDonDienTu.Total = getXML(xml,"DLHDon.NDHDon.TToan.TgTCThue");
                                objHoaDonDienTu.VAT_Amount = getXML(xml,"DLHDon.NDHDon.TToan.TgTThue");
                                objHoaDonDienTu.VAT_Rate = "0";
                                try
                                {
                                    xmlValue = getXML(xml,"DLHDon.NDHDon.TToan.THTTLTSuat.LTSuat.TSuat");
                                    if(!xmlValue) throw "Khong phai nhanh nay!"; // de nhay thang xuong catch ra khoi phan nay!
                                    if (xmlValue == "KKKNT")
                                        ...
                                        if (xmlValue == "KCT")
                                            ...
                                            if (xmlValue.indexOf("KHAC:")>=0)
                                                objHoaDonDienTu.VAT_Rate = "10";
                                            else
                                                objHoaDonDienTu.VAT_Rate = xmlValue.substring(0, xmlValue.length - 1);
                                }
                                catch { objHoaDonDienTu.VAT_Rate = "0"; }
                                xmlValue = getXML(xml,"DLHDon.NDHDon.DSHHDVu");
                                products = [typeof(xmlValue)=="string"?1:xmlValue.length];
                                for (var i = 0; i < products.length; i++)
                                {
                                    var curxml = typeof(xmlValue)=="string"?xmlValue:xmlValue[i];
                                    var htype = 1;
                                    if(curxml.indexOf("<HHDVu")==0) htype=2;
                                    products[i] = {};
                                    if(htype==1)
                                    {
