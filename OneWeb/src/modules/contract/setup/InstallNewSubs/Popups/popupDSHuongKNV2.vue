    ...
    methods: {
        ...
        async TimKiemEnter() {
            ...
            try {
                ...
                // Bổ sung theo yêu cầu của a Lê Cường
                if(this.kieutk.valueid != null && this.thongtintc != ""){
                    var dieukien_bosung_1 = [{"KIEUTK_ID": this.kieutk.valueid,"GIATRI":this.thongtintc}]
                }
                // 
                ...
                var rs = await this.$root.context.post("/web-hopdong/lapdatmoi/fn_timkiem_huongkn_v2", {"ds_para": JSON.stringify({"DK_TRACUU":this.kieutk.value,"RDO_DB":this.isHopDong==1,"RDO_HD":this.isHopDong==2,"DICHVUVT_ID":this.dichvuvt,"TXT_TC":this.thongtintc,"LOAI":this.isHopDong,"DIEUKIEN":dieukien,"TINH_TC":this.tinhkn.value,"DONVI_ID":this.donvi, "DS_KIEU_TK":dieukien_bosung_1 == 0?0:dieukien_bosung_1})}); //DONVI_ID fix code cu = 0, thêm "DS_KIEU_TK" theo yêu cầu của a Lê Cường
                if(rs.data && rs.data.length>0) {
