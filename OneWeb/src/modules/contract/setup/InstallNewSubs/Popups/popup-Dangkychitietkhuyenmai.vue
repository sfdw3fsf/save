  ...
  methods: {
    ...
    actionComplete(args) {
      ...
      if (args.requestType === 'save') {
        ...
        if (args.rowData && args.rowData['nhom_km'] != null)
          for (var item of this.dt)
        ...
        this.$forceUpdate()
        //Them cho phan tinh tien tb / csd BSS-119638
        if(this.kieu_km == 1 && [122,2116,373,175,319].indexOf(this.loaihinh_tb*1)>=0) {
          if(args.data['datcoc_csd']) { 
            for (var item of this.dt)
              if (item.chitietkm_id == args.data['chitietkm_id']) {
                var tyle_vat =  args.data['tyle_vat']?args.data['tyle_vat']:10;
                item['datcoc_csd'] = args.data['datcoc_csd'];
                item['tyle_vat'] = args.data['tyle_vat'];
                item['tien_tb'] = item['datcoc_csd']*100 / (100 + tyle_vat);
                this.$forceUpdate()
              }
          }
        }
      ...
      if (args.requestType === 'beginEdit') {
        ...
        // focus the column
        if (args.rowData && args.rowData['loai_bdkm'] == 3) {
        ...
        } else if (args.rowData && args.rowData['loai_bdkm'] == 1) {
          args.form.elements[this.$refs.grid.ej2Instances.element.getAttribute('id') + 'thang_bdkm'].readOnly = false
        ...
        if (args.rowData && args.rowData['kieu_bdtg'] != 3) args.form.elements[this.$refs.grid.ej2Instances.element.getAttribute('id') + 'thang_bdtg'].readOnly = true
        else args.form.elements[this.$refs.grid.ej2Instances.element.getAttribute('id') + 'thang_bdtg'].readOnly = false

        if (args.rowData && args.rowData['loai_bddc'] == 3) {
        ...
        } else if (args.rowData && args.rowData['loai_bddc'] == 1) {
        ...
        if (args.rowData && args.rowData['kieu_tratruoc'] != 1) this.makeReadOnly(args.form.elements[this.$refs.grid.ej2Instances.element.getAttribute('id') + 'datcoc_csd'], true)
        ...
        if (args.rowData && args.rowData['kieu_kmld'] != 1) {
        ...
        if (args.rowData && args.rowData['kieu_km'] != 1) {
      ...
      if (this.kieu_km == 0) {
        ...
        for (var item of this.dt) {
          ...
          if (item['dangky'] == 1) {
            ...
            //     }
            // }
            if(!dr.THANG_BD && dr.THANG_BDDC) dr.THANG_BD = dr.THANG_BDDC ;
            if(dr.NGAY_KTKM && dr.NGAY_KTKM.match(/^\d{2}\/\d{2}\/\d{4}$/g)) dr.THANG_KTKM = moment(dr.NGAY_KTKM, 'DD/MM/YYYY').format('YYYYMM');
            if(dr.NGAY_KTTG && dr.NGAY_KTTG.match(/^\d{2}\/\d{2}\/\d{4}$/g)) dr.THANG_KTTG = moment(dr.NGAY_KTTG, 'DD/MM/YYYY').format('YYYYMM');
            if(dr.NGAY_KT && dr.NGAY_KT.match(/^\d{2}\/\d{2}\/\d{4}$/g)) dr.THANG_KT = moment(dr.NGAY_KT, 'DD/MM/YYYY').format('YYYYMM');
            if(dr.NGAY_KTDC && dr.NGAY_KTDC.match(/^\d{2}\/\d{2}\/\d{4}$/g)) dr.THANG_KTDC = moment(dr.NGAY_KTDC, 'DD/MM/YYYY').format('YYYYMM');
            if(!dr.THANG_KT && dr.THANG_KTDC) dr.THANG_KT = dr.THANG_KTDC ;
