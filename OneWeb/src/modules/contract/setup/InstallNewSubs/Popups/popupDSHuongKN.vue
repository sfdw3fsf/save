    ...
    <div class="popup-body">
        <div class="row">
            <div class="legend-title mart20">
                Thông tin tìm kiếm
                <div class="right" style="width:150px;">Tỉnh kết nối <select2 v-model="tinhkn.value" class="select-custom" :options="tinhkn.list.map(ls=>({id:ls.TINH_ID,text:ls.TENTINH}))"></select2></div>
            </div>
            <div class="col-sm-6 col-12">
                <div class="box-form">
                    <div class="info-row">
                        ...
                        <div class="value">
                            <select2 v-model="kieutk.value" class="select-custom" :options="dktracuu"></select2>
                        </div>
                    </div>
                    <div class="info-row">
                        <div class="key w90">
                        ...
                        <div class="value">
                            <input type="text" class="form-control" v-model="thongtintc">
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-12">
                <div class="box-form">
                    <div class="info-row">
                        ...
                        <div class="value" :class="{disabled:!dvvt.isenabled}">
                            <select2 v-model="dichvuvt" class="select-custom" :options="dvvt.list.map(ls=>({id:ls.DICHVUVT_ID,text:ls.TEN_DVVT}))"></select2>
                        </div>
                        <div class="key" style="padding-left:10px;">
                            <div class="check-action">
                    ...
                    <div class="info-row">
                        ...
                        <div class="value" :class="{disabled:!daivt.isenabled}">
                            <select2 v-model="donvi" class="select-custom" :options="daivt.list.map(ls=>({id:ls.DONVI_ID,text:ls.TEN_DV}))"></select2>
                        </div>
                        <div class="key" style="padding-left:10px;">
                            <div class="check-action">
...
export default {
    ...
    async mounted() {
        this.loading(true);
        try {
            var rs = await this.$root.context.post("/web-hopdong/lapdatmoi/fn_lay_tt_load_huongkn", {"ds_para": JSON.stringify({"SD_DSHUONGKN_V2":0})});
    ...
    computed: {
        ...
        dktracuu: function() {
            if(this.isHopDong==2) return this.kieutk.hd.map(ls=>({id:ls.KIEU_TK,text:ls.KIEU_TK}));
            else return this.kieutk.db.map(ls=>({id:ls.KIEU_TK,text:ls.KIEU_TK}));
        }
    },
    methods: {
        closeForm() {
        ...
        chapnhan() {
            ...
            if(!obj || !obj['id_sochinh']) this.$emit('form-close',null);
                ...
                this.$emit('form-close',{iid_sochinh:obj['id_sochinh'],iloai:obj['loai'],shuongkn:obj['vrf_vfi']?obj['vrf_vfi']:obj['huong_kn'],vma_tb:obj['vrf_vfi']?obj['vrf_vfi']:obj['ma_tb'],diachi_ld:obj['diachi_ld'],itinh_kn_id:this.tinhkn.value,vrf:obj['vrf_vfi'],});
        },
        ...
        async khachhang_selectedRowChanged(items) {
            ...
            var rs = await this.$root.context.post("/web-hopdong/lapdatmoi/lay_ds_sochinh_ktr1", {
        ...
        async TimKiemEnter() {
            ...
            try {
                ...
                var rs = await this.$root.context.post("/web-hopdong/lapdatmoi/fn_timkiem_huongkn", {"ds_para": JSON.stringify({"DK_TRACUU":this.kieutk.value,"RDO_DB":this.isHopDong==1,"RDO_HD":this.isHopDong==2,"DICHVUVT_ID":this.dichvuvt,"TXT_TC":this.thongtintc,"LOAI":this.isHopDong,"DIEUKIEN":dieukien})});
                if(rs.data && rs.data.length>0) {
                    var apidata = JSON.parse(rs.data);
                    if(apidata['ERROR_CODE']!=1) { this.$toast.error(apidata['MESSAGE']); return; }
                    apidata = apidata['RESULT'];
                    this.khachhang.list = apidata['DS'];
                }
            } catch(ex) { this.$toast.error(''+ex); }
            finally {
                this.loading(false);
