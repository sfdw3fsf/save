<template>
  ...
  >
    <template #modal-header="{ close }">
      <div class="title">
        <span class="icon nc-icon-outline ui-1_settings"></span> Danh sách cột
      </div>
      <div
        @click="close"
                                ...
                                <div class="info-row">
                                    <div class="key w120">
                                        <div class="check-action">
                                            <input type="checkbox" class="check" v-model="chkTuyenCot">
                                            <span class="name">Tuyến cột</span>
                                        </div>
                                    </div>
                                    <div class="value">
                                        <div class="select-custom" id="custom123">
                                          <SelectExt v-model="ddlTuyenCot" :disabled="!chkTuyenCot" :options="this.options.tuyencot"> 
                                            <option disabled value="0">Select one</option> 
                                          </SelectExt> 
                                        </div>
                                    </div>
...
components: { breadcrumb, DataGridEdit },
  ...
  watch: {
    ...
    ddlTuyenCot(newval)
    {
      ...
      {
        TUYENCOT_ID = this.ddlTuyenCot
      }
      this.ddlTuyenCot = newval
      this.NAP_DS_COT(TUYENCOT_ID,this.ddlToQuanLy,null)
    },
    chkTuyenCot(newval)
    {
      this.chkTuyenCot = newval
      this.NAP_DS_COT()
    }
  },
...
onEnter: function() 
{
  ...
  if (this.txtTimKiem == null && this.txtTimKiem == "")
      ...
      this.NAP_DS_COT(null, null, this.txtTimKiem.trim())
},
TimKiem()
{
  let TUYENCOT_ID = ""
...
{
  this.COT_ID = subject.data.COT_ID
},
...
async NAP_DS_TOKT()
{
    ...
    await this.NAP_DS_TUYENCOT()
},

async NAP_DS_TUYENCOT()
{
    const res = await api.lay_ds_tuyen_theo_toql(this.axios, {
      "vtoql_id": this.ddlToQuanLy,
      "loaituyen": 2,//TUYEN_COT
    })
    let items = []
    res.data.data.forEach(function (item) {
      items.push({ id: item.TUYENCOT_ID, text: item.TUYEN_COT })
    })
    this.options.tuyencot = items
    this.ddlTuyenCot = items[0].id
    ...
    await this.NAP_DS_COT(TUYENCOT_ID,this.ddlToQuanLy,null)
...
async NAP_DS_COT(tuyencot_id,toql_id,query)
{
  if (tuyencot_id == null && toql_id == null)
  {
    this.gcDoanCong = "Danh sách cột thỏa mãn điều kiện tìm kiếm"
  }
  else
  {
    this.gcDoanCong = "" 
    if(this.chkTuyenCot == true)
    {
      for(var i =0; i<this.options.tuyencot.length;i++)
      {
        if(this.options.tuyencot[i].id = this.ddlTuyenCot)
        {
          this.gcDoanCong = "Danh sách cột thuộc tuyến: " + this.options.tuyencot[i].text
        }
      }
    }
    else
    {
      this.gcDoanCong = "Danh sách cột chưa xác định tuyến"
    }
  }

  const res = await api.sp_lay_ds_hatang(this.axios, {
    "vtuyen_id": tuyencot_id,
    "vtoql_id": toql_id,
    "vquery": query,
    "vdoituong": "COT"
  })
...
showModal() {
  this.$bvModal.show("DSCot")
},
hideModal() {
    this.$bvModal.hide("DSCot")
},
