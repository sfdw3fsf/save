<template>
  <b-modal ref="popupThongTinHenKhachHang" size="xl" hide-header hide-header-close hide-footer body-class="p-0"
      ...
      <div class="popup-body">
        <div class="grid-stack-box">
          <div class="box-col box-form" style="width: calc(50% - 8px);">
            ...
            <div class="row">
              ...
              <div class="col-sm-12 col-12">
                <div class="info-row">
                  ...
                  <div class="value">
                    <input type="text" id="inputNoiDungHen" ref="inputNoiDungHen"
                      @blur="outFocusInput('inputNoiDungHen')" v-model="input_noidung" class="form-control" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <KTableV1 :columns="columns" :dataSources="dataSources" id="id" tableHeight="200px"
              @dataCheckeds="dataCheckDsThueBao" @onSelectedRow="onSelectedRowDsThueBao" :rowSelectedId="rowSelectedId"
              :allowFilter="false" :allowCheckBox="true" />

          </div>
          <div class="gutter gutter-horizontal" style="width: 16px;"></div>
          <div class="box-col box-form" style="width: calc(50% - 8px);">
            <div class="legend-title">Danh sách hẹn khách hàng</div>
            <KDataGrid :columns="columnsHen" :dataSource="dataSourcesHen" :showColumnCheckbox="false"
              :enable-paging-server="false" :allowPaging="false" :showFilter="false" panelDataHeight="350px"
              :enabledSelectFirstRow="false" @selectedItemsChanged="selectedItemsChangedHen"
...
export default {
  ...
  methods: {
    ...
    async ghiDuLieu() {
      ...
      console.log('log', dulieu.result);
      console.log(isInsert);
      let kq = await this.sp_ins_hdtb_hen(JSON.stringify(dulieu.result), isInsert ? 1 : 2)
      if (kq) {
    ...
    async hienthi_ds_hdtb() {
      try {
        this.loading(true)
        console.log('hdtb_id hieen thi',this.hdtb_id);
        
        this.dataSources = []
        let response = await API.sp_lay_ds_hdtb_theo_hdtb_ban(this.axios, this.hdtb_id)
        this.loading(false)
        if (response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
          ...
          if (this.dataSources.length > 0) {
            ...
            // this.hdtb_id = this.dataSources[0].hdtb_id
            console.log('hdtb_id1', this.hdtb_id);


          }
        } else {
          this.dataSources = []
          this.$toast.error('Thuê bao chưa giao tỉnh thi công')
        }
      } catch (e) {
        this.loading(false)
        if (e.data && e.data.message) {
    ...
    async hienthi_ds_hdtb_hen() {
      try {
        this.loading(true)
        let response = await API.sp_lay_ds_hdtb_hen_bc(this.axios, this.hdtb_id)
        this.loading(false)
        if (response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
          if (response.data.data.thongbao == 'OK') {
            this.dataSourcesHen = response.data.data.ketqua.map((x) => Object.assign(x, {
              group_name: x.ma_tb ? 'Số máy/Acc: ' + x.ma_tb : 'Số máy/Acc: '
            }))
            this.$toast.success('Lấy danh sách hẹn thành công')
          } else {
            this.dataSourcesHen = []
            this.setActiveActions(1)
            this.$toast.error(response.data.data.thongbao)
          }
