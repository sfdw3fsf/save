  ...
  <ejs-dialog :enableResize='true' :allowDragging="true" :visible="false" ref="modal" :header='"Chọn lý do hủy dịch vụ"'
    ...
    <div>
      ...
      <div class="popup-body">
        <div class="box-form">
          <div v-if="banCheo && dichvuvt_id != 9" class="info-row">
            <div class="key w150">Kiểu lắp đặt</div>
            <div class="value">
          ...
          <div v-else-if="banCheo && !kenh_trang && dichvuvt_id == 9">
            <div class="info-row">
              <div class="key w150">Kiểu lắp đặt điểm đầu</div>
              <div class="value">
                <ejs-dropdownlist :showClearButton="false" v-model="cboKieuLapDat.SelectedValue_Dau"
                  :dataSource="cboKieuLapDat.DataSource" :fields="{ text: 'ten_kieuld', value: 'kieuld_id' }"
                  :allowFiltering='true'></ejs-dropdownlist>
              </div>
            </div>
            <div class="info-row">
              <div class="key w150">Kiểu lắp đặt điểm cuối</div>
              <div class="value">
                <ejs-dropdownlist :showClearButton="false" v-model="cboKieuLapDat.SelectedValue_Cuoi"
                  :dataSource="cboKieuLapDat.DataSource" :fields="{ text: 'ten_kieuld', value: 'kieuld_id' }"
                  :allowFiltering='true'></ejs-dropdownlist>
              </div>
            </div>
          ...
          <div class="info-row">
            <div class="key w150">Nhóm hủy</div>
            <div class="value">
...
export default {
  ...
  data: () => ({
    ...
    tinhthicong_id: 0,
    loaitb_id: 0,
    cboKieuLapDat: { DataSource: [], SelectedValue: null, SelectedValue_Dau: null, SelectedValue_Cuoi: null },
    ...
    "kieuLapDat.SelectedValue": {
      handler (val) {
        if (val) {
          ...
          this.cboKieuLapDat.SelectedValue_Dau = val
          this.cboKieuLapDat.SelectedValue_Cuoi = val
        }
      }
    },
  ...
  methods: {
    ...
    onAccept () {
      ...
      if(this.dichvuvt_id != 9){
        let kieuld = {
          kieuld_tc_id: this.cboKieuLapDat.SelectedValue
        }
        this.$emit('onAccept', lydohuy, kieuld)//kieuLd)
      }
      else if(this.dichvuvt_id == 9 && !this.kenh_trang){
        let kieuld = {
          kieuld_tc_id_dau: this.cboKieuLapDat.SelectedValue_Dau,
          kieuld_tc_id_cuoi: this.cboKieuLapDat.SelectedValue_Cuoi,
        }
        if(this.tinh_dau && this.tinh_cuoi){
          if(this.tinh_dau.tinhthicong_id == this.tinh_cuoi.tinhthicong_id){
            if(this.cboKieuLapDat.SelectedValue_Dau != this.cboKieuLapDat.SelectedValue_Cuoi){
              this.$toast.error("Kiểu lắp đặt đầu và cuối phải giống nhau")
              return
            }
            this.$emit('onAccept', lydohuy, kieuld)//kieuld dau va cuoi
          }
          else{
            this.$emit('onAccept', lydohuy, kieuld)//kieuld dau va cuoi
          }
