...
export default {
    ...
    methods:{
        ...
        handleBlur(event, kieu, index){
            if(kieu==1){
                this.ds_tien_km[index].tien=(Math.round((Number(event.target.value)*10/11) * 100+Number.EPSILON) / 100).toString()
                this.ds_tien_km[index].vat=(Math.round((Number(event.target.value)/11) * 100+Number.EPSILON) / 100).toString()
            }else{
                this.ds_tien_km[index].vat=(Math.round((Number(event.target.value)/10) * 100+Number.EPSILON) / 100).toString()
                this.ds_tien_km[index].tongtien=(Math.round((Number(event.target.value)* 11/10) * 100+Number.EPSILON) / 100).toString()
            }
        },
        isNumber(evt) {
        ...
        async lay_ds_tien_kmtt(){
            try{
                this.loading(true)
                let response = await this.axios.post('/web-hopdong/lapdatmoi/lay_ds_tien_kmtt',{
