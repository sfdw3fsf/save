  ...
  methods: {
    ...
    async tsbtnChuyenDB_Click() {
      ...
      if (Number(this.loaitb_id) == LoaiHinhTB.IVAN && this.PHAILAM("RA_KICH_HOAT")) {
        ...
        let check = await this.check_Toantrinh_dvCNTT()
        if (!check) {
          this.$toast.error('Dịch vụ này không thể hoàn thiện ở menu này, hãy giao phiếu sang menu HTHS!')
          return
        }
          await this.tsbtnChuyenDB_Click_1()
      }
    ...
    async check_Toantrinh_dvCNTT () {
      if(this.listbox.cboDichVuVT.value != '15' && this.listbox.cboDichVuVT.value != '14') return true
      try {
        this.loading(true)
