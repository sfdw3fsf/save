    ...
    <div class="modal-content popup-box">
        ...
        <div class="list-actions-top">
            <ul class="list">
                ...
                <li>
                    ...
                    >
                        <span class="icon dropdown-icon one-file-check"></span>Hoàn thiện
                    </a>
                </li>
            </ul>
        </div>
        <div class="popup-body">
            <div class="box-form">
                ...
                <DataGrid v-bind:columns="collumnsPhieu" v-bind:dataSource="options.dsPhieu" ref="danhSach" :showFilter="true" :enabledSelectFirstRow="true" :enableVirtualization="false" :allowPaging="true" :enablePagingServer="false" panelDataHeight="250" :showColumnCheckbox="true"  @selectedItemsChanged="grvDanhSach_FocusedRowChanged" />
            </div>
        </div>
    ...
    methods: {
        ...
        async tsbtnHoanThanh_Click(){
            if(this.data_checked.length == 0){
                this.$toast.warning('Vui lòng chọn thuê bao để hoàn thiện!')
                return
            }
            let newArray = this.data_checked.map(item => ({ "hdtb_id": item.hdtb_id }));
            await api.fn_hths_vattu_tb_docfile(this.axios,{
                    "vds": JSON.stringify(this.changeKeysToUpper(newArray))
                }).then((res) => {
                    if (res.data.data == '1') {
        ...
        grvDanhSach_FocusedRowChanged(values){
            this.data_checked = values;
        },
        close() {
                        ...
                        if (this.kiemTraDuLieuCapVT(columns)) {
                            for (var i = 1; i < data.length; i++) {
                                this.danhsach_excel.push({
                                    ...
                                    MA_VT: data[i][2] != undefined ? data[i][2] : null,
        ...
        kiemTraDuLieuCapVT: function (columns) {
            ...
            if (columns.length <= 1) {
                this.loading(false)
                this.$toast.error('File excel gồm 7 trường: MA_TB, MA_KHO, MA_VT, SOLUONG, SERIAL, MUCDICH, DICHVUVT_ID !')
                return false
            }
            ...
            if (columns[2].toString().toUpperCase() != "MA_VT") {
                this.loading(false)
                this.$toast.error('Tên cột thứ 3 trong file không đúng định dạng : MA_VT')
                return false
            } 
        ...
        async docfile_vattu(dt) {
            ...
            try {
                ...
                let data = this.GetData(await api.docfile_vt_thuebao(this.axios, {
                "vkieu_luachon": this.kieu_luachon,
                "vds_file": JSON.stringify(dt)
                }));
                ...
                if (kq.KETQUA == 0) {
                    this.dt_error = kq.MESSAGE
                    this.$toast.warning("Có " + kq.MESSAGE.length + " bản ghi sai dữ liệu! Bạn hãy kiểm tra trong file!");
                    this.XuatExcel();
                } else {
                    if(this.kieu_luachon == 1){
