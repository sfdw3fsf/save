...
var colLinkQR = function() {
    return {
        template: {
            template: `
                 <div @mouseover="onMouseOver" @mouseleave="onMouseLeave" style="position: relative; overflow: visible;">
                    <button v-if="!data.link_qr" 
                            @click="onViewButtonClick"  
                            class="pad3 mar5 lh14 icon -ap one-eye btn-info btn-view-details" 
                            title="Xem chi tiết">
                    </button>
                    <span v-if="data.link_qr">{{ data.link_qr }}</span>
                    <a v-if="isHovering && data.link_qr" 
                       :href="data.link_qr" 
                       target="_blank"
                       class="link-popup" 
                       style="position: absolute; top: -5px; left: 10%; background-color: #fff; padding: 5px; border: 1px solid #ccc; z-index: 1000;">
                       {{ data.link_qr }}
                    </a>
...
export default {
  ...
  methods: {
    gridCommand_Thaotac: async function (type, dataItem) {
         this.$bvModal
            .msgBoxConfirm("Bạn có muốn tạo QR không?" , { 
              title: 'Xác nhận',
              size: 'sm',
              okTitle: 'Tạo QR',
              cancelTitle: 'Hủy'
            ...
            .then(async (v) => {
              if (v) {
                  ...
                  //if (type == 'viewDetails') { 
                      let arr = []
                      arr.push(dataItem.ma_gd)
                      let param = {
                        "input": arr
                      }
                      const res = await api.fixQRbyMaGD(this.axios, param)
    ...
    tsbtnXoaPhieu_Click: async function() {
      ...
      var input = {
        ...
        nguoidung_id: parseInt(nguoiDungId),
