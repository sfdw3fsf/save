...
<style scoped src="@/modules/QLTN/TaoDanhSachNangGoi/TaoDanhSachNangGoi.scss">
</style>
<script>
import { mapActions, mapState, mapGetters } from "vuex";
import API from "@/modules/QLTN/api/ChuyenPhieuXLNLacHuongAPI.js";
...
export default {
  ...
  methods: {
    ...
    async initModal() {
      if (
        this.danhMucQuanHuyenGetter === undefined ||
        this.danhMucQuanHuyenGetter.length === 0
      )
        await this.getDanhMucQuanHuyen();
      this.danhMucQuanHuyenGetter.forEach((item) => {
        this.params.quan_Options.push({
          id: item.QUAN_ID,
          text: item.TEN_QUAN,
        });
      });
      ...
      if (
        this.danhsachDonViGetter === undefined ||
      ...
      if (this.danhsachDonViGetter.length > 0) {
        this.danhsachDonViGetter.forEach((item) => {
          this.params.donViQL_Options.push({
            id: item.DONVI_ID,
            text: item.TEN_DV,
          });
        });
      ...
      if (
        this.danhSachHinhThucThanhToanGetter === undefined ||
        this.danhSachHinhThucThanhToanGetter.length === 0
      )
    ...
    onClickAccept() {
      if (this.NVTC_grid.items.length > 0)
        this.$emit("acceptChange", {
          ...
          tenNVTC: this.NVTC_grid.selected.TENNV_TC,
        });
      else this.$emit("acceptChange", null);
      this.close();
    },
    recordDoubleClick: function (i, item) {
      this.$emit("acceptChange", {
        quanId: this.params.quan_Selected,
        nhanVienTC_Id: this.NVTC_grid.selected.NHANVIEN_ID,
        maNVTC: this.NVTC_grid.selected.MANV_TC,
    ...
    async getDanhSachPhuongTheoQuan(quanId) {
      try {
        ...
        let response = await API.getDanhSachPhuongTheoQuan(
          this.axios,
          apiParams
        );
        if (
          ...
          response.data.data.forEach((item) => {
            this.params.phuong_Options.push({
              id: item.PHUONG_ID,
    ...
    async getDanhSachPhoApKhuTheoPhuong(phuongId, nhomPho) {
      try {
        this.loading(true);
        if (nhomPho === 1) this.params.pho_Options = [];
        else if (nhomPho === 2) this.params.ap_Options = [];
        else this.params.khu_Options = [];
        let apiParams = {
          pPhuongId: phuongId,
          pNhomPhoId: nhomPho,
        };
        let response = await API.getDanhSachPhoApKhuTheoPhuong(
          this.axios,
          apiParams
        );
        if (
    ...
    async getDanhSachDacDiem() {
      try {
        ...
        if (
          ...
          response.data.data.forEach((item) => {
            this.params.dacDiem_Options.push({
              id: item.DACDIEM_ID,
              text: item.DACDIEM,
            });
          });
    ...
    async getDanhBaNVTNTheoDiaDanh(
      maNVTC = "",
      tenNVTC = "",
      isKeyPress = false
    ) {
      try {
        ...
        let apiParams = {
          ...
          tenNVTC: tenNVTC,
          quanId:
            this.params.quan_Checked & !isKeyPress
              ? this.params.quan_Selected
              : null,
          phuongId:
            this.params.phuong_Checked & !isKeyPress
              ? this.params.phuong_Selected
              : null,
          phoId:
            this.params.pho_Checked & !isKeyPress
              ? this.params.pho_Selected
              : null,
          apId:
            this.params.ap_Checked & !isKeyPress
              ? this.params.ap_Selected
              : null,
          khuId:
            this.params.khu_Checked & !isKeyPress
              ? this.params.khu_Selected
              : null,
          dacDiemId:
            this.params.dacDiem_Checked & !isKeyPress
              ? this.params.dacDiem_Selected
              : null,
        };
        let response = await API.getDanhBaNVTNTheoDiaDanh(
          this.axios,
          apiParams
        );
        if (
  ...
  watch: {
    "params.quan_Checked": async function (newVal, oldVal) {
      if (!newVal) this.params.phuong_Checked = false;
      await this.getDanhBaNVTNTheoDiaDanh();
    },
    ...
    "params.phuong_Checked": async function (newVal, oldVal) {
      ...
      if (this.params.quan_Checked)
        //Incase uncheck Quan will emit an event to checked Phuong
        await this.getDanhBaNVTNTheoDiaDanh();
    },
    ...
    "params.pho_Checked": async function (newVal, oldVal) {
      ...
      if (this.params.phuong_Checked) {
        //Incase uncheck Phuong will emit an event to checked Pho
        await this.getDanhBaNVTNTheoDiaDanh();
      }
    },
    ...
    "params.ap_Checked": async function (newVal, oldVal) {
      await this.onGetDacDiem();
      if (this.params.phuong_Checked) {
        //Incase uncheck Phuong will emit an event to checked Ap
        await this.getDanhBaNVTNTheoDiaDanh();
      }
    },
    ...
    "params.khu_Checked": async function (newVal, oldVal) {
      await this.onGetDacDiem();
      if (this.params.phuong_Checked) {
        //Incase uncheck Phuong will emit an event to checked Khu
        await this.getDanhBaNVTNTheoDiaDanh();
      }
    },
