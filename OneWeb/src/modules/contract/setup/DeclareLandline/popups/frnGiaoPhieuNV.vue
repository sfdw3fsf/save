              ...
              if (ds[j]["so_dt"]) {
                try {
                  try {
                    ...
                    if (!this.vdsphieu) {
                      ...
                      for (let i = 0; i < this.vdsphieu.length; i++) {
                        ...
                        if([7777, 9999].includes(this.huonggiao_id) && rs_noidung.data.data == null) {
                          throw 'loi'
                        }
                  ...
                  if (vdsnoidung.length == 0) {
                    console.log('tichhop/sms/send - vnoidung')
                    console.log(vnoidung)
                    await this.axios.post("/tichhop/sms/send", {msisdn: ds[j]["so_dt"],content: vnoidung});
                  } else {
                    for (let i = 0; i < vdsnoidung.length; i++) {
                      console.log('tichhop/sms/send - vdsnoidung[i]')
                      console.log(vdsnoidung[i])
