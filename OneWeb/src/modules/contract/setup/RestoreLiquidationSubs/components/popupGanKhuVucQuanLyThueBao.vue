...
<script>
/* eslint eqeqeq: 'off', quotes: 'off' */
import VueSelect from '@/modules/search/subscriber/components/form/VueSelect.vue'
export default {
  components: { VueSelect },
  name: 'popupGanKhuVucQuanLyThueBao',
  props: ['modalId', 'data', 'isStrictMode', 'donvi_id', 'hdtb_id'],
  watch: {
  ...
  methods: {
    ...
    async chapnhan () {
      ...
      if (this.options.khuvuc.filter(x => x.khuvuc_id == this.model.khuvuc_id).length > 0) {
        this.$emit('chapnhan', {
          ma_kv: this.options.khuvuc.filter(x => x.khuvuc_id == this.model.khuvuc_id)[0]['ten_kv'],
          hdtb_kv_data: data,
          khuvuc_id: this.model.khuvuc_id
        })
