              ...
              <div class="tab-pane active" id="tabA">
                ...
                <div class="table-content">
                  <DataGrid
                    panelDataHeight="400px"
                    ref="gridXuat"
                    :columns="gridXuat.header"
                    :dataSource="gridXuat.list"
                    :showColumnCheckbox="false"
                    :showFilter="true"
                    :allowPaging="false"
                    :enablePagingServer="false"
  ...
  computed: {
    gridVatTuHeader() {
      return this.gridVatTu.header.filter((e) => {
        ...
        if (e.fieldName == 'CHATLUONG') return ['HNI', 'HCM'].includes(this.$root.token.getMaTinh())
  ...
  methods: {
    ...
    async ds_vt_xuat() {
      ...
      for (let i = 0; i < this.dtLuuVT.length; i++) {
        ...
        console.log('i =' + i)
        console.log(_i.VATTU_ID + '||' + _i.LOHANG)
        for (let k =    0; k < this.dtVatTu.length; k++) {
          let _j = this.dtVatTu[k]
          console.log('k = 0')
          console.log(_j.VATTU_ID + '||' + _j.LOHANG)

          console.log(_i.VATTU_ID == _j.VATTU_ID && _i.LOHANG == _j.LOHANG)
          if (_i.VATTU_ID == _j.VATTU_ID && _i.LOHANG.toUpperCase() == _j.LOHANG.toUpperCase()) {
            console.log('-match')
