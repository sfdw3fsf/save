...
export default {
  ...
  computed: {
    ...
    CboOutPut_Ten: {
      get() {
        console.log(
          this.$refs.cboVatTu.getDataByValue(this.params.p_VatTu).TEN_VT
        );
        return (
          this.$refs.cboVatTu.getDataByValue(this.params.p_VatTu).TEN_VT || ""
        );
  ...
  methods: {
    ...
    async frmNhapText_3(tieuDe, x) {
      ...
      // this.params.rdioCKb = x
      this.Visible.itemIMS = x==1 ? false : true;      
    },
    // Nhập serial thu hồi
    ...
    async btnXacNhan_Click() {
      this.$emit("onXacNhan", this.params.txtNhapText);
      if (!this.params.p_txtNhapText && this.Visible.spMuc == false) {
        ...
        !this.Visible.txtNhapText &&
        !this.Visible.spMuc &&
        this.Visible.dtpNgay &&
        !this.Visible.txtTenVT
      ) {
        this.Close();
        this.DialogResult = DialogResultObj.OK;
        await this.$parent.$parent.XacNhan_frmNhapText();          
