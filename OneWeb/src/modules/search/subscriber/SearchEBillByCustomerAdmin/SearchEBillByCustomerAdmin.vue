<template src="./SearchEBillByCustomerAdmin.html"></template>
<style src="./SearchEBillByCustomerAdmin.scss"></style>
<script>
import SearchEBillByCustomerAdminAPI from './SearchEBillByCustomerAdminAPI'
import breadcrumb from '@/components/breadcrumb'
import moment from 'moment'
import EventBus from '@/utils/eventBus'
import popupTimKiem from './Popup/TimKiemKhachHang.vue'
import Vue from 'vue'
...
//
export default {
    components: { breadcrumb, popupTimKiem, VuePdfApp},
  ...
  methods: {
    ...
    // Chức năng in
          ...
          this.html = null
            ...
            SearchEBillByCustomerAdminAPI.GET_BI_PARAMS(this.axios, {}).then(response => {
        ...
        async viewReport (type) {
            ...
            console.log(request);
            await SearchEBillByCustomerAdminAPI.RUN(this.axios, request)
                .then((response) => {
    ...
    async postDownloadInvPDFFkeyNoPay(fkey) {
            try {
                ...
                let response = await SearchEBillByCustomerAdminAPI.postDownloadInvPDFFkeyNoPay(this.axios, apiBody);
    ...
    async onDownloadInvHandler(item) {
     ...
     async postDownloadInvFkeyNoPayXml(fkey) {
            try {
                ...
                let response = await SearchEBillByCustomerAdminAPI.postDownloadInvFkeyNoPayXml(this.axios, apiBody);
                if (response.data.errorCode == 0) {
                    return response.data.data;
                } else return null;
            }
     ...
     async postDownloadXml(fkey) {
        ...
        async getInvViewNoPay(invToken) {
            try {
               ...
               // console.log(apiBody);
                let response = await SearchEBillByCustomerAdminAPI.getInvViewNoPay(this.axios, apiBody);
                if (response.data.errorCode == 0) {
                    return response.data.data;
                } else return null;
            }
    ...
    ThemBienDong() {
      var pdata = {};
      pdata.vuser_name = this.$auth.getUserName();
      pdata.vcomputer_name = this.$auth.getNguoiDungID();
      pdata.vaction_name = "";
      pdata.vkey_parameter = this.magd;
      pdata.vserver_name = this.$auth.getNguoiDungID();;
      pdata.vnote = "downloadInvNoPay_V2";
      console.log(pdata);
      SearchEBillByCustomerAdminAPI.them_bien_dong(this.axios, pdata).then((response) => {
        if(response && response.data) {
           this.$toast.success('Cập nhật biến động thành công')
        }
      });
    },
    btnSearchClicked() {
      ...
      if(this.magd == false){
        ...
        //  this.Clear();
          ...
          SearchEBillByCustomerAdminAPI.tracuu_ma_gd(this.axios, pdata).then((response) => {
      ...
      if(this.fkey == false){
        ...
        console.log(pdata);
        SearchEBillByCustomerAdminAPI.tra_cuu_fkey(this.axios, pdata).then((response) => {
          if(response.data.errorCode != 0 || response.data.data == "<Data></Data>"){
              this.$toast.error('Không tìm thấy hóa đơn')
          }
          else {
      ...
      if(this.token == false){
        ...
        console.log(pdata);
        SearchEBillByCustomerAdminAPI.tracuu_token(this.axios, pdata).then((response) => {
            ...
            const xmlDoc = this.parseXmlDoc(response.data.data)     
              ...
              //  console.log(Json.HDon.Fkey['#text']);
                 ...
                 // console.log(pdata);
                  SearchEBillByCustomerAdminAPI.tra_cuu_fkey(this.axios, pdata).then((response) => {
                    if(response.data.errorCode != 0 || response.data.data == "<Data></Data>"){
                        this.$toast.error('Không tìm thấy thông tin về hóa đơn')
                    }
                    else {
                    if(response && response.data) {
                        const xmlDoc = this.parseXmlDoc(response.data.data)
    ...
    convert(a){
      for(var i=0; i<a.length; i++){
                  a[i].index['#text'] = i+1;
                  // Json.Data.Item[i].xemchitiet = '<a href="#">Xem chi tiết</a>';
