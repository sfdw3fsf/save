...
import { DBConstants as DatabaseConstants } from '@/const/enums'
import ExportDataModal from '@/modules/contract/profile/ExportData/ExportDataModal.vue'
import ThayDoiThongTinModal from './ThayDoiThongTin/ThayDoiThongTin.vue'
export default {
  components: { breadcrumb,ExportDataModal,ThayDoiThongTinModal },
  name: 'SearchAccount',
  props: [ 'isPopup','vma_tb','vdichvuvt_id','vISKHDN','mathuebao'],
  watch: {
    
  },
  created() {
    if (this.mathuebao.length > 0) {
      this.listbox.dk_tracuu.dk1.value_text = this.mathuebao
    }
  },
  async mounted () {
    ...
    else this.ISKHDN = this.$root.token.getPhanVungID()==97;
    await SearchAccountAPI.get_dichvu_vienthong(this.axios, {}).then(
        (response) => {
    ...
    await SearchAccountAPI.get_kieu_taikhoan(this.axios, {params:{nhomId:(2)}}).then((response) => {
      if(response && response.data && response.data.data && response.data.error_code && response.data.error_code === 'BSS-00000000' && response.data.data) {
    ...
    await SearchAccountAPI.get_kieu_taikhoan(this.axios, {params:{nhomId:(98)}}).then((response) => {
      if(response && response.data && response.data.data && response.data.error_code && response.data.error_code === 'BSS-00000000' && response.data.data) {
        this.listbox.dk_tracuu.list_khdn = response.data.data;
        if(this.listbox.dk_tracuu.list_khdn && this.listbox.dk_tracuu.list_khdn.length>0) {
          this.listbox.dk_tracuu.dk1_khdn.value = this.listbox.dk_tracuu.list_khdn[0].ID;
          this.listbox.dk_tracuu.dk2_khdn.value = this.listbox.dk_tracuu.list_khdn[0].ID;
          this.listbox.dk_tracuu.dk3_khdn.value = this.listbox.dk_tracuu.list_khdn[0].ID;
        }
        //this.filterbox.dichvu_vienthong.list = response.data.data;
      }
    });
    //donvi_dl_id -> đơn vị quản lý (phân quyền đơn vị của người dùng)
    await SearchAccountAPI.get_donvi_ql_tt(this.axios, {}).then((response) => {
      if(response && response.data && response.data.data && response.data.error_code && response.data.error_code === 'BSS-00000000' && response.data.data) {
    ...
    if (this.mathuebao) await this.SearchClient();
  ...
  computed: {
    ...
    searchCondition_KHDN() {
      var dieukiens = [];
      for(var i=1;i<=3;i++)
        if(this.listbox.dk_tracuu[`dk${i}_khdn`].isEnabled && this.listbox.dk_tracuu[`dk${i}_khdn`].value && this.listbox.dk_tracuu[`dk${i}_khdn`].value_text)
          dieukiens.push({ID:this.listbox.dk_tracuu[`dk${i}_khdn`].value,GIATRI:this.listbox.dk_tracuu[`dk${i}_khdn`].value_text});
      return {
        JS_DIEUKIEN:dieukiens,
      };
    }
  },
  data () {
    return {
      ...
      gridbox: {
        khachhang: {
          ...
          cols: [
            {fieldName:'stt',headerText:'STT',width: "80" , allowFiltering: true, },
          ...
          cols_khdn: [
            {fieldName:'stt',headerText:'STT',width: "80" , allowFiltering: true, },
            {fieldName:'ma_kh',headerText:'Mã KH', allowFiltering: true, },
            {fieldName:'full_name',headerText:'Tên khách hàng', allowFiltering: true },
            {fieldName:'mst',headerText:'Mã số thuế',allowFiltering: true, },//format:"number",textAlign:"right",
            {fieldName:'addr',width: "300",headerText:'Địa chỉ', allowFiltering: true },
        ...
        thanhtoan: {
          ...
          cols: [
            {fieldName:'ma_tt',width: "100",headerText:'Mã TT', allowFiltering: true, },
            {fieldName:'matt_cu', width: "120",headerText:'Mã TT cũ', allowFiltering: true, },
            {fieldName:'matb_dd',width: "120",headerText:'Mã đại diện',allowFiltering: true, },
        ...
        thuebao: {
          ...
          cols: [
            {fieldName:'ma_tt',width: "120",headerText:'Mã TT', allowFiltering: true, },
            {fieldName:'ma_tb',width: "120",headerText:'Số máy/Acc', allowFiltering: true, },
            {fieldName:'ma_lt',width: "120",headerText:'Số ảo',allowFiltering: true, },
            ...
            {fieldName:'ma_tb_tinh',headerText:'Mã VTT', allowFiltering: true },
            {fieldName:'ma_vnpti',headerText:'Mã VNPT-I', allowFiltering: true },
  ...
  methods: {
      ...
      await SearchAccountAPI.get_tracuu(this.axios, this.searchCondition).then((response) => {
        ...
        if(response && response.data && response.data.data && response.data.error_code && response.data.error_code === 'BSS-00000000' && response.data.data.length>0) {
          ...
          if(this.gridbox.khachhang.list && this.gridbox.khachhang.list.length>0)
            this.grdKhachHangChanged(this.gridbox.khachhang.list[0]);
    ...
    SearchClient_KHDN() {
      ...
      //SearchAccountAPI.get_khachang_theo_matb(this.axios, {params:{maTB:'canh479',donViDLId:0,dichVuVTId:4}}).then((response) => {
      this.loading(true);
      SearchAccountAPI.post_sp_hienthidanhsachdkh_khdn_frmtracuudanhba(this.axios, {
        p_ds_para:JSON.stringify(this.searchCondition_KHDN),
        p_ds_ttnd:"{}",
        // JSON.stringify({
        //   NHANVIEN_ID:,
        //   DONVI_ID: ,
        ...
        this.gridbox.khachhang.list_khdn=[];
        if(response && response.data && response.data.data && response.data.error_code && response.data.error_code === 'BSS-00000000') {
          var apidata = JSON.parse(response.data.data);
          if(apidata['ERROR_CODE']!=1) {
            this.$toast.error(`Có lỗi khi tìm kiếm dữ liệu: ${apidata['MESSAGE']}`);
          }
          else if(apidata['RESULT'] && apidata['RESULT']['DTHDKH_KHDN'] && apidata['RESULT']['DTHDKH_KHDN'].length>0) {
            this.gridbox.khachhang.list_khdn = this.LowerCasePropertyList(apidata['RESULT']['DTHDKH_KHDN']);
            for(var i=0; i<this.gridbox.khachhang.list_khdn.length;i++)
              this.gridbox.khachhang.list_khdn[i].stt = i+1;
            if(this.gridbox.khachhang.list_khdn && this.gridbox.khachhang.list_khdn.length>0)
            this.gridbox.khachhang.value_khdn = this.gridbox.khachhang.list_khdn[0]; // this.grdKhachHangChanged(this.gridbox.khachhang.list[0]);
            this.$toast.success("Tìm thấy "+this.gridbox.khachhang.list_khdn.length+" thuê bao thoả mãn điều kiện tìm!")
          }
          else {
            this.$toast.error("Không tìm thấy dữ liệu thoả mãn yêu cầu tìm kiếm!");
            this.gridbox.khachhang.list_khdn = [];  
    ...
    grdKhachHangChanged(obj) {
      ...
      Promise.all([api1,api2,]).then(()=>{
        // this.loading(false);
        // if(this.clickcount>1) this.closeForm();
        // this.clickcount = 0;
      });
    },
    ...
    closeForm() {
      if(this.currentkhTab!='khtab2'){
        console.log(this.gridbox.khachhang.value.ma_kh);
        this.$emit('form-close',
        
      { ma_tb:this.gridbox.thuebao.value.ma_tb,
        ma_kh:this.gridbox.khachhang.value.ma_kh,
        ma_tt:this.gridbox.thanhtoan.value.ma_tt,
        thuebao:this.gridbox.thuebao.value, 
        khachhang:this.gridbox.khachhang.value}); //THANGDM1 changed 2021-11-04 them thong tin
        }
      else this.$emit('form-close',{ ma_kh:this.gridbox.khachhang.value_khdn.ma_kh,khachhang:this.gridbox.khachhang.value_khdn});
    },
    terminateForm() {
      this.$emit('form-close');
    },
    thaydoitt_success(){
      ...
      this.$refs.thaydoithongtinModal.hideModal();
    },
    async tsbtnThayDoiTT_Click(){
      this.$refs.thaydoithongtinModal.dt_tb = this.$refs.tblThueBao.getSelectedRecords()[0];
      console.log(this.$refs.tblThueBao.getSelectedRecords()[0]);
      this.$refs.thaydoithongtinModal.dt_kh = this.$refs.tblKhachHang.getSelectedRecords()[0];
      this.$refs.thaydoithongtinModal.showModal();
    },
