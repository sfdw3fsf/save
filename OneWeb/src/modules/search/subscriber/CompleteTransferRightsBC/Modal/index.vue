...
export default {
    ...
    methods: {
        ...
        HT_Kieu_KH_Combobox: async function (){
            let res = await this.$root.context.get('web-ccdv/hoanthienhschuyenquyen/lay_ds_kieu_kh')
        ...
        HT_Nhom_KH_Combobox: async function (kieu_kh){
            let res = await this.$root.context.get('web-ccdv/hoanthienhschuyenquyen/lay_ds_nhom_kh', {
              kieu_kh: kieu_kh
            })
        ...
        Lay_ds_loaikh: async function(nhomKH){
            let res = await this.$root.context.get('web-ccdv/hoanthienhschuyenquyen/lay-ds-loaikh', {
              nhomloaikh_id: nhomKH
            })
            return res.data;
        },
        changeCBO: async function(){
            let res = await this.$root.context.get('web-ccdv/hoanthienhschuyenquyen/lay_ds_nhom_kh', {
              kieu_kh: this.modelCbo.cboKieuKH
            })
            this.modelCbo.vcboNhomKH = res.data
            this.modelCbo.cboNhomKH = res.data[0].NHOMLKH_ID
            this.modelCbo.cboNhomKH = res.data[0].NHOMLKH_ID
        ...
        async onShownModal() {
            ...
            try {
                ...
                let idx_loaikh_id = this.dtList.findIndex(i => i.LOAIKH_ID == this.dataSend.loaikh_id)
                setTimeout(() =>{
                  console.log(idx_loaikh_id)
                  if (idx_loaikh_id > -1) this.$refs.gridKH.selectRow(idx_loaikh_id)
                },500)
