...
export default {
  components: { breadcrumb, KTable, ActionTop, SearchContractModal,searchAccount, ChonPort , KRequiredMarker },
  props:{
        ma_tb:{
            type:String,
            default:''
  ...
  async created() {
      ...
      try{
        this.loading(true)
        await this.load_DS_DichVu_VT();
        await this.load_DS_LoaiHinh_TB();
        await this.load_DS_Router_Cu();
        await this.load_DS_Router_Moi();
        await this.load_DS_LoaiKenh_cu();
        await this.load_DS_LoaiKenh_moi();
        await this.load_loai_node();
        await this.load_DS_LoaiThietbi_cu();
        await this.load_loai_cap_cu_moi();
        this.loading(false)
        this.khoiTaoDauVao()
      }catch(e){
        this.loading(false)
  ...
  methods: {
    ...
    KiemTraDuLieu(){
      // if(!this.ThongTinMoi.txtportFE_moi||this.ThongTinMoi.txtportFE_moi.trim()==''){
      //   this.$toast.error('Hãy nhập Port PE')
      //   this.$refs['ThongTinMoi.txtportFE_moi'].focus()
      //   return false
      // }
      if(!this.ThongTinMoi.txtTramTB_moi||this.ThongTinMoi.txtTramTB_moi.trim()==''){
        this.$toast.error('Hãy nhập Trạm T.Bị bằng cách chọn mục Dslam')
        this.$refs['ThongTinMoi.txtTramTB_moi'].focus()
        return false
      }
      if(!this.ThongTinMoi.txtSvlan_moi||this.ThongTinMoi.txtSvlan_moi.trim()==''){
        this.$toast.error('Hãy nhập SvLan')
        this.$refs['ThongTinMoi.txtSvlan_moi'].focus()
        return false
      }
      if(!this.ThongTinMoi.txtCVlan_moi||this.ThongTinMoi.txtCVlan_moi.trim()==''){
        this.$toast.error('Hãy nhập CvLan')
        this.$refs['ThongTinMoi.txtCVlan_moi'].focus()
        return false
      }
      if(this.params.p_divhvuvt==8){
        if(!this.ThongTinMoi.ttxWanID_moi||this.ThongTinMoi.ttxWanID_moi.trim()==''){
          this.$toast.error('Bắt buộc nhập WanIP')
          this.$refs['ThongTinMoi.ttxWanID_moi'].focus()
          return false
        }
        if(!this.ThongTinMoi.txtLanIP_moi||this.ThongTinMoi.txtLanIP_moi.trim()==''){
          this.$toast.error('Bắt buộc nhập LanIP')
          this.$refs['ThongTinMoi.txtLanIP_moi'].focus()
          return false
        }
      }
      return true
    ...
    async btnCapNhat_click() {
      ...
      if(result.code==1){
        ...
        let datatskt={
          ...
          "doicap_p": this.ThongTinMoi.txtdoicapngon_moi,
          "hopcap_p": this.ThongTinMoi.txthopcapngon_moi,
          "cap_goc": this.ThongTinMoi.txtCapgoc_moi,
          "cap_p": this.ThongTinMoi.txtcapngon_moi,
          "doicap_goc": this.ThongTinMoi.txtdoicaogoc_moi,
          "tucap_goc": this.ThongTinMoi.txtTucapgoc_moi,
          "culy": this.ThongTinMoi.txtculy_moi,
          ...
          "dslam": this.ThongTinMoi.txtdslam_moi,
          "pe_id":this.params.p_router_moi,
          "trangthai_dl": this.params.TrangThaiID,
          "thuebao_cha_id": this.thuebao_cha_id,
          "hdtb_cha_id": this.hdtb_cha_id,
          "huong_kn": this.vhuong_kn,
          "thietbidc_id": this.params.p_loaitbi_moi,
          "site_id": this.ThongTinMoi.txtSiteID_moi,
          "ma_pl": this.ThongTinMoi.txtSoPLHD_moi,
          "loaikenh_id": this.params.p_cboLoaiKenh_moi,
          "loainode_id": this.params.p_nodechinh_moi,
          "tinh_kn_id": this.tinh_kn_id,
          "diachi_kn": this.vdiachi_kn
        }
        
        //let result1=await this.capnhat_doi_tskt_mgwan_v4(datatskt)
        let result1=await this.capnhat_doi_tskt_mgwan_v5({
          p_ds_para:JSON.stringify(datatskt)
        })
        if(result1.code==1){
    ...
    khoiTaoDauVao(){
      this.txtMaTB=this.ma_tb
      this.params.TrangThaiID=this.trangthai_dl
      this.params.p_divhvuvt = this.propDichvuvt_id
    ...
    async capnhat_doi_tskt_mgwan_v4(data){
        try{
            this.loading(true)
            let response=await API.capnhat_doi_tskt_mgwan_v4(this.axios, data)
            this.loading(false)
            if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000') {
                return {
                  code:1,
    ...
    async capnhat_doi_tskt_mgwan_v5(data){
        try{
            this.loading(true)
            let response=await API.capnhat_doi_tskt_mgwan_v5(this.axios, data)
            this.loading(false)
            if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000') {
                return {
                  code:1,
