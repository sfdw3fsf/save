...
export default {
  ...
  methods: {
    ...
    getGroupedCaptionTemplate: function() {
      ...
      if (cs != null) {
        for (let i = 0; i < cs.length; i++) {
          const c = cs[i]
          if (!c.headerText.toString().trim() == '') {
            rs = '${key} - ${count} Bản ghi'
            break
          }else{
            // console.log("test:",c.captionTemplate)
    ...
    grid_RowSelected: function(args) {
      ...
      // Kiểm tra nếu người dùng chọn group
      /*
      if (args.data.hasOwnProperty('sdt_tt')) { 
    ...
    grid_DataBound: function(args) {
      ...
      // Kiểm tra nếu groupModule tồn tại trước khi gọi collapseAll
    if (this.grid.ej2Instances.groupModule) {
        this.grid.ej2Instances.groupModule.collapseAll();
    }
