...
import { L10n, classList, addClass, removeClass } from "@syncfusion/ej2-base";
...
export default {
  ...
  methods: {
    ...
    getGroupedCaptionTemplate: function(){
      ...
      if(cs != null){
        if (cs.length == 1 && !(this.checkIsNull(cs[0].headerText) || cs[0].headerText.toString().trim() == "")) {
          return cs[0].headerText
        }
    ...
    grid_RowSelected: function (args) {
      if (this.addClassWhenRowChecked && args.row != null) {
        let isArray = Array.isArray(args.row)
        if (!isArray) {
          if (args.row.cells != null) {
            args.row.cells.forEach(cell => cell.classList.add(this.addClassWhenRowChecked))
          }
        }
        else {
          args.row.forEach(row => {
            if (row && row.cells != null) {
              row.cells.forEach(cell => cell.classList.add(this.addClassWhenRowChecked))
            }
          })
        }
      }
    ...
    grid_RowDeselected: function (args) {
      if (this.addClassWhenRowChecked && args.row != null) {
        ...
        if (!isArray) {
          if (args.row.cells != null) {
            args.row.cells.forEach(cell => cell.classList.remove(this.addClassWhenRowChecked))
          }
        }
        else {
          args.row.forEach(row => {
            if (row && row.cells != null) {
              row.cells.forEach(cell => cell.classList.remove(this.addClassWhenRowChecked))
            }
          })
        }
      }
    ...
    enableCheckAll: function () {
      var input = this.grid.getHeaderContent().querySelector('.e-checkselectall');
      if (input) {
        var spanEle = input.nextElementSibling;
        if (spanEle && spanEle.parentElement) {
