<template src="./frmKhaoSatKH_Chua_NT.html">
...
export default {
    ...
    methods: {
        ...
        openDialog() {
            this.$refs.frmKhaoSatKH_Chua_NT.show();
        },
        closeDialog() {
            this.$emit("onHideKhaoSat");
            this.$refs.rmKhaoSatKH_Chua_NT.hide();
        },
        async onShow() {
            try {
                ...
                if (response.data.error_code === "BSS-00000000") {
                    ...
                    if ( this.pDsThongTinBaoHong && this.pDsThongTinBaoHong.length > 0 ) {
                        ...
                        if (this.pIsSuaPhieu)
                            ...
                            console.log('dsThongTinTBChuaTraLoi', dsThongTinTBChuaTraLoi)
                        ...
                        if ( dsThongTinTBChuaTraLoi && dsThongTinTBChuaTraLoi.length > 0 ) {
                            ...
                            if (this.pChkKTL) {
                                this.pIsNew = true;
                                console.log('this.CHOPHEP_SUA_OB_KHONG_TL', this.CHOPHEP_SUA_OB_KHONG_TL)
                ...
                if (this.pIsNew) {
                    ...
                    if (await this.kiemTraDuLieu(true)) {
                        ...
                        let rs = await this.$root.context.post(
                            "/web-cskh/api/khao-sat-kh-lm/chua-nt/ghi-lai",
                            data
                        );
                        console.log(data);
        ...
        async kiemTraDuLieu(isThemMoi) {
            ...
            if (!this.pChkKTL) {
                ...
                for (let chIndex = 0; chIndex < this.ketQuaKS.length; chIndex++) {
                    ...
                    console.log('cauHoiPhu', cauHoiPhu)
                    let chpDcTL = false;
                    for (
                        let chpIndex = 0;
        ...
        async onDieuLaiClicked() {
            try {
                ...
                // }
                if (this.pIsNew) {
                    this.$bvModal.show("popup-confirmDieuLai_Chua_NT");
                }
                else {
        ...
        async onDieuLaiConfirmed() {
            try {
                ...
                if (await this.kiemTraDuLieu(true)) {
                    ...
                    let rs = await this.$root.context.post(
                        "/web-cskh/api/khao-sat-kh-lm/chua-nt/dieu-lai",
                        data
                    );
                    if (rs.error_code === "BSS-00000000") {
                        if (rs.data.msgError) {
                            this.$root.toastSuccess(rs.data.msgError);
                        } else
