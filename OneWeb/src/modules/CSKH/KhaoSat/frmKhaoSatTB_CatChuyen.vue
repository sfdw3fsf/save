...
<ejs-dialog
        :enableResize="true"
        :allowDragging="true"
        ref="frmKhaoSatKHTB_CatChuyen"
        :header="'Khảo sát thuê bao cắt chuyển'"
    ...
    <div class="">
        <div class="list-actions-top">
            <ul class="list">
                ...
                <li>
                    <a href="javascript: void(0)" data-toggle="tooltip" title="Lịch sử hỏng">
                        ...
                        <div class="legend-title">
                            <div class="pull-left">
                                ...
                                <div class="check-action normal red marl10">
                                    <input type="checkbox" class="check" v-model="pCboLyDoKTL"/>
                                    <span class="name">Không trả lời</span>
                                </div>
                            </div>
                            <div class="pull-right">
                                <div class="select-custom" style="width: 400px;">
                                    <b-form-select
...
export default {
    ...
    methods: {
        async openDialog() {
            this.$forceUpdate()
            this.$refs.frmKhaoSatKHTB_CatChuyen.show()
            ...
            try {
                ...
                await Promise.all([
                    ...
                    this.hienThiCanhBaoThietHai(),
                    this.getDanhSachLanGoi(),
                    this.getTBTraLoi(),
        ...
        async getLoaiDichVuVienThong(id)
        {
            try
            {
                let dichVuVTResponse = await this.$root.context.get("/web-cskh/api/danh-muc/dich-vu-vt")
                if (dichVuVTResponse.error_code == API_SUCCESS)
                {
                    return dichVuVTResponse.data.filter(x => x.DICHVUVT_ID == id).TEN_DVVT
                }
            }
            catch (e)
            {
        ...
        async getDanhSachLanGoi()
        {
            this.pDsLanGoi = []
            try
            {
                let lanGoiResponse = await this.$root.context.get("/web-cskh/api/xac-nhan-phan-hoi-chat-luong/ds-lan-goi-chua-tra-loi", {id: this.vdkccId, loai: 4})
                if (lanGoiResponse.error_code == API_SUCCESS)
                {
                    this.pDsLanGoi = lanGoiResponse.data.data || []
                }
            }
            catch (e)
            {
        ...
        async getTBTraLoi()
        {
            ...
            try
            {
                let tbtlResponse = await this.$root.context.get("/web-cskh/api/xac-nhan-phan-hoi-chat-luong/ds-thue-bao-tra-loi", {id: this.vdkccId, loai: 6})
                if (tbtlResponse.error_code == API_SUCCESS)
                {
                    this.dtTBTL = tbtlResponse.data.data || []
                    if (this.dtTBTL.length > 0)
                    {
                        this.tbtlId = this.dtTBTL[0].TBTL_ID
                        this.pTxtTenNguoiTL = this.dtTBTL[0].NGUOI_TL
                        this.pTxtSoLH = this.dtTBTL[0].DIENTHOAI_LH
                        this.pTxtGhiChu = this.dtTBTL[0].GHICHU
                        this.pCboLyDoKTL = this.dsTBTL[0].TRANGTHAI == 0
                        if (this.pCboLyDoKTL) {
                            this.pIsNew = true
                            this.tbtlId = 0
                            if (this.dsTBTL[0].LYDOKTL_ID) {
                                this.optLyDoKoTraLoiSelected = this.dsTBTL[0].LYDOKTL_ID
                            }
                        }

                        if (this.dsTBTL[0].NGAYHEN_DEN)
                        {
        ...
        async hienThiThongTinTB()
        {
            try
            {
                let thongTinTBCatChuyenResponse = await this.$root.context.get("/web-cskh/api/khao-sat-tb-cc", {dkccId: this.vdkccId})
                if (thongTinTBCatChuyenResponse.error_code == API_SUCCESS)
                {
                    if (thongTinTBCatChuyenResponse.data.data && thongTinTBCatChuyenResponse.data.data.length > 0)
                    {
                        let record = thongTinTBCatChuyenResponse.data.data[0]
                        this.pTxtThueBaoId = record.THUEBAO_ID
                        this.pTxtMaTB = record.MA_TB
                        this.pTxtTenTB = record.TEN_TB
                        this.pTxtDiaChiLD = record.DIACHI_LD
                        this.pTxtDienThoaiLH = record.DIENTHOAI_LH
                        this.pTxtLoaiHinhTB = await this.getLoaiDichVuVienThong(record.DICHVUVT_ID)
                    }
                }
            }
            catch (e)
        ...
        async hienThiCanhBaoThietHai()
        {
            try
            {
                let response = await this.$root.context.get("/web-cskh/api/khao-sat-tb-cc/canh-bao-thiet-hai", {matb: this.pTxtMaTB})
            ...
            try {
                ...
                if (response.data.error_code === "BSS-00000000") {
                    ...
                    if (dsThongTin.length > 0) {
                        ...
                        this.pTxtSoLH = dsThongTin[0].DIENTHOAI_LH;
                        this.pCboLyDoKTL = dsThongTin[0].TRANGTHAI == 0
        ...
        async kiemTraDuLieu(isThemMoi) {
            ...
            if (this.pCboLyDoKTL && this.optLyDoKoTraLoiSelected == 0) {
            ...
            if (BATBUOC_CHON_TRALOI_KLL == 1 && !this.pCboLyDoKTL && this.getKetQua.length <= 0) {
                this.$root.toastError("Bạn phải chọn câu trả lời!");
                return false;
            }

            if ( this.getKetQua.length > 0 && this.pCboLyDoKTL ) {
            ...
            if (dsTs && dsTs.length > 0) {
                ...
                if (strBatBuoc && this.pCboLyDoKTL) {
        ...
        async onSaveKhaoSat(data) {
            let rs = await this.$root.context.post('/web-cskh/api/khao-sat-tb-cc/ghi-lai', data)
        ...
        taoDuLieu(isThemMoi, isXuatLai) {
          let kq = this.getKetQua
            let data = {
                ...
                lyDoKtlId: this.pCboLyDoKTL ? this.optLyDoKoTraLoiSelected : 0,
