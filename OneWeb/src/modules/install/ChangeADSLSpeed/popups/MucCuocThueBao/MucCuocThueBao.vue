...
name: "MucCuocThueBao",
  ...
  methods:{
    ...
    HienThi_DoiTuongTB(){
      try{
        ...
        if(response.error_code === 'BSS-00000000' && response.data){
          this.dgvDoiTuongTB_DaGan = response.data;
        }
        else{
    ...
    grvDanhSach_FocusedRowChanged(args){
      try
      {
        console.log(args.data.mucuoctb_id);
        console.log(args.data.tungay);
        console.log(args.data.denngay);
        if(args.data.mucuoctb_id == null || args.data.mucuoctb_id == "")
          this.muccuoctb_id = 0;
        else
          this.muccuoctb_id = args.data.mucuoctb_id;
        if (args.data.tungay != null &&  args.data.tungay != "")
          this.dtpNgayBD = moment(args.data.tungay).format("DD/MM/YYYY");
        if (args.data.denngay != null && args.data.denngay != "")
          this.dtpNgayKT = moment(args.data.denngay).format("DD/MM/YYYY");
