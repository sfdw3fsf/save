      ...
      <div class="popup-body">
        <div class="box-form">
          <div class="legend-title">
            ...
            <div class="pull-right red" v-if="rowSelected!=null">Cộng tác viên: {{ten_ht_selected}}</div>
        ...
        onActionClick(action){
            if(action.id=='chapnhan'){
              ...
              this.$emit('accept',{
                ...
                ten_ctv:this.rowSelected.ten_ht,
          ...
          try{
            ...
            if(response && response.data && response.data.error_code && response.data.error_code == 'BSS-00000000' && response.data.data) {
              if(response.data.data && response.data.data.length > 0){
                this.ds_nhomnv = response.data.data.filter(x=>x.NHOMLNV_ID===6).map((x)=>({loainv_id:x.LOAINV_ID,ten:x.TEN}))   //NHOMLNV_ID: 6 là CTV
                this.ds_nhomnv.unshift({loainv_id: 0,ten: '--Tất cả--' })
              }
