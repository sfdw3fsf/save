<template src="@/components/Shared/DataGrid/index.template.html"></template>
<script>
...
/**
 *
 * @author: Custom from IT3.Web2
 */
import { Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate } from '@syncfusion/ej2-vue-grids'
...
export default {
  ...
  provide: {
    grid: [Sort, Filter, Resize, Group, Freeze, CommandColumn, ExcelExport, Edit, Aggregate]
  },
  props: {
    columns: {
      type: Array,
  ...
  created() {
    this.initPaging = this.allowPaging
  },
  computed: {
    getHeight: function () {
      ...
      if (this.dataSource != null) {
        if (this.allowPaging) {
          if (this.enablePagingServer) {
            result = this.dataSource.slice(0)
          } else {
            if (this.filteredDataRecords.length > 0 || this.hasFilter) {
              // console.log('Co filter')
              result = this.filteredDataRecords.slice(this.pageIndexData * this.pageSizeData, this.pageIndexData * this.pageSizeData + this.pageSizeData)
            } else {
              // console.log('Ko co filter', this.hasFilter, this.filteredDataRecords)
              result = this.dataSource.slice(this.pageIndexData * this.pageSizeData, this.pageIndexData * this.pageSizeData + this.pageSizeData)
            }
          }
        } else {
          result = this.dataSource.slice(0)
    ...
    getTotalRecords: function () {
      ...
      if (this.allowPaging) {
        if (this.enablePagingServer) result = this.totalRecords
          ...
          if (this.filteredDataRecords.length > 0 || this.hasFilter) {
            result = this.filteredDataRecords.length
          } else {
            result = this.dataSource == null ? 0 : this.dataSource.length
          }
        }
      }
      return result
  ...
  methods: {
    ...
    getDisableHtmlEncode: function (column) {
      ...
      } else return column.disableHtmlEncode !== false
    },
    getColumnCustomAttributes: function (column) {
      ...
      if (!(css == null || css == undefined || css == '')) {
        ...
        if (!(customAttributes.class == null || customAttributes.class == undefined)) {
          customAttributes.class = ' ' + css
        } else customAttributes.class = css
      }
      return customAttributes
    },
    refeshFilter: function (data) {
      // console.log('call refeshFilter')
      // this.dataSource = data.slice(0);
      this.filteredDataRecords = this.grid.getFilteredRecords().filter((x) => !x.ischecked)
      // console.log(this.filteredDataRecords)
    },
    ...
    getColumnCommands: function () {
      ...
      if (!(this.commands == null || this.commands.length == 0)) {
        for (let i = 0; i < this.commands.length; i++) {
          ...
          if (!(this.commands[i].cssClass == undefined || this.commands[i].cssClass == '')) {
            cssClass += ' ' + this.commands[i].cssClass
          }
          let text = ' '
          if (!(this.commands[i].text == undefined || this.commands[i].text == '')) {
    ...
    getDataValueType: function (value) {
      ...
      if (!(value == null || value == undefined)) {
        ...
        else if (value.match(regNumber) || (typeof value === 'number' && isFinite(value))) {
          type = 'Number'
        } else type = 'String'
      }
      return type
    ...
    getColumnType: function (column) {
      ...
      if (this.dataItems != null) {
        for (let i = 0; i < this.dataItems.length; i++) {
          ...
          if (!(row[column.fieldName] == undefined || row[column.fieldName] == null || row[column.fieldName] === '')) {
    ...
    grid_ActionComplete: function (args) {
      // console.log(`grid_ActionComplete`, args);
      if (args.action == 'filter' || args.action == 'clearFilter') {
        this.flagBeginFilter = false
        if (args.action == 'filter') {
          this.filteredDataRecords = this.grid.getFilteredRecords()
          this.filteredRecords = this.filteredDataRecords.length
          this.allowPaging = this.initPaging
          this.hasFilter = true
      ...
      } else if (args.requestType == 'refresh' && this.flagBeginFilter) {
      } else {
        this.filteredRecords = -1
      }
