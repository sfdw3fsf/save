<template>
<div>
<div class="table-content marb0" style="overflow-x: hidden;" :style="{'max-height':(height?height+'px':'400px')}">
  <table class="table-result nowrap table-gachle" :id="id">
      ...
      <tbody>
          ...
          <tr :class="{ 'highlight' : idx==(currentrecord%recordperpage) }" v-for="(d,idx) in pagedDataList" :key="idx" @click="changeRecord(idx,true)">
              ...
              <td :class="{ 'highlight1' : idx==(currentrecord%recordperpage),'nowrap w30 center':hdx==headerList.length-1,'stickylastcol':(hdx==headerList.length-1) && flcol }" v-for="(h,hdx) in headerList" :key="hdx" @dblclick="$emit('dbClickRow', d)" @click="$emit('rowClick', d)">
                <template v-if="!h.fieldtype && ((hdx<headerList.length-1) || (!lastcoltemplate) || (lastcoltemplate=='0'))">
                  <span>{{ d[h.name] }}</span>
                </template>
                <template v-if="h.fieldtype && h.fieldtype=='checkbox'">
                  <span><input :type="h.fieldtype" v-model="d[h.name]" :true-value="1" :false-value="0"></span>
                </template>
                <template v-if="h.fieldtype && h.fieldtype!='checkbox'">
                  <span><input :type="h.fieldtype" v-model="d[h.name]"></span>
                </template>
                <template v-if="!h.fieldtype && (hdx==headerList.length-1) && lastcoltemplate && (lastcoltemplate=='1')">
                ...
                <template v-if="!h.fieldtype && (hdx==headerList.length-1) && lastcoltemplate && (lastcoltemplate=='2')">
      ...
      for(var header of this.headerList)
        if(header.value && header.value.length>0)
          pagedData = pagedData.filter(a=> {
            ...
            return a[header.name] && (''+a[header.name]).toLowerCase().indexOf((''+header.value.trim()).toLowerCase())>=0
          });
