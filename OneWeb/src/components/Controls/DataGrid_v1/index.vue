...
export default {
  ...
  computed: {
    ...
    dataItems: function () {
      if (this.dataSource != null) {
        let s = this.getPageSize();
        return this.dataSourceFiltered.slice(
          this.currentPageIndex * s,
          this.currentPageIndex * s + s
        );
      }
      return null;
    },
  ...
  methods: {
  /**
   * @summary Lấy thông tin module hỗ trợ pagging server
   */
  ...
  /**
   * @summary Lấy thông tin page size hỗ trợ pagging server
   */
    getPageSize: function () {
      let s = 0;
      let m = this.getModuleName().trim().toLowerCase();
      switch (m) {
        case "ecms":
          s = 10;
          break;
            ...
            if (result) {
              if (dataItem[column.fieldName] != null)
                result = dataItem[column.fieldName]
                  ...
                  .toLowerCase()
                  .includes(
                    this.filterValues[column.fieldName].toLowerCase().trim()
                  );
              else {
    ...
    selectColumnColumnSpan: function () {
      ...
      if (!(this.getGroupedValues == null || this.getGroupedValues.length == 0))
        rs += 1;
      return rs;
    },
    ...
    buttonShowHideGroup_OnClick: function (e) {
      let sp = $(e.target);
      let tr = sp.closest("tr");
      let className = tr.attr("class");
      let newClassName = className == "tree-open" ? "tree-close" : "tree-open";
      tr.removeClass(className);
      tr.addClass(newClassName);
      let tb = tr.closest("table");
      let groupID = tr.data("groupid");
      let rows = tb.find('[data-groupid="' + groupID + '"]');
      if (rows.length > 1) {
        for (let i = 1; i < rows.length; i++) {
          const row = rows[i];
          if (newClassName == "tree-close") $(row).hide();
          else $(row).show();
        }
      }
    },
    getGroupedText: function (headerText, text) {
      if (
        headerText == null ||
