...
export default {
  ...
  watch: {
    ...
    expanded: function(v) {
      if(v) this.$emit("open",null);
      else this.$emit("close",this.selectedItem);
    }
    // value: function (v) {
    //   console.log(v)
      ...
      if (this.showColumnCheckbox == true) {
        ...
        if (this.selectedItems != null) {
          for (let i = 0; i < this.selectedItems.length; i++) {
            ...
            text += item && item[this.textField]?item[this.textField]:"";
          }
        }
        return text;
      } else {
        return this.selectedItem && this.selectedItem[this.textField]
