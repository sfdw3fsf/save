  ...
  <div>
    ...
    <div class="form-control text h-auto">
      <div class="tree-plus tree-icon" style="height: 400px; overflow: auto">
        <vue-tree-item-menu :parent="true" :current="current" @change="setValue" :options="temp_options" :labelField="labelField" :valueField="valueField"  />
      </div>
    </div>
  </div>
</template>
<script>
import VueTreeItem from "./VueTreeItem"
export default {
  data() {
    return {
      temp_options: [],
  ...
  components: {
    'vue-tree-item-menu' : VueTreeItem
  },
  props: {
    options: Array,
    labelField: String,
