  ...
  <div class="sidebar">
    <div class="sidebar-content">
      <div class="search">
        ...
        <input type="text" v-model="txtSearch" placeholder="Tìm kiếm" class="form-control" />
      </div>
...
export default {
  ...
  computed: {
    menuHtml: function() {
      if (this.menuItems != null) return this.getMenuHtml(this.getFiltered(this.txtSearch))
      else return ''
    }
  },
  updated() {
  ...
  methods: {
    emitChuyenTinh: function({ id, reload, recall = ''}) {
      if (id != this.currentPhanVung) {
        this.ChuyenPhanVung(id, reload, recall)
      } else {
        if (recall === 'emitTimKiemLichSuBaoHong') {
          EventBus.$emit('emitTimKiemLichSuBaoHong');
        } else if (recall === 'emitTimKiemLichSuKhieuNai') {
          EventBus.$emit('emitTimKiemLichSuKhieuNai');
        }
      }
    },
    ...
    ChuyenPhanVung: async function(val, reload = true, recall = '') {
      console.log(val)
      console.log(reload)
    ...
    openPage: async function(path, args) {
      ...
      try {
        ...
        if (arg.path) {
          ...
          if (!result && this.$route.path !== arg.path) {
            try {
              await this.$router.push(arg.path)
            } catch (error) {
        ...
        console.log('Lỗi truy cập chức năng: ', error)
      } finally {
        this.loading(false)
    ...
    getFiltered: function(textFiltered) {
      ...
      if (this.menuItems != null) {
        let items = this.menuItems.filter((item) => {
          return this.getText(item.name.toLowerCase()).includes(this.getText(textFiltered.toLowerCase()))
        })
    ...
    getMenuRecursive: function(currentItem, listItems) {
      ...
      //     : "fa fa-circle-o") +
      //   "'></span>";
      if (currentItem.icon != null) tagSpanIcon = "<span class='menu-icon'><span class='" + currentItem.icon + "'></span></span>"
      let items = listItems.filter((f) => {
      ...
      if (href.toLowerCase() == 'javascript:void(0)') {
        if (items.length > 0) {
          tagA += " href='" + href + "' onclick='$(this).parent().toggleClass(\"open\"); return false;'"
        }
      } else {
        if (items.length > 0) {
          tagA +=
      ...
      //   }
      // }
      tagA += '>' + tagSpanIcon + "<span class='menu-item-text'>" + currentItem.name + '</span></a>'
      let tagLi = ''
      if (items.length > 0) {
        let ul = ''
        for (let i = 0; i < items.length; i++) {
        ...
        tagLi = "<li class='haschild" + (this.txtSearch && this.txtSearch.trim().length > 0 ? ' open' : '') + "'>" + tagA + "<span class='down' onclick='$(this).parent().toggleClass(\"open\"); return false;'></span><ul class='child'>" + ul + '</ul></li>'
